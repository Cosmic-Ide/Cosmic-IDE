 ...
 * than the physical display. A TwoDScrollView is a [FrameLayout], meaning you should place
 * one child in it containing the entire contents to scroll; this child may itself be a layout
 * manager with a complex hierarchy of objects. A child that is often used is a [LinearLayout]
...
class TwoDScrollView : FrameLayout {
     ...
     */
    private var mTwoDScrollViewMovedFocus = false
    ...
    fun fling(velocityX: Int, velocityY: Int) {
        if (childCount > 0) {
            ...
            mScroller!!.fling(
                scrollX, scrollY, velocityX, velocityY, 0, right - width, 0, bottom - height
            )
            val movingDown = velocityY > 0
            val movingRight = velocityX > 0
            var newFocused = findFocusableViewInMyBounds(
                movingRight, mScroller!!.finalX, movingDown, mScroller!!.finalY, findFocus()
            )
            if (newFocused == null) {
                newFocused = this
            }
     ...
     */
        ...
        return if (preferredFocusable != null && preferredFocusable.top < bottomWithoutFadingEdge && preferredFocusable.bottom > topWithoutFadingEdge && preferredFocusable.left < rightWithoutFadingEdge && preferredFocusable.right > leftWithoutFadingEdge
        ) {
            preferredFocusable
    ...
    private fun findFocusableViewInBounds(
        topFocus: Boolean, top: Int, bottom: Int, leftFocus: Boolean, left: Int, right: Int
    ): View? {
     ...
     */
         ...
         */
                ...
                if (focusCandidate == null) {
                    ...
                    val viewIsCloserToVerticalBoundary =
                        topFocus && viewTop < focusCandidate.top || !topFocus && viewBottom > focusCandidate.bottom
                    val viewIsCloserToHorizontalBoundary =
                        leftFocus && viewLeft < focusCandidate.left || !leftFocus && viewRight > focusCandidate.right
     ...
     *
     * @param direction the scroll direction: [android.view.View.FOCUS_UP] to go the top of the
     * view or [android.view.View.FOCUS_DOWN] to go the bottom
    ...
    fun fullScroll(direction: Int, horizontal: Boolean): Boolean {
        return if (!horizontal) {
            ...
            scrollAndFocus(direction, mTempRect.top, mTempRect.bottom, 0, 0, 0)
        } else {
            ...
            scrollAndFocus(0, 0, 0, direction, mTempRect.top, mTempRect.bottom)
        }
    }
    ...
    private fun scrollToChild(child: View) {
        ...
        /* Offset from child's local coordinates to TwoDScrollView coordinates */offsetDescendantRectToMyCoords(
            child,
            mTempRect
        )
        val scrollDelta = computeScrollDeltaToGetChildRectOnScreen(mTempRect)
     ...
     *
     *
     * This is more expensive than the default [android.view.ViewGroup] implementation,
    ...
    override fun onRequestFocusInDescendants(direction: Int, previouslyFocusedRect: Rect): Boolean {
        ...
        return nextFocus?.requestFocus(direction, previouslyFocusedRect) ?: false
    }
     ...
     *
     * @param direction the scroll direction: [android.view.View.FOCUS_UP] to go upward [     ][android.view.View.FOCUS_DOWN] to downward
