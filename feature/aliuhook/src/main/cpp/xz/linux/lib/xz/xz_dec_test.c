...
static int xz_dec_test_release(struct inode *i, struct file *f) {
    ...
    if (ret == XZ_OK)
        printk(KERN_INFO
    DEVICE_NAME ": input was truncated\n");
...
static ssize_t xz_dec_test_write(struct file *file, const char __user *buf,
    ...
    printk(KERN_INFO
    DEVICE_NAME ": decoding %zu bytes of input\n",
            size);
    ...
    switch (ret) {
        ...
        case XZ_STREAM_END:
            printk(KERN_INFO
            DEVICE_NAME ": XZ_STREAM_END, "
            "CRC32 = 0x%08X\n", ~crc);
            return size - remaining - (buffers.in_size - buffers.in_pos);

        case XZ_MEMLIMIT_ERROR:
            printk(KERN_INFO
            DEVICE_NAME ": XZ_MEMLIMIT_ERROR\n");
            break;

        case XZ_FORMAT_ERROR:
            printk(KERN_INFO
            DEVICE_NAME ": XZ_FORMAT_ERROR\n");
            break;

        case XZ_OPTIONS_ERROR:
            printk(KERN_INFO
            DEVICE_NAME ": XZ_OPTIONS_ERROR\n");
            break;

        case XZ_DATA_ERROR:
            printk(KERN_INFO
            DEVICE_NAME ": XZ_DATA_ERROR\n");
            break;

        case XZ_BUF_ERROR:
            printk(KERN_INFO
            DEVICE_NAME ": XZ_BUF_ERROR\n");
            break;

        default:
            printk(KERN_INFO
            DEVICE_NAME ": Bug detected!\n");
            break;
    }
...
xz_dec_test_init(void) {
    ...
    printk(KERN_INFO
    DEVICE_NAME ": module loaded\n");
    printk(KERN_INFO
    DEVICE_NAME ": Create a device node with "
