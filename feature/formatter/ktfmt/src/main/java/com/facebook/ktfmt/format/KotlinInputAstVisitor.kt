...
class KotlinInputAstVisitor(
     ...
     */
    private val expressionBreakIndent: Indent.Const =
                    ...
                    is KtQualifiedExpression -> {
                        ...
                        if (selectorExpression !is KtCallExpression) {
                            ...
                            val isTrailingLambda =
                                useBlockLikeLambdaStyle && index == parts.size - 1
                            if (isTrailingLambda) {
                                builder.close()
                            }
                            val argsIndentElse =
                                if (index == parts.size - 1) ZERO else expressionBreakIndent
                            val lambdaIndentElse =
                                if (isTrailingLambda) expressionBreakNegativeIndent else ZERO
                            val negativeLambdaIndentElse =
                                if (isTrailingLambda) expressionBreakIndent else ZERO
                            ...
                            visitCallElement(
                                ...
                                argumentsIndent = Indent.If.make(
                                    nameTag,
                                    expressionBreakIndent,
                                    argsIndentElse
                                ),
                                lambdaIndent = Indent.If.make(nameTag, ZERO, lambdaIndentElse),
                                negativeLambdaIndent = Indent.If.make(
                                    nameTag,
                                    ZERO,
                                    negativeLambdaIndentElse
                                ),
                            )
                        }
                    }
    ...
    private fun visitLambdaExpressionInternal(
        ...
        if (hasStatements) {
            ...
            val blockComments =
                bodyExpression.children().filter { it is PsiComment && it.text.startsWith("/*") }
                    .toList()
            builder.breakOp(Doc.FillMode.UNIFIED, "", bracePlusBlockIndent)
            builder.block(bracePlusBlockIndent) {
                        ...
                        is KtPropertyAccessor -> {
                            builder.block(ZERO) {
                                visitFunctionLikeExpression(
                                    ...
                                    bodyExpression = component.bodyBlockExpression
                                        ?: component.bodyExpression,
