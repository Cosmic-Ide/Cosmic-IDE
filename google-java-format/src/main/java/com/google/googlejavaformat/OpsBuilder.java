...
public final class OpsBuilder {
     ...
     * the input tokens, output them here and optionally complain.
     *
    ...
    /**
     * Returns the {@link Input.Tok}s starting at the current source position, which are satisfied
    ...
    /**
     * Emit an optional token iff it exists on the input. This is used to emit tokens whose
     * existence has been lost in the AST.
     *
    ...
    /**
     * Make the boundary of a region that can be partially formatted. The boundary will be included
     * in the following region, e.g.: [[boundary0, boundary1), [boundary1, boundary2), ...].
     */
        ...
        for (int i = 0; i < opsN; i++) {
            ...
            if (op instanceof Doc.Token) {
                ...
                if (tokenOp.realOrImaginary().isReal()) {
                    ...
                    boolean space =
                            false; // Do we need an extra space after a previous "/*" comment?
                    ...
                    for (Input.Tok tokBefore : token.getToksBefore()) {
                        if (tokBefore.isNewline()) {
                            ...
                            allowBlankAfterLastComment =
                                    tokBefore.isSlashSlashComment()
                                            || (tokBefore.isSlashStarComment()
                                                    && !tokBefore.isJavadocComment());
                        }
                    }
