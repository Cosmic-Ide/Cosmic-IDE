...
public final class JavaInput extends Input {
     ...
     * non-tokens that appear before it, and another list of its non-tokens that appear after it.
     * The concatenation of the texts of all the {@link Token}s' {@link Tok}s, each preceded by the
     * texts of its {@code toksBefore} and followed by the texts of its {@code toksAfter}, equals
     * the input.
     */
    ...
    public JavaInput(String text) throws FormatterException {
        ...
        for (Token token : tokens) {
            ...
            tokenLocations.put(
                    Range.closed(JavaOutput.startTok(token).getPosition(), upper), token);
        }
     ...
     * @param stopTokens a set of tokens which should cause lexing to stop. If one of these is
     *     found, the returned list will include tokens up to but not including that token.
     */
    static ImmutableList<Tok> buildToks(String text, ImmutableSet<TokenKind> stopTokens)
            throws FormatterException {
        stopTokens =
                ImmutableSet.<TokenKind>builder().addAll(stopTokens).add(TokenKind.EOF).build();
        ...
        for (RawTok t : rawToks) {
            ...
            if (Character.isWhitespace(tokText0)) {
                ...
                if (tokText.startsWith("//")
                    ...
                    originalTokText =
                            originalTokText.substring(
                                    0, originalTokText.length() - extraNewline.length());
                }
                isToken = false;
                isNumbered = true;
                strings.add(originalTokText);
                    ...
                    || (tokText0 == '.'
                            && tokText.length() > 1
                            && Character.isDigit(tokText.charAt(1)))) {
    ...
    Range<Integer> characterRangeToTokenRange(Range<Integer> characterRange)
        ...
        Range<Integer> nonEmptyRange =
                characterRange.isEmpty()
                        ? Range.closedOpen(
                                characterRange.lowerEndpoint(), characterRange.lowerEndpoint() + 1)
                        : characterRange;
    ...
    /**
     * Get the navigable map from position to {@link Token}. Used to look for tokens following a
     * given one, and to implement the --offset and --length flags to reformat a character range in
     * the input file.
     *
    ...
    public RangeSet<Integer> characterRangesToTokenRanges(
        ...
        for (Range<Integer> characterRange : characterRanges) {
            tokenRangeSet.add(
                    characterRangeToTokenRange(
                            characterRange.canonical(DiscreteDomain.integers())));
        }
