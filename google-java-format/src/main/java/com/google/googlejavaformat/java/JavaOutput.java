...
public final class JavaOutput extends Output {
    ...
    private final Map<Integer, BlankLineWanted> blankLines =
            new HashMap<>(); // Info on blank lines.
        ...
        if (!range.isEmpty()) {
            ...
            while (iLine < iN
                    && (javaInput.getRanges(iLine).isEmpty()
                            || javaInput.getRanges(iLine).upperEndpoint()
                                    <= range.lowerEndpoint())) {
    ...
    public ImmutableList<Replacement> getFormatReplacements(RangeSet<Integer> iRangeSet0) {
        ...
        for (Range<Integer> iRange : iRangeSet.asRanges()) {
            Range<Integer> range =
                    expandToBreakableRegions(iRange.canonical(DiscreteDomain.integers()));
        ...
        for (Range<Integer> range : breakableRanges.asRanges()) {
            ...
            int replaceTo =
                    min(endTok.getPosition() + endTok.length(), javaInput.getText().length());
