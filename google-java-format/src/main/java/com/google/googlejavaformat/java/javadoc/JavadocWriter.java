...
final class JavadocWriter {
     ...
     * request may be honored, or it may be overridden by a request for "more significant"
     * whitespace, like a newline.
     */
    ...
    void writeFooterJavadocTagStart(Token token) {
        ...
        if (!wroteAnythingSignificant) {
            // Javadoc consists solely of tags. This is frowned upon in general but OK for
            // @Overrides.
    ...
    /**
     * The kind of whitespace that has been requested between the previous and next tokens. The
     * order of the values is significant: It goes from lowest priority to highest. For example, if
     * the previous token requests {@link #BLANK_LINE} after it but the next token requests only
     * {@link #NEWLINE} before it, we insert {@link #BLANK_LINE}.
     */
    ...
    // If this is a hotspot, keep a String of many spaces around, and call append(string, start,
    // end).
    ...
    /**
     * Tokens that are always pinned to the following token. For example, {@code <p>} in {@code
     * <p>Foo bar} (never {@code <p> Foo bar} or {@code <p>\nFoo bar}).
     *
