...
/**
 * RE is an efficient, lightweight regular expression evaluator/matcher class. Regular expressions
 * are pattern descriptions which enable sophisticated matching of strings. In addition to being
 * able to match a string against a pattern, you can also extract parts of the match. This is
 * especially useful in text parsing! Details on the syntax of regular expression patterns are given
 * below.
 *
 * <p>To compile a regular expression (RE), you can simply construct an RE matcher object from the
 * string specification of the pattern, like this:
 *
 * <pre>
 *  RE r = new RE("a*b");
 * </pre>
 *
 * <p>Once you have done this, you can call either of the RE.match methods to perform matching on a
 * String. For example:
 *
 * <pre>
 ...
 * "aaaab".
 *
 * <p>If you were interested in the <i>number</i> of a's which matched the first part of our example
 * expression, you could change the expression to "(a*)b". Then when you compiled the expression and
 * matched it against something like "xaaaab", you would get results like this:
 *
 * <pre>
 ...
 * </pre>
 *
 * You can also refer to the contents of a parenthesized expression within a regular expression
 * itself. This is called a 'backreference'. The first backreference in a regular expression is
 ...
 *
 * <p>The full regular expression syntax accepted by RE is described here:
 *
 * <pre>
 *
 ...
 * </pre>
 *
 * <b>NOTE:</b> Incomplete ranges will be interpreted as &quot;starts from zero&quot; or &quot;ends
 * with last character&quot;. <br>
 * I.e. [-a] is the same as [\\u0000-a], and [a-] is the same as [a-\\uFFFF], [-] means &quot;all
 * characters&quot;.
 *
 * <pre>
 *
 ...
 * </pre>
 *
 * <p>All closure operators (+, *, ?, {m,n}) are greedy by default, meaning that they match as many
 * elements of the string as possible without causing the overall match to fail. If you want a
 * closure to be reluctant (non-greedy), you can simply follow it with a '?'. A reluctant closure
 * will match as few elements of the string as possible when finding matches. {m,n} closures don't
 * currently support reluctancy.
 *
 * <p><b><font face="times roman">Line terminators</font></b> <br>
 ...
 * </ul>
 *
 * <p>RE runs programs compiled by the RECompiler class. But the RE matcher class does not include
 * the actual regular expression compiler for reasons of efficiency. In fact, if you want to
 * pre-compile one or more regular expressions, the 'recompile' class can be invoked from the
 * command line to produce compiled output like this:
 *
 * <pre>
 ...
 * </pre>
 *
 * You can then construct a regular expression matcher (RE) object from the pre-compiled expression
 * re1 and thus avoid the overhead of compiling the expression at runtime. If you require more
 * dynamic regular expressions, you can construct a single RECompiler object and re-use it to
 * compile each expression. Similarly, you can change the program run by a given matcher object at
 * any time. However, RE and RECompiler are not threadsafe (for efficiency reasons, and because
 * requiring thread safety in this class is deemed to be a rare requirement), so you will need to
 * construct a separate compiler or matcher object for each thread (unless you do thread
 * synchronization yourself). Once expression compiled into the REProgram object, REProgram can be
 * safely shared across multiple threads and RE objects. <br>
 *
 * <p><br>
 ...
 *   <li>com.weusours.util.re does not support POSIX equivalence classes ([=foo=] syntax)
 *       (I18N/locale issue)
 *   <li>com.weusours.util.re does not support nested POSIX character classes (definitely should,
 *       but not completely trivial)
 *   <li>com.weusours.util.re Does not support POSIX character collation concepts ([.foo.] syntax)
 *       (I18N/locale issue)
 ...
 *   <li>Not *all* possibilities are considered for greediness when backreferences are involved (as
 *       POSIX suggests should be the case). The POSIX RE "(ac*)c*d[ac]*\1", when matched against
 *       "acdacaa" should yield a match of acdacaa where \1 is "a". This is not the case in this RE
 *       package, and actually Perl doesn't go to this extent either! Until someone actually
 *       complains about this, I'm not sure it's worth "fixing". If it ever is fixed, test #137 in
 *       RETest.txt should be updated.
 * </ul>
 *
 * </font>
 *
...
public class RE implements Serializable {
    ...
    static final char OP_RELUCTANTSTAR =
            '8'; // none/expr       reluctant '*' (mnemonic for char is unshifted '*')
    ...
    /**
     * Constructs a regular expression matcher from a String by compiling it using a new instance of
     * RECompiler. If you will be compiling many expressions, you may prefer to use a single
     * RECompiler object instead.
     *
     ...
     * Permits special flags to be passed in to modify matching behaviour.
     *
    ...
    /**
     * Throws an Error representing an internal error condition probably resulting from a bug in the
     * regular expression compiler (or possibly data corruption). In practice, this should be very
     * rare.
     *
     ...
     * @param lastNode Last valid node (used for matching a subexpression without matching the rest
                ...
                case OP_RELUCTANTMAYBE:
                    {
                        ...
                        do {
                            ...
                            if ((idxNew = matchNodes(next, maxNode, idx)) != -1) {
                                ...
                                && (idx = matchNodes(node + nodeSize, next, idx)) != -1);
                        return -1;
                    }
                ...
                case OP_BACKREF:
                    {
                        ...
                        for (int i = 0; i < l; i++) {
                            if (compareChars(search.charAt(idx++), search.charAt(s + i), caseFold)
                                    != 0) {
                                return -1;
                            }
                        }
                        ...
                        case E_BOUND:
                            {
                                ...
                                if ((Character.isLetterOrDigit(cLast)
                                                == Character.isLetterOrDigit(cNext))
                                        == (opdata == E_BOUND)) {
                                    return -1;
                                }
                            }
                ...
                case OP_POSIXCLASS:
                    {
                        ...
                        switch (opdata) {
                            ...
                            case POSIX_CLASS_LOWER:
                                if (Character.getType(search.charAt(idx))
                ...
                case OP_BRANCH:
                    {
                        ...
                        do {
                            ...
                            node += nextBranch;
                                ...
                                && (instruction[node + offsetOpcode] == OP_BRANCH));
    ...
    /**
     * Match the current regular expression program against the current input string, starting at
     * index i of the input string. This method is only meant for internal use.
     *
    ...
    /**
     * Matches the current regular expression program against a character array, starting at a given
     * index.
     *
     ...
     * works the same way as the Perl function of the same name. Given a regular expression of
     * "[ab]+" and a string to split of "xyzzyababbayyzabbbab123", the result would be the array of
     * Strings "[xyzzy, yyz, 123]".
     *
     * <p>Please note that the first string in the resulting array may be an empty string. This
    ...
    /**
     * Flag bit that indicates that subst should only replace the first occurrence of this regular
     * expression.
     */
    ...
    /**
     * Substitutes a string for this regular expression in another string. This method works like
     * the Perl function of the same name. Given a regular expression of "a*b", a String to
     * substituteIn of "aaaabfooaaabgarplyaaabwackyb" and the substitution String "-", the resulting
     * String returned by subst would be "-foo-garply-wacky-".
     *
     ...
     * @return The string substituteIn with zero or more occurrences of the current regular
     *     expression replaced with the substitution String (if this regular expression object
     *     doesn't match at any position, the original String is returned unchanged).
     */
     ...
     * String returned by subst would be "-foo-garply-wacky-".
     *
     * <p>It is also possible to reference the contents of a parenthesized expression with $0, $1,
     * ... $9. A regular expression of "http://[\\.\\w\\-\\?/~_@&=%]+", a String to substituteIn of
     * "visit us: http://www.apache.org!" and the substitution String "&lt;a
     * href=\"$0\"&gt;$0&lt;/a&gt;", the resulting String returned by subst would be "visit us:
     * &lt;a href=\"http://www.apache.org\"&gt;http://www.apache.org&lt;/a&gt;!".
     *
     * <p><i>Note:</i> $0 represents the whole match.
     *
     * @param substituteIn String to substitute within
     ...
     *     set, only the first occurrence of this regular expression is replaced. If the bit is not
     *     set (REPLACE_ALL), all occurrences of this pattern will be replaced. If the flag
                    ...
                    if ((lCurrentPosition == 0 || substitution.charAt(lCurrentPosition - 1) != '\\')
                        ...
                        if (c >= '0' && c <= '9') {
                            if (bAddedPrefix == false) {
                                ...
                                ret.append(
                                        substitution.substring(
                                                lLastPosition + 2, lCurrentPosition));
                            }
    ...
    /**
     * Returns an array of Strings, whose toString representation matches a regular expression. This
     * method works like the Perl function of the same name. Given a regular expression of "a*b" and
     * an array of String objects of [foo, aab, zzz, aaaab], the array of Strings returned by grep
     * would be [aab, aaaab].
     *
     ...
     * @return negative, 0, or positive integer as the first character less than, equal to, or
