...
/**
 * A regular expression compiler class. This class compiles a pattern string into a regular
 * expression program interpretable by the RE evaluator class. The 'recompile' command line tool
 * uses this compiler to pre-compile regular expressions for use with RE. For a description of the
 * syntax accepted by RECompiler and what you can do with regular expressions, see the documentation
 * for the RE matcher class.
 *
 * @see RE
 * @see recompile
...
public class RECompiler {
    ...
    /**
     * Ensures that n more characters can fit in the program buffer. If n more can't fit, then the
     * size is doubled until it can.
     *
    ...
    /**
     * Inserts a node with a given opcode and opdata at insertAt. The node relative next pointer is
     * initialized to 0.
     *
    ...
    /**
     * Match an escape sequence. Handles quoted chars and octal escapes as well as normal escape
     * characters. Always advances the input stream by the right amount. This code "understands" the
     * subtle difference between an octal escape and a backref. You can access the type of ESC_CLASS
     * or ESC_COMPLEX or ESC_BACKREF by looking at pattern[idx - 1].
     *
                    ...
                    for (; idx < len && hexDigits-- > 0; idx++) {
                        ...
                        if (c >= '0' && c <= '9') {
                            ...
                            if (c >= 'a' && c <= 'f') {
                                ...
                                // If it's not a valid digit or hex letter, the escape must be
                                // invalid
    ...
    /**
     * Absorb an atomic character string. This method is a little tricky because it can un-include
     * the last character of string if a closure operator follows. This is correct because *+? have
     * higher precedence than concatentation (thus ABC* means AB(C*) and NOT (ABC)*).
     *
            ...
            if ((idx + 1) < len) {
                ...
                switch (c) {
                    ...
                    case '+':
                        ...
                        // If the next character is a closure operator and our atom is non-empty,
                        // the
                        // current character should bind to the closure operator rather than the
                        // atom
                        if (lenAtom != 0) {
                            break atomLoop;
                        }
                ...
                case '+':
                    {
                        ...
                        setNextOfEnd(
                                node(RE.OP_GOTO, 0), ret); // one option is to go back to the start
    ...
    /**
     * Compile an expression with possible parens around it. Paren matching is done at this level so
     * we can tie the branch tails together.
     *
    ...
    /**
     * Compiles a regular expression pattern into a program runnable by the pattern matcher class
     * 'RE'.
     *
