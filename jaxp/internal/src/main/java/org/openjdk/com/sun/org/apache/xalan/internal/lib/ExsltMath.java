...
/**
 * This class contains EXSLT math extension functions. It is accessed by specifying a namespace URI
 * as follows:
 *
 * <pre>
...
public class ExsltMath extends ExsltBase {
    ...
    /**
     * The math:max function returns the maximum value of the nodes passed as the argument. The
     * maximum value is defined as follows. The node set passed as an argument is sorted in
     * descending order as it would be by xsl:sort with a data type of number. The maximum is the
     * result of converting the string value of the first node in this sorted list to a number using
     * the number function.
     *
     * <p>If the node set is empty, or if the result of converting the string values of any of the
    ...
    /**
     * The math:min function returns the minimum value of the nodes passed as the argument. The
     * minimum value is defined as follows. The node set passed as an argument is sorted in
     * ascending order as it would be by xsl:sort with a data type of number. The minimum is the
     * result of converting the string value of the first node in this sorted list to a number using
     * the number function.
     *
     * <p>If the node set is empty, or if the result of converting the string values of any of the
    ...
    /**
     * The math:highest function returns the nodes in the node set whose value is the maximum value
     * for the node set. The maximum value for the node set is the same as the value as calculated
     * by math:max. A node has this maximum value if the result of converting its string value to a
     * number as if by the number function is equal to the maximum value, where the equality
     * comparison is defined as a numerical comparison using the = operator.
     *
     * <p>If any of the nodes in the node set has a non-numeric value, the math:max function will
     * return NaN. The definition numeric comparisons entails that NaN != NaN. Therefore if any of
     * the nodes in the node set has a non-numeric value, math:highest will return an empty node
     * set.
     *
     * @param nl The NodeList for the node-set to be evaluated.
     * @return node-set with nodes containing the maximum value found, an empty node-set if any node
    ...
    public static NodeList highest(NodeList nl) {
        ...
        if (Double.isNaN(maxValue)) return highNodes; // empty Nodeset
     ...
     * by math:min. A node has this minimum value if the result of converting its string value to a
     * number as if by the number function is equal to the minimum value, where the equality
     * comparison is defined as a numerical comparison using the = operator.
     *
     * <p>If any of the nodes in the node set has a non-numeric value, the math:min function will
     * return NaN. The definition numeric comparisons entails that NaN != NaN. Therefore if any of
     * the nodes in the node set has a non-numeric value, math:lowest will return an empty node set.
     *
     * @param nl The NodeList for the node-set to be evaluated.
     * @return node-set with nodes containing the minimum value found, an empty node-set if any node
     *     cannot be converted to a number.
     */
    public static NodeList lowest(NodeList nl) {
        ...
        if (Double.isNaN(minValue)) return lowNodes; // empty Nodeset
    ...
    /**
     * The math:constant function returns the specified constant to a set precision. The possible
     * constants are:
     *
     * <pre>
     *  PI
