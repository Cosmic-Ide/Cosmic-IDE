...
import org.openjdk.com.sun.org.apache.xml.internal.utils.DOMHelper;
...
/**
 * This class contains EXSLT set extension functions. It is accessed by specifying a namespace URI
 * as follows:
 *
 * <pre>
...
public class ExsltSets extends ExsltBase {
     ...
     * @return a NodeList containing the nodes in nl1 that precede in document order the first node
     *     in nl2; an empty node-set if the first node in nl2 is not in nl1; all of nl1 if nl2 is
     *     empty.
     * @see <a href="http://www.exslt.org/">EXSLT</a>
     */
    public static NodeList leading(NodeList nl1, NodeList nl2) {
        ...
        if (!ns1.contains(endNode)) return leadNodes; // empty NodeSet
     ...
     * @param nl1 NodeList for first node-set.
     * @param nl2 NodeList for second node-set.
     * @return a NodeList containing the nodes in nl1 that follow in document order the first node
     *     in nl2; an empty node-set if the first node in nl2 is not in nl1; all of nl1 if nl2 is
     *     empty.
     * @see <a href="http://www.exslt.org/">EXSLT</a>
     */
    public static NodeList trailing(NodeList nl1, NodeList nl2) {
        ...
        if (!ns1.contains(startNode)) return trailNodes; // empty NodeSet
    ...
    /**
     * The set:intersection function returns a node set comprising the nodes that are within both
     * the node sets passed as arguments to it.
     *
     * @param nl1 NodeList for first node-set.
     * @param nl2 NodeList for second node-set.
    ...
    public static NodeList intersection(NodeList nl1, NodeList nl2) {
        ...
        for (int i = 0; i < ns1.getLength(); i++) {
            ...
            if (ns2.contains(n)) inter.addElement(n);
        }
    ...
    public static NodeList difference(NodeList nl1, NodeList nl2) {
        ...
        for (int i = 0; i < ns1.getLength(); i++) {
            ...
            if (!ns2.contains(n)) diff.addElement(n);
        }
    ...
    public static NodeList distinct(NodeList nl) {
        ...
        for (int i = 0; i < nl.getLength(); i++) {
            ...
            if (key == null) dist.addElement(currNode);
    ...
    /**
     * The set:has-same-node function returns true if the node set passed as the first argument
     * shares any nodes with the node set passed as the second argument. If there are no nodes that
     * are in both node sets, then it returns false.
     *
     * <p>The Xalan extensions MethodResolver converts 'has-same-node' to 'hasSameNode'.
     *
     * <p>Note: Not to be confused with hasSameNodes in the Xalan namespace, which returns true if
