...
import org.openjdk.com.sun.org.apache.xalan.internal.xslt.EnvironmentCheck;
import org.openjdk.com.sun.org.apache.xml.internal.utils.Hashtree2Node;
...
/**
 * This class contains many of the Xalan-supplied extensions. It is accessed by specifying a
...
public class Extensions {
     ...
     * 1.0 Recommendation. If the argument is a node-set, <code>nodeset</code> returns a node-set.
     * If the argument is a string, number, or boolean, then <code>nodeset</code> returns a node-set
     ...
     * node-set is returned consisting of a single root node with a single text node child that is
     * the result of calling the java <code>toString()</code> method on the passed argument. Most of
     * the actual work here is done in <code>MethodResolver</code> and <code>XRTreeFrag</code>.
     *
     ...
     *     <p>Note: The usage of this extension function in the xalan namespace is deprecated.
     *     Please use the same function in the EXSLT sets extension (http://exslt.org/sets).
     */
     ...
     * @return a NodeList with nodes from nl containing distinct string values. In other words, if
     *     more than one node in nl contains the same string value, only include the first such node
     *     found.
     *     <p>Note: The usage of this extension function in the xalan namespace is deprecated.
     *     Please use the same function in the EXSLT sets extension (http://exslt.org/sets).
     */
     ...
     * Used where the XPath expression is not known until run-time. The expression is evaluated as
     * if the run-time value of the argument appeared in place of the evaluate function call at
     * compile time.
     *
     ...
     *     <p>Note: The usage of this extension function in the xalan namespace is deprecated.
     *     Please use the same function in the EXSLT dynamic extension (http://exslt.org/dynamic).
     */
    ...
    /**
     * Returns a NodeSet containing one text node for each token in the first argument. Delimiters
     * are specified in the second argument. Tokens are determined by a call to <code>
     * StringTokenizer</code>. If the first argument is an empty string or contains only delimiters,
     * the result will be an empty NodeSet.
     *
     * <p>Contributed to XalanJ1 by <a href="mailto:benoit.cerrina@writeme.com">Benoit Cerrina</a>.
     *
    ...
    /**
     * Returns a NodeSet containing one text node for each token in the first argument. Delimiters
     * are whitespace. That is, the delimiters that are used are tab (&#x09), linefeed (&#x0A),
     * return (&#x0D), and space (&#x20). Tokens are determined by a call to <code>StringTokenizer
     * </code>. If the first argument is an empty string or contains only delimiters, the result
     ...
     * environment.
     *
     * <p>Simply calls the {@link
     * org.openjdk.com.sun.org.apache.xalan.internal.xslt.EnvironmentCheck} utility to grab info
     * about the Java environment and CLASSPATH, etc., and then returns the resulting Node.
     * Stylesheets can then maniuplate this data or simply xsl:copy-of the Node. Note that we first
     * attempt to load the more advanced org.apache.env.Which utility by reflection; only if that
     * fails to we still use the internal version. Which is available from <a
