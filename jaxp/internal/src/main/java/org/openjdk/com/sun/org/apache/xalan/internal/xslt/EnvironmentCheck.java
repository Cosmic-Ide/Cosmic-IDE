...
/**
 * Utility class to report simple information about the environment. Simplistic reporting about
 * certain classes found in your JVM may help answer some FAQs for simple problems.
 *
 * <p>Usage-command line: <code>
 * java org.openjdk.com.sun.org.apache.xalan.internal.xslt.EnvironmentCheck [-out outFile]
 * </code>
 *
 * <p>Usage-from program: <code>
 * boolean environmentOK =
 * (new EnvironmentCheck()).checkEnvironment(yourPrintWriter);
 * </code>
 *
 * <p>Usage-from stylesheet: <code><pre>
 *    &lt;?xml version="1.0"?&gt;
 ...
 * </pre></code>
 *
 * <p>Xalan users reporting problems are encouraged to use this class to see if there are potential
 * problems with their actual Java environment <b>before</b> reporting a bug. Note that you should
 * both check from the JVM/JRE's command line as well as temporarily calling checkEnvironment()
 * directly from your code, since the classpath may differ (especially for servlets, etc).
 *
 * <p>Also see http://xml.apache.org/xalan-j/faq.html
 *
 * <p>Note: This class is pretty simplistic: results are not necessarily definitive nor will it find
 * all problems related to environment setup. Also, you should avoid calling this in deployed
 * production code, both because it is quite slow and because it forces classes to get loaded.
 *
 * <p>Note: This class explicitly has very limited compile-time dependencies to enable easy
 * compilation and usage even when Xalan, DOM/SAX/JAXP, etc. are not present.
 *
 * <p>Note: for an improved version of this utility, please see the xml-commons' project Which
...
public class EnvironmentCheck {
    ...
    /**
     * Programmatic entrypoint: Report on basic Java environment and CLASSPATH settings that affect
     * Xalan.
     *
     * <p>Note that this class is not advanced enough to tell you everything about the environment
     * that affects Xalan, and sometimes reports errors that will not actually affect Xalan's
     * behavior. Currently, it very simplistically checks the JVM's environment for some basic
     * properties and logs them out; it will report a problem if it finds a setting or .jar file
     * that is <i>likely</i> to cause problems.
     *
     * <p>Advanced users can peruse the code herein to help them investigate potential environment
     * problems found; other users may simply send the output from this tool along with any bugs
     * they submit to help us in the debugging process.
     *
     * @param pw PrintWriter to send output to; can be sent to a file that will look similar to a
     *     Properties file; defaults to System.out if null
     * @return true if your environment appears to have no major problems; false if potential
     ...
     *
     * <p>Worker method called from various places.
     *
     * <p>Various system and CLASSPATH, etc. properties are put into the hash as keys with a brief
     * description of the current state of that item as the value. Any serious problems will be put
     * in with a key that is prefixed with {@link #ERROR 'ERROR.'} so it stands out in any resulting
     * report; also a key with just that constant will be set as well for any error.
     *
     * <p>Note that some legitimate cases are flaged as potential errors - namely when a developer
     * recompiles xalan.jar on their own - and even a non-error state doesn't guaruntee that
     * everything in the environment is correct. But this will help point out the most common
     * classpath and system property problems that we've seen.
     *
     * @return Hashtable full of useful environment info about Xalan and related system properties,
     *     etc.
     */
     ...
     *
     * <p>This dumps a simple header and then each of the entries in the Hashtable to our
     * PrintWriter; it does special processing for entries that are .jars found in the classpath.
     *
     * @param h Hashtable of items to report on; presumably filled in by our various check*()
     *     methods
     * @return true if your environment appears to have no major problems; false if potential
     *     environment problems found
     * @see #appendEnvironmentReport(Node, Document, Hashtable) for an equivalent that appends to a
     *     Node instead
     */
    ...
    /**
     * Stylesheet extension entrypoint: Dump a basic Xalan environment report from
     * getEnvironmentHash() to a Node.
     *
     * <p>Copy of writeEnvironmentReport that creates a Node suitable for other processing instead
     * of a properties-like text output.
     *
     * @param container Node to append our report to
     * @param factory Document providing createElement, etc. services
     * @param h Hash presumably from {@link #getEnvironmentHash()}
     * @see #writeEnvironmentReport(Hashtable) for an equivalent that writes to a PrintWriter
     *     instead
     */
     ...
     *
     * <p>//@todo NOTE: We don't actually search java.ext.dirs for // *.jar files therein! This
     * should be updated
     *
    ...
    protected void checkSystemProperties(Hashtable h) {
        ...
        try {
            ...
            //  v2 = checkPathForJars(System.getProperty("sun.boot.library.path"), jarNames);   //
            // ?? may not be needed
            //  v3 = checkPathForJars(System.getProperty("java.library.path"), jarNames);   // ??
            // may not be needed
     ...
     *
     * <p>cp should be separated by the usual File.pathSeparator. We then do a simplistic search of
     * the path for any requested .jar filenames, and return a listing of their names and where
     * (apparently) they came from.
     *
     ...
     *
     * <p>Currently does a lookup into a local table of some recent shipped Xalan builds to
     * determine where the .jar probably came from. Note that if you recompile Xalan or Xerces
     * yourself this will likely report a potential error, since we can't certify builds other than
     * the ones we ship. Only reports against selected posted Xalan-J builds.
     *
     * <p>//@todo actually look up version info in manifests
     *
    ...
    protected void checkProcessorVersion(Hashtable h) {
        ...
        try {
            final String XALAN1_VERSION_CLASS =
                    "org.openjdk.com.sun.org.apache.xalan.internal.xslt.XSLProcessorVersion";
        ...
        try {
            ...
            final String XALAN2_2_VERSION_CLASS =
                    "org.openjdk.com.sun.org.apache.xalan.internal.Version";
    ...
    protected void checkParserVersion(Hashtable h) {
        ...
        try {
            final String XERCES1_VERSION_CLASS =
                    "org.openjdk.com.sun.org.apache.xerces.internal.framework.Version";
        ...
        try {
            final String XERCES2_VERSION_CLASS =
                    "org.openjdk.com.sun.org.apache.xerces.internal.impl.Version";
    ...
    /**
     * Manual table of known .jar sizes. Only includes shipped versions of certain projects.
     * key=jarsize, value=jarname ' from ' distro name Note assumption: two jars cannot have the
     * same size!
     *
     * @see #getApparentVersion(String, long)
     */
    ...
    /**
     * Static initializer for jarVersions table. Doing this just once saves time and space.
     *
     * @see #getApparentVersion(String, long)
     */
    static {
        ...
        jarVersions.put(
                new Long(424490), "xalan.jar from Xerces Tools releases - ERROR:DO NOT USE!");
        ...
        jarVersions.put(
                new Long(1808883), "xerces.jar from xalan-j_2_2_D10,D11,D12 or xerces-1_4_3.bin");
