...
final class ForEach extends Instruction {
    ...
    /**
     * The code that is generated by nested for-each loops can appear to some JVMs as if it is
     * accessing un-initialized variables. We must add some code that pushes the default variable
     * value on the stack and pops it into the variable slot. This is done by the
     * Variable.initialize() method. The code that we compile for this loop looks like this:
     *
     * <p>initialize iterator initialize variables <-- HERE!!! goto Iterate Loop: : : (code for
     * <xsl:for-each> contents) : Iterate: node = iterator.next(); if (node != END) goto Loop
     */
