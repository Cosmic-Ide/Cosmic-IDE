...
final class XslElement extends Instruction {
    ...
    /**
     * This method is now deprecated. The new implemation of this class never declares the default
     * NS.
     */
    ...
    /**
     * This method is called when the name of the element is known at compile time. In this case,
     * there is no need to inspect the element name at runtime to determine if a prefix exists,
     * needs to be generated, etc.
     */
    ...
    /**
     * At runtime the compilation of xsl:element results in code that: (i) evaluates the avt for the
     * name, (ii) checks for a prefix in the name (iii) generates a new prefix and create a new
     * qname when necessary (iv) calls startElement() on the handler (v) looks up a uri in the XML
     * when the prefix is not known at compile time (vi) calls namespace() on the handler (vii)
    ...
    public void translate(ClassGenerator classGen, MethodGenerator methodGen) {
        ...
        if (!_ignore) {
            ...
            final int check =
                    cpg.addMethodref(BASIS_LIBRARY_CLASS, "checkQName", "(" + STRING_SIG + ")V");
    ...
    /**
     * Override this method to make sure that xsl:attributes are not copied to output if this
     * xsl:element is to be ignored
     */
