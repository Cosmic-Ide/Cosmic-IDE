...
public final class ResultTreeType extends Type {
    ...
    /**
     * Translates a result tree to object of internal type <code>type</code>. The translation to int
     * is undefined since result trees are always converted to reals in arithmetic expressions.
     *
    ...
    /**
     * Expects an result tree on the stack and pushes a boolean. Translates a result tree to a
     * boolean by first converting it to string.
     *
     * @param classGen A BCEL class generator
     * @param methodGen A BCEL method generator
    ...
    /**
     * Expects an result tree on the stack and pushes a real. Translates a result tree into a real
     * by first converting it to string.
     *
     * @param classGen A BCEL class generator
     * @param methodGen A BCEL method generator
     ...
     * boxed so the translation is just a NOP.
     *
     * @param classGen A BCEL class generator
     * @param methodGen A BCEL method generator
    ...
    public void translateTo(
        ...
        if (_methodName == null) {
            ...
            il.append(methodGen.loadDOM());
            int index =
                    cpg.addInterfaceMethodref(DOM_INTF, "getResultTreeFrag", "(IZ)" + DOM_INTF_SIG);
            ...
            newDom =
                    methodGen.addLocalVariable(
                            "rt_to_reference_dom", Util.getJCRefType(DOM_INTF_SIG), null, null);
    ...
    /**
     * Expects a result tree on the stack and pushes a node-set (iterator). Note that the produced
     * iterator is an iterator for the DOM that contains the result tree, and not the DOM that is
     * currently in use. This conversion here will therefore not directly work with elements such as
     * <xsl:apply-templates> and <xsl:for-each> without the DOM parameter/variable being updates as
     * well.
     *
     * @param classGen A BCEL class generator
     * @param methodGen A BCEL method generator
    ...
    /**
     * Translates a result tree into a non-synthesized boolean. It does not push a 0 or a 1 but
     * instead returns branchhandle list to be appended to the false list.
     *
     * @param classGen A BCEL class generator
     * @param methodGen A BCEL method generator
     * @param type An instance of BooleanType (any)
    ...
    /**
     * Translates a result tree to a Java type denoted by <code>clazz</code>. Expects a result tree
     * on the stack and pushes an object of the appropriate type after coercion. Result trees are
     * translated to W3C Node or W3C NodeList and the translation is done via node-set type.
     *
     * @param classGen A BCEL class generator
     * @param methodGen A BCEL method generator
