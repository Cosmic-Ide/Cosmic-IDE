...
/**
 * A wrapper class that adapts the {@link DTMWSFilter DTMWSFilter} interface to the XSLTC DOM {@link
 * StripFilter StripFilter} interface.
 */
public class DOMWSFilter implements DTMWSFilter {
    ...
    /**
     * Construct an adapter connecting the <code>DTMWSFilter</code> interface to the <code>
     * StripFilter</code> interface.
     *
     ...
     * Normally, this function will be called by the implementation of <code>DTM</code>; it is not
     * normally called directly from user code.
     *
     * @param node int handle of the node.
     * @param dtm the DTM that owns this node
     * @return one of <code>NOTSTRIP</code>, <code>STRIP</code> or <code>INHERIT</code>.
     */
    public short getShouldStripSpace(int node, DTM dtm) {
        if (m_filter != null && dtm instanceof DOM) {
            ...
            if (dtm instanceof DOMEnhancedForDTM) {
                ...
                if (expType >= 0 && expType < mapping.length) type = mapping[expType];
                else type = -1;
