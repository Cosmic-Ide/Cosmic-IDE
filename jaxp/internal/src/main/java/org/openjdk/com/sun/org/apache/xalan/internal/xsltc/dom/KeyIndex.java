...
/**
 * Stores mappings of key values or IDs to DTM nodes. <em>Use of an instance of this class as a
 * {@link DTMAxisIterator} is <b>deprecated.</b></em>
 *
...
public class KeyIndex extends DTMAxisIteratorBase {
    ...
    /**
     * A mapping between values and nodesets for the current document. Used only while building
     * keys.
     */
    ...
    /**
     * Given a context node and the argument to the XPath <code>id</code> function, checks whether
     * the context node is in the set of nodes that results from that reference to the <code>id
     * </code> function. This is used in the implementation of <code>id</code> patterns.
     *
     * @param node The context node
     * @param value The argument to the <code>id</code> function
     * @return <code>1</code> if the context node is in the set of nodes returned by the reference
     *     to the <code>id</code> function; <code>0</code>, otherwise
     */
     ...
     * whether the context node is in the set of nodes that results from that reference to the
     * <code>key</code> function. This is used in the implementation of key patterns.
     *
    ...
    /**
     * Create a {@link KeyIndexIterator} that iterates over the nodes that result from a reference
     * to the XSLT <code>key</code> function or XPath <code>id</code> function.
     *
     * @param keyValue A string or iterator representing the key values or id references
     * @param isKeyCall A <code>boolean</code> indicating whether the iterator is being created for
     *     a reference <code>key</code> or <code>id</code>
     */
    ...
    /**
     * An iterator representing the result of a reference to either the XSLT <code>key</code>
     * function or the XPath <code>id</code> function.
     */
    public class KeyIndexIterator extends MultiValuedNodeHeapIterator {

        /**
         * A reference to the <code>key</code> function that only has one key value or to the <code>
         * id</code> function that has only one string argument can be optimized to ignore the
         * multi-valued heap. This field will be <code>null</code> otherwise.
         */
        private IntegerArray _nodes;

        /**
         * This field contains the iterator representing a node set key value argument to the <code>
         * key</code> function or a node set argument to the <code>id</code> function.
         *
         * <p>Exactly one of this field and {@link #_keyValue} must be <code>null</code>.
         */
        private DTMAxisIterator _keyValueIterator;

        /**
         * This field contains the iterator representing a non-node-set key value argument to the
         * <code>key</code> function or a non-node-set argument to the <code>id</code> function.
         *
         * <p>Exactly one of this field and {@link #_keyValueIterator} must be <code>null</code>.
         */
        private String _keyValue;

        /**
         * Indicates whether this object represents the result of a reference to the <code>key
         * </code> function (<code>true</code>) or the <code>id</code> function (<code>false</code>
         * ).
         */
        private boolean _isKeyIterator;

        /**
         * Represents the DTM nodes retrieved for one key value or one string argument to <code>id
         * </code> for use as one heap node in a {@link MultiValuedNodeHeapIterator}.
         */
        protected class KeyIndexHeapNode extends MultiValuedNodeHeapIterator.HeapNode {
            /**
             * {@link IntegerArray} of DTM nodes retrieved for one key value. Must contain no
             * duplicates and be stored in document order.
             */
            private IntegerArray _nodes;

            /** Position in {@link #_nodes} array of next node to return from this heap node. */
            private int _position = 0;

            /** Marked position. Used by {@link #setMark()} and {@link #gotoMark()} */
            private int _markPosition = -1;

            /**
             * Create a heap node representing DTM nodes retrieved for one key value in a reference
             * to the <code>key</code> function or string argument to the <code>id</code> function.
             */
             ...
             * @return <code>true</code> if and only if the current node for this heap node is
             *     before the current node of the argument heap node in document order.
             */
             ...
             *
             * <p>This has no real effect on this kind of heap node. Instead, the {@link
             * KeyIndexIterator#setStartNode(int)} method should create new instances of this class
             * to represent the effect of changing the context.
             */
        ...
        /**
         * Constructor used when the argument to <code>key</code> or <code>id</code> is not a node
         * set.
         *
         * @param keyValue the argument to <code>key</code> or <code>id</code> cast to a <code>
         *     String</code>
         * @param isKeyIterator indicates whether the constructed iterator represents a reference to
         *     <code>key</code> or <code>id</code>.
         */
        ...
        /**
         * Constructor used when the argument to <code>key</code> or <code>id</code> is a node set.
         *
        ...
        /**
         * Set context node for the iterator. This will cause the iterator to reset itself,
         * reevaluate arguments to the function, look up nodes in the input and reinitialize its
         * internal heap.
         *
         * @param node the context node
        ...
        /**
         * Evaluate the reference to the <code>key</code> or <code>id</code> function with the
         * context specified by {@link #setStartNode(int)} and set up this iterator to iterate over
         * the DTM nodes that are to be returned.
         */
        protected void init() {
            super.init();
            _position = 0;
