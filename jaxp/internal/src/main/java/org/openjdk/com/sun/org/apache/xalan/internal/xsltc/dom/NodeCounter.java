...
public abstract class NodeCounter {
    ...
    /**
     * Set the start node for this counter. The same <tt>NodeCounter</tt> object can be used
     * multiple times by resetting the starting node.
     */
    ...
    protected void setFormatting(
            String format, String lang, String letterValue, String groupSep, String groupSize) {
        _lang = lang;
        _groupSep = groupSep;
    ...
    /**
     * Effectively does the same thing as Integer.parseInt(String s) except instead of throwing a
     * NumberFormatException, it returns 0. This method is used instead of Integer.parseInt() since
     * it does not incur the overhead of throwing an Exception which is expensive.
     *
     * @param s A String to be parsed into an int.
     * @return Either an int represented by the incoming String s, or 0 if the parsing is not
     *     successful.
     */
    ...
    /**
     * Returns the position of <tt>node</tt> according to the level and the from and count patterns.
     * This position is converted into a string based on the arguments passed.
     */
    ...
    /**
     * Returns true if <tt>node</tt> matches the count pattern. By default a node matches the count
     * patterns if it is of the same type as the starting node.
     */
    ...
    /**
     * Returns true if <tt>node</tt> matches the from pattern. By default, no node matches the from
     * pattern.
     */
    ...
    /**
     * Format a sequence of values according to the format paramaters set by calling
     * setFormatting().
     */
    ...
    /**
     * Format a single value based on the appropriate formatting token. This method is based on
     * saxon (Michael Kay) and only implements lang="en".
     */
