...
import org.openjdk.com.sun.org.apache.xalan.internal.xsltc.NodeIterator;
...
public abstract class NodeIteratorBase implements NodeIterator {
    ...
    /**
     * Initialize iterator using a node. If iterator is not restartable, then do nothing. If node is
     * equal to END then subsequent calls to next() must return END.
     */
    ...
    /**
     * Default implementation of getLast(). Stores current position and current node, resets the
     * iterator, counts all nodes and restores iterator to original state.
     */
    public int getLast() {
        if (_last == -1) {
    ...
    /**
     * Indicates if position in this iterator is computed in reverse document order. Note that nodes
     * are always returned in document order.
     */
    ...
    /**
     * Clones and resets this iterator. Note that the cloned iterator is not restartable. This is
     * because cloning is needed for variable references, and the context node of the original
