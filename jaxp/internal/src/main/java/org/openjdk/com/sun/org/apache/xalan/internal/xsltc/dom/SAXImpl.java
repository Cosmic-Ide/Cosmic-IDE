...
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.DTMAxisIterNodeList;
...
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.EmptyIterator;
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.sax2dtm.SAX2DTM2;
import org.openjdk.com.sun.org.apache.xml.internal.serializer.SerializationHandler;
import org.openjdk.com.sun.org.apache.xml.internal.serializer.ToXMLSAXHandler;
...
/**
 * SAXImpl is the core model for SAX input source. SAXImpl objects are usually created from an
 * XSLTCDTMManager.
 *
 * <p>DOMSource inputs are handled using DOM2SAX + SAXImpl. SAXImpl has a few specific fields (e.g.
 * _node2Ids, _document) to keep DOM-related information. They are used when the processing behavior
 * between DOM and SAX has to be different. Examples of these include id function and unparsed
 * entity.
 *
...
public final class SAXImpl extends SAX2DTM2 implements DOMEnhancedForDTM, DOMBuilder {
    ...
    public void setupMapping(
            String[] names, String[] urisArray, int[] typesArray, String[] namespaces) {
        // This method only has a function in DOM adapters
    }

    /**
     * Lookup a namespace URI from a prefix starting at node. This method is used in the execution
     * of xsl:element when the prefix is not known at compile time.
     */
    ...
    /**
     * Create an org.w3c.dom.Node from a node in an iterator The iterator most be started before
    ...
    public NodeList makeNodeList(int index) {
        ...
        if (nodeID < 0) {
            ...
            return (_nodeLists[nodeID] != null)
                    ? _nodeLists[nodeID]
                    : (_nodeLists[nodeID] =
                            new DTMAxisIterNodeList(this, new SingletonIterator(index)));
        } else {
    ...
    /**
     * Create an org.w3c.dom.NodeList from a node iterator The iterator most be started before this
     * method is called
     */
    ...
    public DTMAxisIterator getNodeValueIterator(
            DTMAxisIterator iterator, int type, String value, boolean op) {
    ...
    /**
     * Returns singleton iterator containg the document root Works for them main document (mark ==
     * 0). It cannot be made to point to any other node through setStartNode().
     */
    ...
    /**
     * Get mapping from DOM element/attribute types to external types. This method is used when the
     * document is not fully built.
     */
    private short[] getMapping2(String[] names, String[] uris, int[] types) {
        ...
        for (i = 0; i < namesLength; i++) {
            ...
            generalizedTypes[i] =
                    m_expandedNameTable.getExpandedTypeID(uris[i], names[i], types[i], false);
    ...
    /**
     * Call this when an xml:space attribute is encountered to define the whitespace strip/preserve
     * settings.
     */
    ...
    /**
     * Call this from endElement() to revert strip/preserve setting to whatever it was before the
    ...
    /**
     * Specialized interface used by DOM2SAX. This one has an extra Node parameter to build the Node
     * -> id map.
     */
    public void startElement(
            String uri, String localName, String qname, Attributes attributes, Node node)
            throws SAXException {
    ...
    /**
     * SAX2: Receive notification of ignorable whitespace in element content. Similar to
    ...
    public void print(int node, int level) {
        switch (getNodeType(node)) {
            ...
            default:
                ...
                System.out.print('>');
                for (int child = getFirstChild(node);
                        child != DTM.NULL;
    ...
    /**
     * Similar to getAxisIterator, but this one returns an iterator containing nodes of a typed axis
     * (ex.: child::foo)
     */
    ...
    /**
     * Do not think that this returns an iterator for the namespace axis. It returns an iterator
     * with nodes that belong in a certain namespace, such as with <xsl:apply-templates
     * select="blob/foo:*"/> The 'axis' specifies the axis for the base iterator from which the
    ...
    /**
     * Iterator that handles node tests that test for a namespace, but have a wild card for the
     * local name of the node, i.e., node tests of the form <axis>::<prefix>:*
     */
    public final class NamespaceWildcardIterator extends InternalAxisIteratorBase {
        ...
        /**
         * A nested typed axis iterator that retrieves nodes of the principal node kind for that
         * axis.
         */
    ...
    /**
     * Iterator that returns children within a given namespace for a given node. The functionality
     * chould be achieved by putting a filter on top of a basic child iterator, but a specialised
     ...
     *
     * <p>TODO: Copy namespace declarations. Can't be done until we add namespace nodes and keep
     * track of NS prefixes TODO: Copy comment nodes
     */
    ...
    public String getLanguage(int node) {
        ...
        while (DTM.NULL != parent) {
            if (DTM.ELEMENT_NODE == getNodeType(parent)) {
                int langAttr =
                        getAttributeNode(parent, "http://www.w3.org/XML/1998/namespace", "lang");
    ...
    /**
     * Returns an instance of the DOMBuilder inner class This class will consume the input document
     * through a SAX2 interface and populate the tree.
     */
    ...
    /**
     * Return a SerializationHandler for output handling. This method is used by Result Tree
     * Fragments.
     */
    ...
    public DOM getResultTreeFrag(int initSize, int rtfType, boolean addToManager) {
        if (rtfType == DOM.SIMPLE_RTF) {
            if (addToManager) {
                ...
                SimpleResultTreeImpl rtf =
                        new SimpleResultTreeImpl(
            ...
            if (addToManager) {
                int dtmPos = _dtmManager.getFirstFreeDTMID();
                AdaptiveResultTreeImpl rtf =
                        new AdaptiveResultTreeImpl(
                                _dtmManager,
                ...
                return new AdaptiveResultTreeImpl(
                        _dtmManager, 0, m_wsfilter, initSize, m_buildIdIndex);
            }
        } else {
            return (DOM)
                    _dtmManager.getDTM(
                            null, true, m_wsfilter, true, false, false, initSize, m_buildIdIndex);
        }
    }
