...
import org.openjdk.com.sun.org.apache.xalan.internal.xsltc.dom.AbsoluteIterator;
import org.openjdk.com.sun.org.apache.xalan.internal.xsltc.dom.ArrayNodeListIterator;
import org.openjdk.com.sun.org.apache.xalan.internal.xsltc.dom.DOMAdapter;
import org.openjdk.com.sun.org.apache.xalan.internal.xsltc.dom.MultiDOM;
import org.openjdk.com.sun.org.apache.xalan.internal.xsltc.dom.SingletonIterator;
import org.openjdk.com.sun.org.apache.xalan.internal.xsltc.dom.StepIterator;
...
import org.openjdk.com.sun.org.apache.xml.internal.serializer.NamespaceMappings;
...
/**
 * Standard XSLT functions. All standard functions expect the current node and the DOM as their last
 * two arguments.
 */
public final class BasisLibrary {
    ...
    /**
     * Re-use a single instance of StringBuffer (per thread) in the basis library. StringBuilder is
     * better, however, DecimalFormat only accept StringBuffer
     */
    ...
    public static double roundF(double d) {
        return (d < -0.5 || d > 0.0)
                ? Math.floor(d + 0.5)
                : ((d == 0.0) ? d : (Double.isNaN(d) ? Double.NaN : -0.0));
    }
    ...
    /**
     * XSLT Standard function substring(). Must take a double because of conversions resulting into
     * NaNs and rounding.
     */
    ...
    public static String substringF(String value, double start, double length) {
        if (Double.isInfinite(start) || Double.isNaN(start) || Double.isNaN(length))
            return (EMPTYSTRING);
    ...
    public static String substring_afterF(String value, String substring) {
        ...
        if (index >= 0) return value.substring(index + substring.length());
    ...
    public static String translateF(String value, String from, String to) {
        ...
        for (int j, i = 0; i < valuel; i++) {
            ...
            for (j = 0; j < froml; j++) {
                if (ch == from.charAt(j)) {
                    if (j < tol) result.append(to.charAt(j));
                    break;
                }
            }
            if (j == froml) result.append(ch);
        }
    ...
    public static String normalize_spaceF(String value) {
        ...
        while (i < n && isWhiteSpace(value.charAt(i))) i++;
    ...
    /**
     * External functions that cannot be resolved are replaced with a call to this method. This
     * method will generate a runtime errors. A good stylesheet checks whether the function exists
     * using conditional constructs, and never really tries to call it if it doesn't exist. But
     * simple stylesheets may result in a call to this method. The compiler should generate a
     * warning if it encounters a call to an unresolved external function.
     */
    ...
    /**
     * Utility function to throw a runtime error on the use of an extension function when the secure
    ...
    /**
     * Utility function to throw a runtime error on the use of an extension element when the secure
     * processing feature is set to true.
     */
     ...
     *
     * <p>This is only used in forward-compatibility mode, when the control flow cannot be
     * determined. In 1.0 mode, the error message is emitted at compile time.
     */
    public static void unsupported_ElementF(String qname, boolean isExtension) {
        if (isExtension) runTimeError(UNSUPPORTED_EXT_ERR, qname);
        else runTimeError(UNSUPPORTED_XSL_ERR, qname);
    }
    ...
    public static String system_propertyF(String name) {
        if (name.equals("xsl:version")) return ("1.0");
        if (name.equals("xsl:vendor")) return ("Apache Software Foundation (Xalan XSLTC)");
        if (name.equals("xsl:vendor-url")) return ("http://xml.apache.org/xalan-j");
    ...
    public static String namespace_uriF(int node, DOM dom) {
        ...
        if (colon >= 0) return value.substring(0, colon);
        else return EMPTYSTRING;
    }
    ...
    public static String objectTypeF(Object obj) {
        ...
        else if (obj instanceof DOM) return "RTF";
        else if (obj instanceof DTMAxisIterator) return "node-set";
        else return "unknown";
    }
    ...
    public static boolean compare(
            DTMAxisIterator left, final double rnumber, final int op, DOM dom) {
        int node;
        // left.reset();
        ...
        switch (op) {
            case Operators.EQ:
                while ((node = left.next()) != DTMAxisIterator.END) {
                    if (numberF(dom.getStringValueX(node), dom) == rnumber) return true;
                }
                break;
    ...
    public static boolean testLanguage(String testLang, DOM dom, int node) {
        ...
        if (nodeLang == null) return (false);
        else nodeLang = nodeLang.toLowerCase();
    ...
    /**
     * Utility function: used in RealType to convert a real to a string. Removes the decimal if
     * null. Uses a specialized formatter object for very large and very small numbers that ignores
     * locales, thus using always using "." as a decimal separator.
     */
    ...
    /**
     * Utility function: used to format/adjust a double to a string. The DecimalFormat object comes
     * from the 'formatSymbols' hashtable in AbstractTranslet.
     */
    ...
    /**
     * Utility function: used to convert references to node-sets. If the obj is an instanceof Node
     * then create a singleton iterator.
     */
    ...
    public static DTMAxisIterator node2Iterator(org.w3c.dom.Node node, Translet translet, DOM dom) {
        ...
        org.w3c.dom.NodeList nodelist =
                new org.w3c.dom.NodeList() {
                    public int getLength() {
                        return 1;
                    }

                    public org.w3c.dom.Node item(int index) {
                        if (index == 0) return inNode;
                        else return null;
                    }
                };
    ...
    /**
     * In a perfect world, this would be the implementation for nodeList2Iterator. In reality,
     * though, this causes a ClassCastException in getDTMHandleFromNode because SAXImpl is not an
     * instance of DOM2DTM. So we use the more lengthy implementation below until this issue has
     * been addressed.
     *
    ...
    private static DTMAxisIterator nodeList2IteratorUsingHandleFromNode(
        ...
        if (dom instanceof MultiDOM) dtmManager = ((MultiDOM) dom).getDTMManager();
        for (int i = 0; i < n; ++i) {
    ...
    public static DTMAxisIterator nodeList2Iterator(
        ...
        if (n != dtmHandles.length) throw new InternalRuntimeError("Nodes lost in second pass");
    ...
    /**
     * Utility function: used with nth position filters to convert a sequence of nodes to just one
     * single node (the one at position n).
     */
    ...
    public static void copy(Object obj, SerializationHandler handler, int node, DOM dom) {
        try {
            if (obj instanceof DTMAxisIterator) {
                ...
                if (length > _characterArray.length) _characterArray = new char[length];
    ...
    /**
     * Utility function to check if xsl:attribute has a valid qname This method should only be
     * invoked if the name attribute is an AVT
     */
        ...
        if (firstOccur > 0) {
            ...
            if (firstOccur != lastOccur) {
                ...
                if (!XML11Char.isXML11ValidNCName(oriPrefix)) {
                    // even though the orignal prefix is ignored, it should still get checked for
                    // valid NCName
    ...
    /**
     * Utility function to check if a name is a valid ncname This method should only be invoked if
     * the attribute value is an AVT
     */
    ...
    /**
     * Utility function to check if a name is a valid qname This method should only be invoked if
     * the attribute value is an AVT
     */
    ...
    public static String startXslElement(
            String qname, String namespace, SerializationHandler handler, DOM dom, int node) {
        try {
    ...
    static {
        String resource =
                "org.openjdk.com.sun.org.apache.xalan.internal.xsltc.runtime.ErrorMessages";
    ...
    /**
     * Utility method to allow setting parameters of the form {namespaceuri}localName which get
     * mapped to an instance variable in the class Hence a parameter of the form
     * "{http://foo.bar}xyz" will be replaced with the corresponding values by the BasisLibrary's
     * utility method mapQNametoJavaName and thus get mapped to legal java variable names
     */
    public static String mapQNameToJavaName(String base) {
        return replace(
                ...
                ".-:/{}?#%*",
                new String[] {
                    "$dot$", "$dash$", "$colon$", "$slash$", "", "$colon$", "$ques$", "$hash$",
                    "$per$", "$aster$"
                });
    }
