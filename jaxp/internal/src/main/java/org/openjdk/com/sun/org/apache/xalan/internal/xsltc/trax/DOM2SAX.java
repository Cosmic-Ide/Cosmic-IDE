...
public class DOM2SAX implements XMLReader, Locator {
    ...
    /**
     * Begin the scope of namespace prefix. Forward the event to the SAX handler only if the prefix
     * is unknown or it is mapped to a different URI.
     */
    ...
    /**
     * If the DOM was created using a DOM 1.0 API, the local name may be null. If so, get the local
     * name from the qualified name before generating the SAX event.
     */
    ...
    /**
     * Traverse the DOM and generate SAX events for a handler. A startElement() event passes all
