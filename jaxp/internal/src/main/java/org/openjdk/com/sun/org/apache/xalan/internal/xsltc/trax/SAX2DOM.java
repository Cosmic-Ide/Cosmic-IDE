...
public class SAX2DOM implements ContentHandler, LexicalHandler, Constants {
    ...
    /**
     * JAXP document builder factory. Create a single instance and use synchronization because the
     * Javadoc is not explicit about thread safety.
     */
    ...
    public SAX2DOM(Node root, Node nextSibling, boolean useServicesMechanism)
    ...
    private void appendTextNode() {
        if (_textBuffer.length() > 0) {
            ...
            if (last == _root && _nextSiblingCache != null) {
                _lastSibling =
                        last.insertBefore(
                                _document.createTextNode(_textBuffer.toString()),
                                _nextSiblingCache);
            } else {
    ...
    public void startElement(String namespace, String localName, String qName, Attributes attrs) {
        ...
        if (_namespaceDecls != null) {
            ...
            for (int i = 0; i < nDecls; i++) {
                ...
                if (prefix == null || prefix.equals(EMPTYSTRING)) {
                    tmp.setAttributeNS(
                            XMLNS_URI, XMLNS_PREFIX, (String) _namespaceDecls.elementAt(i));
                } else {
                    tmp.setAttributeNS(
                            XMLNS_URI,
                            XMLNS_STRING + prefix,
        ...
        if (last == _root && _nextSibling != null) last.insertBefore(tmp, _nextSibling);
        else last.appendChild(tmp);
    ...
    public void processingInstruction(String target, String data) {
        ...
        if (pi != null) {
            if (last == _root && _nextSibling != null) last.insertBefore(pi, _nextSibling);
            else last.appendChild(pi);
    ...
    public void comment(char[] ch, int start, int length) {
        ...
        if (comment != null) {
            if (last == _root && _nextSibling != null) last.insertBefore(comment, _nextSibling);
            else last.appendChild(comment);
    ...
    private Document createDocument(boolean useServicesMechanism)
            throws ParserConfigurationException {
        if (_factory == null) {
