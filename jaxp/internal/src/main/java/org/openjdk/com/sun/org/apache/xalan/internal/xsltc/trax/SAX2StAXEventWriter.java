...
public class SAX2StAXEventWriter extends SAX2StAXBaseWriter {
    ...
    private void writeStartDocument() throws SAXException {
        try {
            if (docLocator == null) writer.add(eventFactory.createStartDocument());
            else {
                try {
                    writer.add(
        ...
        try {
            ...
            writer.add(
                    eventFactory.createStartElement(
                            qname[0], uri, qname[1], events[1].iterator(), events[0].iterator()));
        ...
        for (int i = 0, s = attributes.getLength(); i < s; i++) {
            ...
            if ("xmlns".equals(attrQName) || "xmlns".equals(attrPrefix)) {
                ...
                if (attrPrefix.length() > 0) {
                    ...
                    attribute =
                            eventFactory.createAttribute(attrPrefix, attrURI, attrLocal, attrValue);
