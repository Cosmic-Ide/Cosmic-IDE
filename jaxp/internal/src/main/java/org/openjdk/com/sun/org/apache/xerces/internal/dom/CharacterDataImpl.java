 ...
 *
 * <p>Since ProcessingInstructionImpl inherits from this class to reuse the setNodeValue method,
 * this class isn't declared as implementing the interface CharacterData. This is done by relevant
 * subclasses (TexImpl, CommentImpl).
 *
...
public abstract class CharacterDataImpl extends ChildNode {
    ...
    /**
     * Convenience wrapper for calling setNodeValueInternal when we are not performing a replacement
     * operation
     */
    ...
    /**
     * This function added so that we can distinguish whether setNodeValue has been called from some
     * other DOM functions. or by the client.
     *
     * <p>This is important, because we do one type of Range fix-up, from the high-level functions
     * in CharacterData, and another type if the client simply calls setNodeValue(value).
     */
    ...
    /**
     * Sets the content, possibly firing related events, and updating ranges (via notification to
     * the document)
     */
     ...
     *     the permitted length of strings. If so, getData() will throw this DOMException advising
     *     the user to instead retrieve the data in chunks via the substring() operation.
     */
    ...
    /**
     * Report number of characters currently stored in this node's data. It may be 0, meaning that
    ...
    /**
     * Concatenate additional characters onto the end of the data stored in this node. Note that
     * this, and insert(), are the paths by which a DOM could wind up accumulating more data than
     * the language's strings can easily handle. (See above discussion.)
     *
     ...
     *
     * @throws DOMException(INDEX_SIZE_ERR) if offset is negative or greater than length, or if
     *     count is negative.
     * @throws DOMException(NO_MODIFICATION_ALLOWED_ERR) if node is readonly.
     */
     ...
     * events are spawned. This version of the deleteData operation allows us to do so. It is not
    ...
    /**
     * NON-DOM INTERNAL: Within DOM actions, we sometimes need to be able to control which mutation
     * events are spawned. This version of the insertData operation allows us to do so. It is not
     * intended for use by application programs.
     */
     ...
     * @param data The new string to be inserted at offset in place of the removed data. Note that
     *     the entire string will be inserted -- the count parameter does not affect insertion, and
     *     the new data may be longer or shorter than the substring it replaces.
     * @throws DOMException(INDEX_SIZE_ERR) if offset is negative or greater than length, or if
     *     count is negative.
     * @throws DOMException(NO_MODIFICATION_ALLOWED_ERR) if node is readonly.
     */
     ...
     * stored data may exceed the length that can be returned in a single string, the only way to
     ...
     * @throws DOMException(INDEX_SIZE_ERR) if offset is negative or greater than length, or if
     *     count is negative.
     * @throws DOMException(WSTRING_SIZE_ERR) In some implementations, count may exceed the
     *     permitted length of strings. If so, substring() will throw this DOMException advising the
