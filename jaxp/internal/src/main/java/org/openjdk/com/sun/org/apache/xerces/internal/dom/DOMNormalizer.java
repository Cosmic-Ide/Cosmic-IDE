...
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dtd.DTDGrammar;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dtd.XMLDTDDescription;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dtd.XMLDTDValidator;
...
import org.openjdk.com.sun.org.apache.xerces.internal.parsers.XMLGrammarPreparser;
...
import org.openjdk.com.sun.org.apache.xerces.internal.util.XMLGrammarPoolImpl;
...
/**
 * This class adds implementation for normalizeDocument method. It acts as if the document was going
 * through a save and load cycle, putting the document in a "normal" form. The actual result depends
 * on the features being set and governing what operations actually take place. See
 * setNormalizationFeature for details. Noticeably this method normalizes Text nodes, makes the
 * document "namespace wellformed", according to the algorithm described below in pseudo code, by
 * adding missing namespace declaration attributes and adding or changing namespace prefixes,
 * updates the replacement tree of EntityReference nodes, normalizes attribute values, etc. Mutation
 * events, when supported, are generated to reflect the changes occuring on the document. See
 * Namespace normalization for details on how namespace declaration attributes and prefixes are
 * normalized.
 *
 * <p>NOTE: There is an initial support for DOM revalidation with XML Schema as a grammar. The tree
 * might not be validated correctly if entityReferences, CDATA sections are present in the tree. The
 * PSVI information is not exposed, normalized data (including element default content is not
 * available).
 *
 * @xerces.experimental
...
public class DOMNormalizer implements XMLDocumentHandler {
    ...
    protected void normalizeDocument(CoreDocumentImpl document, DOMConfigurationImpl config) {
        ...
        if ((fConfiguration.features & DOMConfigurationImpl.VALIDATE) != 0) {
            String schemaLang =
                    (String) fConfiguration.getProperty(DOMConfigurationImpl.JAXP_SCHEMA_LANGUAGE);

            if (schemaLang != null && schemaLang.equals(Constants.NS_XMLSCHEMA)) {
                fValidationHandler =
                        CoreDOMImplementationImpl.singleton.getValidator(
        ...
        try {
            ...
            if (e == abort) return; // processing aborted by the user
     ...
     * document in a "normal" form. The actual result depends on the features being set and
     * governing what operations actually take place. See setNormalizationFeature for details.
     ...
     *
     * @param node Modified node or null. If node is returned, we need to normalize again starting
     *     on the node returned.
            ...
            case Node.DOCUMENT_TYPE_NODE:
                {
                    ...
                    fConfiguration.setProperty(
                            ...
                            createGrammarPool(docType));
                    fDTDValidator.reset(fConfiguration);
                    fDTDValidator.startDocument(
                            new SimpleLocator(
                                    fDocument.fDocumentURI, fDocument.fDocumentURI, -1, -1),
                            fDocument.encoding,
                            fNamespaceContext,
                            null);
                    fDTDValidator.doctypeDecl(
                            docType.getName(), docType.getPublicId(), docType.getSystemId(), null);
            ...
            case Node.ELEMENT_NODE:
                {
                            ...
                            if (fNamespaceValidation) {
                                ...
                                wellformed =
                                        CoreDocumentImpl.isXMLName(
                                                node.getNodeName(), fDocument.isXML11Version());
                            }
                            if (!wellformed) {
                                String msg =
                    ...
                    AttributeMap attributes =
                            (elem.hasAttributes()) ? (AttributeMap) elem.getAttributes() : null;
                            ...
                            for (int i = 0; i < attributes.getLength(); ++i) {
                                ...
                                if (fDocument.errorChecking
                                        && ((fConfiguration.features
                                                        & DOMConfigurationImpl.WELLFORMED)
                                                != 0)) {
                                    isAttrValueWF(
                                            fErrorHandler,
                                            fError,
                    ...
                    if (DEBUG_ND) {
                        // normalized subtree
                        System.out.println(
                                "***The children of {" + node.getNodeName() + "} are normalized");
                        for (kid = elem.getFirstChild(); kid != null; kid = next) {
                            next = kid.getNextSibling();
            ...
            case Node.COMMENT_NODE:
                {
                    ...
                    if ((fConfiguration.features & DOMConfigurationImpl.COMMENTS) == 0) {
                        ...
                        if (fDocument.errorChecking
                                && ((fConfiguration.features & DOMConfigurationImpl.WELLFORMED)
                                        != 0)) {
            ...
            case Node.CDATA_SECTION_NODE:
                {
                    ...
                    if ((fConfiguration.features & DOMConfigurationImpl.SPLITCDATA) != 0) {
                        ...
                        isCDataWF(
                                fErrorHandler, fError, fLocator, value, fDocument.isXML11Version());
                    }
                    break;
                }
            ...
            case Node.TEXT_NODE:
                {
                    ...
                    if (next != null && next.getNodeType() == Node.TEXT_NODE) {
                        ...
                        if (nextType == -1
                                || !(((fConfiguration.features & DOMConfigurationImpl.ENTITIES) == 0
                                                && nextType == Node.ENTITY_NODE)
                                        || ((fConfiguration.features
                                                                & DOMConfigurationImpl.COMMENTS)
                                                        == 0
            ...
            case Node.PROCESSING_INSTRUCTION_NODE:
                {
                    ...
                    // do the well-formed valid PI target name , data check when application has set
                    // the value of well-formed feature to true
                    if (fDocument.errorChecking
                            && (fConfiguration.features & DOMConfigurationImpl.WELLFORMED) != 0) {
    ...
    private XMLGrammarPool createGrammarPool(DocumentTypeImpl docType) {
        ...
        preParser.setProperty(
                Constants.XERCES_PROPERTY_PREFIX + Constants.XMLGRAMMAR_POOL_PROPERTY, pool);

        String internalSubset = docType.getInternalSubset();
        XMLInputSource is = new XMLInputSource(docType.getPublicId(), docType.getSystemId(), null);

        if (internalSubset != null) is.setCharacterStream(new StringReader(internalSubset));
        try {
            DTDGrammar g =
                    (DTDGrammar) preParser.preparseGrammar(XMLGrammarDescription.XML_DTD, is);
        ...
        if (attributes != null) {
            ...
            for (int k = 0; k < attributes.getLength(); ++k) {
                ...
                if (uri != null && uri.equals(NamespaceContext.XMLNS_URI)) {
                    ...
                    // "namespace-declarations" == false; Discard all namespace declaration
                    // attributes
                    if ((fConfiguration.features & DOMConfigurationImpl.NSDECL) == 0) {
                        continue;
                    }
            ...
            if (element.getLocalName() == null) {
                ...
                if (uri != null && uri.length() > 0) {
                    ...
                    fNamespaceContext.declarePrefix(
                            XMLSymbols.EMPTY_STRING, XMLSymbols.EMPTY_STRING);
                }
            }
        }
                ...
                if (uri != null) { // attribute has namespace !=null
                    ...
                    if (fDocument.errorChecking
                        ...
                        if (fDocument.isXMLVersionChanged()) {
                            boolean wellformed =
                                    CoreDocumentImpl.isXMLName(
                                            attr.getNodeName(), fDocument.isXML11Version());
                            if (!wellformed) {
                                String msg =
                                        DOMMessageFormatter.formatMessage(
                                                DOMMessageFormatter.DOM_DOMAIN,
    ...
    /**
     * Adds a namespace attribute or replaces the value of existing namespace attribute with the
     * given prefix and value for URI. In case prefix is empty will add/update default namespace
     * declaration.
     *
     * @param prefix
     * @param uri
     ...
     */
    public static final void isCDataWF(
                ...
                if (XML11Char.isXML11Invalid(c)) {
                    ...
                    if (count < datalength && dataarray[count] == ']') {
                        ...
                        if (count < datalength && dataarray[count] == '>') {
                            ...
                            String msg =
                                    DOMMessageFormatter.formatMessage(
                                            DOMMessageFormatter.XML_DOMAIN, "CDEndInContent", null);
                            reportDOMError(
                                    errorHandler,
                                    error,
                                    locator,
     ...
     * @param isXML11Version = true if XML 1.1
     */
    public static final void isXMLCharWF(
            DOMErrorHandler errorHandler,
            DOMErrorImpl error,
            DOMLocatorImpl locator,
            String datavalue,
     ...
     * @param isXML11Version = true if XML 1.1
     */
    public static final void isCommentWF(
            DOMErrorHandler errorHandler,
            DOMErrorImpl error,
            DOMLocatorImpl locator,
            String datavalue,
        ...
        if (isXML11Version) {
            ...
            while (i < datalength) {
                ...
                if (XML11Char.isXML11Invalid(c)) {
                    ...
                    String msg =
                            DOMMessageFormatter.formatMessage(
                                    DOMMessageFormatter.XML_DOMAIN, "DashDashInComment", null);
                    // invalid: '--' in comment
                    reportDOMError(
                            errorHandler,
                            error,
     ...
     * @param value
     */
    public static final void isAttrValueWF(
            DOMErrorHandler errorHandler,
            DOMErrorImpl error,
            DOMLocatorImpl locator,
     ...
     *
     * <p>If the error is fatal, the processing will be always aborted.
     */
    public static final void reportDOMError(
            DOMErrorHandler errorHandler,
            DOMErrorImpl error,
            DOMLocatorImpl locator,
            String message,
    ...
    protected final void updateQName(Node node, QName qname) {
        ...
        qname.prefix =
                (prefix != null && prefix.length() != 0) ? fSymbolTable.addSymbol(prefix) : null;
    ...
    protected final class XMLAttributesProxy implements XMLAttributes {
        ...
        public int addAttribute(QName qname, String attrType, String attrValue) {
            ...
            if (index < 0) {
                ...
                AttrImpl attr =
                        (AttrImpl)
                                ((CoreDocumentImpl) fElement.getOwnerDocument())
                                        .createAttributeNS(
                                                qname.uri, qname.rawname, qname.localpart);
     ...
     *     during the parsing of this document. However, it is <em>strongly</em> recommended that a
     *     locator be supplied that can at least report the system identifier of the document.
    ...
    public void endElement(QName element, Augmentations augs) throws XNIException {
        ...
        if (augs != null) {
            ...
            if (elementPSVI != null) {
                ...
                if ((fConfiguration.features & DOMConfigurationImpl.DTNORMALIZATION) != 0) {
                    if (normalizedValue != null) elementNode.setTextContent(normalizedValue);
                } else {
