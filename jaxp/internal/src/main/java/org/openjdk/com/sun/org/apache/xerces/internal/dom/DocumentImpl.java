...
import org.openjdk.com.sun.org.apache.xerces.internal.dom.events.EventImpl;
import org.openjdk.com.sun.org.apache.xerces.internal.dom.events.MutationEventImpl;
 ...
 *
 * <p>The DocumentImpl class also implements the DOM Level 2 DocumentTraversal interface. This
 * interface is comprised of factory methods needed to create NodeIterators and TreeWalkers. The
 * process of creating NodeIterator objects also adds these references to this document. After
 * finishing with an iterator it is important to remove the object using the remove methods in this
 * implementation. This allows the release of the references from the iterator objects to the DOM
 * Nodes.
 *
...
public class DocumentImpl extends CoreDocumentImpl
    ...
    /**
     * NON-DOM extension: Create and return a NodeIterator. The NodeIterator is added to a list of
     * NodeIterators so that it can be removed to free up the DOM Nodes it references.
     *
    ...
    /**
     * Create and return a NodeIterator. The NodeIterator is added to a list of NodeIterators so
     * that it can be removed to free up the DOM Nodes it references.
     *
     * @param root The root of the iterator.
     * @param whatToShow The whatToShow mask.
     * @param filter The NodeFilter installed. Null means no filter.
    ...
    public NodeIterator createNodeIterator(
            Node root, int whatToShow, NodeFilter filter, boolean entityReferenceExpansion) {
        ...
        NodeIterator iterator =
                new NodeIteratorImpl(this, root, whatToShow, filter, entityReferenceExpansion);
        if (iterators == null) {
    ...
    /**
     * This is not called by the developer client. The developer client uses the detach() function
     * on the NodeIterator itself.
     *
     * <p>This function is called from the NodeIterator#detach().
     */
    ...
    /**
     * Not a client function. Called by Range.detach(), so a Range can remove itself from the list
     * of Ranges.
     */
     ...
     *
     * <p>Create and return Event objects.
     *
     * @param type The eventType parameter specifies the type of Event interface to be created. If
     *     the Event interface specified is supported by the implementation this method will return
     *     a new Event of the interface type requested. If the Event is to be dispatched via the
     *     dispatchEvent method the appropriate event init method must be called after creation in
     *     order to initialize the Event's values. As an example, a user wishing to synthesize some
     *     kind of Event would call createEvent with the parameter "Events". The initEvent method
     *     could then be called on the newly created Event to set the specific type of Event to be
     *     dispatched and set its context information.
     * @return Newly created Event
     * @exception DOMException NOT_SUPPORTED_ERR: Raised if the implementation does not support the
     *     type of Event interface requested
     * @since WD-DOM-Level-2-19990923
     */
    public Event createEvent(String type) throws DOMException {
        if (type.equalsIgnoreCase("Events") || "Event".equals(type)) return new EventImpl();
    ...
    /**
     * Store event listener registered on a given node This is another place where we could use weak
     * references! Indeed, the node here won't be GC'ed as long as some listener is registered on
     * it, since the eventsListeners table will have a reference to the node.
     */
    ...
    class LEntry implements Serializable {
         ...
         * @param useCaptue True iff listener is registered on capturing phase rather than at-target
         *     or bubbling
         */
     ...
     *
     * <p>Register an event listener with this Node. A listener may be independently registered as
     * both Capturing and Bubbling, but may only be registered once per role; redundant
     * registrations are ignored.
     *
     * @param node node to add listener to
     * @param type Event name (NOT event group!) to listen for.
     * @param listener Who gets called when event is dispatched
     * @param useCapture True iff listener is registered on capturing phase rather than at-target or
     *     bubbling
     */
    protected void addEventListener(
        ...
        if (type == null || type.equals("") || listener == null) return;
     ...
     * Introduced in DOM Level 2.
     *
     * <p>Deregister an event listener previously registered with this Node. A listener must be
     * independently removed from the Capturing and Bubbling roles. Redundant removals (of listeners
     ...
     * Introduced in DOM Level 2.
     *
     * <p>Distribution engine for DOM Level 2 Events.
     *
     * <p>Event propagation runs as follows:
     *
     * <ol>
     *   <li>Event is dispatched to a particular target node, which invokes this code. Note that the
     *       event's stopPropagation flag is cleared when dispatch begins; thereafter, if it has
     *       been set before processing of a node commences, we instead immediately advance to the
     *       DEFAULT phase.
     *   <li>The node's ancestors are established as destinations for events. For capture and bubble
     *       purposes, node ancestry is determined at the time dispatch starts. If an event handler
     *       alters the document tree, that does not change which nodes will be informed of the
     *       event.
     *   <li>CAPTURING_PHASE: Ancestors are scanned, root to target, for Capturing listeners. If
     *       found, they are invoked (see below).
     *   <li>AT_TARGET: Event is dispatched to NON-CAPTURING listeners on the target node. Note that
     *       capturing listeners on this node are _not_ invoked.
     *   <li>BUBBLING_PHASE: Ancestors are scanned, target to root, for non-capturing listeners.
     *   <li>Default processing: Some DOMs have default behaviors bound to specific nodes. If this
     *       DOM does, and if the event's preventDefault flag has not been set, we now return to the
     *       target node and process its default handler for this event, if any.
     * </ol>
     *
     * <p>Note that registration of handlers during processing of an event does not take effect
     * during this phase of this event; they will not be called until the next time this node is
     * visited by dispatchEvent. On the other hand, removals take effect immediately.
     *
     * <p>If an event handler itself causes events to be dispatched, they are processed
     * synchronously, before processing resumes on the event which triggered them. Please be aware
     * that this may result in events arriving at listeners "out of order" relative to the actual
     * sequence of requests.
     *
     * <p>Note that our implementation resets the event's stop/prevent flags when dispatch begins. I
     * believe the DOM's intent is that event objects be redispatchable, though it isn't stated in
     * those terms.
     *
     * @param node node to dispatch to
     * @param event the event object to be dispatched to registered EventListeners
     * @return true if the event's <code>preventDefault()</code> method was invoked by an
     *     EventListener; otherwise false.
     */
    protected boolean dispatchEvent(NodeImpl node, Event event) {
        ...
        if (!evt.initialized || evt.type == null || evt.type.equals("")) {
            String msg =
                    DOMMessageFormatter.formatMessage(
                            DOMMessageFormatter.DOM_DOMAIN, "UNSPECIFIED_EVENT_TYPE_ERR", null);
        ...
        if (lc.captures > 0) {
            ...
            for (int j = pv.size() - 1; j >= 0; --j) {
                if (evt.stopPropagation) break; // Someone set the flag. Phase ends.
    ...
    /**
     * NON-DOM INTERNAL: DOMNodeInsertedIntoDocument and ...RemovedFrom... are dispatched to an
     * entire subtree. This is the distribution code therefor. They DO NOT bubble, thanks be, but
     * may be captured.
     *
     * <p>Similar to code in dispatchingEventToSubtree however this method is only used on the
     * target node and does not start a dispatching chain on the sibling of the target node as this
     * is not part of the subtree ***** At the moment I'm being sloppy and using the normal capture
     * dispatcher on every node. This could be optimized hugely by writing a capture engine that
     * tracks our position in the tree to update the capture chain without repeated chases up to
     * root.
     *
    ...
    /**
     * NON-DOM INTERNAL: Return object for getEnclosingAttr. Carries (two values, the Attr node
     * affected (if any) and its previous string value. Simple struct, no methods.
     */
    ...
    /**
     * NON-DOM INTERNAL: Convenience wrapper for calling dispatchAggregateEvents when the context
     * was established by <code>savedEnclosingAttr</code>.
     *
     * @param node node to dispatch to
    ...
    /**
     * NON-DOM INTERNAL: Generate the "aggregated" post-mutation events DOMAttrModified and
     * DOMSubtreeModified. Both of these should be issued only once for each user-requested mutation
     * operation, even if that involves multiple changes to the DOM. For example, if a DOM operation
     * makes multiple changes to a single Attr before returning, it would be nice to generate only
     * one DOMAttrModified, and multiple changes over larger scope but within a recognizable single
     * subtree might want to generate only one DOMSubtreeModified, sent to their lowest common
     * ancestor.
     *
     * <p>To manage this, use the "internal" versions of insert and remove with MUTATION_LOCAL, then
     * make an explicit call to this routine at the higher level. Some examples now exist in our
     * code.
     *
     * @param node The node to dispatch to
     * @param enclosingAttr The Attr node (if any) whose value has been changed as a result of the
     *     DOM operation. Null if none such.
     * @param oldValue The String value previously held by the enclosingAttr. Ignored if none such.
     * @param change Type of modification to the attr. See MutationEvent.attrChange
     */
    protected void dispatchAggregateEvents(
            NodeImpl node, AttrImpl enclosingAttr, String oldvalue, short change) {
    ...
    /**
     * NON-DOM INTERNAL: Pre-mutation context check, in preparation for later generating
     * DOMAttrModified events. Determines whether this node is within an Attr
     *
    ...
    protected void saveEnclosingAttr(NodeImpl node) {
        ...
        if (lc.total > 0) {
            ...
            while (true) {
                ...
                else if (type == TEXT_NODE) eventAncestor = eventAncestor.parentNode();
                else return;
        ...
        if (mutationEvents) {
            ...
            if (lc.total > 0) {
                ...
                if (eventAncestor != null) { // Might have been orphan Attr
                    ...
                    if (eventAncestor.getNodeType() == DOCUMENT_NODE) {
                        MutationEventImpl me = new MutationEventImpl();
                        me.initMutationEvent(
        ...
        if (mutationEvents) {
            ...
            if (previous == null) {
                ...
                dispatchAggregateEvents(
                        attr.ownerNode, attr, previous.getNodeValue(), MutationEvent.MODIFICATION);
            }
        }
    }
