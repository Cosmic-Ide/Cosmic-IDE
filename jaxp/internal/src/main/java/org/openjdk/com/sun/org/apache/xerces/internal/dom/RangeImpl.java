 ...
 *
 * <p>Please see the API documentation for the interface classes and use the interfaces in your
 * client programs.
 *
 * @xerces.internal
 */
public class RangeImpl implements Range {
    ...
    /**
     * The constructor. Clients must use DocumentRange.createRange(), because it registers the Range
     * with the document, so it can be fixed-up.
     */
    ...
    public short compareBoundaryPoints(short how, Range sourceRange) throws DOMException {
        if (fDocument.errorChecking) {
            ...
            // WRONG_DOCUMENT_ERR: Raised if the two Ranges are not in the same Document or
            // DocumentFragment.
        ...
        int depthDiff = 0;
        for (Node n = endPointA; n != null; n = n.getParentNode()) depthDiff++;
        ...
        for (Node pA = endPointA.getParentNode(), pB = endPointB.getParentNode();
                pA != pB;
    ...
    public void insertNode(Node newNode) throws DOMException, RangeException {
        ...
        if (fStartContainer.getNodeType() == Node.TEXT_NODE) {
            ...
            currentChildren =
                    parent.getChildNodes().getLength(); // holds number of kids before insertion
    ...
    public void surroundContents(Node newParent) throws DOMException, RangeException {
        ...
        if (realStart != realEnd) {
            throw new RangeExceptionImpl(
                    ...
                    DOMMessageFormatter.formatMessage(
                            DOMMessageFormatter.DOM_DOMAIN, "BAD_BOUNDARYPOINTS_ERR", null));
        }
    ...
    public String toString() {
        ...
        if (fEndContainer.getNodeType() != Node.TEXT_NODE
            ...
            if (stopNode == null) stopNode = nextNode(fEndContainer, false);
        }
    ...
    /**
     * Signal other Ranges to update their start/end containers/offsets. The data has already been
     * split into the two Nodes.
     */
    ...
    /**
     * This function is called from the DOM. This node has already been inserted into the DOM.
     * Fix-up any offsets.
     */
    ...
    /**
     * This function is called within Range instead of Node.removeChild, so that the range can
     * remember that it is actively removing this child.
     */
    ...
    /**
     * This function must be called by the DOM _BEFORE_ a node is deleted, because at that time it
     * is connected in the DOM tree, which we depend on.
     */
    ...
    /**
     * This is the master routine invoked to visit the nodes selected by this range. For each such
     * node, different actions are taken depending on the value of the <code>how</code> argument.
     *
     * @param how Specifies what type of traversal is being requested (extract, clone, or delete).
     *     Legal values for this argument are:
     *     <ol>
     *       <li><code>EXTRACT_CONTENTS</code> - will produce a document fragment containing the
     *           range's content. Partially selected nodes are copied, but fully selected nodes are
     *           moved.
     *       <li><code>CLONE_CONTENTS</code> - will leave the context tree of the range undisturbed,
     *           but sill produced cloned content in a document fragment
     *       <li><code>DELETE_CONTENTS</code> - will delete from the context tree of the range, all
     *           fully selected nodes.
     *     </ol>
     *
     * @return Returns a document fragment containing any copied or extracted nodes. If the <code>
     *     how</code> parameter was <code>DELETE_CONTENTS</code>, the return value is null.
     */
    private DocumentFragment traverseContents(int how) throws DOMException {
        ...
        */
        ...
        // case 1: same container
        if (fStartContainer == fEndContainer) return traverseSameContainer(how);
        ...
        for (Node c = fEndContainer, p = c.getParentNode();
            ...
            if (p == fStartContainer) return traverseCommonStartContainer(c, how);
            ++endContainerDepth;
        }
        ...
        for (Node c = fStartContainer, p = c.getParentNode();
                p != null;
                c = p, p = p.getParentNode()) {
            if (p == fEndContainer) return traverseCommonEndContainer(c, how);
            ++startContainerDepth;
        }
    ...
    /**
     * Visits the nodes selected by this range when we know a-priori that the start and end
     * containers are the same. This method is invoked by the generic <code>traverse</code> method.
     *
     * @param how Specifies what type of traversal is being requested (extract, clone, or delete).
     *     Legal values for this argument are:
     *     <ol>
    ...
    private DocumentFragment traverseSameContainer(int how) {
        ...
        if (how != DELETE_CONTENTS) frag = fDocument.createDocumentFragment();

        // If selection is empty, just return the fragment
        if (fStartOffset == fEndOffset) return frag;
        ...
        if (fStartContainer.getNodeType() == Node.TEXT_NODE) {
            ...
            if (how == DELETE_CONTENTS) return null;
        ...
        while (cnt > 0) {
            ...
            if (frag != null) frag.appendChild(xferNode);
            --cnt;
            n = sibling;
        }

        // Nothing is partially selected, so collapse to start point
        if (how != CLONE_CONTENTS) collapse(true);
        return frag;
    }

    /**
     * Visits the nodes selected by this range when we know a-priori that the start and end
     * containers are not the same, but the start container is an ancestor of the end container.
     * This method is invoked by the generic <code>traverse</code> method.
     *
    ...
    private DocumentFragment traverseCommonStartContainer(Node endAncestor, int how) {
        ...
        if (frag != null) frag.appendChild(n);
        ...
        while (cnt > 0) {
            ...
            Node xferNode = traverseFullySelected(n, how);
            if (frag != null) frag.insertBefore(xferNode, frag.getFirstChild());
            --cnt;
            n = sibling;
        }
        // Collapse to just before the endAncestor, which
    ...
    /**
     * Visits the nodes selected by this range when we know a-priori that the start and end
     * containers are not the same, but the end container is an ancestor of the start container.
     * This method is invoked by the generic <code>traverse</code> method.
     *
    ...
    /**
     * Visits the nodes selected by this range when we know a-priori that the start and end
     * containers are not the same, and we also know that neither the start nor end container is an
     * ancestor of the other. This method is invoked by the generic <code>traverse</code> method.
     *
     * @param startAncestor Given a common ancestor of the start and end containers, this parameter
     *     is the ancestor (or self) of the start container that is a direct child of the common
     *     ancestor.
     * @param endAncestor Given a common ancestor of the start and end containers, this parameter is
     *     the ancestor (or self) of the end container that is a direct child of the common
     *     ancestor.
     * @param how Specifies what type of traversal is being requested (extract, clone, or delete).
     *     Legal values for this argument are:
     *     <ol>
     ...
     *     how</code> parameter was <code>DELETE_CONTENTS</code>, the return value is null.
     */
    private DocumentFragment traverseCommonAncestors(
            Node startAncestor, Node endAncestor, int how) {
        DocumentFragment frag = null;
        if (how != DELETE_CONTENTS) frag = fDocument.createDocumentFragment();
    ...
    /**
     * Traverses the "right boundary" of this range and operates on each "boundary node" according
     * to the <code>how</code> parameter. It is a-priori assumed by this method that the right
     * boundary does not contain the range's start container.
     *
     * <p>A "right boundary" is best visualized by thinking of a sample tree:
     *
     * <pre>
     *                 A
     *                /|\
     ...
     * </pre>
     *
     * Imagine first a range that begins between the "E" and "F" nodes and ends between the "I" and
     * "J" nodes. The start container is "B" and the end container is "D". Given this setup, the
     * following applies:
     *
     * <p>Partially Selected Nodes: B, D<br>
     * Fully Selected Nodes: F, G, C, H, I
     *
     * <p>The "right boundary" is the highest subtree node that contains the ending container. The
     * root of this subtree is always partially selected.
     *
     * <p>In this example, the nodes that are traversed as "right boundary" nodes are: H, I, and D.
     *
     * @param root The node that is the root of the "right boundary" subtree.
     * @param how Specifies what type of traversal is being requested (extract, clone, or delete).
     *     Legal values for this argument are:
     *     <ol>
     *       <li><code>EXTRACT_CONTENTS</code> - will produce a node containing the boundaries
     *           content. Partially selected nodes are copied, but fully selected nodes are moved.
     *       <li><code>CLONE_CONTENTS</code> - will leave the context tree of the range undisturbed,
     *           but will produced cloned content.
     *       <li><code>DELETE_CONTENTS</code> - will delete from the context tree of the range, all
     *           fully selected nodes within the boundary.
     *     </ol>
     *
    ...
    private Node traverseRightBoundary(Node root, int how) {
        ...
        if (next == root) return traverseNode(next, isFullySelected, false, how);
        ...
        while (parent != null) {
            ...
            if (how != DELETE_CONTENTS) clonedGrandParent.appendChild(clonedParent);
    ...
    /**
     * Traverses the "left boundary" of this range and operates on each "boundary node" according to
     * the <code>how</code> parameter. It is a-priori assumed by this method that the left boundary
     * does not contain the range's end container.
     *
     * <p>A "left boundary" is best visualized by thinking of a sample tree:
     *
     * <pre>
     *
     *                 A
     ...
     * Fully Selected Nodes: F, G, C, H, I
     *
     * <p>The "left boundary" is the highest subtree node that contains the starting container. The
     * root of this subtree is always partially selected.
     *
     * <p>In this example, the nodes that are traversed as "left boundary" nodes are: F, G, and B.
     *
    ...
    private Node traverseLeftBoundary(Node root, int how) {
        ...
        if (next == root) return traverseNode(next, isFullySelected, true, how);
        ...
        while (parent != null) {
            while (next != null) {
                ...
                if (how != DELETE_CONTENTS) clonedParent.appendChild(clonedChild);
                isFullySelected = true;
                next = nextSibling;
            }
            if (parent == root) return clonedParent;
    ...
    /**
     * Utility method for traversing a single node. Does not properly handle a text node containing
     * both the start and end offsets. Such nodes should have been previously detected and been
     * routed to traverseTextNode.
     *
     ...
     *     Note that although the DOM 2 specification says that a text node that is boththe start
     *     and end container is not selected, we treat it here as if it were partially selected.
     * @param isLeft Is true if we are traversing the node as part of navigating the "left boundary"
     *     of the range. If this value is false, it implies we are navigating the "right boundary"
     *     of the range.
     * @param how Specifies what type of traversal is being requested (extract, clone, or delete).
     *     Legal values for this argument are:
     *     <ol>
     *       <li><code>EXTRACT_CONTENTS</code> - will simply return the original node.
     *       <li><code>CLONE_CONTENTS</code> - will leave the context tree of the range undisturbed,
     *           but will return a cloned node.
     *       <li><code>DELETE_CONTENTS</code> - will delete the node from it's parent, but will
     *           return null.
     *     </ol>
     *
    ...
    private Node traverseNode(Node n, boolean isFullySelected, boolean isLeft, int how) {
        if (isFullySelected) return traverseFullySelected(n, how);
        if (n.getNodeType() == Node.TEXT_NODE) return traverseTextNode(n, isLeft, how);
        return traversePartiallySelected(n, how);
    }

    /**
     * Utility method for traversing a single node when we know a-priori that the node if fully
     * selected.
     *
     * @param n The node to be traversed.
     * @param how Specifies what type of traversal is being requested (extract, clone, or delete).
    ...
    /**
     * Utility method for traversing a single node when we know a-priori that the node if partially
    ...
    /**
     * Utility method for traversing a text node that we know a-priori to be on a left or right
     * boundary of the range. This method does not properly handle text nodes that contain both the
    ...
    private Node traverseTextNode(Node n, boolean isLeft, int how) {
        ...
        if (how != CLONE_CONTENTS) n.setNodeValue(oldNodeValue);
        if (how == DELETE_CONTENTS) return null;
    ...
    /**
     * Finds the root container for the given node and determines if that root container is legal
     * with respect to the DOM 2 specification. At present, that means the root container must be
     * either an attribute, a document, or a document fragment.
     */
    ...
    /**
     * Utility method to retrieve a child node by index. This method assumes the caller is trying to
     * find out which node is selected by the given index. Note that if the index is greater than
     * the number of children, this implies that the first node selected is the parent node itself.
     *
     ...
     *     the offset is less than zero, or if the offset is greater than the number of children,
    ...
    private Node getSelectedNode(Node container, int offset) {
        if (container.getNodeType() == Node.TEXT_NODE) return container;
