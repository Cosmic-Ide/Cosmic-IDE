...
import org.openjdk.com.sun.org.apache.xerces.internal.util.XMLAttributesIteratorImpl;
import org.openjdk.com.sun.org.apache.xerces.internal.util.XMLChar;
import org.openjdk.com.sun.org.apache.xerces.internal.util.XMLStringBuffer;
...
import org.openjdk.com.sun.org.apache.xerces.internal.utils.XMLLimitAnalyzer;
...
import org.openjdk.com.sun.org.apache.xerces.internal.xni.parser.XMLDocumentScanner;
import org.openjdk.com.sun.org.apache.xerces.internal.xni.parser.XMLInputSource;
import org.openjdk.com.sun.xml.internal.stream.XMLBufferListener;
import org.openjdk.com.sun.xml.internal.stream.XMLEntityStorage;
import org.openjdk.com.sun.xml.internal.stream.XMLInputFactoryImpl;
...
public class XMLDocumentFragmentScannerImpl extends XMLScanner
    ...
    private static final Boolean[] FEATURE_DEFAULTS = {
        Boolean.TRUE, null, Boolean.FALSE, Boolean.FALSE, Boolean.TRUE
    };

    /** Recognized properties. */
    private static final String[] RECOGNIZED_PROPERTIES = {
        SYMBOL_TABLE, ERROR_REPORTER, ENTITY_MANAGER, XML_SECURITY_PROPERTY_MANAGER
    };

    /** Property defaults. */
    private static final Object[] PROPERTY_DEFAULTS = {null, null, null, EXTERNAL_ACCESS_DEFAULT};
    ...
    /**
     * Document system identifier. REVISIT: So what's this used for? - NG protected String
     * fDocumentSystemId; ****
     */
    protected String fPITarget;

    // xxx do we need to create an extra XMLString object... look for using fTempString for
    // collecting all the data values
    ...
    /**
     * comma-delimited list of protocols that are allowed for the purpose of accessing external dtd
    ...
    /**
     * CHANGED: Using XMLAttributesIteratorImpl instead of XMLAttributesImpl. This class implements
     * Iterator interface so we can directly give Attributes in the form of iterator.
     */
     ...
     * Scans a document.
     *
     * @param complete True if the scanner should scan the document completely, pushing all events
     *     to the registered document handler. A value of false indicates that that the scanner
     *     should only scan the next portion of the document and return. A scanner instance is
     *     permitted to completely scan a document if it does not support this "pull" scanning
     *     model.
        ...
        do {
            switch (event) {
                ...
                case XMLStreamConstants.SPACE:
                    // check if getCharacterData() is the right function to retrieve
                    // ignorableWhitespace information.
    ...
    public void reset(XMLComponentManager componentManager) throws XMLConfigurationException {
        ...
        fSecurityManager =
                (XMLSecurityManager) componentManager.getProperty(Constants.SECURITY_MANAGER, null);
        ...
        fExternalSubsetResolver =
                (resolver instanceof ExternalSubsetResolver)
    ...
    public void reset(PropertyManager propertyManager) {
        ...
        // fDocumentSystemId = null;
        fNamespaces =
                ((Boolean) propertyManager.getProperty(XMLInputFactory.IS_NAMESPACE_AWARE))
                        .booleanValue();
        ...
        Boolean bo =
                (Boolean)
                        propertyManager.getProperty(
                                XMLInputFactoryImpl.IS_REPLACING_ENTITY_REFERENCES);
        fReplaceEntityReferences = bo.booleanValue();
        bo =
                (Boolean)
                        propertyManager.getProperty(
                                XMLInputFactoryImpl.IS_SUPPORTING_EXTERNAL_ENTITIES);
        ...
        // if fIsCoalesce is set to false, take the value of fReplaceEntityReferences as set by
        // application
        ...
        fSecurityManager =
                (XMLSecurityManager) propertyManager.getProperty(Constants.SECURITY_MANAGER);
        resetCommon();
    } // reset(XMLComponentManager)
    ...
    public void setProperty(String propertyId, Object value) throws XMLConfigurationException {
        ...
        if (propertyId.startsWith(Constants.XERCES_PROPERTY_PREFIX)) {
            ...
            if (suffixLength == Constants.ENTITY_RESOLVER_PROPERTY.length()
                ...
                fExternalSubsetResolver =
                        (value instanceof ExternalSubsetResolver)
                                ? (ExternalSubsetResolver) value
                                : null;
                return;
            }
        }
    ...
    public void endEntity(String name, Augmentations augs) throws IOException, XNIException {
        ...
        /**
         * // flush possible pending output buffer - see scanContent if (fInScanContent &&
         * fStringBuffer.length != 0 && fDocumentHandler != null) {
         * fDocumentHandler.characters(fStringBuffer, null); fStringBuffer.length = 0; // make sure
         * we know it's been flushed }
         */
    ...
    protected void scanXMLDeclOrTextDecl(boolean scanningTextDecl)
        ...
        /// xxx see where its used.. this is not used anywhere. it may be useful for entity to store
        // this information
        ...
        if (encoding != null
                && !fEntityScanner.getCurrentEntity().isEncodingExternallySpecified()) {
    ...
    /**
     * Scans a processing data. This is needed to handle the situation where a document starts with
     * a processing instruction whose target name <em>starts with</em> "xml". (e.g. xmlfoo)
     *
     ...
     * </pre>
     *
     * <p><strong>Note:</strong> Called after scanning past '&lt;!--'
     */
    ...
    void addElement(String rawname) {
        if (fElementPointer < ELEMENT_ARRAY_LENGTH) {
            ...
            if (fElementStack.fDepth < MAX_DEPTH_LIMIT) {
                ...
                if (column > 0) {
                    short pointer =
                            getElementPointer((short) fElementStack.fDepth, (short) (column - 1));
            ...
            if (rawname != null && skipFromTheBuffer(rawname)) {
                ...
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println(
                            "Element "
                                    + fElementRawname
                                    + " was SKIPPED at pointer location = "
                                    + fLastPointerLocation);
                }
                return true;
            } else {
            ...
            if (fElementArray[pointer] != null && skipFromTheBuffer(fElementArray[pointer])) {
                if (DEBUG_SKIP_ALGORITHM) {
                    ...
                    System.out.println(
                            "Element "
                                    + fElementRawname
                                    + " was SKIPPED at depth = "
                                    + fElementStack.fDepth
                                    + " column = "
                                    + column);
                    System.out.println();
                }
    ...
    /**
     * Scans a start element. This method will handle the binding of namespace information and
     * notifying the handler of the start of the element.
     *
     * <p>
     *
     * <pre>
     ...
     * </pre>
     *
     * <p><strong>Note:</strong> This method assumes that the leading '&lt;' character has been
     * consumed.
     *
     * <p><strong>Note:</strong> This method uses the fElementQName and fAttributes variables. The
     * contents of these variables will be destroyed. The caller should copy important information
     * out of these variables before calling this method. NB: Content in fAttributes is valid only
     * till the state of the parser is XMLEvent.START_ELEMENT
     *
    ...
    protected boolean scanStartElement() throws IOException, XNIException {
        ...
        if (!fSkip || fAdd) {
            ...
            if (DEBUG)
                System.out.println(
                        "Element scanned in start element is " + fElementQName.toString());
            if (DEBUG_SKIP_ALGORITHM) {
                if (fAdd) {
                    System.out.println(
        ...
        // when the elements are being added , we need to check if we are set for skipping the
        // elements
        if (fAdd) {
        ...
        if (!seekCloseOfStartTag()) {
            ...
            do {
                ...
                if (fSecurityManager != null
                        && !fSecurityManager.isNoLimit(fElementAttributeLimit)
        ...
        if (fEmptyElement) {
            ...
            // We should not be popping out the context here in endELement becaause the namespace
            // context is still
            ...
            if (dtdGrammarUtil != null) dtdGrammarUtil.startElement(fElementQName, fAttributes);
            if (fDocumentHandler != null) {
                // complete element and attributes are traversed in this function so we can send a
                // callback
                // here.
        ...
        if (DEBUG_START_END_ELEMENT)
            System.out.println(
                    this.getClass().toString() + "<<< scanStartElement(): " + fEmptyElement);
     ...
     * </pre>
     *
     * <p><strong>Note:</strong> This method assumes that the next character on the stream is the
     * first character of the attribute name.
     *
     * <p><strong>Note:</strong> This method uses the fAttributeQName and fQName variables. The
     * contents of these variables will be destroyed.
     *
     * @param attributes The attributes list for the scanned attribute.
     */

    /**
     * protected void scanAttribute(AttributeIteratorImpl attributes) throws IOException,
     * XNIException { if (DEBUG_START_END_ELEMENT) System.out.println(">>> scanAttribute()");
     *
     * <p>// name if (fNamespaces) { fEntityScanner.scanQName(fAttributeQName); } else { String name
     * = fEntityScanner.scanName(); fAttributeQName.setValues(null, name, name, null); }
     *
     * <p>// equals fEntityScanner.skipSpaces(); if (!fEntityScanner.skipChar('=')) {
     * reportFatalError("EqRequiredInAttribute", new Object[]{fAttributeQName.rawname}); }
     * fEntityScanner.skipSpaces();
     *
     * <p>// content int oldLen = attributes.getLength();
     */
    /**
     ...
     *
     * <p>// WFC: Unique Att Spec if (oldLen == attributes.getLength()) {
    ...
    protected void scanAttribute(XMLAttributes attributes) throws IOException, XNIException {
        ...
        scanAttributeValue(
                tmpStr, fTempString2, fAttributeQName.rawname, attributes, attIndex, isVC);
        ...
        // WFC: Unique Att Spec
        // attributes count will be same if the current attribute  name already exists for this
        // element name.
     ...
     *
     * <p><strong>Note:</strong> This method uses the fTempString and fStringBuffer variables.
     *
     * @param complete True if the CDATA section is to be scanned completely.
    ...
    protected boolean scanCDATASection(XMLStringBuffer contentBuffer, boolean complete)
        ...
        while (true) {
            ...
            if (!fEntityScanner.scanData("]]>", contentBuffer)) {
                break;
                /**
                 * We dont need all this code if we pass ']]>' as delimeter.. int brackets = 2;
                 * while (fEntityScanner.skipChar(']')) { brackets++; }
                 *
                 * <p>//When we find more than 2 square brackets if (fDocumentHandler != null &&
                 * brackets > 2) { //we dont need to clear the buffer.. //contentBuffer.clear(); for
                 * (int i = 2; i < brackets; i++) { contentBuffer.append(']'); }
                 * fDocumentHandler.characters(contentBuffer, null); }
                 *
                 * <p>if (fEntityScanner.skipChar('>')) { break; } if (fDocumentHandler != null) {
                 * //we dont need to clear the buffer now.. //contentBuffer.clear();
                 * contentBuffer.append("]]"); fDocumentHandler.characters(contentBuffer, null); }
                 */
            } else {
     ...
     * </pre>
     *
     * <p><strong>Note:</strong> This method uses the fElementQName variable. The contents of this
     * variable will be destroyed. The caller should copy the needed information out of this
    ...
    protected int scanEndElement() throws IOException, XNIException {
        if (DEBUG_START_END_ELEMENT)
            System.out.println(this.getClass().toString() + ">>> scanEndElement()");
        ...
        // REVISIT: if the string is not the same as expected.. we need to do better error
        // handling..
        // We can skip this for now... In any case if the string doesn't match -- document is not
        // well formed.
        ...
        if (dtdGrammarUtil != null) dtdGrammarUtil.endElement(endElementName);
     ...
     *
     * @return returns true if the new entity is started. If it was built-in entity 'false' is
     *     returned.
     * @throws IOException Thrown if i/o error occurs.
     * @throws XNIException Thrown if handler throws exception upon notification.
     */
    protected void scanEntityReference(XMLStringBuffer content) throws IOException, XNIException {
        ...
        if ((isEE && !fSupportExternalEntities)
                || (!isEE && !fReplaceEntityReferences)
                || foundBuiltInRefs) {
        ...
        if (!fEntityStore.isDeclaredEntity(name)) {
            ...
            } else reportFatalError("EntityNotDeclared", new Object[] {name});
        }
        ...
        // set the scaner state to content.. parser will automatically revive itself at any point of
        // time.
        // setScannerState(SCANNER_STATE_CONTENT);
        // return true ;
    } // scanEntityReference()
     ...
     *     <p>we really dont need to call this function -- this function is only required when we
     *     integrate with rest of Xerces2. SO maintaining the current behavior and still calling
     *     this function to hanlde built-in entity reference.
     */
    ...
    String checkAccess(String systemId, String allowedProtocols) throws IOException {
        ...
        return SecuritySupport.checkAccess(
                expandedSystemId, allowedProtocols, Constants.ACCESS_EXTERNAL_ALL);
    }
    ...
    protected class ElementStack2 {
        ...
        public void resize() {
            /**
             * int length = fElements.length; Element [] temp = new Element[length * 2];
             * System.arraycopy(fElements, 0, temp, 0, length); fElements = temp;
             */
            // resize QNames
        ...
        /**
         * This function doesn't increase depth. The function in this function is broken down into
         * two functions for efficiency. <@see>matchElement</see>. This function just returns the
         * pointer to the object and its values are set.
         *
        ...
        public QName nextElement() {
            ...
            if (fCount == fQName.length) {
                ...
                fAdd = false;
                if (DEBUG_SKIP_ALGORITHM)
                    System.out.println("SKIPPING STOPPED, fShouldSkip = " + fShouldSkip);
        ...
        /**
         * Note that this function is considerably different than nextElement() This function just
         * returns the previously stored elements
         */
    ...
    /**
     * Element stack. This stack operates without synchronization, error checking, and it re-uses
     * objects instead of throwing popped items away.
     *
     * @author Andy Clark, IBM
     */
    protected class ElementStack {
         ...
         *
         * <p><strong>Note:</strong> The QName values are copied into the stack. In other words, the
         * caller does <em>not</em> orphan the element to the stack. Also, the QName object returned
         * is <em>not</em> orphaned to the caller. It should be considered read-only.
         *
        ...
        public QName getNext() {
            ...
            if (DEBUG_SKIP_ALGORITHM) {
                System.out.println(
                        "Element at fPosition = "
                                + fPosition
                                + " is "
                                + fElements[fPosition].rawname);
            }
        ...
        /**
         * This function should be called only when element was skipped sucessfully. 1. Increase the
         * depth - because element was sucessfully skipped. 2. Store the position of the element
         * token in array "last opened tag" at depth. 3. increase the position counter so as to
        ...
        public boolean matchElement(QName element) {
            ...
            if (fLastDepth > fDepth && fDepth <= 3) {
                if (DEBUG_SKIP_ALGORITHM) {
                    ...
                    System.out.println(
                            "Depth = "
                                    + fDepth
                                    + " Checking if INCOMING element "
                                    + element.rawname
                                    + " match STORED ELEMENT "
                                    + fElements[fDepth - 1].rawname);
                }
            ...
            if (match) {
                ...
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println(
                            "At depth = " + fDepth + "array position is = " + (fCount - 1));
                }
            ...
            if (fCount == fElements.length) {
                fSkip = false;
                fAdd = false;
                // reposition the stack -- it seems to be too complex document and there is no
                // symmerty in structure
                reposition();
                if (DEBUG_SKIP_ALGORITHM) {
            ...
            if (DEBUG_SKIP_ALGORITHM) {
                if (match) {
                    System.out.println(
                            "Storing fPosition = " + fInt[fDepth] + " at fDepth = " + fDepth);
                } else {
                    System.out.println(
                            "Storing fCount = " + fInt[fDepth] + " at fDepth = " + fDepth);
                }
            }
            // store the last depth
            fLastDepth = fDepth;
         ...
         *
         * @return Returns the actual QName object. Callee should use this object to store the
         *     details of next element encountered.
         */
        public QName nextElement() {
            if (fSkip) {
                fDepth++;
         ...
         *
         * <p><strong>Note:</strong> The object returned is <em>not</em> orphaned to the caller.
         * Therefore, the caller should consider the object to be read-only.
         */
        public QName popElement() {
            ...
            if (fSkip || fAdd) {
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println(
                            "POPPING Element, at position "
                                    + fInt[fDepth]
                                    + " element at that count is = "
                                    + fElements[fInt[fDepth]].rawname);
                    System.out.println("");
                }
                ...
                if (DEBUG_SKIP_ALGORITHM) {
                    System.out.println(
                            "Retrieveing element at depth = "
                                    + fDepth
                                    + " is "
                                    + fElements[fDepth].rawname);
                }
                return fElements[--fDepth];
            }
        ...
        /**
         * Reposition the stack. fInt [] contains all the opened tags at particular depth. Transfer
         * all the opened tags starting from depth '2' to the current depth and reposition them as
         * per the depth.
         */
        ...
        /**
         * This function is as a result of optimization done for endElement -- we dont need to set
         * the value for every end element encouterd. For Well formedness checks we can have the
         * same QName object that was pushed. the values will be set only if application need to
         * know about the endElement -- neeraj.bajaj@sun.com
         */
    ...
    /**
     * Drives the parser to the next state/event on the input. Parser is guaranteed to stop at the
     * next state/event.
     *
     * <p>Internally XML document is divided into several states. Each state represents a sections
     * of XML document. When this functions returns normally, it has read the section of XML
     * document and returns the state corresponding to section of document which has been read. For
     * optimizations, a particular driver can read ahead of the section of document (state returned)
    ...
    /**
     * Driver to handle content scanning. This driver is capable of reading the fragment of XML
     * document. When it has finished reading fragment of XML documents, it can pass the job of
     * reading to another driver.
     *
     * <p>This class has been modified as per the new design which is more suited to efficiently
     * build pull parser. Lot of performance improvements have been done and the code has been added
     * to support stax functionality/features.
     *
     * @author Neeraj Bajaj, Sun Microsystems
     * @author Andy Clark, IBM
     * @author Eric Ye, IBM
    ...
    protected class FragmentContentDriver implements Driver {
        ...
        /**
         * SCANNER_STATE_CONTENT and SCANNER_STATE_START_OF_MARKUP are two super states of the
         * parser. At any point of time when in doubt over the current state of the parser, the
         * state should be set to SCANNER_STATE_CONTENT. Parser will automatically revive itself and
         * will set state of the parser to one of its sub state. sub states are defined in the
         * parser on the basis of different XML component like SCANNER_STATE_ENTITY_REFERENCE ,
         * SCANNER_STATE_START_ELEMENT, SCANNER_STATE_CDATA etc.. These sub states help the parser
         * to have fine control over the parsing. These are the different milepost, parser stops at
         * each sub state (milepost). Based on this state it is decided if paresr needs to stop at
         * next milepost ??
         */
        public void decideSubState() throws IOException {
            while (fScannerState == SCANNER_STATE_CONTENT
        ...
        /**
         * Drives the parser to the next state/event on the input. Parser is guaranteed to stop at
         * the next state/event. Internally XML document is divided into several states. Each state
         * represents a sections of XML document. When this functions returns normally, it has read
         * the section of XML document and returns the state corresponding to section of document
         * which has been read. For optimizations, a particular driver can read ahead of the section
         * of document (state returned) just read and can maintain a different internal state.
         *
         * <p>State returned corresponds to Stax states.
         *
         * @return state representing the section of document just read.
         * @throws IOException Thrown on i/o error.
         * @throws XNIException Thrown on parse error.
            ...
            while (true) {
                try {
                    if (DEBUG_NEXT) {
                        ...
                        System.out.println(
                                "Entering the FragmentContentDriver with = "
                                        + getScannerStateName(fScannerState));
                    }

                    // decide the actual sub state of the scanner.For more information refer to the
                    // javadoc of
                    // decideSubState.
                    ...
                    if (fIsCoalesce) {
                        ...
                        if (fLastSectionWasCharacterData) {
                            ...
                            // if we dont encounter any CDATA or ENITY REFERENCE and current state
                            // is also not SCANNER_STATE_CHARACTER_DATA
                    ...
                    if (DEBUG_NEXT) {
                        System.out.println(
                                "Actual scanner state set by decideSubState is = "
                                        + getScannerStateName(fScannerState));
                    }
                    ...
                    switch (fScannerState) {
                        ...
                        case SCANNER_STATE_START_ELEMENT_TAG:
                            {
                                ...
                                // xxx this function returns true when element is empty.. can be
                                // linked to end element event.
                        ...
                        case SCANNER_STATE_CHARACTER_DATA:
                            {
                                if (DEBUG_COALESCE) {
                                    System.out.println(
                                            "fLastSectionWasCData = " + fLastSectionWasCData);
                                    System.out.println("fIsCoalesce = " + fIsCoalesce);
                                }
                                // if last section was either entity reference or cdata or character
                                // data we should be using buffer
                                fUsebuffer =
                                ...
                                // set the fTempString length to 0 before passing it on to
                                // scanContent
                                // scanContent sets the correct co-ordinates as per the content read
                                fTempString.length = 0;
                                int c = fEntityScanner.scanContent(fTempString);
                                ...
                                if (fEntityScanner.skipChar('<')) {
                                    ...
                                    if (fEntityScanner.skipChar('/')) {
                                        ...
                                        setScannerState(SCANNER_STATE_START_OF_MARKUP);
                                        // there can be cdata ahead if coalesce is true we should
                                        // call again
                                        if (fIsCoalesce) {
                                            fUsebuffer = true;
                                            fLastSectionWasCharacterData = true;
                                    ...
                                    // in case last section was either entity reference or cdata or
                                ...
                                if (c == '\r') {
                                    ...
                                    // happens when there is the character reference &#13;
                                    // xxx: We know the next chracter.. we should just skip it and
                                    // add ']' directlry
                                    fEntityScanner.scanChar();
                                    fUsebuffer = true;
                                    ...
                                    // remember where we are in case we get an endEntity before we
                                    // could flush the buffer out - this happens when we're parsing
                                    // an
                                    // entity which ends with a ]
                                    fInScanContent = true;
                                ...
                                do {
                                    ...
                                    // we need not to grow the buffer only when isCoalesce() is not
                                    // true;
                                    ...
                                    // we should not be iterating again if fIsCoalesce is not set to
                                    // true
                                ...
                                if (DEBUG)
                                    System.out.println(
                                            "USING THE BUFFER, STRING START="
                                                    + fContentBuffer.toString()
                                                    + "=END");
                                // if fIsCoalesce is true there might be more data so call
                                // fDriver.next()
                                if (fIsCoalesce) {
                                    fLastSectionWasCharacterData = true;
                                    continue;
                        ...
                        case SCANNER_STATE_END_ELEMENT_TAG:
                            {
                                if (fEmptyElement) {
                                    ...
                                    fEmptyElement = false;
                                    setScannerState(SCANNER_STATE_CONTENT);
                                    // check the case when there is comment after single element
                                    // document
                                    ...
                                    if (elementDepthIsZeroHook()) {
                                        // if element depth is zero , it indicates the end of the
                                        // document
                                        // the state shouldn't be set, because it is set by
                                        // elementDepthIsZeroHook() function
                        ...
                        case SCANNER_STATE_CDATA:
                            { // SCANNER_STATE_CDATA: {
                                ...
                                setScannerState(SCANNER_STATE_CONTENT);
                                // 1. if fIsCoalesce is set to true we set the variable
                                // fLastSectionWasCData to true
                                // and just call fDispatche.next(). Since we have set the scanner
                                // state to
                                // SCANNER_STATE_CONTENT (super state) parser will automatically
                                // recover and
                                // behave appropriately. When isCoalesce is set to true we dont need
                                // to reportCDATA event
                                // 2. Check if application has set for reporting CDATA event
                                // 3. if the application has neither set the fIsCoalesce to true nor
                                // fReportCdataEvent
                        ...
                        case SCANNER_STATE_REFERENCE:
                            {
                                ...
                                if (fIsCoalesce
                                        && (fLastSectionWasEntityReference
                                                || fLastSectionWasCData
                                                || fLastSectionWasCharacterData)) {
                                    // fLastSectionWasEntityReference or fLastSectionWasCData are
                                    // only
                                ...
                                if (fEntityScanner.skipChar('#')) {
                                    ...
                                    // return CHARACTERS
                                    if (fScannerState == SCANNER_STATE_BUILT_IN_REFS
                                            && !fIsCoalesce) {
                                        setScannerState(SCANNER_STATE_CONTENT);
                                        return XMLEvent.CHARACTERS;
                                    }

                                    // if there was a text declaration, call next() it will be taken
                                    // care.
                                ...
                                // Wether it was character reference, entity reference or built-in
                                // entity
                        ...
                        case SCANNER_STATE_TEXT_DECL:
                            {
                                ...
                                // now that we've straightened out the readers, we can read in
                                // chunks:
                                fEntityManager.fCurrentEntity.mayReadChunks = true;
                                setScannerState(SCANNER_STATE_CONTENT);
                                // xxx: we don't return any state, so how do we get to know about
                                // TEXT declarations.
                                // it seems we have to careful when to allow function issue a
                                // callback
                        ...
                        case SCANNER_STATE_ROOT_ELEMENT:
                            {
                                if (scanRootElementHook()) {
                                    fEmptyElement = true;
                                    // rest would be taken care by fTrailingMiscDriver set by
                                    // scanRootElementHook
                                    return XMLEvent.START_ELEMENT;
                                }
                                setScannerState(SCANNER_STATE_CONTENT);
        ...
        /**
         * Add the count of the content buffer and check if the accumulated value exceeds the limit
         *
         * @param buffer content buffer
         */
        protected void checkLimit(XMLStringBuffer buffer) {
            if (fLimitAnalyzer.isTracking(fCurrentEntityName)) {
                fLimitAnalyzer.addValue(
                        Limit.GENERAL_ENTITY_SIZE_LIMIT, fCurrentEntityName, buffer.length);
                ...
                if (fSecurityManager.isOverLimit(Limit.TOTAL_ENTITY_SIZE_LIMIT, fLimitAnalyzer)) {
                    ...
                    reportFatalError(
                            ...
                            new Object[] {
                                fLimitAnalyzer.getTotalValue(Limit.TOTAL_ENTITY_SIZE_LIMIT),
    ...
    /**
     * this function gets an XMLString (which is used to store the attribute value) from the special
     * pool maintained for attributes. fAttributeCacheUsedCount tracks the number of attributes that
     * has been consumed from the pool. if all the attributes has been consumed, it adds a new
     * XMLString inthe pool and returns the same XMLString.
     *
