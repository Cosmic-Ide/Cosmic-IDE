...
import org.openjdk.com.sun.org.apache.xerces.internal.util.NamespaceContextWrapper;
...
import org.openjdk.com.sun.xml.internal.stream.StaxErrorReporter;
import org.openjdk.com.sun.xml.internal.stream.XMLEntityStorage;
import org.openjdk.com.sun.xml.internal.stream.dtd.nonvalidating.DTDGrammar;
import org.openjdk.com.sun.xml.internal.stream.dtd.nonvalidating.XMLNotationDecl;
import org.openjdk.com.sun.xml.internal.stream.events.EntityDeclarationImpl;
import org.openjdk.com.sun.xml.internal.stream.events.NotationDeclarationImpl;
...
/**
 * This class implements org.openjdk.javax.xml.stream.XMLStreamReader. It makes use of XML*Scanner
 * classes to derive most of its functionality. If desired, Application can reuse this instance by
 * calling reset() and setInputSource().
 *
...
public class XMLStreamReaderImpl implements org.openjdk.javax.xml.stream.XMLStreamReader {
    ...
    protected static final String READER_IN_DEFINED_STATE = Constants.READER_IN_DEFINED_STATE;
    ...
    // make Global NamespaceContextWrapper object,  fScanner.getNamespaceContext() is dynamic object
    // and ita value changes
    ...
    public XMLStreamReaderImpl(InputStream inputStream, String encoding, PropertyManager props)
        ...
        XMLInputSource inputSource =
                new XMLInputSource(
                        null, null, null, new BufferedInputStream(inputStream), encoding);
        // pass the input source to document scanner impl.
        setInputSource(inputSource);
    }
    ...
    public XMLStreamReaderImpl(Reader reader, PropertyManager props) throws XMLStreamException {
        ...
        XMLInputSource inputSource =
                new XMLInputSource(null, null, null, new BufferedReader(reader), null);
        // pass the input source to document scanner impl.
        setInputSource(inputSource);
    }
    ...
    public void setInputSource(XMLInputSource inputSource) throws XMLStreamException {
        ...
        // this instances is free for reuse if parser has reached END_DOCUMENT state or application
        // has
        // called close()
        fReuse = false;
        ...
        try {
            ...
            if (fReaderInDefinedState) {
                ...
                if (versionStr == null) versionStr = getVersion();
    ...
    void init(PropertyManager propertyManager) throws XMLStreamException {
        ...
        // set Stax internal properties -- Note that these instances are being created in
        // XMLReaderImpl.
        // 1.SymbolTable
    ...
    /**
     * This function tells if this instances is available for reuse. One must call reset() and
     * setInputSource() to be able to reuse this instance.
     */
    ...
    public void reset() {
        ...
        // default value for this property is true. However, this should be false when using
        // XMLEventReader... Ugh..
        // because XMLEventReader should not have defined state.
        fReaderInDefinedState =
                ((Boolean) fPropertyManager.getProperty(READER_IN_DEFINED_STATE)).booleanValue();
        fBindNamespaces =
                ((Boolean) fPropertyManager.getProperty(XMLInputFactory.IS_NAMESPACE_AWARE))
                        .booleanValue();
        versionStr = null;
    }
    ...
    public String getLocalName() {
        ...
        throw new IllegalStateException(
                "Method getLocalName() cannot be called for "
                        + getEventTypeString(fEventType)
                        + " event.");
    } // getLocalName()
    ...
    public String getPIData() {
        if (fEventType == XMLEvent.PROCESSING_INSTRUCTION) {
            ...
            throw new java.lang.IllegalStateException(
                    "Current state of the parser is "
                            + getEventTypeString(fEventType)
                            + " But Expected state is "
    ...
    /**
     * @return the prefix of the current event, or null if the event does not have a prefix. For
     *     START_ELEMENT and END_ELEMENT, return XMLConstants.DEFAULT_NS_PREFIX when no prefix is
     *     available.
     */
    ...
    public char[] getTextCharacters() {
        if (fEventType == XMLEvent.CHARACTERS
                || fEventType == XMLEvent.COMMENT
                || fEventType == XMLEvent.CDATA
                || fEventType == XMLEvent.SPACE) {
            return fScanner.getCharacterData().ch;
        } else {
            throw new IllegalStateException(
                    "Current state = "
                            + getEventTypeString(fEventType)
                            + " is not among the states "
                            + getEventTypeString(XMLEvent.CHARACTERS)
                            + " , "
    ...
    public String getVersion() {
        // apply SAP's patch: the default version in the scanner was set to 1.0 because of DOM and
        // SAX
        // so this patch is a workaround of the difference between StAX and DOM
        // SAPJVM: Return null if the XML version has not been declared (as specified in the
        // JavaDoc).
    ...
    public boolean hasValue() {
        if (fEventType == XMLEvent.START_ELEMENT
                ...
                || fEventType == XMLEvent.PROCESSING_INSTRUCTION
                || fEventType == XMLEvent.COMMENT
                || fEventType == XMLEvent.CHARACTERS) {
            return true;
        } else {
            return false;
        }
        ...
        try {
            ...
            if (fScanner.fScannerState == fScanner.SCANNER_STATE_DTD_EXTERNAL) {
                Boolean isValidating =
                        (Boolean) fPropertyManager.getProperty(XMLInputFactory.IS_VALIDATING);
    ...
    private void switchToXML11Scanner() throws IOException {
        ...
        org.openjdk.com.sun.org.apache.xerces.internal.xni.NamespaceContext oldNamespaceContext =
    ...
    public int getAttributeCount() {
        // xxx: recognize SAX properties namespace, namespace-prefix to get XML Namespace
        // declarations
        ...
        if (fEventType == XMLEvent.START_ELEMENT || fEventType == XMLEvent.ATTRIBUTE) {
            ...
            throw new java.lang.IllegalStateException(
                    "Current state is not among the states "
    ...
    public QName getAttributeName(int index) {
        // State should be either START_ELEMENT or ATTRIBUTE
        if (fEventType == XMLEvent.START_ELEMENT || fEventType == XMLEvent.ATTRIBUTE) {
            return convertXNIQNametoJavaxQName(
                    fScanner.getAttributeIterator().getQualifiedName(index));
        } else {
            throw new java.lang.IllegalStateException(
                    "Current state is not among the states "
                            + getEventTypeString(XMLEvent.START_ELEMENT)
    ...
    public String getAttributeValue(String namespaceURI, String localName) {
        ...
        if (fEventType == XMLEvent.START_ELEMENT || fEventType == XMLEvent.ATTRIBUTE) {
            ...
            if (namespaceURI == null) { // sjsxp issue 70
                ...
                return fScanner.getAttributeIterator()
                        .getValue(namespaceURI.length() == 0 ? null : namespaceURI, localName);
            }
    ...
    public String getNamespaceURI(int index) {
        if (fEventType == XMLEvent.START_ELEMENT
                ...
                || fEventType == XMLEvent.NAMESPACE) {
            // namespaceContext is dynamic object.
            return fScanner.getNamespaceContext()
                    .getURI(fScanner.getNamespaceContext().getDeclaredPrefixAt(index));
        } else {
            throw new IllegalStateException(
                    "Current state "
                            + getEventTypeString(fEventType)
    ...
    public Object getProperty(java.lang.String name) throws java.lang.IllegalArgumentException {
        ...
        if (fPropertyManager != null) {
            ...
            } else return fPropertyManager.getProperty(name);
        }
        return null;
    }
     ...
     * Returns the current value of the parse event as a string, this returns the string value of a
     * CHARACTERS event, returns the value of a COMMENT, the replacement value for an
     * ENTITY_REFERENCE, or the String value of the DTD
     *
     * @return the current text or null
    ...
    public String getText() {
        if (fEventType == XMLEvent.CHARACTERS
            ...
            if (name != null) {
                if (fScanner.foundBuiltInRefs) return fScanner.getCharacterData().toString();
                ...
                else return ((Entity.InternalEntity) en).text;
            } else return null;
            ...
            throw new IllegalStateException(
                    "Current state "
                            ...
                            + ", "
                            + getEventTypeString(XMLEvent.DTD)
                            + " valid for getText() ");
        }
    } // getText
     ...
     * "sourceStart" is copied into "destination" starting at "targetStart". Up to "length"
     * characters are copied. The number of characters actually copied is returned.
     *
     ...
     * @throws IndexOutOfBoundwhile(isCharacters()) ;sException if length < 0 or targetStart +
    ...
    public int getTextCharacters(int sourceStart, char[] target, int targetStart, int length)
        ...
        // getTextStart() + sourceStart should not be greater than the lenght of number of
        // characters
        // present
        int copiedLength = 0;
    ...
    public boolean hasText() {
        ...
        if (fEventType == XMLEvent.CHARACTERS
            ...
            String name = fScanner.getEntityName();
            if (name != null) {
                if (fScanner.foundBuiltInRefs) return true;
            ...
            if (fEventType == XMLEvent.DTD) return fScanner.fSeenDoctypeDecl;
        }
        return false;
    }
    ...
    /**
     * Skips any insignificant events (COMMENT and PROCESSING_INSTRUCTION) until a START_ELEMENT or
     * END_ELEMENT is reached. If other than space characters are encountered, an exception is
     * thrown. This method should be used when processing element-only content because the parser is
     * not able to recognize ignorable whitespace if then DTD is missing or not interpreted.
     *
    ...
    public boolean standaloneSet() {
        // xxx: it requires if the standalone was set in the document ? This is different that if
        // the document
        // is standalone
     ...
     * @return
     */
    public org.openjdk.javax.xml.namespace.QName convertXNIQNametoJavaxQName(
            org.openjdk.com.sun.org.apache.xerces.internal.xni.QName qname) {
        if (qname == null) return null;
        // xxx: prefix definition ?
        if (qname.prefix == null) {
            return new org.openjdk.javax.xml.namespace.QName(qname.uri, qname.localpart);
        } else {
            return new org.openjdk.javax.xml.namespace.QName(
                    qname.uri, qname.localpart, qname.prefix);
        }
    }
