...
import org.openjdk.com.sun.org.apache.xerces.internal.impl.validation.ValidationState;
...
/**
 * The DTD validator. The validator implements a document filter: receiving document events from the
 * scanner; validating the content and structure; augmenting the InfoSet, if applicable; and
 * notifying the parser of the information resulting from the validation process.
 *
 * <p>Formerly, this component also handled DTD events and grammar construction. To facilitate the
 * development of a meaningful DTD grammar caching/preparsing framework, this functionality has been
 * moved into the XMLDTDLoader class. Therefore, this class no longer implements the DTDFilter or
 ...
 *   <li>http://apache.org/xml/features/validation/dynamic
...
public class XMLDTDValidator
    ...
    /**
     * Dynamic validation. This state of this feature is only useful when the validation feature is
    ...
    /**
     * Element children. This data structure is a growing stack that holds the children of elements
     * from the root to the current element depth. This structure never gets "deeper" than the
     * deepest element. Space is re-used once each element is closed.
     *
     * <p><strong>Note:</strong> This is much more efficient use of memory than creating new arrays
     * for each element depth.
     *
     * <p><strong>Note:</strong> The use of this data structure is for validation "on the way out".
     * If the validation model changes to "on the way in", then this data structure is not needed.
     */
    ...
    /**
     * Element children offset stack. This stack refers to offsets into the <code>fElementChildren
     * </code> array.
     *
    ...
    public void doctypeDecl(
        ...
        try {
            eid =
                    XMLEntityManager.expandSystemId(
                            systemId, fDocLocation.getExpandedSystemId(), false);
        } catch (java.io.IOException e) {
        }
        XMLDTDDescription grammarDesc =
                new XMLDTDDescription(
                        publicId, systemId, fDocLocation.getExpandedSystemId(), eid, rootElement);
        ...
        if (fDTDGrammar == null) {
            ...
            // key may cause an unexpected grammar to be retrieved from the grammar pool. This
            // scenario
        ...
        if (fPerformValidation && fElementDepth >= 0 && fDTDGrammar != null) {
            ...
            if (fTempElementDecl.type == XMLElementDecl.TYPE_EMPTY) {
                fErrorReporter.reportError(
                        XMLMessageFormatter.XML_DOMAIN,
                        "MSG_CONTENT_INVALID_SPECIFIED",
                        new Object[] {fCurrentElement.rawname, "EMPTY", "comment"},
                        XMLErrorReporter.SEVERITY_ERROR);
            }
        }
        // call handlers
        ...
        if (fPerformValidation && fElementDepth >= 0 && fDTDGrammar != null) {
            ...
            if (fTempElementDecl.type == XMLElementDecl.TYPE_EMPTY) {
                fErrorReporter.reportError(
                        XMLMessageFormatter.XML_DOMAIN,
                        "MSG_CONTENT_INVALID_SPECIFIED",
                        new Object[] {fCurrentElement.rawname, "EMPTY", "processing instruction"},
                        XMLErrorReporter.SEVERITY_ERROR);
            }
        }
        // call handlers
        ...
        if (fPerformValidation && fElementDepth >= 0 && fDTDGrammar != null) {
            ...
            if (fTempElementDecl.type == XMLElementDecl.TYPE_EMPTY) {
                fErrorReporter.reportError(
                        XMLMessageFormatter.XML_DOMAIN,
                        "MSG_CONTENT_INVALID_SPECIFIED",
                        new Object[] {fCurrentElement.rawname, "EMPTY", "ENTITY"},
                        XMLErrorReporter.SEVERITY_ERROR);
            }
            if (fGrammarBucket.getStandalone()) {
                XMLDTDLoader.checkStandaloneEntityRef(
                        name, fDTDGrammar, fEntityDecl, fErrorReporter);
            }
        }
        if (fDocumentHandler != null) {
     ...
     * called immediately following the startParameterEntity call.
     *
     * <p><strong>Note:</strong> This method is only called for external parameter entities
    ...
    public final boolean validate() {
        ...
        return (fSchemaType != Constants.NS_XMLSCHEMA)
                && (!fDynamicValidation && fValidation || fDynamicValidation && fSeenDoctypeDecl)
    ...
    protected void addDTDDefaultAttrsAndValidate(
            QName elementName, int elementIndex, XMLAttributes attributes) throws XNIException {
        ...
        while (attlistIndex != -1) {
                ...
                if (fTempAttDecl != null) {
                    ...
                    System.out.println(
                            "attlistIndex "
                                    ...
                                    + "\n"
                                    + "attName : '"
                                    + (fTempAttDecl.name.localpart)
                                    + "'\n"
                                    + "attType : "
                                    + fTempAttDecl.simpleType.type
                                    + "\n"
                                    + "attDefaultType : "
                                    + fTempAttDecl.simpleType.defaultType
                                    + "\n"
                                    + "attDefaultValue : '"
                                    + fTempAttDecl.simpleType.defaultValue
                                    + "'\n"
            ...
            if (!specified) {
                if (required) {
                    ...
                    // add attribute
                    fTempQName.setValues(
                            attPrefix, attLocalpart, attRawName, fTempAttDecl.name.uri);
            ...
            if (ampIndex + 1 < valLength && nonNormalizedValue.charAt(ampIndex + 1) != '#') {
                ...
                if (entIndex > -1) {
                    ...
                    if (fEntityDecl.inExternal
                            || (entityName = getExternalEntityRefInAttrValue(fEntityDecl.value))
                                    != null) {
                        return entityName;
                    }
                }
    ...
    protected void validateDTDattribute(
            QName element, String attValue, XMLAttributeDecl attributeDecl) throws XNIException {
    ...
    /**
     * Normalize the attribute value of a non CDATA attributes collapsing sequences of space
     * characters (x20)
     *
     ...
     *
     * <p>This is the method of primary concern to the validator. This method is called upon the
     * scanner reaching the end tag of an element. At that time, the element's children must be
     * structurally validated, so it calls this method. The index of the element being checked (in
     * the decl pool), is provided as well as an array of element name indexes of the children. The
     * validator must confirm that this element can have these children in this order.
     *
     * <p>This can also be called to do 'what if' testing of content models just to see if they
     * would be valid.
     *
     * <p>Note that the element index is an index into the element decl pool, whereas the children
     * indexes are name indexes, i.e. into the string pool.
     *
     ...
     *
     * @param elementIndex The index within the <code>ElementDeclPool</code> of this element.
     * @param childCount The number of entries in the <code>children</code> array.
     * @param children The children of this element.
     * @return The value -1 if fully valid, else the 0 based index of the child that first failed.
     *     If the value returned is equal to the number of children, then additional content is
     *     required to reach a valid ending state.
    ...
    private String getAttributeTypeName(XMLAttributeDecl attrDecl) {
        ...
        switch (attrDecl.simpleType.type) {
            case XMLSimpleType.TYPE_ENTITY:
                {
                    return attrDecl.simpleType.list
                            ? XMLSymbols.fENTITIESSymbol
     ...
     */
    protected boolean handleStartElement(
            QName element, XMLAttributes attributes, Augmentations augs) throws XNIException {
        ...
        if (!fSeenRootElement) {
            ...
            //  Outcome: report validation errors only against schemas (no errors produced from DTD
            // validator)
            //
            // [3] XML instance document has DTD and XML schemas:
            // [a] if schema language is not set outcome - validation errors reported against both
            // grammars: DTD and schemas.
    ...
    protected void startNamespaceScope(
            QName element, XMLAttributes attributes, Augmentations augs) {}
