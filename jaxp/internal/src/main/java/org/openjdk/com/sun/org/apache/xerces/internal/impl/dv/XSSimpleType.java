...
/**
 * This interface <code>XSSimpleType</code> represents the simple type definition of schema
 * component and defines methods to query the information contained. Any simple type (atomic, list
 * or union) will implement this interface. It inherits from <code>XSTypeDecl</code>.
 *
 * @xerces.internal
...
public interface XSSimpleType extends XSSimpleTypeDefinition {
    ...
    /**
     * constants defined for the values of 'whitespace' facet. see <a
     * href='http://www.w3.org/TR/xmlschema-2/#dt-whiteSpace'>XML Schema Part 2: Datatypes </a>
     */
    ...
    /**
     * Constant defined for the primitive built-in simple tpyes. see <a
     * href='http://www.w3.org/TR/xmlschema-2/#built-in-primitive-datatypes'>XML Schema Part 2:
     * Datatypes </a>
     */
    /** "string" type */
    ...
    /**
     * return an ID representing the built-in primitive base type. REVISIT: This method is
     * (currently) for internal use only. the constants returned from this method are not finalized
     * yet. the names and values might change in the further.
     *
    ...
    /**
     * validate a given string value, represented by content.toString(). note that if content is a
     * StringBuffer, for performance reasons, it's possible that the content of the string buffer is
     * modified.
     *
     * @param content the string value that needs to be validated
     * @param context the validation context
     ...
     * <code>XSFacets</code> is used to pass the value of different facets.
     *
     * @param facets the value of all the facets
     * @param presentFacet bit combination value of the costraining facet constants which are
     *     present.
     * @param fixedFacet bit combination value of the costraining facet constants which are fixed.
     * @param context the validation context
    ...
    /**
     * Check the order of the two actual values. (May not be supported by all simple types. REVISIT:
     * Andy believes that a compare() method is necessary. I don't see the necessity for schema (the
     * only place where we need to compare two values is to check min/maxIn/Exclusive facets, but we
     * only need a private method for this case.) But Andy thinks XPATH potentially needs this
     * compare() method.
     *
     * @param value1 the first value
     * @prarm value2 the second value
     * @return > 0 if value1 > value2 = 0 if value1 == value2 < = if value1 < value2
     */
    // public short compare(Object value1, Object value2);

    /**
     * Check whether this type is or is derived from ID. REVISIT: this method makes ID special,
     * which is not a good design. but since ID is not a primitive, there doesn't seem to be a clean
     * way of doing it except to define special method like this.
     *
     ...
     * @exception DatatypeException union datatypes don't have whitespace facet associated with them
     */
