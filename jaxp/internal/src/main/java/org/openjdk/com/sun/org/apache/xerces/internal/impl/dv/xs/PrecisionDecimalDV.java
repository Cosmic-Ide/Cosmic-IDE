...
class PrecisionDecimalDV extends TypeValidator {
    ...
    static final class XPrecisionDecimal {
        ...
        // sign: 0 for absent; 1 for positive values; -1 for negative values (except in case of INF,
        // -INF)
        int sign = 1;
        // total digits. >= 1
        int totalDigits = 0;
        ...
        void initD(String content) throws NumberFormatException {
            ...
            if (intEnd < len) {
                ...
                if (content.charAt(intEnd) != '.'
                        ...
                        && content.charAt(intEnd) != 'e') throw new NumberFormatException();
        ...
        private static String canonicalToStringForHashCode(
                String ivalue, String fvalue, int sign, int pvalue) {
        ...
        public boolean equals(Object val) {
            ...
            if (!(val instanceof XPrecisionDecimal)) return false;
        ...
        private int compareFractionalPart(XPrecisionDecimal oval) {
            if (fvalue.equals(oval.fvalue)) return EQUAL;
        ...
        private void truncateTrailingZeros(StringBuffer fValue, StringBuffer otherFValue) {
            for (int i = fValue.length() - 1; i >= 0; i--)
                if (fValue.charAt(i) == '0') fValue.deleteCharAt(i);
                else break;

            for (int i = otherFValue.length() - 1; i >= 0; i--)
                if (otherFValue.charAt(i) == '0') otherFValue.deleteCharAt(i);
                else break;
        }
        ...
        public int compareTo(XPrecisionDecimal val) {
            ...
            if (ivalue.equals("INF") || val.ivalue.equals("INF")) {
                if (ivalue.equals(val.ivalue)) return EQUAL;
            ...
            if (ivalue.equals("-INF") || val.ivalue.equals("-INF")) {
                if (ivalue.equals(val.ivalue)) return EQUAL;
                else if (ivalue.equals("-INF")) return LESS_THAN;
                return GREATER_THAN;
            }

            if (sign != val.sign) return sign > val.sign ? GREATER_THAN : LESS_THAN;
        ...
        private int compare(XPrecisionDecimal val) {
            ...
            if (pvalue != 0 || val.pvalue != 0) {
                if (pvalue == val.pvalue) return intComp(val);
                else {

                    if (intDigits + pvalue != val.intDigits + val.pvalue)
                        return intDigits + pvalue > val.intDigits + val.pvalue
                                ? GREATER_THAN
                                : LESS_THAN;
                    ...
                    if (pvalue > val.pvalue) {
                        ...
                        return compareDecimal(
                                buffer.toString(), val.ivalue, fbuffer.toString(), val.fvalue);
                    } else {
                        ...
                        return compareDecimal(
                                ivalue, buffer.toString(), fvalue, fbuffer.toString());
                    }
                }
            } else {
                return intComp(val);
         ...
         * @return
         */
        private int compareDecimal(
                String iValue, String fValue, String otherIValue, String otherFValue) {
            int ret = iValue.compareTo(otherIValue);
            if (ret != 0) return ret > 0 ? GREATER_THAN : LESS_THAN;

            if (fValue.equals(otherFValue)) return EQUAL;
        ...
        public boolean isIdentical(XPrecisionDecimal decimal) {
            if (ivalue.equals(decimal.ivalue)
                    && (ivalue.equals("INF") || ivalue.equals("-INF") || ivalue.equals("NaN")))
                return true;

            if (sign == decimal.sign
                    && intDigits == decimal.intDigits
                    && fracDigits == decimal.fracDigits
                    && pvalue == decimal.pvalue
                    && ivalue.equals(decimal.ivalue)
                    && fvalue.equals(decimal.fvalue)) return true;
            return false;
        }
    }
    /* (non-Javadoc)
    ...
    public Object getActualValue(String content, ValidationContext context)
        ...
        try {
            ...
            throw new InvalidDatatypeValueException(
                    "cvc-datatype-valid.1.2.1", new Object[] {content, "precisionDecimal"});
        }
    }
