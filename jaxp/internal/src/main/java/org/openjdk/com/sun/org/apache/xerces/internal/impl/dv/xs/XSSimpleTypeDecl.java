...
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dv.DatatypeException;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dv.InvalidDatatypeFacetException;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dv.InvalidDatatypeValueException;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dv.ValidatedInfo;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dv.ValidationContext;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dv.XSFacets;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dv.XSSimpleType;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.xpath.regex.RegularExpression;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.xs.SchemaSymbols;
import org.openjdk.com.sun.org.apache.xerces.internal.impl.xs.util.ShortListImpl;
...
import org.openjdk.com.sun.org.apache.xerces.internal.xs.XSFacet;
import org.openjdk.com.sun.org.apache.xerces.internal.xs.XSMultiValueFacet;
...
public class XSSimpleTypeDecl implements XSSimpleType, TypeInfo {
    ...
    private ShortList[]
            fEnumerationItemType; // used in case fenumerationType value is LIST or LISTOFUNION
    ...
    protected XSSimpleTypeDecl setRestrictionValues(
            XSSimpleTypeDecl base,
            String name,
            String uri,
            short finalSet,
    ...
    // set values for list.
    protected XSSimpleTypeDecl setListValues(
            String name,
            String uri,
            short finalSet,
            XSSimpleTypeDecl itemType,
    ...
    public short getPrimitiveKind() {
        if (fVariety == VARIETY_ATOMIC && fValidationDV != DV_ANYSIMPLETYPE) {
            if (fValidationDV == DV_ID || fValidationDV == DV_IDREF || fValidationDV == DV_ENTITY) {
                ...
                return DV_DECIMAL;
            } else if (Constants.SCHEMA_1_1_SUPPORT
                    && (fValidationDV == DV_YEARMONTHDURATION
                            || fValidationDV == DV_DAYTIMEDURATION)) {
                return DV_DURATION;
            } else {
    ...
    /**
     * Returns the closest built-in type category this type represents or derived from. For example,
     * if this simple type is a built-in derived type integer the <code>INTEGER_DV</code> is
     * returned.
     */
    ...
    /**
     * If variety is <code>atomic</code> the primitive type definition (a built-in primitive
     * datatype definition or the simple ur-type definition) is available, otherwise <code>null
     * </code>.
     */
    public XSSimpleTypeDefinition getPrimitiveType() {
        if (fVariety == VARIETY_ATOMIC && fValidationDV != DV_ANYSIMPLETYPE) {
            ...
            while (pri.fBase != fAnySimpleType) pri = pri.fBase;
            return pri;
        } else {
            // REVISIT: error situation. runtime exception?
            return null;
    ...
    /**
     * If variety is <code>list</code> the item type definition (an atomic or union simple type
    ...
    /**
     * If variety is <code>union</code> the list of member type definitions (a non-empty sequence of
     * simple type definitions) is available, otherwise an empty <code>XSObjectList</code>.
     */
    ...
    void applyFacets(
        ...
        if ((presentFacet & FACET_PATTERN) != 0) {
            if ((allowedFacet & FACET_PATTERN) == 0) {
                ...
                try {
                    ...
                    reportError(
                            "InvalidRegex", new Object[] {facets.pattern, e.getLocalizedMessage()});
                }
                if (regex != null) {
                    fPattern = new Vector();
                ...
                for (int i = 0; i < enumVals.size(); i++) {
                    ...
                    try {
                        ...
                        reportError(
                                ...
                                new Object[] {enumVals.elementAt(i), this.getBaseType().getName()});
                    }
                }
        ...
        if ((presentFacet & FACET_MAXINCLUSIVE) != 0) {
            if ((allowedFacet & FACET_MAXINCLUSIVE) == 0) {
                ...
                try {
                    fMaxInclusive =
                            fBase.getActualValue(facets.maxInclusive, context, tempInfo, true);
                    ...
                    reportError(
                            "FacetValueFromBase",
                            new Object[] {
                                fTypeName, facets.maxInclusive, "maxInclusive", fBase.getName()
                            });
                }
        ...
        if ((presentFacet & FACET_MAXEXCLUSIVE) != 0) {
            if ((allowedFacet & FACET_MAXEXCLUSIVE) == 0) {
                ...
                try {
                    fMaxExclusive =
                            fBase.getActualValue(facets.maxExclusive, context, tempInfo, true);
                    ...
                    reportError(
                            "FacetValueFromBase",
                            new Object[] {
                                fTypeName, facets.maxExclusive, "maxExclusive", fBase.getName()
                            });
                }
                ...
                // maxExclusive <= base.maxInclusive
                    ...
                    if (fDVs[fValidationDV].compare(fMaxExclusive, fBase.fMaxInclusive) > 0) {
                        reportError(
                                ...
                                new Object[] {facets.maxExclusive, fBase.fMaxInclusive});
                    }
                }
            }
        }
        ...
        if ((presentFacet & FACET_MINEXCLUSIVE) != 0) {
            if ((allowedFacet & FACET_MINEXCLUSIVE) == 0) {
                ...
                try {
                    fMinExclusive =
                            fBase.getActualValue(facets.minExclusive, context, tempInfo, true);
                    ...
                    reportError(
                            "FacetValueFromBase",
                            new Object[] {
                                fTypeName, facets.minExclusive, "minExclusive", fBase.getName()
                            });
                }
                ...
                // minExclusive >= base.minInclusive
                    ...
                    if (fDVs[fValidationDV].compare(fMinExclusive, fBase.fMinInclusive) < 0) {
                        reportError(
                                ...
                                new Object[] {facets.minExclusive, fBase.fMinInclusive});
                    }
                }
            }
        }
        ...
        if ((presentFacet & FACET_MININCLUSIVE) != 0) {
            if ((allowedFacet & FACET_MININCLUSIVE) == 0) {
                ...
                try {
                    fMinInclusive =
                            fBase.getActualValue(facets.minInclusive, context, tempInfo, true);
                    ...
                    reportError(
                            "FacetValueFromBase",
                            new Object[] {
                                fTypeName, facets.minInclusive, "minInclusive", fBase.getName()
                            });
                }
        ...
        if (fFacetsDefined != 0) {
            ...
            if (((fFacetsDefined & FACET_MAXEXCLUSIVE) != 0)
                    && ((fFacetsDefined & FACET_MAXINCLUSIVE) != 0)) {
                reportError(
                        "maxInclusive-maxExclusive",
                        new Object[] {fMaxInclusive, fMaxExclusive, fTypeName});
            }
            ...
            if (((fFacetsDefined & FACET_MINEXCLUSIVE) != 0)
                ...
                reportError(
                        ...
                        new Object[] {fMinInclusive, fMinExclusive, fTypeName});
            }
            ...
            if (((fFacetsDefined & FACET_MAXINCLUSIVE) != 0)
                ...
                if (result != -1 && result != 0)
                    reportError(
                            ...
                            new Object[] {fMinInclusive, fMaxInclusive, fTypeName});
            }

            // check 4.3.8.c2 must: minExclusive <= maxExclusive ??? minExclusive < maxExclusive
            if (((fFacetsDefined & FACET_MAXEXCLUSIVE) != 0)
                    && ((fFacetsDefined & FACET_MINEXCLUSIVE) != 0)) {
                result = fDVs[fValidationDV].compare(fMinExclusive, fMaxExclusive);
                if (result != -1 && result != 0)
                    reportError(
                            "minExclusive-less-than-equal-to-maxExclusive",
                            new Object[] {fMinExclusive, fMaxExclusive, fTypeName});
            }
            ...
            if (((fFacetsDefined & FACET_MAXINCLUSIVE) != 0)
                ...
                if (fDVs[fValidationDV].compare(fMinExclusive, fMaxInclusive) != -1)
                    reportError(
                            ...
                            new Object[] {fMinExclusive, fMaxInclusive, fTypeName});
            }
            ...
            if (((fFacetsDefined & FACET_MAXEXCLUSIVE) != 0)
                ...
                if (fDVs[fValidationDV].compare(fMinInclusive, fMaxExclusive) != -1)
                    reportError(
                            ...
                            new Object[] {fMinInclusive, fMaxExclusive, fTypeName});
            }
            ...
            if ((fFacetsDefined & FACET_LENGTH) != 0) {
                if ((fBase.fFacetsDefined & FACET_MINLENGTH) != 0 && fLength < fBase.fMinLength) {
                    ...
                    reportError(
                            "length-minLength-maxLength.1.1",
                            new Object[] {
                                fTypeName,
                ...
                if ((fBase.fFacetsDefined & FACET_MAXLENGTH) != 0 && fLength > fBase.fMaxLength) {
                    ...
                    reportError(
                            "length-minLength-maxLength.2.1",
                            new Object[] {
                                fTypeName,
                                Integer.toString(fLength),
                                Integer.toString(fBase.fMaxLength)
                            });
                }
                if ((fBase.fFacetsDefined & FACET_LENGTH) != 0) {
                    ...
                    if (fLength != fBase.fLength)
                        reportError(
                                ...
                                new Object[] {
                                    Integer.toString(fLength),
                                    Integer.toString(fBase.fLength),
                                    fTypeName
                                });
                }
            }
            ...
            if ((fBase.fFacetsDefined & FACET_LENGTH) != 0
                ...
                if ((fFacetsDefined & FACET_MINLENGTH) != 0) {
                    ...
                    if (fMinLength != fBase.fMinLength) {
                        reportError(
                                "length-minLength-maxLength.1.2.b",
                                new Object[] {
                                    fTypeName,
                                    Integer.toString(fMinLength),
                                    Integer.toString(fBase.fMinLength)
                ...
                if ((fFacetsDefined & FACET_MAXLENGTH) != 0) {
                    ...
                    if (fMaxLength != fBase.fMaxLength) {
                        reportError(
                                "length-minLength-maxLength.2.2.b",
                                new Object[] {
                                    fTypeName,
                                    Integer.toString(fMaxLength),
            ...
            if (((fFacetsDefined & FACET_MINLENGTH) != 0)) {
                if ((fBase.fFacetsDefined & FACET_MAXLENGTH) != 0) {
                    ...
                    if ((fBase.fFixedFacet & FACET_MINLENGTH) != 0
                            && fMinLength != fBase.fMinLength) {
                        reportError(
                                "FixedFacetValue",
                                new Object[] {
            ...
            if (((fFacetsDefined & FACET_MAXLENGTH) != 0)
                ...
                if (fMaxLength < fBase.fMinLength) {
                    reportError(
                            "minLength-less-than-equal-to-maxLength",
                            new Object[] {
                                Integer.toString(fBase.fMinLength), Integer.toString(fMaxLength)
                            });
                }
            }
            ...
            if ((fFacetsDefined & FACET_MAXLENGTH) != 0) {
                if ((fBase.fFacetsDefined & FACET_MAXLENGTH) != 0) {
                    if (((fBase.fFixedFacet & FACET_MAXLENGTH) != 0)
                            && fMaxLength != fBase.fMaxLength) {
                        reportError(
                                "FixedFacetValue",
                                new Object[] {
            ...
            */
            ...
            if (((fFacetsDefined & FACET_TOTALDIGITS) != 0)) {
                if (((fBase.fFacetsDefined & FACET_TOTALDIGITS) != 0)) {
                    if ((fBase.fFixedFacet & FACET_TOTALDIGITS) != 0
                            && fTotalDigits != fBase.fTotalDigits) {
                        reportError(
                                "FixedFacetValue",
                                new Object[] {
                                    "totalDigits",
                                    Integer.toString(fTotalDigits),
                                    Integer.toString(fBase.fTotalDigits),
                                    fTypeName
                                });
                    }
            ...
            if (((fFacetsDefined & FACET_FRACTIONDIGITS) != 0)) {
                if (((fBase.fFacetsDefined & FACET_FRACTIONDIGITS) != 0)) {
                    if (((fBase.fFixedFacet & FACET_FRACTIONDIGITS) != 0
                                    && fFractionDigits != fBase.fFractionDigits)
                        ...
                        reportError(
                                ...
                                new Object[] {
                                    "fractionDigits",
                                    Integer.toString(fFractionDigits),
                                    Integer.toString(fBase.fFractionDigits),
                                    fTypeName
                                });
                    }
                    ...
                    reportError(
                            "FixedFacetValue",
                            new Object[] {
                                "fractionDigits", Integer.toString(fFractionDigits), "0", fTypeName
                            });
                }
            }
            ...
            if ((fFacetsDefined & FACET_WHITESPACE) != 0
                    && (fBase.fFacetsDefined & FACET_WHITESPACE) != 0) {
                if ((fBase.fFixedFacet & FACET_WHITESPACE) != 0
                        && fWhiteSpace != fBase.fWhiteSpace) {
                    reportError(
                            "FixedFacetValue",
                            new Object[] {
                                "whiteSpace",
                                whiteSpaceValue(fWhiteSpace),
                                whiteSpaceValue(fBase.fWhiteSpace),
                                fTypeName
                            });
                }
                ...
                if (fWhiteSpace == WS_PRESERVE && fBase.fWhiteSpace == WS_COLLAPSE) {
                    reportError(
                            "whiteSpace-valid-restriction.1", new Object[] {fTypeName, "preserve"});
                }
                if (fWhiteSpace == WS_REPLACE && fBase.fWhiteSpace == WS_COLLAPSE) {
                    reportError(
                            "whiteSpace-valid-restriction.1", new Object[] {fTypeName, "replace"});
                }
        ...
        // inherit maxLength
        if ((fFacetsDefined & FACET_MAXLENGTH) == 0
                && (fBase.fFacetsDefined & FACET_MAXLENGTH) != 0) {
            fFacetsDefined |= FACET_MAXLENGTH;
            fMaxLength = fBase.fMaxLength;
            maxLengthAnnotation = fBase.maxLengthAnnotation;
        }
        ...
        if ((fFacetsDefined & FACET_ENUMERATION) == 0
                && (fBase.fFacetsDefined & FACET_ENUMERATION) != 0) {
            fFacetsDefined |= FACET_ENUMERATION;
            fEnumeration = fBase.fEnumeration;
            enumerationAnnotations = fBase.enumerationAnnotations;
        }
        ...
        if (((fBase.fFacetsDefined & FACET_MAXEXCLUSIVE) != 0)
                && !((fFacetsDefined & FACET_MAXEXCLUSIVE) != 0)
                && !((fFacetsDefined & FACET_MAXINCLUSIVE) != 0)) {
            fFacetsDefined |= FACET_MAXEXCLUSIVE;
            fMaxExclusive = fBase.fMaxExclusive;
            maxExclusiveAnnotation = fBase.maxExclusiveAnnotation;
        ...
        if ((fPatternType == SPECIAL_PATTERN_NONE)
                && (fBase.fPatternType != SPECIAL_PATTERN_NONE)) {
            fPatternType = fBase.fPatternType;
        }
    ...
    protected ValidatedInfo getActualEnumValue(
            String lexical, ValidationContext ctx, ValidatedInfo info)
            throws InvalidDatatypeValueException {
        return fBase.validateWithInfo(lexical, ctx, info);
    }

    /** validate a value, and return the compiled form */
    public ValidatedInfo validateWithInfo(
            String content, ValidationContext context, ValidatedInfo validatedInfo)
            throws InvalidDatatypeValueException {
    ...
    private void checkFacets(ValidatedInfo validatedInfo) throws InvalidDatatypeValueException {
        ...
        if (((fFacetsDefined & FACET_ENUMERATION) != 0)) {
            ...
            for (int i = 0; i < enumSize; i++) {
                ...
                if ((primitiveType1 == primitiveType2
                    ...
                    if (primitiveType1 == XSConstants.LIST_DT
                        ...
                        final int typeList2Length =
                                enumItemType != null ? enumItemType.getLength() : 0;
            ...
            if (!present) {
                throw new InvalidDatatypeValueException(
                        "cvc-enumeration-valid", new Object[] {content, fEnumeration.toString()});
            }
        }
        ...
        if ((fFacetsDefined & FACET_MAXINCLUSIVE) != 0) {
            ...
            if (compare != -1 && compare != 0) {
                throw new InvalidDatatypeValueException(
                        "cvc-maxInclusive-valid", new Object[] {content, fMaxInclusive, fTypeName});
            }
        }
        ...
        if ((fFacetsDefined & FACET_MAXEXCLUSIVE) != 0) {
            ...
            if (compare != -1) {
                throw new InvalidDatatypeValueException(
                        "cvc-maxExclusive-valid", new Object[] {content, fMaxExclusive, fTypeName});
            }
        }
        ...
        if ((fFacetsDefined & FACET_MININCLUSIVE) != 0) {
            ...
            if (compare != 1 && compare != 0) {
                throw new InvalidDatatypeValueException(
                        "cvc-minInclusive-valid", new Object[] {content, fMinInclusive, fTypeName});
            }
        }
        ...
        if ((fFacetsDefined & FACET_MINEXCLUSIVE) != 0) {
            ...
            if (compare != 1) {
                throw new InvalidDatatypeValueException(
                        "cvc-minExclusive-valid", new Object[] {content, fMinExclusive, fTypeName});
            }
        }
    }
    ...
    private Object getActualValue(
        ...
        if ((fFacetsDefined & FACET_PATTERN) != 0) {
            if (fPattern.size() == 0 && nvalue.length() > 0) {
                throw new InvalidDatatypeValueException(
                        "cvc-pattern-valid", new Object[] {content, "(empty string)", fTypeName});
            }
            ...
            for (int idx = fPattern.size() - 1; idx >= 0; idx--) {
                ...
                if (!regex.matches(nvalue)) {
                    throw new InvalidDatatypeValueException(
                            "cvc-pattern-valid",
                            new Object[] {content, fPatternStr.elementAt(idx), fTypeName});
                }
            }
        }
        ...
        if (fVariety == VARIETY_ATOMIC) {
            ...
            for (int i = 0; i < countOfTokens; i++) {
                ...
                avalue[i] =
                        fItemType.getActualValue(
                                parsedList.nextToken(), context, validatedInfo, false);
                if (context.needFacetChecking()
            ...
            validatedInfo.actualValueType =
                    isUnion ? XSConstants.LISTOFUNION_DT : XSConstants.LIST_DT;
            ...
            final Object _content =
                    (fMemberTypes.length > 1 && content != null) ? content.toString() : content;
            for (int i = 0; i < fMemberTypes.length; i++) {
                try {
                    ...
                    Object aValue =
                            fMemberTypes[i].getActualValue(_content, context, validatedInfo, true);
                    if (context.needFacetChecking()
    ...
    public static String normalize(String content, short ws) {
        ...
        if (len == 0 || ws == WS_PRESERVE) return content;
    ...
    public boolean isDefinedFacet(short facetName) {
        if (fValidationDV == DV_ANYSIMPLETYPE || fValidationDV == DV_ANYATOMICTYPE) {
            return false;
        }
    ...
    /**
     * [facets]: all facets defined on this type. The value is a bit combination of FACET_XXX
    ...
    public boolean isFixedFacet(short facetName) {
        if ((fFixedFacet & facetName) != 0) return true;
        if (fValidationDV == DV_INTEGER) return facetName == FACET_FRACTIONDIGITS;
        return false;
    }

    /** [facets]: all defined facets for this type which are fixed. */
    public short getFixedFacets() {
        if (fValidationDV == DV_INTEGER) return (short) (fFixedFacet | FACET_FRACTIONDIGITS);
        return fFixedFacet;
    }

    /**
     * Convenience method. Returns a value of a single constraining facet for this simple type
     * definition. This method must not be used to retrieve values for <code>enumeration</code> and
     * <code>pattern</code> facets.
     *
     * @param facetName The name of the facet, i.e. <code>FACET_LENGTH, FACET_TOTALDIGITS </code>
     *     (see <code>XSConstants</code>). To retrieve the value for a pattern or an enumeration,
     *     see <code>enumeration</code> and <code>pattern</code>.
    ...
    public StringList getLexicalEnumeration() {
        if (fLexicalEnumeration == null) {
            if (fEnumeration == null) return StringListImpl.EMPTY_LIST;
            int size = fEnumeration.size();
            String[] strs = new String[size];
            for (int i = 0; i < size; i++) strs[i] = fEnumeration.elementAt(i).toString();
    ...
    /**
     * A list of enumeration type values (as a list of ShortList objects) if it exists, otherwise
     * returns null
     */
    public ObjectList getEnumerationItemTypeList() {
        if (fEnumerationItemTypeList == null) {
            if (fEnumerationItemType == null) return null;
            fEnumerationItemTypeList =
                    new AbstractObjectList() {
                        public int getLength() {
                        ...
                        public boolean contains(Object item) {
                            ...
                            for (int i = 0; i < fEnumerationItemType.length; i++)
                                if (fEnumerationItemType[i] == item) return true;
                            return false;
                        }
    ...
    public StringList getLexicalPattern() {
        if (fPatternType == SPECIAL_PATTERN_NONE
                ...
                && fPatternStr == null) return StringListImpl.EMPTY_LIST;
        if (fLexicalPattern == null) {
            ...
            for (int i = 0; i < size; i++) strs[i] = (String) fPatternStr.elementAt(i);
    ...
    /**
     * [annotations]: a set of annotations for this simple type component if it exists, otherwise an
     * empty <code>XSObjectList</code>.
     */
    ...
    private void setOrdered() {
        ...
        // For all "primitive" types {value} is as specified in the table in Fundamental Facets
        // (C.1).
        if (fVariety == VARIETY_ATOMIC) {
        ...
        // 1. If every member of {member type definitions} is derived from a common ancestor other
        // than the simple ur-type, then {value} is the same as that ancestor's ordered facet.
        // 2. If every member of {member type definitions} has a {value} of false for the ordered
    ...
    private void setBounded() {
        if (fVariety == VARIETY_ATOMIC) {
            ...
            for (int i = 0; i < memberTypes.length; i++) {
                if (!memberTypes[i].getBounded()
                        || (ancestorId != getPrimitiveDV(memberTypes[i].fValidationDV))) {
                    this.fBounded = false;
                    return;
                }
            }
    ...
    private short getPrimitiveDV(short validationDV) {
        ...
        if (validationDV == DV_ID || validationDV == DV_IDREF || validationDV == DV_ENTITY) {
            ...
            return DV_DECIMAL;
                ...
                && (validationDV == DV_YEARMONTHDURATION || validationDV == DV_DAYTIMEDURATION)) {
            return DV_DURATION;
        } else {
            return validationDV;
        }
    ...
    /**
     * Checks if a type is derived from another by restriction, given the name and namespace. See:
     ...
     * @return boolean True if the ancestor type is derived from the reference type by the
     *     specifiied derivation method.
     */
     ...
     * See: http://www.w3.org/TR/2004/REC-DOM-Level-3-Core-20040407/core.html#TypeInfo-isDerivedFrom
     *
     * @param ancestorNS The namspace of the ancestor type declaration
     * @param ancestorName The name of the ancestor type declaration
            ...
            if (((XSSimpleTypeDecl) type).getVariety() == VARIETY_ABSENT
                ...
                for (int i = 0; i < ((XSSimpleTypeDecl) type).getMemberTypes().getLength(); i++) {
                    return isDerivedByAny(
                            ancestorNS,
                            ancestorName,
                            (XSTypeDefinition) ((XSSimpleTypeDecl) type).getMemberTypes().item(i));
                }
    ...
    /**
     * DOM Level 3 Checks if a type is derived from another by restriction. See:
     * http://www.w3.org/TR/2004/REC-DOM-Level-3-Core-20040407/core.html#TypeInfo-isDerivedFrom
     *
     * @param ancestorNS The namspace of the ancestor type declaration
     * @param ancestorName The name of the ancestor type declaration
    ...
    private boolean isDerivedByRestriction(
            String ancestorNS, String ancestorName, XSTypeDefinition type) {
        XSTypeDefinition oldType = null;
        while (type != null && type != oldType) {
            if ((ancestorName.equals(type.getName()))
                    && ((ancestorNS != null && ancestorNS.equals(type.getNamespace()))
     ...
     */
    private boolean isDerivedByUnion(
            String ancestorNS, String ancestorName, XSTypeDefinition type) {
            ...
            for (int i = 0; i < memberTypes.getLength(); i++) {
                ...
                if (memberTypes.item(i) != null) {
                    ...
                    if (isDerivedByRestriction(
                            ancestorNS,
                            ancestorName,
                            (XSSimpleTypeDefinition) memberTypes.item(i))) {
                        return true;
                    }
                }
            }
    ...
    static final XSSimpleTypeDecl fAnyAtomicType =
            new XSSimpleTypeDecl(
                    ...
                    false,
                    true,
                    XSSimpleTypeDecl.ANYATOMICTYPE_DT);
    ...
    /**
     * A wrapper of ValidationContext, to provide a way of switching to a different Namespace
     * declaration context.
     */
     ...
     * Note: This method must not be used to retrieve values for <code>enumeration</code> and <code>
     * pattern</code> facets.
     */
    public XSObjectList getFacets() {
        if (fFacets == null && (fFacetsDefined != 0 || fValidationDV == DV_INTEGER)) {
            ...
            if (fValidationDV == DV_INTEGER) {
                facets[count] =
                        new XSFacetImpl(FACET_FRACTIONDIGITS, "0", true, fractionDigitsAnnotation);
                count++;
            ...
            fFacets =
                    (count > 0) ? new XSObjectListImpl(facets, count) : XSObjectListImpl.EMPTY_LIST;
        }
        return (fFacets != null) ? fFacets : XSObjectListImpl.EMPTY_LIST;
    }

    /**
     * A list of enumeration and pattern constraining facets if it exists, otherwise an empty <code>
     * XSObjectList</code>.
     */
    public XSObjectList getMultiValueFacets() {
        if (fMultiValueFacets == null
            ...
            if ((fFacetsDefined & FACET_PATTERN) != 0
                ...
                facets[count] =
                        new XSMVFacetImpl(
                                FACET_PATTERN, this.getLexicalPattern(), patternAnnotations);
                count++;
            }
