...
public class UCSReader extends Reader {
    ...
    /**
     * Constructs an ASCII reader from the specified input stream using the default buffer size. The
     * Endian-ness and whether this is UCS-2 or UCS-4 needs also to be known in advance.
     *
     ...
     * and whether this is UCS-2 or UCS-4 needs also to be known in advance.
     *
     * @param inputStream The input stream.
     ...
     * method.
     *
     * @return The character read, as an integer in the range 0 to 127 (<tt>0x00-0x7f</tt>), or -1
        ...
        if (fEncoding >= 4) {
            ...
            System.err.println(
                    "b0 is "
                            ...
                            + " b3 "
                            + (b3 & 0xff));
            if (fEncoding == UCS4BE) return (b0 << 24) + (b1 << 16) + (b2 << 8) + b3;
            else return (b3 << 24) + (b2 << 16) + (b1 << 8) + b0;
        } else { // UCS-2
    ...
    public int read(char ch[], int offset, int length) throws IOException {
        ...
        for (int i = 0; i < numChars; i++) {
            ...
            if (fEncoding >= 4) {
                ...
                else ch[offset + i] = (char) ((b3 << 24) + (b2 << 16) + (b1 << 8) + b0);
            } else { // UCS-2
