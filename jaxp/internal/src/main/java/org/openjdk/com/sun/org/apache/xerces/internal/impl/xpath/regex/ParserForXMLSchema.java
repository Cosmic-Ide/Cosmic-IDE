...
class ParserForXMLSchema extends RegexParser {
     ...
     *
     * <p>c-c-expression ::= '[' c-group ']' c-group ::= positive-c-group | negative-c-group |
     * c-c-subtraction positive-c-group ::= (c-range | c-c-escape)+ negative-c-group ::= '^'
     * positive-c-group c-c-subtraction ::= (positive-c-group | negative-c-group) subtraction
     * subtraction ::= '-' c-c-expression c-range ::= single-range | from-to-range single-range ::=
     * multi-c-escape | category-c-escape | block-c-escape | &lt;any XML char&gt; cc-normal-c ::=
     * &lt;any character except [, ], \&gt; from-to-range ::= cc-normal-c '-' cc-normal-c
     *
            ...
            if (!end) { // if not shorthands...
                if (type == T_CHAR) {
                    ...
                    if (c == '-' && this.chardata != ']' && !firstloop)
                        throw this.ex(
                                "parser.cc.8", this.offset - 2); // if regex = '[-]' then invalid
                }
                if (this.read() != T_CHAR
                        || this.chardata != '-'
                        || c == '-' && firstloop) { // Here is no '-'.
                    ...
                    // c '-' ']' -> '-' is a single-range.
                    if (type == T_CHAR
                            && this.chardata == ']') { // if - is at the last position of the group
                        if (!this.isSet(RegularExpression.IGNORE_CASE) || c > 0xffff) {
                            tok.addRange(c, c);
                        } else {
                            addCaseInsensitiveChar(tok, c);
