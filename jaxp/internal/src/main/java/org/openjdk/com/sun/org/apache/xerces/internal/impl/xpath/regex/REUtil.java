...
public final class REUtil {
    ...
    /**
     * Sample entry. <div>Usage: <KBD>com.sun.org.apache.xerces.internal.utils.regex.REUtil
        ...
        try {
            ...
            for (int i = 0; i < argv.length; i++) {
                if (argv[i].length() == 0 || argv[i].charAt(0) != '-') {
                    if (pattern == null) pattern = argv[i];
                    else if (target == null) target = argv[i];
                    else System.err.println("Unnecessary: " + argv[i]);
            ...
            for (int i = 0; i < match.getNumberOfGroups(); i++) {
                ...
                if (match.getBeginning(i) < 0) System.out.println("-1");
                else {
