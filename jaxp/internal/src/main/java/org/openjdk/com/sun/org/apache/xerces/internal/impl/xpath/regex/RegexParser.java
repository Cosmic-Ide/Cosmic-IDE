...
class RegexParser {
    ...
    public void setLocale(Locale locale) {
        try {
            if (locale != null) {
                this.resources =
                        SecuritySupport.getResourceBundle(
                                "org.openjdk.com.sun.org.apache.xerces.internal.impl.xpath.regex.message",
                                locale);
            } else {
                this.resources =
                        SecuritySupport.getResourceBundle(
            ...
            throw new RuntimeException(
                    "Installation Problem???  Couldn't load messages: " + mre.getMessage());
        }
    }
    ...
    synchronized Token parse(String regex, int options) throws ParseException {
        ...
        if (parenCount < 0) throw ex("parser.factor.0", this.offset);
    ...
    */
            ...
            case '(':
                ...
                switch (ch) {
                    ...
                    case '=':
                        ...
                        break;
                    case '!':
                        ret = T_NEGATIVELOOKAHEAD;
                        break;
                    case '[':
                        ret = T_SET_OPERATIONS;
                        break;
                    case '>':
                        ret = T_INDEPENDENT;
                        break;
    ...
    /**
     * regex ::= term (`|` term)* term ::= factor+ factor ::= ('^' | '$' | '\A' | '\Z' | '\z' | '\b'
     * | '\B' | '\<' | '\>' | atom (('*' | '+' | '?' | minmax ) '?'? )?) | '(?=' regex ')' | '(?!'
     * regex ')' | '(?&lt;=' regex ')' | '(?&lt;!' regex ')' atom ::= char | '.' | range | '(' regex
     * ')' | '(?:' regex ')' | '\' [0-9] | '\w' | '\W' | '\d' | '\D' | '\s' | '\S' | category-block
     */
    ...
    Token processStar(Token tok) throws ParseException {
        ...
        } else return Token.createClosure(tok);
    }
    ...
    Token processPlus(Token tok) throws ParseException {
        ...
        } else return Token.createConcat(tok, Token.createClosure(tok));
    }
    ...
    Token processModifiers() throws ParseException {
        ...
        if (ch == ':') {
            ...
            this.next();
            tok = Token.createModifierGroup(this.parseRegex(), add, mask);
        } else throw ex("parser.factor.3", this.offset);
    ...
    Token processBackreference() throws ParseException {
        ...
        if (this.parennumber <= refnum) throw ex("parser.parse.2", this.offset - 2);
     ...
     * '?' | minmax ) '?'? )?) | '(?=' regex ')' | '(?!' regex ')' | '(?&lt;=' regex ')' | '(?&lt;!'
     * regex ')' | '(?#' [^)]* ')' minmax ::= '{' min (',' max?)? '}' min ::= [0-9]+ max ::= [0-9]+
     */
    ...
    /**
     * atom ::= char | '.' | char-class | '(' regex ')' | '(?:' regex ')' | '\' [0-9] | '\w' | '\W'
     * | '\d' | '\D' | '\s' | '\S' | category-block | '(?>' regex ')' char ::= '\\' | '\' [efnrt] |
     * bmp-code | character-1
     */
    Token parseAtom() throws ParseException {
        ...
        switch (ch) {
            ...
            case T_DOT:
                 ...
                 * '\]' | '\\' | '\' [,-efnrtv] | bmp-code | character-2 bmp-char ::= '\' 'u'
            ...
            case T_CHAR:
                ...
                if (REUtil.isHighSurrogate(high)
                        && this.read() == T_CHAR
                        && REUtil.isLowSurrogate(this.chardata)) {
                    char[] sur = new char[2];
                    sur[0] = (char) high;
    ...
    /**
     * char-class ::= '[' ( '^'? range ','?)+ ']' range ::= '\d' | '\w' | '\s' | category-block |
     * range-char | range-char '-' range-char range-char ::= '\[' | '\]' | '\\' | '\' [,-efnrtv] |
     * bmp-code | character-2 bmp-code ::= '\' 'u' [0-9a-fA-F] [0-9a-fA-F] [0-9a-fA-F] [0-9a-fA-F]
     */
    protected RangeToken parseCharacterClass(boolean useNrange) throws ParseException {
        ...
        while ((type = this.read()) != T_EOF) {
            ...
            // POSIX Character class such as [:alnum:]
                ...
                RangeToken range =
                        Token.getRange(
                                name, positive, this.isSet(RegularExpression.XMLSCHEMA_MODE));
    ...
    Token getTokenForShorthand(int ch) {
        ...
        switch (ch) {
            case 'd':
                tok =
                        this.isSet(RegularExpression.USE_UNICODE_CATEGORY)
                                ? Token.getRange("Nd", true)
                                : Token.token_0to9;
                break;
            case 'D':
                tok =
                        this.isSet(RegularExpression.USE_UNICODE_CATEGORY)
                                ? Token.getRange("Nd", false)
                                : Token.token_not_0to9;
                break;
            case 'w':
                tok =
                        this.isSet(RegularExpression.USE_UNICODE_CATEGORY)
                                ? Token.getRange("IsWord", true)
                                : Token.token_wordchars;
                break;
            case 'W':
                tok =
                        this.isSet(RegularExpression.USE_UNICODE_CATEGORY)
                                ? Token.getRange("IsWord", false)
                                : Token.token_not_wordchars;
                break;
            case 's':
                tok =
                        this.isSet(RegularExpression.USE_UNICODE_CATEGORY)
                                ? Token.getRange("IsSpace", true)
                                : Token.token_spaces;
                break;
            case 'S':
                tok =
                        this.isSet(RegularExpression.USE_UNICODE_CATEGORY)
                                ? Token.getRange("IsSpace", false)
