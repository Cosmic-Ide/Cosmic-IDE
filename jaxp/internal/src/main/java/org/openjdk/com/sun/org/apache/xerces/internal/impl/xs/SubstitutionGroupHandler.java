...
public class SubstitutionGroupHandler {
    ...
    public XSElementDecl getMatchingElemDecl(QName element, XSElementDecl exemplar) {
        ...
        if (exemplar.fScope != XSConstants.SCOPE_GLOBAL) return null;

        // if the decl blocks substitution, return false
        if ((exemplar.fBlock & XSConstants.DERIVATION_SUBSTITUTION) != 0) return null;

        // get grammar of the element
        SchemaGrammar sGrammar = fGrammarBucket.getGrammar(element.uri);
        if (sGrammar == null) return null;
        ...
        if (substitutionGroupOK(eDecl, exemplar, exemplar.fBlock)) return eDecl;
    ...
    protected boolean substitutionGroupOK(
            XSElementDecl element, XSElementDecl exemplar, short blockingConstraint) {
        // For an element declaration (call it D) to be validly substitutable for another element
        // declaration (call it C) subject to a blocking constraint (a subset of {substitution,
        // extension, restriction}, the value of a {disallowed substitutions}) one of the following
        // must be true:
        // 1. D and C are the same element declaration.
        if (element == exemplar) return true;

        // 2 All of the following must be true:
        // 2.1 The blocking constraint does not contain substitution.
        if ((blockingConstraint & XSConstants.DERIVATION_SUBSTITUTION) != 0) return false;

        // 2.2 There is a chain of {substitution group affiliation}s from D to C, that is, either
        // D's {substitution group affiliation} is C, or D's {substitution group affiliation}'s
        ...
        // 2.3 The set of all {derivation method}s involved in the derivation of D's {type
        // definition} from C's {type definition} does not intersect with the union of the blocking
        // constraint, C's {prohibited substitutions} (if C is complex, otherwise the empty set) and
        // the {prohibited substitutions} (respectively the empty set) of any intermediate {type
        // definition}s in the derivation of D's {type definition} from C's {type definition}.
    ...
    private boolean typeDerivationOK(
            XSTypeDefinition derived, XSTypeDefinition base, short blockingConstraint) {
        ...
        if (type != base) {
            // If the base is a union, check if "derived" is allowed through any of the member
            // types.
    ...
    public boolean inSubstitutionGroup(XSElementDecl element, XSElementDecl exemplar) {
        // [Definition:]  Every element declaration (call this HEAD) in the {element declarations}
        ...
        // 2 PSG is closed with respect to {substitution group affiliation}, that is, if any element
        // declaration in the {element declarations} has a {substitution group affiliation} in PSG,
        // then it is also in PSG itself.
        // HEAD's actual substitution group is then the set consisting of each member of PSG such
        // that all of the following must be true:
        // 1 Its {abstract} is false.
        // 2 It is validly substitutable for HEAD subject to an empty blocking constraint, as
        // defined in Substitution Group OK (Transitive) (3.3.6).
    ...
    /**
     * get all elements that can substitute the given element, according to the spec, we shouldn't
     * consider the {block} constraints.
     *
     * <p>from the spec, substitution group of a given element decl also contains the element
     * itself. but the array returned from this method doesn't containt this element.
     */
    ...
    private boolean getDBMethods(
            XSTypeDefinition typed, XSTypeDefinition typeb, OneSubGroup methods) {
