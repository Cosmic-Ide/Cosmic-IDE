...
/**
 * The XML representation for an attribute group declaration schema component is a global
 * <attributeGroup> element information item
 *
 * @xerces.internal
...
public class XSAttributeGroupDecl implements XSAttributeGroupDefinition {
    ...
    public XSAttributeUse getAttributeUse(String namespace, String name) {
        for (int i = 0; i < fAttrUseNum; i++) {
            if ((fAttributeUses[i].fAttrDecl.fTargetNamespace == namespace)
                    && (fAttributeUses[i].fAttrDecl.fName == name)) return fAttributeUses[i];
        }
    ...
    public void removeProhibitedAttrs() {
        ...
        //                    if (fAttributeUses[i].fAttrDecl.fName ==
        // pUses[fAttrUseNum-pCount].fAttrDecl.fName &&
        //                        fAttributeUses[i].fAttrDecl.fTargetNamespace ==
        // pUses[fAttrUseNum-pCount].fAttrDecl.fTargetNamespace) {
        //                        continue OUTER;
        //                    }
        //                }
     ...
     * is found, an Object[] is returned. This contains the arguments for the error message
     * describing the error. The last element in the array (at index arr.length - 1) is the the
     * error code. Returns null if there is no error.
     *
     * <p>REVISIT: is there a better way of returning the appropriate information for the error?
     *
    ...
    public Object[] validRestrictionOf(String typeName, XSAttributeGroupDecl baseGroup) {
        ...
        for (int i = 0; i < fAttrUseNum; i++) {
            ...
            baseAttrUse =
                    (XSAttributeUseImpl)
                            baseGroup.getAttributeUse(attrDecl.fTargetNamespace, attrDecl.fName);
            if (baseAttrUse != null) {
                //
                ...
                // derivation-ok-restriction.  Constraint 2.1.1
                //
                if (!XSConstraints.checkSimpleDerivationOk(
                        attrDecl.fType, baseAttrDecl.fType, baseAttrDecl.fType.getFinal())) {
                    errorArgs =
                            new Object[] {
                                typeName,
                                attrDecl.fName,
                                attrDecl.fType.getName(),
                                baseAttrDecl.fType.getName(),
                ...
                //
                int baseConsType =
                        baseAttrUse.fConstraintType != XSConstants.VC_NONE
                                ? baseAttrUse.fConstraintType
                                : baseAttrDecl.getConstraintType();
                int thisConstType =
                        attrUse.fConstraintType != XSConstants.VC_NONE
                                ? attrUse.fConstraintType
                ...
                if (baseConsType == XSConstants.VC_FIXED) {
                    ...
                    if (thisConstType != XSConstants.VC_FIXED) {
                        errorArgs =
                                new Object[] {
                                    typeName, attrDecl.fName, "derivation-ok-restriction.2.1.3.a"
                                };
                        return errorArgs;
                    } else {
                        // check the values are the same.
                        ValidatedInfo baseFixedValue =
                                (baseAttrUse.fDefault != null
                                        ? baseAttrUse.fDefault
                                        : baseAttrDecl.fDefault);
                        ValidatedInfo thisFixedValue =
                                (attrUse.fDefault != null ? attrUse.fDefault : attrDecl.fDefault);
                        if (!baseFixedValue.actualValue.equals(thisFixedValue.actualValue)) {
                            errorArgs =
                                    new Object[] {
                                        ...
                                        "derivation-ok-restriction.2.1.3.b"
                                    };
                            return errorArgs;
                        }
                    }
                ...
                if (baseGroup.fAttributeWC == null) {
                    errorArgs =
                            new Object[] {
                                typeName, attrDecl.fName, "derivation-ok-restriction.2.2.a"
                            };
                    return errorArgs;
        ...
        for (int i = 0; i < baseGroup.fAttrUseNum; i++) {
            ...
            if (baseAttrUse.fUse == SchemaSymbols.USE_REQUIRED) {
                ...
                if (getAttributeUse(baseAttrDecl.fTargetNamespace, baseAttrDecl.fName) == null) {
                    errorArgs =
                            new Object[] {
                                typeName, baseAttrUse.fAttrDecl.fName, "derivation-ok-restriction.3"
                            };
                    return errorArgs;
                }
            }
