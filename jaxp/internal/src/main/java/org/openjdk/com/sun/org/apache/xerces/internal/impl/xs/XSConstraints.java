...
public class XSConstraints {
    ...
    /**
     * check whether derived is valid derived from base, given a subset of {restriction,
     * extension}.B
     */
    public static boolean checkTypeDerivationOk(
            XSTypeDefinition derived, XSTypeDefinition base, short block) {
    ...
    /**
     * check whether simple type derived is valid derived from base, given a subset of {restriction,
     * extension}.
     */
    public static boolean checkSimpleDerivationOk(
            XSSimpleType derived, XSTypeDefinition base, short block) {
        // if derived is anySimpleType, then it's valid only if the base
        // is ur-type
        if (derived == SchemaGrammar.fAnySimpleType) {
            return (base == SchemaGrammar.fAnyType || base == SchemaGrammar.fAnySimpleType);
    ...
    /**
     * check whether complex type derived is valid derived from base, given a subset of
    ...
    /**
     * Note: this will be a private method, and it assumes that derived is not anySimpleType, and
     * base is not anyType. Another method will be introduced for public use, which will call this
     * method.
     */
    private static boolean checkSimpleDerivation(
            XSSimpleType derived, XSSimpleType base, short block) {
        ...
        // 2.2.2 D's base type definition is not the simple ur-type definition and is validly
        ...
        // 2.2.4 B's {variety} is union and D is validly derived from a type definition in B's
        // {member type definitions} given the subset, as defined by this constraint.
    ...
    /**
     * Note: this will be a private method, and it assumes that derived is not anyType. Another
     * method will be introduced for public use, which will call this method.
     */
    private static boolean checkComplexDerivation(
            XSComplexTypeDecl derived, XSTypeDefinition base, short block) {
        // 2.1 B and D must be the same type definition.
        if (derived == base) return true;

        // 1 If B and D are not the same type definition, then the {derivation method} of D must not
        // be in the subset.
        ...
        // 2.3.2.1 If D's {base type definition} is complex, then it must be validly derived from B
        ...
        // 2.3.2.2 If D's {base type definition} is simple, then it must be validly derived from B
        // given the subset as defined in Type Derivation OK (Simple) (3.14.6).
    ...
    /**
     * check whether a value is a valid default for some type returns the compiled form of the value
     * The parameter value could be either a String or a ValidatedInfo object
     */
    public static Object ElementDefaultValidImmediate(
            XSTypeDefinition type, String value, ValidationContext context, ValidatedInfo vinfo) {

        XSSimpleType dv = null;

        // e-props-correct
        // For a string to be a valid default with respect to a type definition the appropriate case
        // among the following must be true:
        // 1 If the type definition is a simple type definition, then the string must be valid with
        // respect to that definition as defined by String Valid (3.14.4).
        if (type.getTypeCategory() == XSTypeDefinition.SIMPLE_TYPE) {
            dv = (XSSimpleType) type;
        }

        // 2 If the type definition is a complex type definition, then all of the following must be
        // true:
        else {
            ...
            // 2.2.2 If the {content type} is mixed, then the {content type}'s particle must be
    ...
    static void reportSchemaError(
            XMLErrorReporter errorReporter, SimpleLocator loc, String key, Object[] args) {
        if (loc != null) {
            ...
            errorReporter.reportError(
                    XSMessageFormatter.SCHEMA_DOMAIN, key, args, XMLErrorReporter.SEVERITY_ERROR);
        }
    }

    /**
     * used to check the 3 constraints against each complex type (should be each model group):
     * Unique Particle Attribution, Particle Derivation (Restriction), Element Declrations
     * Consistent.
     */
    public static void fullSchemaChecking(
                    ...
                    if (types[j].fParticle != null) {
                        ...
                        try {
                            checkElementDeclsConsistent(
                                    types[j], types[j].fParticle, elemTable, SGHandler);
                        } catch (XMLSchemaException e) {
                            reportSchemaError(
                                    errorReporter, ctLocators[j], e.getKey(), e.getArgs());
                        }
                    }
                }
    ...
    */
    ...
    public static void checkElementDeclsConsistent(
    ...
    public static void findElemInTable(
            XSComplexTypeDecl type, XSElementDecl elem, SymbolHash elemDeclHash)
            throws XMLSchemaException {
        ...
        if ((existingElem = (XSElementDecl) (elemDeclHash.get(name))) == null) {
            ...
            if (elem.fType != existingElem.fType) {
                ...
                throw new XMLSchemaException(
                        "cos-element-consistent", new Object[] {type.fName, elem.fName});
            }
        }
    }
    ...
    private static boolean particleValidRestriction(
            ...
            case XSParticleDecl.PARTICLE_ELEMENT:
                {
                    switch (bType) {
                        ...
                        case XSParticleDecl.PARTICLE_ELEMENT:
                            {
                                checkNameAndTypeOK(
                                        (XSElementDecl) dParticle.fValue,
                                        dMinOccurs,
                                        dMaxOccurs,
                                        (XSElementDecl) bParticle.fValue,
                                        bMinOccurs,
                                        bMaxOccurs);
                        ...
                        case XSParticleDecl.PARTICLE_WILDCARD:
                            {
                                checkNSCompat(
                                        ...
                                        dMinOccurs,
                                        dMaxOccurs,
                                        (XSWildcardDecl) bParticle.fValue,
                                        bMinOccurs,
                                        bMaxOccurs,
                                        checkWCOccurrence);
                                return bExpansionHappened;
            ...
            case XSParticleDecl.PARTICLE_WILDCARD:
                {
                    switch (bType) {
                        ...
                        case XSParticleDecl.PARTICLE_WILDCARD:
                            {
                                checkNSSubset(
                                        (XSWildcardDecl) dParticle.fValue,
                                        dMinOccurs,
                                        dMaxOccurs,
                                        (XSWildcardDecl) bParticle.fValue,
                                        bMinOccurs,
            ...
            case XSModelGroupImpl.MODELGROUP_SEQUENCE:
                {
                    switch (bType) {
                        ...
                        case XSModelGroupImpl.MODELGROUP_CHOICE:
                            {
                                ...
                                int max1 =
                                        (dMaxOccurs == SchemaSymbols.OCCURRENCE_UNBOUNDED)
                                                ? dMaxOccurs
                                                : dMaxOccurs * dChildren.size();
                                checkMapAndSum(
                                        dChildren,
                                        min1,
                                        max1,
                        ...
                        case XSParticleDecl.PARTICLE_ELEMENT:
                            {
                                throw new XMLSchemaException(
                                        "cos-particle-restrict.2", new Object[] {"seq:elt"});
                            }
    ...
    private static XSParticleDecl getNonUnaryGroup(XSParticleDecl p) {
        ...
        if (p.fType == XSParticleDecl.PARTICLE_ELEMENT
                || p.fType == XSParticleDecl.PARTICLE_WILDCARD) return p;

        if (p.fMinOccurs == 1
                && p.fMaxOccurs == 1
                && p.fValue != null
                && ((XSModelGroupImpl) p.fValue).fParticleCount == 1)
    ...
    private static Vector removePointlessChildren(XSParticleDecl p) {
        ...
        if (p.fType == XSParticleDecl.PARTICLE_ELEMENT
                || p.fType == XSParticleDecl.PARTICLE_WILDCARD) return null;
    ...
    private static void checkNameAndTypeOK(
            XSElementDecl dElement, int dMin, int dMax, XSElementDecl bElement, int bMin, int bMax)
            throws XMLSchemaException {
        ...
        if (dElement.fName != bElement.fName
            ...
            throw new XMLSchemaException(
                    ...
                    new Object[] {
                        dElement.fName,
                        dElement.fTargetNamespace,
                        bElement.fName,
                        bElement.fTargetNamespace
                    });
        }
        ...
        if (!checkOccurrenceRange(dMin, dMax, bMin, bMax)) {
            throw new XMLSchemaException(
                    "rcase-NameAndTypeOK.3",
                    new Object[] {
                        dElement.fName,
                        Integer.toString(dMin),
                        dMax == SchemaSymbols.OCCURRENCE_UNBOUNDED
                                ? "unbounded"
                                : Integer.toString(dMax),
                        Integer.toString(bMin),
                        bMax == SchemaSymbols.OCCURRENCE_UNBOUNDED
                                ? "unbounded"
                                : Integer.toString(bMax)
                    });
        }
        ...
        if (bElement.getConstraintType() == XSConstants.VC_FIXED) {
            ...
            if (dElement.fType.getTypeCategory() == XSTypeDefinition.SIMPLE_TYPE
                    || ((XSComplexTypeDecl) dElement.fType).fContentType
            ...
            if (!isSimple
                            && !bElement.fDefault.normalizedValue.equals(
                                    dElement.fDefault.normalizedValue)
                    || isSimple
                            && !bElement.fDefault.actualValue.equals(
                                    dElement.fDefault.actualValue)) {
                throw new XMLSchemaException(
        ...
        if (((blockSet1 & blockSet2) != blockSet2)
                || (blockSet1 == XSConstants.DERIVATION_NONE
                        && blockSet2 != XSConstants.DERIVATION_NONE))
        ...
        if (!checkTypeDerivationOk(
            ...
            throw new XMLSchemaException(
                    ...
                    new Object[] {
                        dElement.fName, dElement.fType.getName(), bElement.fType.getName()
                    });
        }
    }

    private static void checkIDConstraintRestriction(
            XSElementDecl derivedElemDecl, XSElementDecl baseElemDecl) throws XMLSchemaException {
        // TODO
    ...
    private static void checkNSCompat(
            XSElementDecl elem,
            int min1,
            int max1,
        ...
        if (checkWCOccurrence && !checkOccurrenceRange(min1, max1, min2, max2)) {
            throw new XMLSchemaException(
                    ...
                    new Object[] {
                        ...
                        max1 == SchemaSymbols.OCCURRENCE_UNBOUNDED
                                ? "unbounded"
                                : Integer.toString(max1),
                        Integer.toString(min2),
                        max2 == SchemaSymbols.OCCURRENCE_UNBOUNDED
                                ? "unbounded"
                                : Integer.toString(max2)
                    });
        }
        ...
        if (!wildcard.allowNamespace(elem.fTargetNamespace)) {
            throw new XMLSchemaException(
                    "rcase-NSCompat.1", new Object[] {elem.fName, elem.fTargetNamespace});
        }
    }

    private static void checkNSSubset(
            XSWildcardDecl dWildcard,
            int min1,
            int max1,
            XSWildcardDecl bWildcard,
            int min2,
            int max2)
            throws XMLSchemaException {
        ...
        if (dWildcard.weakerProcessContents(bWildcard)) {
            throw new XMLSchemaException(
                    "rcase-NSSubset.3",
                    new Object[] {
                        dWildcard.getProcessContentsAsString(),
                        bWildcard.getProcessContentsAsString()
                    });
        }
    }

    private static void checkNSRecurseCheckCardinality(
            Vector children,
            int min1,
            int max1,
            SubstitutionGroupHandler dSGHandler,
            XSParticleDecl wildcard,
            int min2,
            int max2,
            boolean checkWCOccurrence)
            throws XMLSchemaException {
        ...
        if (checkWCOccurrence && !checkOccurrenceRange(min1, max1, min2, max2)) {
            throw new XMLSchemaException(
                    "rcase-NSRecurseCheckCardinality.2",
                    new Object[] {
                        Integer.toString(min1),
                        max1 == SchemaSymbols.OCCURRENCE_UNBOUNDED
                                ? "unbounded"
    ...
    private static void checkRecurse(
            Vector dChildren,
            int min1,
            int max1,
            SubstitutionGroupHandler dSGHandler,
    ...
    private static void checkRecurseUnordered(
            Vector dChildren,
            int min1,
            int max1,
            SubstitutionGroupHandler dSGHandler,
    ...
    private static void checkRecurseLax(
            Vector dChildren,
            int min1,
            int max1,
            SubstitutionGroupHandler dSGHandler,
    ...
    private static void checkMapAndSum(
            Vector dChildren,
            int min1,
            int max1,
            SubstitutionGroupHandler dSGHandler,
    ...
    public static boolean overlapUPA(
            XSElementDecl element1, XSElementDecl element2, SubstitutionGroupHandler sgHandler) {
    ...
    public static boolean overlapUPA(
            XSElementDecl element, XSWildcardDecl wildcard, SubstitutionGroupHandler sgHandler) {
    ...
    public static boolean overlapUPA(
            Object decl1, Object decl2, SubstitutionGroupHandler sgHandler) {
