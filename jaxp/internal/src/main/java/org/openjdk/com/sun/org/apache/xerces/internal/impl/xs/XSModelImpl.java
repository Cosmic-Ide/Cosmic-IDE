...
import org.openjdk.com.sun.org.apache.xerces.internal.xs.XSNamespaceItemList;
...
/**
 * Implements XSModel: a read-only interface that represents an XML Schema, which could be
 * components from different namespaces.
 *
 * @xerces.internal
...
public final class XSModelImpl extends AbstractList implements XSModel, XSNamespaceItemList {
    ...
    /**
     * Construct an XSModelImpl, by storing some grammars and grammars imported by them to this
     * object.
     *
    ...
    /**
     * A set of namespace schema information information items (of type <code>XSNamespaceItem</code>
     * ), one for each namespace name which appears as the target namespace of any schema component
     * in the schema used for that assessment, and one for absent if any schema component in the
     * schema had no target namespace. For more information see schema information.
     */
    ...
    /**
     * Returns a list of top-level components, i.e. element declarations, attribute declarations,
     * etc.
     *
     ...
     * @return A list of top-level definitions of the specified type in <code>objectType</code> or
     *     an empty <code>XSNamedMap</code> if no such definitions exist.
     */
    public synchronized XSNamedMap getComponents(short objectType) {
        ...
        if (fGlobalComponents[objectType] == null) {
            ...
            if (objectType == XSTypeDefinition.COMPLEX_TYPE
                ...
                fGlobalComponents[objectType] =
                        new XSNamedMap4Types(fNamespaces, tables, fGrammarCount, objectType);
            } else {
                fGlobalComponents[objectType] =
                        new XSNamedMapImpl(fNamespaces, tables, fGrammarCount);
            }
        }
    ...
    /**
     * Convenience method. Returns a list of top-level component declarations that are defined
     * within the specified namespace, i.e. element declarations, attribute declarations, etc.
     *
    ...
    public XSAttributeDeclaration getAttributeDeclaration(
            String name, String namespace, String loc) {
        SchemaGrammar sg = (SchemaGrammar) fGrammarMap.get(null2EmptyString(namespace));
        if (sg == null) {
            return null;
        }
     ...
     *     exist.
     */
    public XSModelGroupDefinition getModelGroupDefinition(
            String name, String namespace, String loc) {
        SchemaGrammar sg = (SchemaGrammar) fGrammarMap.get(null2EmptyString(namespace));
        if (sg == null) {
            return null;
    ...
    /**
     * [annotations]: a set of annotations if it exists, otherwise an empty <code>XSObjectList
     * </code>.
     */
    ...
    /**
     * REVISIT: to expose identity constraints from XSModel. For now, we only expose whether there
     * are any IDCs. We also need to add these methods to the public XSModel interface.
     */
    ...
    /**
     * Convenience method. Returns a list containing the members of the substitution group for the
     * given <code>XSElementDeclaration</code> or an empty <code>XSObjectList</code> if the
     * substitution group contains no members.
     *
     * @param head The substitution group head.
     * @return A list containing the members of the substitution group for the given <code>
     *     XSElementDeclaration</code> or an empty <code>XSObjectList</code> if the substitution
    ...
    /**
     * The number of <code>XSNamespaceItem</code>s in the list. The range of valid child object
     ...
     * @return The <code>XSNamespaceItem</code> at the <code>index</code>th position in the <code>
     *     XSNamespaceItemList</code>, or <code>null</code> if the index specified is not valid.
     */
