...
/**
 * The element declaration schema component traverser. <element abstract = boolean : false block =
 * (#all | List of (extension | restriction | substitution)) default = string final = (#all | List
 * of (extension | restriction)) fixed = string form = (qualified | unqualified) id = ID maxOccurs =
 * (nonNegativeInteger | unbounded) : 1 minOccurs = nonNegativeInteger : 1 name = NCName nillable =
 * boolean : false ref = QName substitutionGroup = QName type = QName {any attributes with
 * non-schema namespace . . .}> Content: (annotation?, ((simpleType | complexType)?, (unique | key |
 * keyref)*)) </element>
 *
 * @xerces.internal
...
class XSDElementTraverser extends XSDAbstractTraverser {
     ...
     *
     * <p>To handle the recursive cases efficiently, we delay the traversal and return an empty
     * particle node. We'll fill in this particle node later after we've done with all the global
     * declarations. This method causes a number of data structures in the schema handler to be
     * filled in.
     *
     * @param elmDecl
     * @param schemaDoc
    ...
    XSParticleDecl traverseLocal(
        ...
        if (fDeferTraversingLocalElements) {
            ...
            fSchemaHandler.fillInLocalElemInfo(
                    elmDecl, schemaDoc, allContextFlags, parent, particle);
        } else {
     ...
     * Traverse a locally declared element (or an element reference).
     *
     * <p>This is the real traversal method. It's called after we've done with all the global
     * declarations.
     *
     * @param index
     */
    protected void traverseLocal(
        ...
        if (elmDecl.getAttributeNode(SchemaSymbols.ATT_REF) != null) {
            if (refAtt != null) {
                element =
                        (XSElementDecl)
                                fSchemaHandler.getGlobalDecl(
                                        schemaDoc, XSDHandler.ELEMENT_TYPE, refAtt, elmDecl);
                    ...
                    if (text != null) {
                        annotation =
                                traverseSyntheticAnnotation(
                                        elmDecl, text, attrValues, false, schemaDoc);
                    }
                }
                ...
                // 2.2 If ref is present, then all of <complexType>, <simpleType>, <key>, <keyref>,
                // <unique>, nillable, default, fixed, form, block and type must be absent, i.e.
                // only minOccurs, maxOccurs, id are allowed in addition to ref, along with
                // <annotation>.
                if (child != null) {
                    reportSchemaError(
                            "src-element.2.2",
                            new Object[] {refAtt.rawname, DOMUtil.getLocalName(child)},
                            child);
                }
            } else {
                element = null;
        ...
        if (refAtt != null) {
            ...
            particle.fAnnotations =
                    ((element != null) ? element.fAnnotations : XSObjectListImpl.EMPTY_LIST);
        }
        Long defaultVals = (Long) attrValues[XSAttributeChecker.ATTIDX_FROMDEFAULT];
        checkOccurrences(
                particle,
                SchemaSymbols.ELT_ELEMENT,
                (Element) elmDecl.getParentNode(),
                allContextFlags,
    ...
    XSElementDecl traverseGlobal(Element elmDecl, XSDocumentInfo schemaDoc, SchemaGrammar grammar) {
        ...
        XSElementDecl element =
                traverseNamedElement(elmDecl, attrValues, schemaDoc, grammar, true, null);
        fAttrChecker.returnAttrArray(attrValues, schemaDoc);
        return element;
    }
    ...
    XSElementDecl traverseNamedElement(
            ...
            if ((element.fBlock != XSConstants.DERIVATION_ALL)
                ...
                reportSchemaError(
                        ...
                        new Object[] {
                            element.fName,
                            "block",
                            "must be (#all | List of (extension | restriction | substitution))"
                        },
                        elmDecl);
            }
        }
        ...
        if (subGroupAtt != null) {
            element.fSubGroup =
                    (XSElementDecl)
                            fSchemaHandler.getGlobalDecl(
                                    schemaDoc, XSDHandler.ELEMENT_TYPE, subGroupAtt, elmDecl);
        }
        ...
        if (child != null && DOMUtil.getLocalName(child).equals(SchemaSymbols.ELT_ANNOTATION)) {
            ...
            if (text != null) {
                annotation =
                        traverseSyntheticAnnotation(elmDecl, text, attrValues, false, schemaDoc);
            }
        }
        ...
        if (child != null) {
            ...
            if (childName.equals(SchemaSymbols.ELT_COMPLEXTYPE)) {
                elementType =
                        fSchemaHandler.fComplexTypeTraverser.traverseLocal(
        ...
        if (elementType == null && typeAtt != null) {
            elementType =
                    (XSTypeDefinition)
                            fSchemaHandler.getGlobalDecl(
                                    schemaDoc, XSDHandler.TYPEDECL_TYPE, typeAtt, elmDecl);
            if (elementType == null) {
        ...
        if (child != null) {
            ...
            while (child != null
                ...
                if (childName.equals(SchemaSymbols.ELT_KEY)
                    ...
                    fSchemaHandler.fUniqueOrKeyTraverser.traverse(
                            child, element, schemaDoc, grammar);
                    if (DOMUtil.getAttrValue(child, SchemaSymbols.ATT_NAME).length() != 0) {
                        fSchemaHandler.checkForDuplicateNames(
                                (schemaDoc.fTargetNamespace == null)
                                        ? "," + DOMUtil.getAttrValue(child, SchemaSymbols.ATT_NAME)
                                        : schemaDoc.fTargetNamespace
                                                + ","
                                                + DOMUtil.getAttrValue(
                                                        child, SchemaSymbols.ATT_NAME),
                                fSchemaHandler.ATTRIBUTE_TYPE,
                                fSchemaHandler.getIDRegistry(),
                                fSchemaHandler.getIDRegistry_sub(),
                                child,
                                schemaDoc);
                    }
        ...
        if (nameAtt == null) {
            if (isGlobal)
                reportSchemaError(
                        "s4s-att-must-appear",
                        new Object[] {SchemaSymbols.ELT_ELEMENT, SchemaSymbols.ATT_NAME},
                        elmDecl);
        ...
        // 2.2 If ref is present, then all of <complexType>, <simpleType>, <key>, <keyref>,
        // <unique>, nillable, default, fixed, form, block and type must be absent, i.e. only
        // minOccurs, maxOccurs, id are allowed in addition to ref, along with <annotation>.
        ...
        // 2 If there is a {value constraint}, the canonical lexical representation of its value
        // must be valid with respect to the {type definition} as defined in Element Default Valid
        // (Immediate) (3.3.6).
        if (element.fDefault != null) {
            fValidationState.setNamespaceSupport(schemaDoc.fNamespaceSupport);
            if (XSConstraints.ElementDefaultValidImmediate(
                            element.fType,
                            element.fDefault.normalizedValue,
                            fValidationState,
                            element.fDefault)
                    == null) {
                reportSchemaError(
                        "e-props-correct.2",
                        new Object[] {nameAtt, element.fDefault.normalizedValue},
                        elmDecl);
        ...
        // 4 If there is an {substitution group affiliation}, the {type definition} of the element
        // declaration must be validly derived from the {type definition} of the {substitution group
        // affiliation}, given the value of the {substitution group exclusions} of the {substitution
        // group affiliation}, as defined in Type Derivation OK (Complex) (3.4.6) (if the {type
        // definition} is complex) or as defined in Type Derivation OK (Simple) (3.14.6) (if the
        // {type definition} is simple).
        if (element.fSubGroup != null) {
            if (!XSConstraints.checkTypeDerivationOk(
                    element.fType, element.fSubGroup.fType, element.fSubGroup.fFinal)) {
                reportSchemaError(
                        "e-props-correct.4",
                        new Object[] {nameAtt, subGroupAtt.prefix + ":" + subGroupAtt.localpart},
                        elmDecl);
                element.fSubGroup = null;
            }
        }

        // 5 If the {type definition} or {type definition}'s {content type} is or is derived from ID
        // then there must not be a {value constraint}.
        if (element.fDefault != null) {
