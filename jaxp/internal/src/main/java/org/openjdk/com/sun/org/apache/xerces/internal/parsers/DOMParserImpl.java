...
/**
 * This is Xerces DOM Builder class. It uses the abstract DOM parser with a document scanner, a dtd
 * scanner, and a validator, as well as a grammar pool.
 *
...
public class DOMParserImpl extends AbstractDOMParser implements LSParser, DOMConfiguration {
    ...
    /**
     * When the application provides a filter, the parser will call out to the filter at the
     * completion of the construction of each <code>Element</code> node. The filter implementation
     * can choose to remove the element from the document being constructed (unless the element is
     * the document element) or to terminate the parse early. If the document is being validated
    ...
    public Object getParameter(String name) throws DOMException {
        if (name.equalsIgnoreCase(Constants.DOM_COMMENTS)) {
            ...
            boolean infoset =
                    fConfiguration.getFeature(NAMESPACES)
    ...
    public boolean canSetParameter(String name, Object value) {
        ...
        if (value instanceof Boolean) {
            ...
            if (name.equalsIgnoreCase(Constants.DOM_ERROR_HANDLER)) {
                ...
                if (value instanceof String || value == null) return true;
                return false;
            } else if (name.equalsIgnoreCase(DOCUMENT_CLASS_NAME)) {
                return true;
            }
    ...
    public Document parseURI(String uri) throws LSException {
        ...
        try {
            ...
            if (e != Abort.INSTANCE) {
                ...
                throw (LSException)
                        DOMUtil.createLSException(LSException.PARSE_ERR, e).fillInStackTrace();
            }
        }
    ...
    /**
     * Parse an XML document or fragment from a resource identified by an <code>LSInput</code> and
     * insert the content into an existing document at the position epcified with the <code>
     * contextNode</code> and <code>action</code> arguments. When parsing the input stream the
     ...
     *     node being inserted and the existing children of the context node. The set of possible
     *     actions is defined above.
     * @exception DOMException HIERARCHY_REQUEST_ERR: Thrown if this action results in an invalid
    ...
    public void abort() {
        ...
        if (fBusy) {
            ...
            if (currentThread != null) {
                ...
                if (currentThread == Thread.currentThread()) throw Abort.INSTANCE;
    ...
    private class AbortHandler
        ...
        public void startExternalSubset(
                XMLResourceIdentifier identifier, Augmentations augmentations) throws XNIException {
            throw Abort.INSTANCE;
        }
    ...
    private static DOMException newFeatureNotFoundError(String name) {
        String msg =
                DOMMessageFormatter.formatMessage(
                        DOMMessageFormatter.DOM_DOMAIN, "FEATURE_NOT_FOUND", new Object[] {name});
    ...
    private static DOMException newTypeMismatchError(String name) {
        String msg =
                DOMMessageFormatter.formatMessage(
                        DOMMessageFormatter.DOM_DOMAIN, "TYPE_MISMATCH_ERR", new Object[] {name});
