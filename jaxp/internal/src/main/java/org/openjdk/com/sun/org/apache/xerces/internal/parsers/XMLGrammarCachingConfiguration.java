...
import org.openjdk.com.sun.org.apache.xerces.internal.impl.dtd.XMLDTDLoader;
...
/**
 * This configuration provides a generic way of using Xerces's grammar caching facilities. It
 * extends the XIncludeAwareParserConfiguration and thus may validate documents according to XML
 * schemas or DTD's. It also allows the user to preparse a grammar, and to lock the grammar pool
 * implementation such that no more grammars will be added.
 *
 * <p>Using the com.sun.org.apache.xerces.internal.xni.parser property, an application may
 * instantiate a Xerces SAX or DOM parser with this configuration. When invoked in this manner, the
 * default behaviour will be elicited; to use this configuration's specific facilities, the user
 * will need to reference it directly.
 *
...
public class XMLGrammarCachingConfiguration extends XIncludeAwareParserConfiguration {
    ...
    /**
     * Parse a grammar from a location identified by an URI. This method also adds this grammar to
     * the XMLGrammarPool
     *
     * @param type The type of the grammar to be constructed
     * @param uri The location of the grammar to be constructed. <strong>The parser will not expand
     *     this URI or make it available to the EntityResolver</strong>
