...
public class URI implements Serializable {
    ...
    /**
     * URI punctuation mark characters: -_.!~*'() - these, combined with alphanumerics, constitute
     ...
     * fields as necessary. If the specification does not follow the "generic URI" syntax, the
     * specification is parsed into a scheme and scheme-specific part (stored as the path) only.
     * Construct a relative URI if boolean is assigned to "true" and p_uriSpec is not valid absolute
     * URI, instead of throwing an exception.
     *
     * @param p_uriSpec the URI specification string (cannot be null or empty)
    ...
    /**
     * Construct a new URI from a base URI and a URI specification string. The URI specification
     * string may be a relative URI. Construct a relative URI if boolean is assigned to "true" and
     * p_uriSpec is not valid absolute URI and p_base is null instead of throwing an exception.
     *
     * @param p_base the base URI (cannot be null if p_uriSpec is null or empty)
     * @param p_uriSpec the URI specification string (cannot be null or empty if p_base is null)
     * @param allowNonAbsoluteURI true to permit non-absolute URIs, false otherwise.
     ...
     * @exception MalformedURIException if any of the parameters violates syntax rules or semantic
     *     rules
     */
    public URI(
     ...
     *     otherwise.
     * @exception MalformedURIException if p_base is null and p_uriSpec is not an absolute URI or if
        ...
        if (host.length() > 0) {
            ...
            if (hasPort) {
                ...
                if (portStr.length() > 0) {
                    ...
                    /**
                     * for (int i = 0; i < portStr.length(); i++) { if (!isDigit(portStr.charAt(i)))
                     * { throw new MalformedURIException( portStr + " is invalid. Port should only
                     * contain digits!"); } }*
                     */
                    // REVISIT: Remove this code.
     ...
     * @exception MalformedURIException if p_host is not a valid IP address or DNS hostname.
     */
     ...
     * @exception MalformedURIException if p_port is not -1 and not a valid port number
     */
     ...
     *
     * <p>Note: This method overwrites server based authority if it previously existed in this URI.
     *
     * @param authority the registry based authority for this URI
     * @exception MalformedURIException it authority is not a well formed registry based authority
     */
     ...
     * @exception MalformedURIException if p_addToPath contains syntax errors
     */
     ...
     * @exception MalformedURIException if p_queryString is not null and this URI does not conform
     ...
     * @exception MalformedURIException if p_fragment is not null and this URI does not conform to
    ...
    /**
     * Determine whether a string is syntactically capable of representing a valid IPv4 address,
     * IPv6 reference or the domain name of a network host. A valid IPv4 address consists of four
     * decimal digit groups separated by a '.'. Each group must consist of one to three digits. See
     * RFC 2732 Section 3, and RFC 2373 Section 2.2, for the definition of IPv6 references. A
     * hostname consists of domain labels (each of which must begin and end with an alphanumeric but
     ...
     *
     * <p><code>IPv4address = 1*3DIGIT "." 1*3DIGIT "." 1*3DIGIT "." 1*3DIGIT</code>
     *
    ...
    /**
     * Determines whether a string is an IPv6 reference as defined by RFC 2732, where IPv6address is
     * defined in RFC 2373. The IPv6 address is parsed according to Section 2.2 of RFC 2373, with
     * the additional constraint that the address be composed of 128 bits of information.
     *
     * <p><code>IPv6reference = "[" IPv6address "]"</code>
     *
     * <p>Note: The BNF expressed in RFC 2373 Appendix B does not accurately describe section 2.2,
     * and was in fact removed from RFC 3513, the successor of RFC 2373.
     *
        ...
        return (index == end)
                || (index != -1
                        && isWellFormedIPv4Address(
                                address.substring(
                                        (counter[0] > prevCount) ? index + 1 : index, end)));
    }
     ...
     * @return the index of the next character to scan, or -1 if the string cannot match a valid
     *     IPv6 address
     */
