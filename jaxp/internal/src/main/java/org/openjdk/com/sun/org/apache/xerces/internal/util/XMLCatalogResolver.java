...
/**
 * The catalog resolver handles the resolution of external identifiers and URI references through
 * XML catalogs. This component supports XML catalogs defined by the <a
 * href="http://www.oasis-open.org/committees/entity/spec.html">OASIS XML Catalogs
 * Specification</a>. It encapsulates the <a href="http://xml.apache.org/commons/">XML Commons</a>
 * resolver. An instance of this class may be registered on the parser as a SAX entity resolver, as
 * a DOM LSResourceResolver or as an XNI entity resolver by setting the property
 * (http://apache.org/xml/properties/internal/entity-resolver).
 *
 * <p>It is intended that this class may be used standalone to perform catalog resolution outside of
 * a parsing context. It may be shared between several parsers and the application.
 *
...
public class XMLCatalogResolver implements XMLEntityResolver, EntityResolver2, LSResourceResolver {
    ...
    /**
     * Indicates whether the application desires that the parser or some other component performing
     * catalog resolution should use the literal system identifier instead of the expanded system
     * identifier.
     */
    ...
    /**
     * Constructs a catalog resolver with the given list of entry files and the preference for
     * whether system or public matches are preferred.
     *
     * @param catalogs an ordered array list of absolute URIs
    ...
    /**
     * Sets the initial list of catalog entry files. If there were any catalog mappings cached from
     * the previous list they will be replaced by catalog mappings from the new list the next time
    ...
    /**
     * Returns the preference for whether system or public matches are preferred. This is used in
     * the absence of any occurence of the <code>prefer</code> attribute on the <code>catalog</code>
     * entry of a catalog. If this property has not yet been explicitly set its value is <code>true
     * </code>.
     *
    ...
    /**
     * Sets the preference for whether system or public matches are preferred. This is used in the
     * absence of any occurence of the <code>prefer</code> attribute on the <code>catalog</code>
     * entry of a catalog.
     *
     * @param preferPublic the prefer public setting
     */
    ...
    /**
     * Returns the preference for whether the literal system identifier should be used when
     * resolving system identifiers when both it and the expanded system identifier are available.
     * If this property has not yet been explicitly set its value is <code>true</code>.
     *
     * @return the preference for using literal system identifers for catalog resolution
    ...
    /**
     * Sets the preference for whether the literal system identifier should be used when resolving
     * system identifiers when both it and the expanded system identifier are available.
     *
     * <p>The literal system identifier is the URI as it was provided before absolutization. It may
     * be embedded within an entity. It may be provided externally or it may be the result of
     * redirection. For example, redirection may have come from the protocol level through HTTP or
     * from an application's entity resolver.
     *
     * <p>The expanded system identifier is an absolute URI which is the result of resolving the
     * literal system identifier against a base URI.
     *
     * @param useLiteralSystemId the preference for using literal system identifers for catalog
     *     resolution
     */
     ...
     * <code>null</code>. This method returns an input source if an entry was found in the catalog
     * for the given external identifier. It should be overrided if other behaviour is required.
     *
    ...
    /**
     * Locates an external subset for documents which do not explicitly provide one. This method
     * always returns <code>null</code>. It should be overrided if other behaviour is required.
     *
    ...
    /**
     * Resolves an external entity. If the entity cannot be resolved, this method should return
     * <code>null</code>. This method only calls <code>resolveIdentifier</code> and returns an input
     * source if an entry was found in the catalog. It should be overrided if other behaviour is
     * required.
     *
     ...
     * if no mapping exists. If the system identifier is an URN in the <code>publicid</code>
     * namespace it is converted into a public identifier by URN "unwrapping" as specified in the
    ...
    /**
     * Returns the URI mapping in the catalog for the given external identifier or <code>null</code>
     * if no mapping exists. Public identifiers are normalized before comparison.
     *
     ...
     * mapping exists. URI comparison is case sensitive. If the URI reference is an URN in the
     * <code>publicid</code> namespace it is converted into a public identifier by URN "unwrapping"
     * as specified in the XML Catalogs specification and then resolution is performed following the
    ...
    private void init(String[] catalogs, boolean preferPublic) {
        ...
        fResolverCatalogManager.setCatalogClassName(
                "org.openjdk.com.sun.org.apache.xml.internalresolver.Catalog");
