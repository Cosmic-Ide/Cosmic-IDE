...
/**
 * This interface is used to resolve external parsed entities. The application can register an
 * object that implements this interface with the parser configuration in order to intercept
 * entities and resolve them explicitly. If the registered entity resolver cannot resolve the
 * entity, it should return <code>null</code> so that the parser will try to resolve the entity
 * using a default mechanism.
 *
