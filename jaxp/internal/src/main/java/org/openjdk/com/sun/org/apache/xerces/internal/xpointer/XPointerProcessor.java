...
/**
 * The XPointerProcessor is responsible for parsing an XPointer expression and and providing scheme
 * specific resolution of the document fragment pointed to be the pointer.
 *
 * @xerces.internal
 */
public interface XPointerProcessor {
    ...
    /**
     * Parses an XPointer expression. It performs scheme specific processing depending on the
     * pointer parts and sets up a Vector of XPointerParts in the order (left-to-right) they appear
     * in the XPointer expression.
     *
     * @param xpointer A String representing the xpointer expression.
     * @throws XNIException Thrown if the xpointer string does not conform to the XPointer Framework
     *     syntax or the syntax of the pointer part does not conform to its definition for its
     *     scheme.
     */
    ...
    /**
     * Returns true if the XPointer expression resolves to the current resource fragment or Node
     * which is part of the input resource being streamed else returns false.
     *
     * @return True if the xpointer expression matches a node/fragment in the resource else returns
     *     false.
     * @throws XNIException Thrown to signal an error
     */
