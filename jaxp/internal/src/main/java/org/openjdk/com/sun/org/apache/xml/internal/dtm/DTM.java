...
public interface DTM {
    ...
    /**
     * The node is a <code>namespace node</code>. Note that this is not currently a node type
     * defined by the DOM API.
     */
     ...
     *
     * <p>%TBD% Note that we currently have no way to support the DOM's old getAttribute() call,
     * which accesses only the qname.
     *
     ...
     * @param inScope true if all namespaces in scope should be returned, false if only the node's
     *     own namespace declarations should be returned.
     ...
     *
     * @param baseHandle handle to original node from where the first child was relative to (needed
     *     to return nodes in document order).
     ...
     * Note that if the DTM is configured so it can contain multiple documents, this call will
     * return the Document currently under construction -- but may return null if it's between
     * documents. Generally, you should use getOwnerDocument(nodeHandle) or
     * getDocumentRoot(nodeHandle) instead.
     *
     * @return int Node handle of document, or DTM.NULL if a shared DTM can not tell us which
     *     Document is currently active.
     */
    public int getDocument();

    /**
     * Given a node handle, find the owning document node. This version mimics the behavior of the
     * DOM call by the same name.
     *
     * @param nodeHandle the id of the node.
     ...
     *     representing the chunk's start position within the returned character buffer and the
     *     length of the chunk.
     ...
     * handle, return the prefix used to map to the namespace. (As defined in Namespaces, this is
     * the portion of the name before any colon character).
     *
     ...
     * usable URI or that our caller can map it to one. %REVIEW% Resolve Public Identifiers... or
    ...
    /**
     * 2. [element content whitespace] A boolean indicating whether a text node represents white
     * space appearing within element content (see [XML], 2.10 "White Space Handling"). Note that
     * validating XML processors are required by XML 1.0 to provide this information... but that DOM
     * Level 2 did not support it, since it depends on knowledge of the DTD which DOM2 could not
     * guarantee would be available.
     *
     * <p>If there is no declaration for the containing element, an XML processor must assume that
     * the whitespace could be meaningful and return false. If no declaration has been read, but the
     * [all declarations processed] property of the document information item is false (so there may
     * be an unread declaration), then the value of this property is indeterminate for white space
     ...
     * start-tag of its element, or was defaulted from the DTD (or schema).
     *
     ...
     *     CoroutineParser) and thus require that the transformation and the parse run
     ...
     * insertionCursor.appendChild(document.importNode(newChild))) where the insert point is the
     * last element that was appended (or the last one popped back to by an end-element operation).
     *
