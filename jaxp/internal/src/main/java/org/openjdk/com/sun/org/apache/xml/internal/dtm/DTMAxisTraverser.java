...
public abstract class DTMAxisTraverser {
     ...
     * iteration will go into an infinate loop. So to traverse an axis, the first function must be
     * used to get the first node.
     *
     ...
     *     traversal, typically the node-handle returned by the previous traversal step. For the
     *     first traversal step, context should be set equal to current. Note that in order to test
     *     whether context is in the set, you must use the first() method instead.
