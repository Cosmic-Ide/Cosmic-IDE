...
public interface DTMFilter {
    ...
    /**
     * Show <code>Attr</code> nodes. This is meaningful only when creating an iterator or
     * tree-walker with an attribute node as its <code>root</code>; in this case, it means that the
     * attribute node will appear in the first position of the iteration or traversal. Since
     * attributes are never children of other nodes, they do not appear when traversing over the
     * main document tree.
     */
     ...
     * expanded while the tree was being constructed, these nodes will not appear and this mask has
     * no effect.
     */
     ...
     * tree-walker with a <code>Notation</code> node as its <code>root</code>; in this case, it
     * means that the <code>Notation</code> node will appear in the first position of the traversal.
     * Since notations are not part of the document tree, they do not appear when traversing over
     ...
     * Normally, this function will be called by the implementation of <code>DTMIterator</code>; it
     * is not normally called directly from user code.
     *
     ...
     * is not normally called directly from user code.
     *
     * <p>TODO: Should this be setNameMatch(expandedName) followed by accept()? Or will we really be
     * testing a different name at every invocation?
     *
     * <p>%REVIEW% Under what circumstances will this be used? The cases I've considered are just as
     * easy and just about as efficient if the name test is performed in the DTMIterator... -- Joe
     *
     * <p>%REVIEW% Should that 0xFFFF have a mnemonic assigned to it? Also: This representation is
     * assuming the expanded name is indeed split into high/low 16-bit halfwords. If we ever change
     * the balance between namespace and localname bits (eg because we decide there are many more
     * localnames than namespaces, which is fairly likely), this is going to break. It might be
     * safer to encapsulate the details with a makeExpandedName method and make that responsible for
     * setting up the wildcard version as well.
     *
     * @param nodeHandle int Handle of the node.
     * @param whatToShow one of SHOW_XXX values.
     * @param expandedName a value defining the exanded name as defined in the DTM interface. Wild
     *     cards will be defined by 0xFFFF in the namespace and/or localname portion of the
     *     expandedName.
     * @return one of FILTER_ACCEPT, FILTER_REJECT, or FILTER_SKIP.
     */
