...
public interface DTMIterator {
    ...
    /**
     * The root node of the <code>DTMIterator</code>, as specified when it was created. Note the
     * root node is not the root node of the document tree, but the context node from where the
     * iteration begins and ends.
     *
    ...
    /**
     * Reset the root node of the <code>DTMIterator</code>, overriding the value specified when it
     * was created. Note the root node is not the root node of the document tree, but the context
     * node from where the iteration begins.
     *
    ...
    /**
     * Reset the iterator to the start. After resetting, the next node returned will be the root
     * node -- or, if that's filtered out, the first node within the root's subtree which is _not_
     ...
     * of constants is defined above. Nodes not accepted by <code>whatToShow</code> will be skipped,
     * but their children may still be considered.
     *
     ...
     * to the iterator. If false, they and their descendants will be rejected. Note that this
     * rejection takes precedence over <code>whatToShow</code> and the filter.
     *
     ...
     * reference node and set <code>expandEntityReferences</code> to true when creating the
     * iterator. To produce a view of the document that has entity reference nodes but no entity
     * expansion, use the <code>whatToShow</code> flags to show the entity reference node and set
     * <code>expandEntityReferences</code> to false.
     *
     * <p>NOTE: In Xalan's use of DTM we will generally have fully expanded entity references when
     * the document tree was built, and thus this flag will have no effect.
     *
     ...
     * a <code>DTMIterator</code> has setRoot called, the first call to <code>nextNode()</code>
     * returns that root or (if it is rejected by the filters) the first node within its subtree
     * which is not filtered out.
     *
     * @return The next node handle in the set being iterated over, or <code>DTM.NULL</code> if
    ...
    /**
     * Get the current node in the iterator. Note that this differs from the DOM's NodeIterator,
     * where the current position lies between two nodes (as part of the maintain-relative-position
     * semantic).
     *
    ...
    /**
     * If setShouldCacheNodes(true) is called, then nodes will be cached, enabling random access,
     * and giving the ability to do sorts and the like. They are not cached by default.
     *
     * <p>%REVIEW% Shouldn't the other random-access methods throw an exception if they're called on
     * a DTMIterator with this flag set false?
     *
