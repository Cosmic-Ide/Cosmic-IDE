 ...
 * "org.openjdk.com.sun.org.apache.xml.internalutils.DTMFactory". This property names a concrete
 * subclass of the DTMFactory abstract class. If the property is not defined, a platform default is
 * be used.
 *
 ...
 * org.openjdk.javax.xml.transform.TransformerFactory in terms of its factory support.
 *
 * <p>State: In progress!!
 */
public abstract class DTMManager {
     ...
     * instance using the default <code>DTMManager</code> implementation, which is <code>
     * com.sun.org.apache.xml.internal.dtm.ref.DTMManagerDefault</code>.
     * </ul>
     *
     ...
     *
     * <p>Note that DTMIterators may not be an exact match to DOM NodeIterators. They are
     * initialized and used in much the same way as a NodeIterator, but their response to document
     * mutation is not currently defined.
     *
    ...
    /**
     * This value, set at compile time, controls how many bits of the DTM node identifier numbers
     * are used to identify a node within a document, and thus sets the maximum number of nodes per
     ...
     *
     * <p>(FuncGenerateKey currently uses the node identifier directly and thus is affected when
     * this changes. The IDKEY results will still be _correct_ (presuming no other breakage), but
     * simple equality comparison against the previous "golden" files will probably complain.)
     */
    ...
    /**
     * A configuration error. Originally in ObjectFactory. This is the only portion used in this
     * package
     */
