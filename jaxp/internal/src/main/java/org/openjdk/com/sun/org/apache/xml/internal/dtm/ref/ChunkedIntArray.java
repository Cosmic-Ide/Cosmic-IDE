...
/**
 * <code>ChunkedIntArray</code> is an extensible array of blocks of integers. (I'd consider Vector,
 * but it's unable to handle integers except by turning them into Objects.)
 *
 * <p>Making this a separate class means some call-and-return overhead. But doing it all inline
 * tends to be fragile and expensive in coder time, not to mention driving up code size. If you want
 * to inline it, feel free. The Java text suggest that private and Final methods may be inlined, and
 * one can argue that this beast need not be made subclassable...
 *
 * <p>%REVIEW% This has strong conceptual overlap with the IntVector class. It would probably be a
 * good thing to merge the two, when time permits.
 *
 * <p>
 */
final class ChunkedIntArray {
    ...
    /**
     * Create a new CIA with specified record size. Currently record size MUST be a power of two...
     * and in fact is hardcoded to 4.
     */
        ...
        if (this.slotsize < slotsize)
            throw new ArrayIndexOutOfBoundsException(
                    XMLMessages.createXMLMessage(
                            ...
                            new Object[] {
                                Integer.toString(slotsize)
                            })); // "ChunkedIntArray("+slotsize+") not currently supported");
    ...
    /**
     * Append a 4-integer record to the CIA, starting with record 1. (Since arrays are initialized
     * to all-0, 0 has been reserved as the "unknown" value in DTM.)
     *
    ...
    /**
     * Retrieve an integer from the CIA by record number and column within the record, both 0-based
     * (though position 0 is reserved for special purposes).
     *
    ...
    /**
     * Disard the highest-numbered record. This is used in the string-buffer CIA; when only a single
     * characters() chunk has been recieved, its index is moved into the Text node rather than being
     * referenced by indirection into the text accumulator.
     */
    void discardLast() {
        --lastUsed;
    }
    ...
    /**
     * Overwrite the integer found at a specific record and column. Used to back-patch existing
     * records, most often changing their "next sibling" reference from 0 (unknown) to something
     * meaningful
     *
     * @param position int Record number
    ...
    /**
     * Overwrite an entire (4-integer) record at the specified index. Mostly used to create record
     * 0, the Document node.
     *
    ...
    /**
     * Retrieve the contents of a record into a user-supplied buffer array. Used to reduce
     * addressing overhead when code will access several columns of the record.
     *
     * @param position int Record number
     * @param buffer int[] Integer array provided by user, must be large enough to hold a complete
     *     record.
     */
    ...
    class ChunksVector {
        ...
        void addElement(int[] value) {
            if (pos >= m_mapSize) {
                ...
                while (pos >= m_mapSize) m_mapSize += BLOCKSIZE;
