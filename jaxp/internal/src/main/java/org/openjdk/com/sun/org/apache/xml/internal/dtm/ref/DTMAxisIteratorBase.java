...
public abstract class DTMAxisIteratorBase implements DTMAxisIterator {
     ...
     * _not_ the node's handle within the DTM. Also, don't confuse it with the current (most
     * recently returned) position.
     */
    ...
    /**
     * The position of the marked node within the iteration; a saved itaration state that we may
     * want to come back to. Note that only one mark is maintained; there is no stack.
     */
    ...
    /**
     * True if this iteration can be restarted. False otherwise (eg, if we are iterating over a
     * stream that can not be re-scanned, or if the iterator was produced by cloning another
     * iterator.)
     */
    protected boolean _isRestartable = true;

    /**
     * Get start to END should 'close' the iterator, i.e. subsequent call to next() should return
     * END.
     *
     ...
     *
     * @return This default method returns just returns this DTMAxisIterator, after setting the
     *     flag. (Returning "this" permits C++-style chaining of method calls into a single
     *     expression.)
     */
     ...
     *
     * <p>This may be an expensive operation when called the first time, since it may have to
     * iterate through a large part of the document to produce its answer.
     *
    ...
    /**
     * @return The position of the current node within the set, as defined by XPath. Note that this
