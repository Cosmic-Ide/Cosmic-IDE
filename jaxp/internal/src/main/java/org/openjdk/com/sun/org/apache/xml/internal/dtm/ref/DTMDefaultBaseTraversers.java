...
public abstract class DTMDefaultBaseTraversers extends DTMDefaultBase {
    ...
    /**
     * This returns a stateless "traverser", that can navigate over an XPath axis, though perhaps
            ...
            default:
                throw new DTMException(
                        XMLMessages.createXMLMessage(
                                ...
                                })); // "Unknown axis traversal type: "+axis);
        }
    ...
    private class AncestorOrSelfTraverser extends AncestorTraverser {
        ...
        /**
         * By the nature of the stateless traversal, the context node can not be returned or the
         * iteration will go into an infinate loop. To see if the self node should be processed, use
         * this function.
         *
         ...
         * By the nature of the stateless traversal, the context node can not be returned or the
         * iteration will go into an infinate loop. To see if the self node should be processed, use
         * this function. If the context node does not match the expanded type ID, this function
         * will return false.
         *
         * @param context The context node of this traversal.
         * @param expandedTypeID The expanded type ID that must match.
    ...
    private class AttributeTraverser extends DTMAxisTraverser {
        ...
        public int next(int context, int current, int expandedTypeID) {
            ...
            } while (NULL != (current = getNextAttribute(current)));
    ...
    private class ChildTraverser extends DTMAxisTraverser {
        ...
        /**
         * Get the next indexed node that matches the expanded type ID. Before calling this
         * function, one should first call {@link #isIndexed(int) isIndexed} to make sure that the
         * index can contain nodes that match the given expanded type ID.
         *
        ...
        /**
         * By the nature of the stateless traversal, the context node can not be returned or the
         * iteration will go into an infinate loop. So to traverse an axis, the first function must
         * be used to get the first node.
         *
         * <p>This method needs to be overloaded only by those axis that process the self node. <\p>
         *
         * @param context The context node of this traversal. This is the point that the traversal
         *     starts from.
         * @return the first node in the traversal.
         */
        public int first(int context) {
    ...
    private abstract class IndexedDTMAxisTraverser extends DTMAxisTraverser {
         ...
         * @return true if it is OK to call the {@link #getNextIndexed(int, int, int)
         *     getNextIndexed} method.
         */
        protected final boolean isIndexed(int expandedTypeID) {
            return (m_indexing
                    && ExpandedNameTable.ELEMENT == m_expandedNameTable.getType(expandedTypeID));
        }
        ...
        /**
         * Tell if the axis has been fully processed to tell if a the wait for an arriving node
         * should terminate. This method must be implemented be a derived class.
         *
         * @param axisRoot The root identity of the axis.
    ...
    private class DescendantTraverser extends IndexedDTMAxisTraverser {
        /**
         * Get the first potential identity that can be returned. This should be overridded by
         * classes that need to return the self node.
         *
         ...
         * starts after the root ends. May be sufficient for this class's logic, but misleadingly
         * named!
         *
    ...
    private class DescendantOrSelfTraverser extends DescendantTraverser {
        ...
        /**
         * Get the first potential identity that can be returned, which is the axis context, in this
         * case.
         *
         * @param identity The node identity of the root context of the traversal.
    ...
    private class FollowingSiblingTraverser extends DTMAxisTraverser {
        ...
        public int next(int context, int current, int expandedTypeID) {
            ...
            while (NULL != (current = getNextSibling(current))) {
                if (getExpandedTypeID(current) == expandedTypeID) return current;
            }
    ...
    private class NamespaceDeclsTraverser extends DTMAxisTraverser {
        ...
        public int next(int context, int current, int expandedTypeID) {
            ...
            do {
                if (getExpandedTypeID(current) == expandedTypeID) return current;
            } while (NULL != (current = getNextNamespaceNode(context, current, false)));
    ...
    private class NamespaceTraverser extends DTMAxisTraverser {
        ...
        public int next(int context, int current, int expandedTypeID) {
            ...
            do {
                if (getExpandedTypeID(current) == expandedTypeID) return current;
            } while (NULL != (current = getNextNamespaceNode(context, current, true)));
    ...
    private class PrecedingSiblingTraverser extends DTMAxisTraverser {
        ...
        public int next(int context, int current, int expandedTypeID) {
            ...
            while (NULL != (current = getPreviousSibling(current))) {
                if (getExpandedTypeID(current) == expandedTypeID) return current;
            }
    ...
    private class DescendantOrSelfFromRootTraverser extends DescendantTraverser {
        ...
        /**
         * Get the first potential identity that can be returned, which is the axis root context in
         * this case.
         *
         * @param identity The node identity of the root context of the traversal.
