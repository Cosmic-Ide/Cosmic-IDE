...
import org.openjdk.com.sun.org.apache.xml.internal.utils.FastStringBuffer;
...
/**
 * This is the implementation of the DTM document interface. It receives requests from an XML
 * content handler similar to that of an XML DOM or SAX parser to store information from the xml
 * document in an array based dtm table structure. This informtion is used later for document
 * navigation, query, and SAX event dispatch functions. The DTM can also be used directly as a
 * document composition model for an application. The requests received are:
 *
 * <ul>
 *   <li>initiating DTM to set the doc handle
 *   <li>resetting DTM for data structure reuse
 *   <li>hinting the end of document to adjust the end of data structure pointers
 *   <li>createnodes (element, comment, text, attribute, ....)
 *   <li>hinting the end of an element to patch parent and siblings
 *   <li>
 *   <li>setting application provided symbol name stringpool data structures
 * </ul>
 *
 * <p>State: In progress!! %REVIEW% I _think_ the SAX convention is that "no namespace" is expressed
 * as "" rather than as null (which is the DOM's convention). What should DTM expect? What should it
 * do with the other?
 *
 * <p>Origin: the implemention is a composite logic based on the DTM of XalanJ1 and DocImpl,
 * DocumentImpl, ElementImpl, TextImpl, etc. of XalanJ2
 */
public class DTMDocumentImpl
    ...
    /**
     * If we're building the model incrementally on demand, we need to be able to tell the source
     * when to send us more data.
     *
     * <p>Note that if this has not been set, and you attempt to read ahead of the current build
     * point, we'll probably throw a null-pointer exception. We could try to wait-and-retry instead,
     * as a very poor fallback, but that has all the known problems with multithreading on
     * multiprocessors and we Don't Want to Go There.
     *
     ...
     * Construct a DTM.
     *
     * @param documentNumber the ID number assigned to this document. It will be shifted up into the
     *     high bits and returned as part of all node ID numbers, so those IDs indicate which
     *     document they came from as well as a location within the document. It is the DTMManager's
    ...
    /**
     * Bind a IncrementalSAXSource to this DTM. If we discover we need nodes that have not yet been
     * built, we will ask this object to send us more events, and it will manage interactions with
     * its data sources.
     *
     * <p>Note that we do not actually build the IncrementalSAXSource, since we don't know what
     * source it's reading from, what thread that source will run in, or when it will run.
     *
    ...
    /**
     * Wrapper for ChunkedIntArray.append, to automatically update the previous sibling's "next"
     * reference (if necessary) and periodically wake a reader who may have encountered incomplete
     * data and entered a wait state.
     *
    ...
    /**
     * Set a reference pointer to the element name symbol table. %REVIEW% Should this really be
     * Public? Changing it while DTM is in use would be a disaster.
     *
    ...
    /**
     * Set a reference pointer to the namespace URI symbol table. %REVIEW% Should this really be
     * Public? Changing it while DTM is in use would be a disaster.
     *
     * @param poolRef DTMStringPool reference to an instance of table.
     */
    ...
    /**
     * Set a reference pointer to the prefix name symbol table. %REVIEW% Should this really be
     * Public? Changing it while DTM is in use would be a disaster.
     *
     * @param poolRef DTMStringPool reference to an instance of table.
     */
     ...
     *     built-in SAX ContentHandler, the IncrementalSAXSource if we're bound to one and should
     ...
     *
     * @return null if this model doesn't respond to lexical SAX events, "this" if the DTM object
     *     has a built-in SAX ContentHandler, the IncrementalSAXSource if we're bound to one and
     *     should receive the SAX stream via it for incremental build purposes...
     */
    ...
    public void startElement(
        ...
        /**/ System.out.println(
                "Prefix=" + prefix + " index=" + m_prefixNames.stringToIndex(prefix));
        appendStartElement(
        ...
        for (int i = nAtts - 1; i >= 0; --i) {
            ...
            if (!(qName.startsWith("xmlns:") || "xmlns".equals(qName))) {
                ...
                if (!("xmlns".equals(prefix) || "xmlns".equals(qName)))
                    appendAttribute(
                            m_nsNames.stringToIndex(atts.getURI(i)),
                            m_localNames.stringToIndex(localName),
                            m_prefixNames.stringToIndex(prefix),
                            atts.getType(i).equalsIgnoreCase("ID"),
    ...
    public void startDTD(
            java.lang.String name, java.lang.String publicId, java.lang.String systemId)
            throws org.xml.sax.SAXException {
        // No-op in DTM
    }
     ...
     *
     * <p>The DTMManager will invoke this method when the dtm is created.
     *
    ...
    //      /**
    //       * Factory method; creates an Element node not associated with a given name space in
    // this document.
    //       *
    //       * The node created will be chained according to its natural order of request
    //       * received.  %TBD% It can be rechained later via the optional DTM writable interface.
    ...
    //      {
    //              // This method wraps around the index valued interface of the createElement
    // interface.
    //              // The raw string values are stored into the current DTM name symbol tables.
    // The method
    //              // method will then use the index values returned to invoke the other
    // createElement()
    //              // onverted to index values modified to match a
    //              // method.
    //              int nsIndex = NULL;
    //              int nameIndex = m_localNames.stringToIndex(name);
    //              // note - there should be no prefix separator in the name because it is not
    // associated
    //              // with a name space
    ...
    //      // using String value parameters passed in from a content handler or application
    //      /**
    //       * Factory method; creates an Element node associated with a given name space in this
    // document.
    //       *
    //       * The node created will be chained according to its natural order of request
    //       * received.  %TBD% It can be rechained later via the optional DTM writable interface.
    ...
    //              int nameIndex = m_localNames.stringToIndex(name);
    //              // The prefixIndex is not needed by the indexed interface of the createElement
    // method
    ...
    //      throws DTMException
    //      {
    //              m_char.append(ch, start, length);               // store the chunk to the
    // text/comment string table

    //              // create a Text Node
    //              // %TBD% may be possible to combine with appendNode()to replace the next chunk
    // of code
    //              int w0 = TEXT_NODE;
    //              // W1: Parent
    //              int w1 = currentParent;
    ...
    //      throws DTMException
    //      {
    //              m_char.append(ch, start, length);               // store the comment string to
    // the text/comment string table
     ...
     * (dtm.getFirstChild(nodeHandle)!=DTM.NULL) is just as fast and almost as self-evident. But
    ...
    /**
     * Given a node handle, advance to its last child. If not yet resolved, waits for more nodes to
     ...
     * @param inScope true if all namespaces in scope should be returned, false if only the
     ...
     *
     * <p>%TBD% This currently uses the DTM-internal definition of sibling; eg, the last attr's next
     * sib is the first child. In the old DTM, the DOM proxy layer provided the additional logic for
     * the public view. If we're rewriting for XPath emulation, that test must be done here.
     *
     * <p>%TBD% CODE INTERACTION WITH INCREMENTAL PARSE - If not yet resolved, should wait for more
     * nodes to be added to the document and tries again.
     *
     * @param nodeHandle int Handle of the node.
    ...
    /**
     * Given a node handle, find its preceeding sibling. WARNING: DTM is asymmetric; this operation
     * is resolved by search, and is relatively expensive.
     *
    ...
    public int getPreviousSibling(int nodeHandle) {
        ...
        int kid = NULL;
        for (int nextkid = getFirstChild(parent);
                nextkid != nodeHandle;
                nextkid = getNextSibling(nextkid)) {
            kid = nextkid;
        }
     ...
     * attribute; if an attr, we advance to the next attr on the same node.
     *
     * @param nodeHandle int Handle of the node.
     ...
     *
     * <p>%TBD% THIS METHOD DOES NOT MATCH THE CURRENT SIGNATURE IN THE DTM INTERFACE. FIX IT, OR
     * JUSTIFY CHANGING THE DTM API.
     *
    ...
    /**
     * Given a node handle, advance to its next descendant. If not yet resolved, waits for more
     * nodes to be added to the document and tries again.
     *
     ...
     * @param axisContextHandle the start of the axis that is being traversed.
     * @param nodeHandle the id of the node.
     * @return int Node-number of preceding sibling, or DTM.NULL to indicate none exists.
     */
     ...
     *
     * @param nodeHandle the id of the node.
     * @return int Node handle of owning document, or NULL if the nodeHandle is a document.
     */
    ...
    /**
     * Given a node handle, find the owning document node. This has the DTM semantics; a Document
     ...
     * http://www.w3.org/TR/xpath#data-model for the definition of a node's string-value). Note that
     * a single text node may have multiple text chunks.
     *
     * <p>EXPLANATION: This method is an artifact of the fact that the underlying m_chars object may
     * not store characters in a single contiguous array -- for example,the current FastStringBuffer
     * may split a single node's text across multiple allocation units. This call tells us how many
     * separate accesses will be required to retrieve the entire content. PLEASE NOTE that this may
     * not be the same as the number of SAX characters() events that caused the text node to be
     * built in the first place, since m_chars buffering may be on different boundaries than the
     * parser's buffers.
     *
     * @param nodeHandle The node ID.
     ...
     * <p>EXPLANATION: This method is an artifact of the fact that the underlying m_chars object may
     * not store characters in a single contiguous array -- for example,the current FastStringBuffer
     * may split a single node's text across multiple allocation units. This call retrieves a single
     * contiguous portion of the text -- as much as m-chars was able to store in a single allocation
     * unit. PLEASE NOTE that this may not be the same granularityas the SAX characters() events
     * that caused the text node to be built in the first place, since m_chars buffering may be on
    ...
    private static final String[] fixednames = {
        ...
        "#text", // Attr, Text
        "#cdata_section",
        null, // CDATA, EntityReference
        null,
        null, // Entity, PI
        "#comment",
        "#document", // Comment, Document
        null,
        "#document-fragment", // Doctype, DocumentFragment
        null
    }; // Notation
     ...
     *
     * <p>%REVIEW% What's the local name of something other than Element/Attr? Should this be
     * DOM-style (undefined unless namespaced), or other?
     *
     * @param nodeHandle the id of the node.
     * @return String Local name of this node.
     */
     ...
     *
     * <p>%REVIEW% Are you sure you want "" for no prefix? %REVIEW% Should this be DOM-style
     * (undefined unless namespaced), or other?
     *
     * @param nodeHandle the id of the node.
    ...
    /**
     * Given a node handle, return its DOM-style namespace URI (As defined in Namespaces, this is
     * the declared URI which this node's prefix -- or default in lieu thereof -- was mapped to.)
     *
     * @param nodeHandle the id of the node.
     * @return String URI value of this node's namespace, or null if no namespace was resolved.
     */
    ...
    /**
     * Return the base URI of the document entity. If it is not known (because the document was
     * parsed from a socket connection or from standard input, for example), the value of this
     * property is unknown.
     *
     ...
     * property is derived from the optional standalone document declaration in the XML declaration
     * at the beginning of the document entity, and has no value if there is no standalone document
     * declaration.
     *
     * @param nodeHandle The node id, which can be any valid node handle.
    ...
    /**
     * Return a string representing the XML version of the document. This property is derived from
     * the XML declaration optionally present at the beginning of the document entity, and has no
     * value if there is no XML declaration.
     *
     ...
     * boolean. If it is false, then certain properties (indicated in their descriptions below) may
     * be unknown. If it is true, those properties are never unknown.
     *
     * @return <code>true</code> if all declarations were processed {}; <code>false</code>
     *     otherwise.
     */
     ...
     *
     * <p>1. [system identifier] The system identifier of the external subset, if it exists.
    ...
    /**
     * Return the public identifier of the external subset, normalized as described in 4.2.2
     * External Entities [XML]. If there is no external subset or if it has no public identifier,
     ...
     * no such element exists, returns <code>DTM.NULL</code>. Behavior is not defined if more than
     * one element has this <code>ID</code>. Attributes (including those with the name "ID") are not
     * of type ID unless so defined by DTD/Schema information available to the DTM implementation.
     ...
     * processor, and we would have to support some form of plug-in resolver to handle this
     * properly. Currently, we simply return the System Identifier if present, and hope that it a
     * usable URI or that our caller can map it to one. TODO: Resolve Public Identifiers... or
     * consider changing function name.
     *
     * <p>If we find a relative URI reference, XML expects it to be resolved in terms of the base
     * URI of the document. The DOM doesn't do that for us, and it isn't entirely clear whether that
     * should be done here; currently that's pushed up to a higher level of our application. (Note
     * that DOM Level 1 didn't store the document's base URI.) TODO: Consider resolving Relative
     * URIs.
     *
     * <p>(The DOM's statement that "An XML processor may choose to completely expand entities
     * before the structure model is passed to the DOM" refers only to parsed entities, not
     * unparsed, and hence doesn't affect this function.)
     *
     * @param name A string containing the Entity Name of the unparsed entity.
     * @return String containing the URI of the Unparsed Entity, or an empty string if no such
     *     entity exists.
     */
     ...
     * the DTM document.
     *
     * <p>%REVEIW% Presumes a 1:1 mapping from DTM to Document, since we aren't saying which
     * Document to query...?
     */
     ...
     * appearing within element content (see [XML], 2.10 "White Space Handling"). Note that
     * validating XML processors are required by XML 1.0 to provide this information. If there is no
     * declaration for the containing element, this property has no value for white space
     * characters. If no declaration has been read, but the [all declarations processed] property of
     * the document information item is false (so there may be an unread declaration), then the
     * value of this property is unknown for white space characters. It is always false for
    ...
    /**
     * 10. [all declarations processed] This property is not strictly speaking part of the infoset
     * of the document. Rather it is an indication of whether the processor has read the complete
     * DTD. Its value is a boolean. If it is false, then certain properties (indicated in their
    ...
    /**
     * 5. [specified] A flag indicating whether this attribute was actually specified in the
     * start-tag of its element, or was defaulted from the DTD.
     *
    ...
    /**
     * Append a child to the end of the child list of the current node. Please note that the node is
    ...
    /**
     * Append a text child at the current insertion point. Assumes that the actual content of the
     * text has previously been appended to the m_char buffer (shared with the builder).
     *
    ...
    /**
     * Append a comment child at the current insertion point. Assumes that the actual content of the
     * comment has previously been appended to the m_char buffer (shared with the builder).
     *
     * @param m_char_current_start int Starting offset of node's content in m_char.
     * @param contentLength int Length of node's content in m_char.
     */
    ...
    /**
     * Append an Element child at the current insertion point. This Element then _becomes_ the
     * insertion point; subsequent appends become its lastChild until an appendEndElement() call is
     * made.
     *
     * <p>Assumes that the symbols (local name, namespace URI and prefix) have already been added to
     * the pools
     *
     * <p>Note that this _only_ handles the Element node itself. Attrs and namespace nodes are
     * unbundled in the ContentHandler layer and appended separately.
     *
    ...
    /**
     * Append a Namespace Declaration child at the current insertion point. Assumes that the symbols
     * (namespace URI and prefix) have already been added to the pools
     *
     * @param prefixIndex: Index within the prefix string pool
     * @param namespaceIndex: Index within the namespaceURI string pool
     * @param isID: If someone really insists on writing a bad DTD, it is theoretically possible for
     *     a namespace declaration to also be declared as being a node ID. I don't really want to
     *     support that stupidity, but I'm not sure we can refuse to accept it.
     */
    ...
    /**
     * Append an Attribute child at the current insertion point. Assumes that the symbols (namespace
     * URI, local name, and prefix) have already been added to the pools, and that the content has
     * already been appended to m_char. Note that the attribute's content has been flattened into a
     * single string; DTM does _NOT_ attempt to model the details of entity references within
     * attribute values.
     *
     ...
     * @param isID boolean True if this attribute was declared as an ID (for use in supporting
     *     getElementByID).
     * @param m_char_current_start int Starting offset of node's content in m_char.
     * @param contentLength int Length of node's content in m_char.
     */
    ...
    /**
     * This is a shortcut to the iterators that implement the supported XPath axes (only
     * namespace::) is not supported. Returns a bare-bones iterator that must be initialized with a
    ...
    /**
     * Terminate the element currently acting as an insertion point. Subsequent insertions will
     * occur as the last child of this element's parent.
     */
    ...
    /**
     * A dummy routine to satisify the abstract interface. If the DTM implememtation that extends
     * the default base requires notification of registration, they can override this method.
     */
    public void documentRegistration() {}

    /**
     * A dummy routine to satisify the abstract interface. If the DTM implememtation that extends
     * the default base requires notification when the document is being released, they can override
     * this method
     */
