...
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.dom2dtm.DOM2DTM;
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.dom2dtm.DOM2DTMdefaultNamespaceDeclarationNode;
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.sax2dtm.SAX2DTM;
...
public class DTMManagerDefault extends DTMManager {
     ...
     * several prefix numbers, if extended node indexing is in use; in that case, m_dtm_offsets[]
     * will used to control which prefix maps to which section of the DTM.
     *
    ...
    /**
     * Map from DTM identifier numbers to offsets. For small DTMs with a single identifier, this
     * will always be 0. In overflow addressing, where additional identifiers are allocated to
     * access nodes beyond the range of a single Node Handle, this table is used to map the handle's
     * node field into the actual node identifier.
     *
     * <p>This array grows as necessary; see addDTM().
     *
     * <p>This array grows as necessary; see addDTM(). Growth is uncommon... but access needs to be
     * blindingly fast since it's used in node addressing. (And at the moment, that includes
     * accessing it from DTMDefaultBase, which is why this is not Protected or Private.)
     */
    ...
    /**
     * Add a DTM to the DTM table. This convenience call adds it as the "base DTM ID", with offset
     * 0. The other version of addDTM should be used if you want to add "extended" DTM IDs with
     * nonzero offsets.
     *
                    ...
                    boolean haveXercesParser =
                            (null != reader)
                                    && (reader.getClass()
                                            .getName()
                                            .equals(
                                                    "org.openjdk.com.sun.org.apache.xerces.internal.parsers.SAXParser"));
                            ...
                            try {
                                coParser =
                                        (IncrementalSAXSource)
                                                Class.forName(
                                                                "org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.IncrementalSAXSource_Xerces")
                                                        .newInstance();
                            } catch (Exception ex) {
                                ex.printStackTrace();
                                coParser = null;
    ...
    public synchronized int getDTMHandleFromNode(org.w3c.dom.Node node) {
        ...
        if (node instanceof DTMNodeProxy) return ((DTMNodeProxy) node).getDTMNodeNumber();
        else {
            ...
            DOM2DTM dtm =
                    (DOM2DTM)
                            getDTM(
                                    new org.openjdk.javax.xml.transform.dom.DOMSource(root),
                                    false,
                                    null,
                                    true,
                                    true);
    ...
    /**
     * This method returns the SAX2 parser to use with the InputSource obtained from this URI. It
     * may return null if any SAX2-conformant XML parser can be used, or if getInputSource() will
     * also return null. The parser must be free for use (i.e., not currently in use for another
     * parse(). After use of the parser is completed, the releaseXMLReader(XMLReader) must be
     * called.
     *
    ...
    public synchronized XMLReader getXMLReader(Source inputSource) {
        ...
        try {
            XMLReader reader =
                    (inputSource instanceof SAXSource)
                            ? ((SAXSource) inputSource).getXMLReader()
                            : null;
     ...
     * If overflow addressing is in use, other DTM IDs may also be assigned to this DTM.
     *
     * @param dtm The DTM which (hopefully) contains this node.
     * @return The DTM ID (as the high bits of a NodeHandle, not as our internal index), or -1 if
     *     the DTM doesn't belong to this manager.
     */
    ...
    /**
     * Release the DTMManager's reference(s) to a DTM, making it unmanaged. This is typically done
     * as part of returning the DTM to the heap after we're done with it.
     *
     ...
     * @return true if the DTM was released, false if shouldHardDelete was set and we decided not
     *     to.
     */
