...
import org.openjdk.com.sun.org.apache.xml.internal.dtm.DTMDOMException;
...
public class DTMNodeProxy
    ...
    /**
     * Ask whether we support a given DOM feature. In fact, we do not _fully_ support any DOM
     * feature -- we're a read-only subset -- so arguably we should always return false. Or we could
     * say that we support DOM Core Level 2 but all nodes are read-only. Unclear which answer is
     * least misleading.
     *
     * <p>NON-DOM method. This was present in early drafts of DOM Level 2, but was renamed
     * isSupported. It's present here only because it's cheap, harmless, and might help some poor
     * fool who is still trying to use an early Working Draft of the DOM.
     *
     * @param feature
     * @param version
     * @return false
    ...
    /**
     * Ask whether we support a given DOM feature. In fact, we do not _fully_ support any DOM
     * feature -- we're a read-only subset -- so arguably we should always return false.
     *
     * @param feature
     * @param version
     * @return false
    ...
    /**
     * This is a bit of a problem in DTM, since a DTM may be a Document Fragment and hence not have
     * a clear-cut Document Element. We can make it work in the well-formed cases but would that be
    ...
    private final void traverseChildren(
            Vector listVector, Node tempNode, String tagname, boolean isTagNameWildCard) {
        if (tempNode == null) {
            return;
        } else {
     ...
     * href='http://www.w3.org/TR/2001/WD-DOM-Level-3-Core-20010605'>Document Object Model (DOM)
     * Level 3 Core Working Draft of 5 June 2001.</a>.
     *
     * <p>An attribute specifying, as part of the XML declaration, the encoding of this document.
     * This is <code>null</code> when unspecified.
     *
     * @since DOM Level 3
     ...
     * Level 3 Core Working Draft of 5 June 2001.</a>.
     *
     * <p>An attribute specifying, as part of the XML declaration, whether this document is
     * standalone.
     *
     * @since DOM Level 3
     *     <p>NEEDSDOC ($objectName$) @return
     ...
     * false</code>, the implementation is free to not test every possible error case normally
     * defined on DOM operations, and not raise any <code>DOMException</code>. In case of error, the
     * behavior is undefined. This attribute is <code>true</code> by defaults.
     *
     * @since DOM Level 3
     *     <p>NEEDSDOC ($objectName$) @return
     */
     ...
     * Level 3 Core Working Draft of 5 June 2001.</a>.
     *
     * <p>An attribute specifying, as part of the XML declaration, the version number of this
     * document. This is <code>null</code> when unspecified.
     *
     * @since DOM Level 3
     *     <p>NEEDSDOC ($objectName$) @return
     */
    ...
    static class DTMNodeProxyImplementation implements DOMImplementation {
        ...
        public Document createDocument(
                String namespaceURI, String qualfiedName, DocumentType doctype) {
         ...
         * arguably we should always return false. On the other hand, it may be more practically
         * useful to return true and simply treat the whole DOM as read-only, failing on the methods
         * we can't support. I'm not sure which would be more useful to the caller.
         */
        @Override
         ...
         * binding-specific casting methods but is not necessarily expected to, as discussed in
         * Mixed DOM implementations .
         *
         * @param feature The name of the feature requested (case-insensitive).
         * @param version This is the version number of the feature to test. If the version is
         *     <code>null</code> or the empty string, supporting any version of the feature will
         *     cause the method to return an object that supports at least one version of the
         *     feature.
         * @return Returns an object which implements the specialized APIs of the specified feature
         *     and version, if any, or <code>null</code> if there is no object which implements
         *     interfaces associated with that feature. If the <code>DOMObject</code> returned by
         *     this method implements the <code>Node</code> interface, it must delegate to the
         *     primary core <code>Node</code> and not return results inconsistent with the primary
         *     core <code>Node</code> such as attributes, childNodes, etc.
         * @since DOM Level 3
         */
        @Override
    ...
    /**
     * DOM Level 3 Look up the namespace URI associated to the given prefix, starting from this
     * node. Use lookupNamespaceURI(null) to lookup the default namespace
     *
     * @param namespaceURI
    ...
    /**
     * DOM Level 3 Look up the prefix associated to the given namespace URI, starting from this
     * node.
     *
     * @param namespaceURI
