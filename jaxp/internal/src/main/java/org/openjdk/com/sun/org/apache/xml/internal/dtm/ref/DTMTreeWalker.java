...
import org.openjdk.com.sun.org.apache.xml.internal.utils.NodeConsumer;
 ...
 * As such, it's more like the Visitor design pattern than like the DOM's TreeWalker.
 *
 * <p>I think normally this class should not be needed, because of DTM#dispatchToEvents.
 *
 * @xsl.usage advanced
 */
public class DTMTreeWalker {
    ...
    /**
     * Perform a non-recursive pre-order/post-order traversal, operating as a Visitor. startNode
     * (preorder) and endNode (postorder) are invoked for each node as we traverse over them, with
     * the result that the node is written out to m_contentHandler.
     *
     * @param pos Node in the tree at which to start (and end) traversal -- in other words, the root
     *     of the subtree to traverse over.
     ...
     * @param top Node in the tree where to end traversal. If top==DTM.NULL, run through end of
     *     document.
     * @throws TransformerException
     */
    ...
    protected void startNode(int node) throws org.xml.sax.SAXException {
        ...
        switch (m_dtm.getNodeType(node)) {
            ...
            case DTM.ELEMENT_NODE:
                ...
                for (int nsn = dtm.getFirstNamespaceNode(node, true);
                        DTM.NULL != nsn;
                ...
                this.m_contentHandler.startElement(
                        ns, m_dtm.getLocalName(node), m_dtm.getNodeName(node), attrs);
                break;
            ...
            case DTM.TEXT_NODE:
                {
                    if (nextIsRaw) {
                        ...
                        m_contentHandler.processingInstruction(
                                org.openjdk.javax.xml.transform.Result.PI_DISABLE_OUTPUT_ESCAPING,
                                "");
                        dispatachChars(node);
                        m_contentHandler.processingInstruction(
                                org.openjdk.javax.xml.transform.Result.PI_ENABLE_OUTPUT_ESCAPING,
                                "");
                    } else {
                        dispatachChars(node);
                    }
            ...
            case DTM.ENTITY_REFERENCE_NODE:
                {
                    if (m_contentHandler instanceof LexicalHandler) {
                        ((LexicalHandler) this.m_contentHandler)
                                .startEntity(m_dtm.getNodeName(node));
                    } else {
    ...
    protected void endNode(int node) throws org.xml.sax.SAXException {
        ...
        switch (m_dtm.getNodeType(node)) {
            ...
            case DTM.ELEMENT_NODE:
                ...
                if (null == ns) ns = "";
                this.m_contentHandler.endElement(
                        ns, m_dtm.getLocalName(node), m_dtm.getNodeName(node));

                for (int nsn = m_dtm.getFirstNamespaceNode(node, true);
                        DTM.NULL != nsn;
