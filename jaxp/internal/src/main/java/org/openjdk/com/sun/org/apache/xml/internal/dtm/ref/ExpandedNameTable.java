...
public class ExpandedNameTable {
    ...
    /**
     * The initial capacity of the hash table. Use a bigger number to avoid the cost of expanding
     * the table.
     */
    ...
    /**
     * Given an expanded name represented by namespace, local name and node type, return an ID. If
     * the expanded-name does not exist in the internal tables, the entry will be created, and the
     * ID will be returned. Any additional nodes that are created that have this expanded name will
     * use this ID.
     *
     ...
     * @param localName The local name
     * @param type The node type
     * @param searchOnly If it is true, we will only search for the expanded name. -1 is return is
    ...
    /**
     * Increases the capacity of and internally reorganizes the hashtable, in order to accommodate
     * and access its entries more efficiently. This method is called when the number of keys in the
    ...
    public final int getLocalNameID(int ExpandedNameID) {
        ...
        if (m_extendedTypes[ExpandedNameID].getLocalName().equals("")) return 0;
    ...
    public final int getNamespaceID(int ExpandedNameID) {
        // ExtendedType etype = m_extendedTypes[ExpandedNameID];
        if (m_extendedTypes[ExpandedNameID].getNamespace().equals("")) return 0;
        else return ExpandedNameID;
    }
    ...
    /**
     * Inner class which represents a hash table entry. The field next points to the next entry
     * which is hashed into the same bucket in the case of "hash collision".
     */
