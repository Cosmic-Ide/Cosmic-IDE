...
import org.openjdk.com.sun.org.apache.xml.internal.utils.ThreadControllerWrapper;
...
/**
 * IncrementalSAXSource_Filter implements IncrementalSAXSource, using a standard SAX2 event source
 * as its input and parcelling out those events gradually in reponse to deliverMoreNodes() requests.
 * Output from the filter will be passed along to a SAX handler registered as our listener, but
 * those callbacks will pass through a counting stage which periodically yields control back to the
 * controller coroutine.
 *
 * <p>%REVIEW%: This filter is not currenly intended to be reusable for parsing additional
 * streams/documents. We may want to consider making it resettable at some point in the future. But
 * it's a small object, so that'd be mostly a convenience issue; the cost of allocating each time is
 * trivial compared to the cost of processing any nontrival stream.
 *
 * <p>For a brief usage example, see the unit-test main() method.
 *
 * <p>This is a simplification of the old CoroutineSAXParser, focusing specifically on filtering.
 * The resulting controller protocol is _far_ simpler and less error-prone; the only controller
 * operation is deliverMoreNodes(), and the only requirement is that deliverMoreNodes(false) be
 * called if you want to discard the rest of the stream and the previous deliverMoreNodes() didn't
 * return false.
 *
 * <p>This class is final and package private for security reasons. Please see CR 6537912 for
 * further details.
 */
final class IncrementalSAXSource_Filter
        ...
        if (fControllerCoroutineID == -1 || fSourceCoroutineID == -1)
            throw new RuntimeException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_COJOINROUTINESET_FAILED,
     ...
     *
     * <p>Just a convenience routine; obviously you can explicitly register this as a listener with
     * the same effect.
     */
    ...
    public void characters(char[] ch, int start, int length) throws org.xml.sax.SAXException {
        ...
        if (clientContentHandler != null) clientContentHandler.characters(ch, start, length);
    }

    public void endDocument() throws org.xml.sax.SAXException {
        // EXCEPTION: In this case we need to run the event BEFORE we yield.
        if (clientContentHandler != null) clientContentHandler.endDocument();
    ...
    public void setDocumentLocator(Locator locator) {
        ...
        if (clientContentHandler != null) clientContentHandler.setDocumentLocator(locator);
    }
    ...
    public void comment(char[] ch, int start, int length) throws org.xml.sax.SAXException {
        if (null != clientLexicalHandler) clientLexicalHandler.comment(ch, start, length);
    }

    public void endCDATA() throws org.xml.sax.SAXException {
        if (null != clientLexicalHandler) clientLexicalHandler.endCDATA();
    }

    public void endDTD() throws org.xml.sax.SAXException {
        if (null != clientLexicalHandler) clientLexicalHandler.endDTD();
    }
    ...
    public void startCDATA() throws org.xml.sax.SAXException {
        if (null != clientLexicalHandler) clientLexicalHandler.startCDATA();
    }
    ...
    public void error(SAXParseException exception) throws SAXException {
        if (null != clientErrorHandler) clientErrorHandler.error(exception);
    }
    ...
    /**
     * @return the CoroutineManager this CoroutineFilter object is bound to. If you're using the
     *     do...() methods, applications should only need to talk to the CoroutineManager once, to
     *     obtain the application's Coroutine ID.
     */
    ...
    /**
     * In the SAX delegation code, I've inlined the count-down in the hope of encouraging compilers
     * to deliver better performance. However, if we subclass (eg to directly connect the output to
     * a DTM builder), that would require calling super in order to run that logic... which seems
     * inelegant. Hence this routine for the convenience of subclasses: every [frequency]
     * invocations, issue a co_yield.
     *
     * @param moreExepected Should always be true unless this is being called at the end of
    ...
    /**
     * co_entry_pause is called in startDocument() before anything else happens. It causes the
     * filter to wait for a "go ahead" request from the controller before delivering any events.
     * Note that the very first thing the controller tells us may be "I don't need events after
     * all"!
     */
     ...
     *
     * <p>When moreRemains==true, we are pausing after delivering events, to ask if more are needed.
     * We will resume the controller thread with co_resume(Boolean.TRUE, ...) When control is passed
     * back it may indicate Boolean.TRUE indication to continue delivering events Boolean.FALSE
     * indication to discontinue events and shut down.
     *
     * <p>When moreRemains==false, we shut down immediately without asking the controller's
     * permission. Normally this means end of document has been reached.
     *
     * <p>Shutting down a IncrementalSAXSource_Filter requires terminating the incoming SAX event
     * stream. If we are in control of that stream (if it came from an XMLReader passed to our
     * startReader() method), we can do so very quickly by throwing a reserved exception to it. If
     * the stream is coming from another source, we can't do that because its caller may not be
     * prepared for this "normal abnormal exit", and instead we put ourselves in a "spin" mode where
    ...
    private void co_yield(boolean moreRemains) throws SAXException {
        ...
        try // Coroutine manager might throw no-such.
        {
            ...
            if (moreRemains) {
                ...
                arg =
                        fCoroutineManager.co_resume(
                                Boolean.TRUE, fSourceCoroutineID, fControllerCoroutineID);
            }
            ...
            if (arg == Boolean.FALSE) {
                ...
                fCoroutineManager.co_exit_to(
                        Boolean.FALSE, fSourceCoroutineID, fControllerCoroutineID);
            }
        } catch (NoSuchMethodException e) {
    ...
    /**
     * Launch a thread that will run an XMLReader's parse() operation within a thread, feeding
     * events to this IncrementalSAXSource_Filter. Mostly a convenience routine, but has the
     * advantage that -- since we invoked parse() -- we can halt parsing quickly via a StopException
     * rather than waiting for the SAX stream to end by itself.
     *
    ...
    public void startParse(InputSource source) throws SAXException {
        ...
        if (fXMLReader == null)
            throw new SAXException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_XMLRDR_NOT_BEFORE_STARTPARSE,
    ...
    /**
     * Used to quickly terminate parse when running under a startParse() thread. Only its type is
     * important.
     */
     ...
     * This is intended to be called from one of our partner routines, and serves to encapsulate the
     * details of how incremental parsing has been achieved.
     *
     * @param parsemore If true, tells the incremental filter to generate another chunk of output.
     *     If false, tells the filter that we're satisfied and it can terminate parsing of this
     *     document.
     * @return Boolean.TRUE if there may be more events available by invoking deliverMoreNodes()
     *     again. Boolean.FALSE if parsing has run to completion (or been terminated by
     *     deliverMoreNodes(false). Or an exception object if something malfunctioned. %REVIEW% We
     *     _could_ actually throw the exception, but that would require runinng deliverMoreNodes()
     *     in a try/catch... and for many applications, exception will be simply be treated as "not
     *     TRUE" in any case.
     */
    ...
    // ================================================================
    /**
     * Simple unit test. Attempt coroutine parsing of document indicated by first argument (as a
     * URI), report progress.
     */
    /*
