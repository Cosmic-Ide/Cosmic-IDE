...
import org.openjdk.com.sun.org.apache.xml.internal.serialize.XMLSerializer;
...
/**
 * IncrementalSAXSource_Xerces takes advantage of the fact that Xerces1 incremental mode is already
 * a coroutine of sorts, and just wraps our IncrementalSAXSource API around it.
 *
 * <p>Usage example: See main().
 *
 * <p>Status: Passes simple main() unit-test. NEEDS JAVADOC.
 */
public class IncrementalSAXSource_Xerces implements IncrementalSAXSource {
     ...
     * incremental parsing is only supported if this constructor is used, due to limitations in the
     * Xerces2 API (as of Beta 3). If you don't like that restriction, tell the Xerces folks that
    ...
    /**
     * Create a IncrementalSAXSource_Xerces wrapped around an existing SAXParser. Currently this
     * works only for recent releases of Xerces-1. Xerces-2 incremental is currently possible only
     * if we are allowed to create the parser instance, due to limitations in the API exposed by
    ...
    // ================================================================
    /**
     * startParse() is a simple API which tells the IncrementalSAXSource to begin reading a
     * document.
     *
    ...
    public void startParse(InputSource source) throws SAXException {
        if (fIncrementalParser == null)
            throw new SAXException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_STARTPARSE_NEEDS_SAXPARSER,
                            null)); // "startParse needs a non-null SAXParser.");
        if (fParseInProgress)
            throw new SAXException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_STARTPARSE_WHILE_PARSING,
        ...
        if (!ok)
            throw new SAXException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_COULD_NOT_INIT_PARSER,
