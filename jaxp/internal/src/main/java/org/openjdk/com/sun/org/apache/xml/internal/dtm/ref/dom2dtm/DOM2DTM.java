 ...
 */
package org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.dom2dtm;
...
import org.openjdk.com.sun.org.apache.xml.internal.utils.StringBufferPool;
import org.openjdk.com.sun.org.apache.xml.internal.utils.TreeWalker;
import org.openjdk.com.sun.org.apache.xml.internal.utils.XMLCharacterRecognizer;
...
public class DOM2DTM extends DTMDefaultBaseIterators {
    ...
    /**
     * True iff the first element has been processed. This is used to control synthesis of the
     * implied xml: namespace declaration node.
     */
    boolean m_processedFirstElement = false;

    /**
     * true if ALL the nodes in the m_root subtree have been processed; false if our incremental
     * build has not yet finished scanning the DOM tree.
     */
     ...
     * Text/CDATASection nodes in the DOM have been coalesced into a single DTM Text node); this
     * table points only to the first in that sequence.
     */
     ...
     * @param forceNodeType If not DTM.NULL, overrides the DOM node type. Used to force nodes to
     *     Text rather than CDATASection when their coalesced value includes ordinary Text nodes
     *     (current DTM behavior).
    ...
    /**
     * This method iterates to the next node that will be added to the table. Each call to this
     * method adds a new node to the table, unless the end is reached, in which case it returns
     * null.
     *
        ...
        if (next == null) {
            ...
            if (JJK_DEBUG) {
                ...
                for (int i = 0; i < m_nodes.size(); ++i)
                    System.out.println(
                            i + ":\t" + m_firstch.elementAt(i) + "\t" + m_nextsib.elementAt(i));
            }
                    ...
                    attrIndex =
                            addNode(
                                    new DOM2DTMdefaultNamespaceDeclarationNode(
                                            ...
                                            NAMESPACE_DECL_NS,
                                            makeNodeHandle(
                                                    ((attrIndex == NULL) ? nextindex : attrIndex)
                                                            + 1)),
                                    nextindex,
                                    attrIndex,
                                    NULL);
     ...
     *
     * <p>%OPT% An XPath-like search (walk up DOM to root, tracking path; walk down DTM
     * reconstructing path) might be considerably faster on later nodes in large documents. That
     * might also imply improving this call to handle nodes which would be in this DTM but have not
     * yet been built, which might or might not be a Good Thing. %REVIEW% This relies on being able
     * to test node-identity via object-identity. DTM2DOM proxying is a great example of a case
     * where that doesn't work. DOM Level 3 will provide the isSameNode() method to fix that, but
     * until then this is going to be flaky.
     *
     ...
     *
     * <p>%OPT% This will be pretty slow. %REVIEW% This relies on being able to test node-identity
     * via object-identity. DTM2DOM proxying is a great example of a case where that doesn't work.
     * DOM Level 3 will provide the isSameNode() method to fix that, but until then this is going to
     * be flaky.
     *
     * @param node A node, which may be null.
     * @return The node handle or <code>DTM.NULL</code>.
    ...
    /**
     * Utility function: Given a DOM Text node, determine whether it is logically followed by
     * another Text or CDATASection node. This may involve traversing into Entity References.
     *
    ...
    /**
     * Bind an IncrementalSAXSource to this DTM. NOT RELEVANT for DOM2DTM, since we're wrapped
     * around an existing DOM.
     *
     ...
     * also consider xml:space if/when we address this. DOM Level 3 may solve the problem for us.
     *
     * <p>%REVIEW% Note that as a DOM-level operation, it can be argued that this routine
     * _shouldn't_ perform any processing beyond what the DOM already does, and that whitespace
     * stripping and so on belong at the DTM level. If you want a stripped DOM view, wrap DTM2DOM
     * around DOM2DTM.
     *
     * @param node Node whose subtree is to be walked, gathering the contents of all Text or
     *     CDATASection nodes.
