...
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.DTMDefaultBase;
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.ExpandedNameTable;
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.ExtendedType;
...
import org.openjdk.com.sun.org.apache.xml.internal.utils.XMLStringDefault;
...
/**
 * SAX2DTM2 is an optimized version of SAX2DTM which is used in non-incremental situation. It is
 * used as the super class of the XSLTC SAXImpl. Many of the interfaces in SAX2DTM and
 * DTMDefaultBase are overridden in SAX2DTM2 in order to allow fast, efficient access to the DTM
 * model. Some nested iterators in DTMDefaultBaseIterators are also overridden in SAX2DTM2 for
 * performance reasons.
 *
 * <p>Performance is the biggest consideration in the design of SAX2DTM2. To make the code most
 * efficient, the incremental support is dropped in SAX2DTM2, which means that you should not use it
 * in incremental situation. To reduce the overhead of pulling data from the DTM model, a few core
 * interfaces in SAX2DTM2 have direct access to the internal arrays of the SuballocatedIntVectors.
 *
 * <p>The design of SAX2DTM2 may limit its extensibilty. If you have a reason to extend the SAX2DTM
 * model, please extend from SAX2DTM instead of this class.
 *
 * <p>TODO: This class is currently only used by XSLTC. We need to investigate the possibility of
 * also using it in Xalan-J Interpretive. Xalan's performance is likely to get an instant boost if
 * we use SAX2DTM2 instead of SAX2DTM in non-incremental case.
 *
 * <p>%MK% The code in this class is critical to the XSLTC_DTM performance. Be very careful when
 * making changes here!
 */
public class SAX2DTM2 extends SAX2DTM {
    ...
    public final class ChildrenIterator extends InternalAxisIteratorBase {
         ...
         * return END.
         *
         * <p>If the iterator is not restartable, this has no effect. %REVIEW% Should it
         * return/throw something in that case, or set current node to END, to indicate
         * request-not-honored?
         *
    ...
    public final class ParentIterator extends InternalAxisIteratorBase {
        ...
        /**
         * Set start to END should 'close' the iterator, i.e. subsequent call to next() should
         * return END.
         *
         * @param node Sets the root of the iteration.
         * @return A DTMAxisIterator set to the start of the iteration.
        ...
        public DTMAxisIterator setStartNode(int node) {
            ...
            if (_isRestartable) {
                ...
                if (node != DTM.NULL) _currentNode = _parent2(makeNodeIdentity(node));
                else _currentNode = DTM.NULL;
        ...
        /**
         * Set the node type of the parent that we're looking for. Note that this does _not_ mean
         * "find the nearest ancestor of this type", but "yield the parent if it is of this type".
         *
        ...
        /**
         * Get the next node in the iteration. In this case, we return only the immediate parent,
         * _if_ it matches the requested nodeType.
         *
         * @return The next node handle in the iteration, or END.
         */
        public int next() {
            int result = _currentNode;
            if (result == END) return DTM.NULL;
    ...
    /**
     * Iterator that returns children of a given type for a given node. The functionality chould be
     * achieved by putting a filter on top of a basic child iterator, but a specialised iterator is
     * used for efficiency (both speed and size of translet).
     */
    public final class TypedChildrenIterator extends InternalAxisIteratorBase {
        ...
        public DTMAxisIterator setStartNode(int node) {
            ...
            if (_isRestartable) {
                _startNode = node;
                _currentNode =
                        (node == DTM.NULL) ? DTM.NULL : _firstch2(makeNodeIdentity(_startNode));
     ...
     * node, filtered by extended type ID.
     */
        ...
        public DTMAxisIterator setStartNode(int node) {
            ...
            if (_isRestartable) {
                ...
                if (ExpandedNameTable.ATTRIBUTE == type || ExpandedNameTable.NAMESPACE == type) {
                    ...
                    if (NULL != _currentNode) _currentNode = _firstch2(_currentNode);
    ...
    /**
     * Iterator that returns preceding nodes of agiven type for a given node. This includes the node
     * set {root+1, start-1}, but excludes all ancestors.
     */
    ...
    /**
     * Iterator that returns the ancestors of a given node in document order. (NOTE! This was
     * changed from the XSLTC code!)
     */
    public class AncestorIterator extends InternalAxisIteratorBase {
        ...
        /**
         * Get start to END should 'close' the iterator, i.e. subsequent call to next() should
         * return END.
         *
                    ...
                    while (nodeID != END) {
                        ...
                        if (eType == nodeType) {
                            if (m_size >= m_ancestors.length) {
                                int[] newAncestors = new int[m_size * 2];
                                System.arraycopy(
                                        m_ancestors, 0, newAncestors, 0, m_ancestors.length);
                                m_ancestors = newAncestors;
                            }
    ...
    public class DescendantIterator extends InternalAxisIteratorBase {
        ...
        /**
         * Tell if this node identity is a descendant. Assumes that the node info for the element
         * has already been obtained.
         *
         * <p>This one-sided test works only if the parent has been previously tested and is known
         * to be a descendent. It fails if the parent is the _startNode's next sibling, or indeed
         * any node that follows _startNode in document order. That may suffice for this iterator,
         * but it's not really an isDescendent() test. %REVIEW% rename?
         *
    ...
    /**
     * %OPT% If the offset and length of a Text node are within certain limits, we store a bitwise
     * encoded value into an int, using 10 bits (max. 1024) for length and 21 bits for offset. We
     * can save two SuballocatedIntVector calls for each getStringValueX() and
     ...
     *
     * <p>This one is less efficient than _exptype2. It is only used during DTM building. _exptype2
     * is used after the document is fully built.
     */
     ...
     *
     * <p>This one is only used by DOMAdapter.getExpandedTypeID(int), which is mostly called from
     * the compiled translets.
     */
     ...
     *
     * <p>%OPT% This one is different from SAX2DTM.processingInstruction() in that we do not use
     * extended types for PI nodes. The name of the PI is saved in the DTMStringPool.
     *
     ...
     *
     * <p>Given a node handle, get the index of the node's first attribute.
     *
     ...
     *
     * <p>Given a node identity, get the index of the node's first attribute.
     *
     ...
     *
     * <p>Given a node identity for an attribute, advance to the next attribute.
     *
     * @param identity int identity of the attribute node. This <strong>must</strong> be an
     *     attribute node.
     ...
     *
     * <p>Given a node handle and an expanded type ID, get the index of the node's attribute of that
     * type, if any.
     *
     * @param nodeHandle int Handle of the node.
     ...
     *
     * <p>Given a node handle, return its XPath- style localname. (As defined in Namespaces, this is
     * the portion of the name after any colon character).
     *
     ...
     *
     * <p>Given a node handle, return the XPath node name. This should be the name as described by
     * the XPath data model, NOT the DOM- style name.
     *
     * @param nodeHandle the id of the node.
     ...
     *
     * <p>Given a node handle, return its DOM-style node name. This will include names such as #text
     * or #document.
     *
     * @param nodeHandle the id of the node.
     ...
     *
     * <p>This method is only used by Xalan-J Interpretive. It is not used by XSLTC.
     *
     * <p>If the caller supplies an XMLStringFactory, the getStringValue() interface in SAX2DTM will
     * be called. Otherwise just calls getStringValueX() and wraps the returned String in an
     * XMLString.
     *
     * <p>Get the string-value of a node as a String object (see
     * http://www.w3.org/TR/xpath#data-model for the definition of a node's string-value).
     *
    ...
    public XMLString getStringValue(int nodeHandle) {
        ...
        if (type == DTM.ELEMENT_NODE || type == DTM.DOCUMENT_NODE) {
            ...
            if (dataIndex >= 0) {
                if (m_xstrf != null)
                    return m_xstrf.newstr(
                            m_chars, dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
                else
                    return new XMLStringDefault(
                            m_chars.getString(
                                    dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX));
            } else {
                if (m_xstrf != null)
                    return m_xstrf.newstr(
                            m_chars,
     ...
     *
     * <p>%OPT% This is one of the most often used interfaces. Performance is critical here. This
     * one is different from SAX2DTM.getStringValue(int) in that it returns a String instead of a
     * XMLString.
     *
     * <p>Get the string- value of a node as a String object (see http: //www. w3.
    ...
    public final String getStringValueX(final int nodeHandle) {
        ...
        if (type == DTM.ELEMENT_NODE || type == DTM.DOCUMENT_NODE) {
            ...
            int dataIndex = m_dataOrQName.elementAt(identity);
            if (dataIndex >= 0) {
                return m_chars.getString(
                        dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
            } else {
                return m_chars.getString(
                        m_data.elementAt(-dataIndex), m_data.elementAt(-dataIndex + 1));
            }
        } else {
            int dataIndex = m_dataOrQName.elementAt(identity);
    ...
    public String getStringValue() {
        ...
        if ((_exptype2(child) == DTM.TEXT_NODE) && (_nextsib2(child) == DTM.NULL)) {
            ...
            if (dataIndex >= 0)
                ...
                return m_chars.getString(
                        m_data.elementAt(-dataIndex), m_data.elementAt(-dataIndex + 1));
        } else return getStringValueX(getDocument());
    }

    /**
     * The optimized version of SAX2DTM.dispatchCharactersEvents(int, ContentHandler, boolean).
     *
     * <p>Directly call the characters method on the passed ContentHandler for the string-value of
     * the given node (see http://www.w3.org/TR/xpath#data-model for the definition of a node's
    ...
    public final void dispatchCharactersEvents(int nodeHandle, ContentHandler ch, boolean normalize)
        ...
        if (type == DTM.ELEMENT_NODE || type == DTM.DOCUMENT_NODE) {
            ...
            if (dataIndex >= 0) {
                if (normalize)
                    m_chars.sendNormalizedSAXcharacters(
                            ch, dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
                else
                    m_chars.sendSAXcharacters(
                            ch, dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
            } else {
                if (normalize)
                    m_chars.sendNormalizedSAXcharacters(
                            ch, m_data.elementAt(-dataIndex), m_data.elementAt(-dataIndex + 1));
                else
                    m_chars.sendSAXcharacters(
                            ch, m_data.elementAt(-dataIndex), m_data.elementAt(-dataIndex + 1));
            }
    ...
    protected final void copyTextNode(final int nodeID, SerializationHandler handler)
        ...
        if (nodeID != DTM.NULL) {
            ...
            if (dataIndex >= 0) {
                m_chars.sendSAXcharacters(
                        handler, dataIndex >>> TEXT_LENGTH_BITS, dataIndex & TEXT_LENGTH_MAX);
            } else {
                m_chars.sendSAXcharacters(
                        handler, m_data.elementAt(-dataIndex), m_data.elementAt(-dataIndex + 1));
            }
        }
    }
     ...
     * @param handler The SerializationHandler
     * @param inScope true if all namespaces in scope should be copied, false if only the namespace
     *     declarations should be copied.
     */
    protected final void copyNS(final int nodeID, SerializationHandler handler, boolean inScope)
        ...
        if (m_namespaceDeclSetElements != null
                ...
                && ((SuballocatedIntVector) m_namespaceDeclSets.elementAt(0)).size() == 1) return;
     ...
     *
     * @baseID The node identity of the base node, which can be an element, attribute or namespace
     *     node.
    ...
    protected final void copyAttributes(final int nodeID, SerializationHandler handler)
        ...
        for (int current = getFirstAttributeIdentity(nodeID);
                current != DTM.NULL;
