...
import org.openjdk.com.sun.org.apache.xml.internal.resolver.readers.CatalogReader;
...
import org.openjdk.com.sun.org.apache.xml.internal.resolver.readers.TR9401CatalogReader;
 ...
 *
 * <p>This class implements the semantics of OASIS Open Catalog files (defined by <a
 * href="http://www.oasis-open.org/html/a401.htm">OASIS Technical Resolution 9401:1997 (Amendment 2
 * to TR 9401)</a>).
 *
 * <p>The primary purpose of the Catalog is to associate resources in the document with local system
 * identifiers. Some entities (document types, XML entities, and notations) have names and all of
 * them can have either public or system identifiers or both. (In XML, only a notation can have a
 * public identifier without a system identifier, but the methods implemented in this class obey the
 * Catalog semantics from the SGML days when system identifiers were optional.)
 *
 * <p>The system identifiers returned by the resolution methods in this class are valid, i.e. usable
 * by, and in fact constructed by, the <tt>java.net.URL</tt> class. Unfortunately, this class seems
 * to behave in somewhat non-standard ways and the system identifiers returned may not be directly
 * usable in a browser or filesystem context.
 *
 * <p>This class recognizes all of the Catalog entries defined in TR9401:1997:
 *
 * <ul>
 *   <li><b>BASE</b> changes the base URI for resolving relative system identifiers. The initial
 *       base URI is the URI of the location of the catalog (which is, in turn, relative to the
 *       location of the current working directory at startup, as returned by the <tt>user.dir</tt>
 *       system property).
 *   <li><b>CATALOG</b> processes other catalog files. An included catalog occurs logically at the
 *       end of the including catalog.
 *   <li><b>DELEGATE_PUBLIC</b> specifies alternate catalogs for some public identifiers. The
 *       delegated catalogs are not loaded until they are needed, but they are cached once loaded.
 *   <li><b>DELEGATE_SYSTEM</b> specifies alternate catalogs for some system identifiers. The
 *       delegated catalogs are not loaded until they are needed, but they are cached once loaded.
 *   <li><b>DELEGATE_URI</b> specifies alternate catalogs for some URIs. The delegated catalogs are
 *       not loaded until they are needed, but they are cached once loaded.
 *   <li><b>REWRITE_SYSTEM</b> specifies alternate prefix for a system identifier.
 *   <li><b>REWRITE_URI</b> specifies alternate prefix for a URI.
 *   <li><b>SYSTEM_SUFFIX</b> maps any system identifier that ends with a particular suffix to
 *       another system identifier.
 *   <li><b>URI_SUFFIX</b> maps any URI that ends with a particular suffix to another URI.
 *   <li><b>DOCTYPE</b> associates the names of root elements with URIs. (In other words, an XML
 *       processor might infer the doctype of an XML document that does not include a doctype
 *       declaration by looking for the DOCTYPE entry in the catalog which matches the name of the
 *       root element of the document.)
 *   <li><b>DOCUMENT</b> provides a default document.
 *   <li><b>DTDDECL</b> recognized and silently ignored. Not relevant for XML.
 *   <li><b>ENTITY</b> associates entity names with URIs.
 *   <li><b>LINKTYPE</b> recognized and silently ignored. Not relevant for XML.
 *   <li><b>NOTATION</b> associates notation names with URIs.
 *   <li><b>OVERRIDE</b> changes the override behavior. Initial behavior is set by the system
 *       property <tt>xml.catalog.override</tt>. The default initial behavior is 'YES', that is,
 *       entries in the catalog override system identifiers specified in the document.
 *   <li><b>PUBLIC</b> maps a public identifier to a system identifier.
 *   <li><b>SGMLDECL</b> recognized and silently ignored. Not relevant for XML.
 *   <li><b>SYSTEM</b> maps a system identifier to another system identifier.
 *   <li><b>URI</b> maps a URI to another URI.
 * </ul>
 *
 * <p>Note that BASE entries are treated as described by RFC2396. In particular, this has the
 * counter-intuitive property that after a BASE entry identifing "http://example.com/a/b/c" as the
 * base URI, the relative URI "foo" is resolved to the absolute URI "http://example.com/a/b/foo".
 * You must provide the trailing slash if you do not want the final component of the path to be
 * discarded as a filename would in a URI for a resource: "http://example.com/a/b/c/".
 *
 * <p>Note that subordinate catalogs (all catalogs except the first, including CATALOG and DELEGATE*
 * catalogs) are only loaded if and when they are required.
 *
 * <p>This class relies on classes which implement the CatalogReader interface to actually load
 * catalog files. This allows the catalog semantics to be implemented for TR9401 text-based
 * catalogs, XML catalogs, or any number of other storage formats.
 *
 * <p>Additional catalogs may also be loaded with the {@link #parseCatalog} method.
 * </dl>
 *
 * <p><b>Change Log:</b>
 *
 ...
 *   <dt>2.0
 *   <dd>
 *       <p>Rewrite to use CatalogReaders.
 *   <dt>1.1
 *   <dd>
 *       <p>Allow quoted components in <tt>xml.catalog.files</tt> so that URLs containing colons can
 *       be used on Unix. The string passed to <tt>xml.catalog.files</tt> can now have the form:
 *       <pre>
 ...
 * </pre>
 *       <p>(Where ":" is the separater character in this example.)
 *       <p>If an unquoted path contains an embedded double or single quote character, no special
 *       processig is performed on that character. No path can contain separater characters, double,
 *       and single quotes simultaneously.
 *       <p>Fix bug in calculation of BASE entries: if a catalog contains multiple BASE entries,
 *       each is relative to the preceding base, not the default base URI of the catalog.
 *   <dt>1.0.1
 *   <dd>
 *       <p>Fixed a bug in the calculation of the list of subordinate catalogs. This bug caused an
 *       infinite loop where parsing would alternately process two catalogs indefinitely.
 * </dl>
 *
 * @see CatalogReader
 * @see CatalogEntry
 ...
 * @version 1.0
 *     <p>Derived from public domain code originally published by Arbortext, Inc.
 */
public class Catalog {
    ...
    /**
     * The base URI for relative system identifiers in the catalog. This may be changed by BASE
     ...
     *
     * <p>This list is initially established by <code>loadSystemCatalogs</code> when it parses the
     * system catalog list, but CATALOG entries may contribute to it during the course of parsing.
     *
    ...
    /**
     * A vector of catalog files constructed during processing of CATALOG entries in the current
     * catalog.
     *
     * <p>This two-level system is actually necessary to correctly implement the semantics of the
     * CATALOG entry. If one catalog file includes another with a CATALOG entry, the included
     * catalog logically occurs <i>at the end</i> of the including catalog, and after any preceding
     * CATALOG entries. In other words, the CATALOG entry cannot insert anything into the middle of
     * a catalog file.
     *
     * <p>When processing reaches the end of each catalog files, any elements on this vector are
     * added to the front of the <code>catalogFiles</code> vector.
     *
     * @see #catalogFiles
     */
     ...
     *
     * <p>The semantics of Catalog resolution are such that each catalog is effectively a list of
     * Catalogs (in other words, a recursive list of Catalog instances).
     *
     * <p>Catalogs that are processed as the result of CATALOG or DELEGATE* entries are subordinate
     * to the catalog that contained them, but they may in turn have subordinate catalogs.
     *
     * <p>Catalogs are only loaded when they are needed, so this vector initially contains a list of
     * Catalog filenames (URLs). If, during processing, one of these catalogs has to be loaded, the
     * resulting Catalog object is placed in the vector, effectively caching it for the next query.
     */
     ...
     *
     * <p>This two-level system has two purposes; first, it allows us to sort the DELEGATE* entries
     * by the length of the partial public identifier so that a linear search encounters them in the
     * correct order and second, it puts them all at the end of the Catalog.
     *
     * <p>When processing reaches the end of each catalog file, any elements on this vector are
     * added to the end of the <code>catalogEntries</code> vector. This assures that matching PUBLIC
     * keywords are encountered before DELEGATE* entries.
     */
     ...
     *
     * <p>This hash maps MIME types to elements in the readerArr vector. This allows the Catalog to
     * quickly locate the reader for a particular MIME type.
     */
     ...
     *
     * <p>This vector contains all of the readers in the order that they were added. In the event
     * that a catalog is read from a file, where the MIME type is unknown, each reader is attempted
     ...
     *
     * <p>The constructor interrogates the relevant system properties using the default (static)
     * CatalogManager and initializes the catalog data structures.
     */
    public Catalog() {
        // nop;
    }
     ...
     *
     * <p>The constructor interrogates the relevant system properties using the specified Catalog
     * Manager and initializes the catalog data structures.
     */
     ...
     *
     * <p>This method allows you to add a new CatalogReader to the catalog. The reader will be
     * associated with the specified mimeType. You can only have one reader per mimeType.
     *
     * <p>In the absence of a mimeType (e.g., when reading a catalog directly from a file on the
     * local system), the readers are attempted in the order that you add them to the Catalog.
     *
     * <p>Note that subordinate catalogs (created by CATALOG or DELEGATE* entries) get a copy of the
     * set of readers present in the primary catalog when they are created. Readers added
     * subsequently will not be available. For this reason, it is best to add all of the readers
     * before the first call to parse a catalog.
     *
     ...
     *
     * <p>This method is used internally when constructing a new catalog. It copies the current
     * reader associations over to the new catalog.
     *
     ...
     *
     * <p>This method constructs a new instance of the running Catalog class (which might be a
     * subtype of com.sun.org.apache.xml.internal.resolver.Catalog). All new catalogs are managed by
     * the same CatalogManager.
     *
     * <p>N.B. All Catalog subtypes should call newCatalog() to construct a new Catalog. Do not
     * simply use "new Subclass()" since that will confuse future subclasses.
     */
     ...
     *
     * <p>All catalog files loaded by this catalog will have the initial override setting specified
     * by this default.
     */
     ...
     *
     * <p>The method adds all of the catalogs specified in the <tt>xml.catalog.files</tt> property
     * to the Catalog list.
     *
     * @throws MalformedURLException One of the system catalogs is identified with a filename that
     *     is not a valid URL.
     ...
     * @throws MalformedURLException The fileName cannot be turned into a valid URL.
     ...
     * Parse a catalog file, augmenting internal data structures.
     *
     * <p>Catalogs retrieved over the net may have an associated MIME type. The MIME type can be
     * used to select an appropriate reader.
     *
     * @param mimeType The MIME type of the catalog file.
     * @param is The InputStream from which the catalog should be read
     * @throws CatalogException Failed to load catalog mimeType.
     * @throws IOException Error reading catalog file.
     */
     ...
     * jar:file:///path/to/filename.jar!/path/to/catalog.xml". That URI doesn't survive
     * transmogrification through the URI processing that the parseCatalog(String) performs and
     * passing it as an input stream doesn't set the base URI appropriately.
     *
     * <p>Written by Stefan Wachter (2002-09-26)
     *
     ...
     *
     * <p>Catalogs may refer to other catalogs, this method parses all of the currently pending
     * catalog files.
     */
        ...
        try {
            ...
            try {
                ...
                catalogManager.debug.message(
                        1, "Malformed URL on catalog filename", fixSlashes(fileName));
                base = null;
            }
        }
     ...
     *
     * <p>This method processes each Catalog entry, changing mapped relative system identifiers into
     * absolute ones (based on the current base URI), and maintaining other information about the
     * current catalog.
     *
     ...
     *
     * <p>This method exists to allow subclasses to deal with unknown entry types.
     */
     ...
     *
     * <p>This method recursively parses all of the subordinate catalogs. If this method does not
     * throw an exception, you can be confident that no subsequent call to any resolve*() method
     * will either, with two possible exceptions:
     *
     * <ol>
     *   <li>
     *       <p>Delegated catalogs are re-parsed each time they are needed (because a variable list
     *       of them may be needed in each case, depending on the length of the matching partial
     *       public identifier).
     *       <p>But they are parsed by this method, so as long as they don't change or disappear
     *       while the program is running, they shouldn't generate errors later if they don't
     *       generate errors now.
     *   <li>
     *       <p>If you add new catalogs with <code>parseCatalog</code>, they won't be loaded until
     *       they are needed or until you call <code>parseAllCatalogs</code> again.
     * </ol>
     *
     * <p>On the other hand, if you don't call this method, you may successfully parse documents
     * without having to load all possible catalogs.
     *
     * @throws MalformedURLException The filename (URL) for a subordinate or delegated catalog is
     *     not a valid URL.
     * @throws IOException Error reading some subordinate or delegated catalog file.
     */
     ...
     *
     * @param entityName The name of the entity (element) for which a doctype is required.
     * @param publicId The nominal public identifier for the doctype (as provided in the source
     *     document).
     * @param systemId The nominal system identifier for the doctype (as provided in the source
     *     document).
     * @return The system identifier to use for the doctype.
     * @throws MalformedURLException The formal system identifier of a subordinate catalog cannot be
    ...
    public String resolveDoctype(String entityName, String publicId, String systemId)
        ...
        catalogManager.debug.message(
                3, "resolveDoctype(" + entityName + "," + publicId + "," + systemId + ")");
     ...
     *
     * @param entityName The name of the entity for which a system identifier is required.
     * @param publicId The nominal public identifier for the entity (as provided in the source
     *     document).
     * @param systemId The nominal system identifier for the entity (as provided in the source
     *     document).
    ...
    public String resolveEntity(String entityName, String publicId, String systemId)
        ...
        catalogManager.debug.message(
                3, "resolveEntity(" + entityName + "," + publicId + "," + systemId + ")");
     ...
     *
     * @param notationName The name of the notation for which a doctype is required.
     * @param publicId The nominal public identifier for the notation (as provided in the source
     *     document).
     * @param systemId The nominal system identifier for the notation (as provided in the source
     *     document).
    ...
    public String resolveNotation(String notationName, String publicId, String systemId)
        ...
        catalogManager.debug.message(
                3, "resolveNotation(" + notationName + "," + publicId + "," + systemId + ")");
     ...
     *
     * <p>This method searches the Catalog and returns the system identifier specified for the given
     * system or public identifiers. If no appropriate PUBLIC or SYSTEM entry is found in the
     * Catalog, null is returned.
     *
     * @param publicId The public identifier to locate in the catalog. Public identifiers are
     *     normalized before comparison.
     * @param systemId The nominal system identifier for the entity in question (as provided in the
     *     source document).
     * @throws MalformedURLException The formal system identifier of a subordinate catalog cannot be
     *     turned into a valid URL.
     * @throws IOException Error reading subordinate catalog file.
     * @return The system identifier to use. Note that the nominal system identifier is not returned
     *     if a match is not found in the catalog, instead null is returned to indicate that no
     *     match was found.
     */
     ...
     * <p>This method searches the Catalog and returns the system identifier specified for the given
     * system or public identifiers. If no appropriate PUBLIC or SYSTEM entry is found in the
     * Catalog, delegated Catalogs are interrogated.
     *
     * <p>There are four possible cases:
     *
     * <ul>
     *   <li>If the system identifier provided matches a SYSTEM entry in the current catalog, the
     *       SYSTEM entry is returned.
     *   <li>If the system identifier is not null, the PUBLIC entries that were encountered when
     *       OVERRIDE YES was in effect are interrogated and the first matching entry is returned.
     *   <li>If the system identifier is null, then all of the PUBLIC entries are interrogated and
     *       the first matching entry is returned. This may not be the same as the preceding case,
     *       if some PUBLIC entries are encountered when OVERRIDE NO is in effect. In XML, the only
     *       place where a public identifier may occur without a system identifier is in a notation
     *       declaration.
     *   <li>Finally, if the public identifier matches one of the partial public identifiers
     *       specified in a DELEGATE* entry in the Catalog, the delegated catalog is interrogated.
     *       The first time that the delegated catalog is required, it will be retrieved and parsed.
     *       It is subsequently cached.
     * </ul>
     *
     * @param entityType The CatalogEntry type for which this query is being conducted. This is
     *     necessary in order to do the approprate query on a delegated catalog.
     * @param entityName The name of the entity being searched for, if appropriate.
     * @param publicId The public identifier of the entity in question.
     * @param systemId The nominal system identifier for the entity in question (as provided in the
     *     source document).
     * @throws MalformedURLException The formal system identifier of a delegated catalog cannot be
     *     turned into a valid URL.
     ...
     *     match was found.
     */
    protected synchronized String resolveLocalPublic(
     ...
     *
     * <p>If a SYSTEM entry exists in the Catalog for the system ID specified, return the mapped
     * value.
     *
     * <p>On Windows-based operating systems, the comparison between the system identifier provided
     * and the SYSTEM entries in the Catalog is case-insensitive.
     *
     ...
     *
     * <p>If a SYSTEM entry exists in the catalog file for the system ID specified, return the
     * mapped value.
     *
     * @param systemId The system ID to locate in the catalog
     ...
     *
     * <p>If a URI entry exists in the Catalog for the URI specified, return the mapped value.
     *
     * <p>URI comparison is case sensitive.
     *
     * @param uri The URI to locate in the catalog.
     * @return The resolved URI.
     * @throws MalformedURLException The system identifier of a subordinate catalog cannot be turned
     *     into a valid URL.
     * @throws IOException Error reading subordinate catalog file.
     */
     ...
     *
     * <p>If a URI entry exists in the catalog file for the URI specified, return the mapped value.
     *
     * @param uri The URI to locate in the catalog
     ...
     *
     * <p>This method searches the Catalog and returns the system identifier specified for the given
     * entity type with the given name, public, and system identifiers. In some contexts, these may
     * be null.
     *
     * @param entityType The CatalogEntry type for which this query is being conducted. This is
     *     necessary in order to do the approprate query on a subordinate catalog.
     * @param entityName The name of the entity being searched for, if appropriate.
     * @param publicId The public identifier of the entity in question (as provided in the source
     *     document).
     * @param systemId The nominal system identifier for the entity in question (as provided in the
     *     source document). This parameter is overloaded for the URI entry type.
     * @throws MalformedURLException The formal system identifier of a delegated catalog cannot be
     *     turned into a valid URL.
     * @throws IOException Error reading delegated catalog file.
     * @return The system identifier to use. Note that the nominal system identifier is not returned
     ...
     *     match was found.
     */
    protected synchronized String resolveSubordinateCatalogs(
            int entityType, String entityName, String publicId, String systemId)
            throws MalformedURLException, IOException {
     ...
     *
     * @param b The 8-bit integer that represents th byte. (Bytes are signed but encoding needs to
     *     look at the bytes unsigned.)
     ...
     *
     * <p>This method always constructs the {@link #localDelegate} vector so that it is ordered by
     * length of partial public identifier.
     *
