 ...
 *
 * <p>Instances of this class represent individual entries in a Catalog.
 *
 * <p>Each catalog entry has a unique name and is associated with an arbitrary number of arguments
 * (all strings). For example, the TR9401 catalog entry "PUBLIC" has two arguments, a public
 * identifier and a system identifier. Each entry has a unique numeric type, assigned automatically
 * when the entry type is created.
 *
 * <p>The number and type of catalog entries is maintained <em>statically</em>. Catalog classes, or
 * their subclasses, can add new entry types, but all Catalog objects share the same global pool of
 * types.
 *
...
public class CatalogEntry {
    ...
    /**
     * The entryTypes vector maps catalog entry names (e.g., 'BASE' or 'SYSTEM') to their type (1,
     * 2, etc.). Names are case sensitive.
     */
    protected static Hashtable entryTypes = new Hashtable();

    /**
     * The entryTypes vector maps catalog entry types to the number of arguments they're required to
     * have.
     */
     ...
     *
     * @param name The name of the catalog entry type. This must be unique among all types and is
     *     case-sensitive. (Adding a duplicate name effectively replaces the old type with the new
     *     type.)
     * @param numArgs The number of arguments that this entry type is required to have. There is no
     *     provision for variable numbers of arguments.
     ...
     * @throws InvalidCatalogEntryTypeException if no such entry type exists.
     * @throws InvalidCatalogEntryException if the wrong number of arguments is passed.
     */
     ...
     *
     * <p>Catalogs sometimes need to adjust the catlog entry parameters, for example to make a
     * relative URI absolute with respect to the current base URI. But in general, this function
     * should only be called shortly after object creation to do some sort of cleanup. Catalog
     * entries should not mutate over time.
     *
     * @param argNum The argument number (arguments are numbered from 0).
