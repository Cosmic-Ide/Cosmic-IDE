 ...
 *
 * <p>This class is used to read XML Catalogs using the SAX. This reader has an advantage over the
 * DOM-based reader in that it functions on the stream of SAX events. It has the disadvantage that
 * it cannot look around in the tree.
 *
 ...
 * In order to get around this problem, the SAXCatalogReader relies on a set of external
...
public class SAXCatalogReader implements CatalogReader, ContentHandler, DocumentHandler {
    ...
    /**
     * Set if something goes horribly wrong. It allows the class to ignore the rest of the events
     * that are received.
     */
     ...
     *
     * <p>This is a bit of a hack. Anyway, whenever we read for a catalog, we extract the debug
     * object from the catalog's manager so that we can use it to print messages.
     *
     * <p>In production, we don't really expect any messages so it doesn't really matter. But it's
    ...
    public void readCatalog(Catalog catalog, String fileUrl)
        ...
        try {
            ...
            catalog.getCatalogManager()
                    .debug
                    .message(1, "Failed to load catalog, file not found", url.toString());
        }
    }
    ...
    public void startElement(String name, AttributeList atts) throws SAXException {
        ...
        if (saxParser == null) {
            ...
            try {
                saxParser = (SAXCatalogParser) Class.forName(saxParserClass).newInstance();
        ...
        if (saxParser == null) {
            ...
            if (saxParserClass == null) {
                ...
                if (namespaceURI == null) {
                    ...
                    debug.message(
                            2, "No Catalog parser for " + "{" + namespaceURI + "}" + localName);
                }
                return;
            }
