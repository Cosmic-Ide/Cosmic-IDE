...
import org.openjdk.com.sun.org.apache.xerces.internal.dom.DOMErrorImpl;
import org.openjdk.com.sun.org.apache.xerces.internal.dom.DOMLocatorImpl;
import org.openjdk.com.sun.org.apache.xerces.internal.dom.DOMMessageFormatter;
...
/**
 * Base class for a serializer supporting both DOM and SAX pretty serializing of XML/HTML/XHTML
 * documents. Derives classes perform the method-specific serializing, this class provides the
 * common serializing mechanisms.
 *
 * <p>The serializer must be initialized with the proper writer and output format before it can be
 * used by calling {@link #setOutputCharStream} or {@link #setOutputByteStream} for the writer and
 * {@link #setOutputFormat} for the output format.
 *
 * <p>The serializer can be reused any number of times, but cannot be used concurrently by two
 * threads.
 *
 ...
 * #serialize(Document)} and SAX serializing is done by firing SAX events and using the serializer
 * as a document handler. This also applies to derived class.
 *
 ...
 *
 * <p>For elements that are not specified as whitespace preserving, the serializer will potentially
 * break long text lines at space boundaries, indent lines, and serialize elements on separate
 * lines. Line terminators will be regarded as spaces, and spaces at beginning of line will be
 * stripped.
 *
 * <p>When indenting, the serializer is capable of detecting seemingly element content, and
 * serializing these elements indented on separate lines. An element is serialized indented when it
 * is the first or last child of an element, or immediate following or preceding another element.
 *
...
public abstract class BaseMarkupSerializer
    ...
    /**
     * Holds array of all element states that have been entered. The array is automatically resized.
     * When leaving an element, it's state is not removed but reused when later returning to the
     * same nesting level.
     */
    private ElementState[] _elementStates;

    /**
     * The index of the next state to place in the array, or one plus the index of the current
     * state. When zero, we are in no state.
     */
    private int _elementStateCount;

    /**
     * Vector holding comments and PIs that come before the root element (even after it), see {@link
     * #serializePreRoot}.
     */
    private Vector _preRoot;

    /**
     * If the document has been started (header serialized), this flag is set to true so it's not
     * started twice.
     */
    protected boolean _started;

    /**
     * True if the serializer has been prepared. This flag is set to false when the serializer is
     * reset prior to using it, and to true after it has been prepared for usage.
     */
    private boolean _prepared;

    /**
     * Association between namespace URIs (keys) and prefixes (values). Accumulated here prior to
     * starting an element and placing this list in the element state.
     */
    ...
    /**
     * The output format associated with this serializer. This will never be a null reference. If no
     * format was passed to the constructor, the default one for this document type will be used.
     * The format object is never changed by the serializer.
     */
    ...
    /**
     * Protected constructor can only be used by derived class. Must initialize the serializer
     * before serializing any document, by calling {@link #setOutputCharStream} or {@link
     * #setOutputByteStream} first
     */
    ...
    public boolean reset() {
        if (_elementStateCount > 1) {
            String msg =
                    DOMMessageFormatter.formatMessage(
                            DOMMessageFormatter.SERIALIZER_DOMAIN, "ResetInMiddle", null);
    ...
    /**
     * Serializes the DOM element using the previously specified writer and output format. Throws an
     * exception only if an I/O exception occured while serializing.
     *
    ...
    public void serialize(Element elem) throws IOException {
        ...
        serializeNode(elem);
        _printer.flush();
        if (_printer.getException() != null) throw _printer.getException();
    }

    /**
     * Serializes a node using the previously specified writer and output format. Throws an
     * exception only if an I/O exception occured while serializing.
     *
     ...
     * Throws an exception only if an I/O exception occured while serializing.
     *
     * @param elem The element to serialize
     * @throws IOException An I/O exception occured while serializing
     */
    ...
    public void ignorableWhitespace(char[] chars, int start, int length) throws SAXException {
        ...
        try {
            ...
            if (_indenting) {
                ...
                for (i = start; length-- > 0; ++i) _printer.printText(chars[i]);
            }
        } catch (IOException except) {
            throw new SAXException(except);
        }
    ...
    public void processingInstructionIO(String target, String code) throws IOException {
        ...
        if (index >= 0) fStrBuffer.append("<?").append(target.substring(0, index));
        else fStrBuffer.append("<?").append(target);
        if (code != null) {
            fStrBuffer.append(' ');
            index = code.indexOf("?>");
            if (index >= 0) fStrBuffer.append(code.substring(0, index));
            else fStrBuffer.append(code);
        }
        fStrBuffer.append("?>");
        ...
        if (isDocumentState()) {
            if (_preRoot == null) _preRoot = new Vector();
    ...
    public void comment(String text) throws IOException {
        ...
        index = text.indexOf("-->");
        if (index >= 0) fStrBuffer.append("<!--").append(text.substring(0, index)).append("-->");
        else fStrBuffer.append("<!--").append(text).append("-->");
    ...
    /**
     * Called at the end of the document to wrap it up. Will flush the output stream and throw an
     * exception if any I/O error occured while serializing.
     *
    ...
    public void startPrefixMapping(String prefix, String uri) throws SAXException {
        if (_prefixes == null) _prefixes = new Hashtable();
    ...
    /**
     * Serialize the DOM node. This method is shared across XML, HTML and XHTML serializers and the
     * differences are masked out in a separate {@link #serializeElement}.
     *
    ...
    protected void serializeNode(Node node) throws IOException {
        ...
        switch (node.getNodeType()) {
                    ...
                    if (text != null) {
                        if (fDOMFilter != null
                            ...
                            switch (code) {
                                ...
                                || getElementState().preserveSpace
                                || (text.replace('\n', ' ').trim().length() != 0)) characters(text);
                    }
                    break;
                }
            ...
            case Node.COMMENT_NODE:
                {
                    ...
                    if (!_format.getOmitComments()) {
                        ...
                        if (text != null) {
                            ...
                            if (fDOMFilter != null
                                    && (fDOMFilter.getWhatToShow() & NodeFilter.SHOW_COMMENT)
                                            != 0) {
                                short code = fDOMFilter.acceptNode(node);
                                switch (code) {
                                    case NodeFilter.FILTER_REJECT:
            ...
            case Node.PROCESSING_INSTRUCTION_NODE:
                {
                    if (fDOMFilter != null
                            && (fDOMFilter.getWhatToShow() & NodeFilter.SHOW_PROCESSING_INSTRUCTION)
                                    != 0) {
                        short code = fDOMFilter.acceptNode(node);
                        switch (code) {
                            case NodeFilter.FILTER_REJECT:
                        ...
                        // DOM Level 1 -- does implementation have methods?
                            ...
                            try {
                                ...
                                if (getPublicId.getReturnType().equals(String.class)) {
                                    docTypePublicId =
                                            (String) getPublicId.invoke(docType, (Object[]) null);
                                }
                            } catch (Exception e) {
                                // ignore
                            }
                            try {
                                ...
                                if (getSystemId.getReturnType().equals(String.class)) {
                                    docTypeSystemId =
                                            (String) getSystemId.invoke(docType, (Object[]) null);
                                }
                            } catch (Exception e) {
                                // ignore
                            }
    ...
    protected void serializeDocument() throws IOException {
        ...
        if (!_started) {
            ...
            if (!_format.getOmitXMLDeclaration()) {
                ...
                if (_format.getVersion() != null) buffer.append(_format.getVersion());
                else buffer.append("1.0");
                buffer.append('"');
        ...
        if (!_format.getOmitDocumentType()) {
            if (_docTypeSystemId != null) {
                ...
                if (_docTypePublicId != null) {
                    ...
                    if (_indenting) {
                        _printer.breakLine();
                        for (int i = 0; i < 18 + name.length(); ++i) _printer.printText(" ");
    ...
    /**
     * Must be called by a method about to print any type of content. If the element was just
     * opened, the opening tag is closed and will be matched to a closing tag. Returns the current
     * element state with <tt>empty</tt> and <tt>afterElement</tt> set to false.
     *
    ...
    /**
     * Called to print the text contents in the prevailing element format. Since this method is
     * capable of printing text as CDATA, it is used for that purpose as well. White space handling
     * is determined by the current element state. In addition, the output format can dictate
    ...
    /**
     * Called to serializee the DOM element. The element is serialized based on the serializer's
    ...
    /**
     * Comments and PIs cannot be serialized before the root element, because the root element
     * serializes the document type, which generally comes first. Instead such PIs and comments are
     * accumulated inside a vector and serialized by calling this method. Will be called when the
     * root element is serialized and when the document finished serializing.
     *
     * @throws IOException An I/O exception occured while serializing
     */
        ...
        for (int index = 0; index < length; ++index) {
            ...
            if (ch == ']'
                ...
                if (fDOMErrorHandler != null) {
                    // REVISIT: this means that if DOM Error handler is not registered we don't
                    // report any
     ...
     * is printed as if by calling {@link #printText(String,boolean,boolean)} with a call to {@link
     * Printer#breakLine} for each new line. If spaces are not preserved, the text is broken at
     * space boundaries if longer than the line width; Multiple spaces are printed as such, but
     * spaces at beginning of line are removed.
     *
     * @param text The text to print
    ...
    protected void printText(
            char[] chars, int start, int length, boolean preserveSpace, boolean unescaped)
            throws IOException {
        int index;
        char ch;
        ...
        if (preserveSpace) {
            ...
            while (length-- > 0) {
                ...
                if (ch == ' ' || ch == '\f' || ch == '\t' || ch == '\n' || ch == '\r')
                    _printer.printSpace();
                else if (unescaped) _printer.printText(ch);
                else printEscaped(ch);
            }
        }
    }
    ...
    /**
     * Print a document type public or system identifier URL. Encapsulates the URL in double quotes,
     * escapes non-printing characters and print it equivalent to {@link #printText}.
     *
    ...
    /**
     * Enter a new element state for the specified element. Tag name and space preserving is
     * specified, element state is initially empty.
     *
     * @return Current element state, or null
     */
    protected ElementState enterElementState(
            String namespaceURI, String localName, String rawName, boolean preserveSpace) {
        ElementState state;
    ...
    /**
     * Leave the current element state and return to the state of the parent element. If this was
     * the root element, return to the state of the document.
     *
    ...
    protected ElementState leaveElementState() {
        if (_elementStateCount > 0) {
            ...
            String msg =
                    DOMMessageFormatter.formatMessage(
                            DOMMessageFormatter.SERIALIZER_DOMAIN, "Internal", null);
            throw new IllegalStateException(msg);
        }
    }

    /**
     * Returns true if in the state of the document. Returns true before entering any element and
     * after leaving the root element.
     *
    ...
    protected String getPrefix(String namespaceURI) {
        ...
        if (_elementStateCount == 0) return null;
        else {
