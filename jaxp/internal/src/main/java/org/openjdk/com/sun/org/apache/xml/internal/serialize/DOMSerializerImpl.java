...
import org.openjdk.com.sun.org.apache.xerces.internal.dom.DOMNormalizer;
import org.openjdk.com.sun.org.apache.xerces.internal.dom.DOMStringListImpl;
 ...
 * calls to <CODE>XMLSerializer</CODE>. LSSerializer provides an API for serializing (writing) a DOM
 * document out in an XML document. The XML data is written to an output stream. During
 * serialization of XML data, namespace fixup is done when possible as defined in DOM Level 3 Core,
 * Appendix B.
 *
...
public class DOMSerializerImpl implements LSSerializer, DOMConfiguration {
    ...
    protected static final short FORMAT_PRETTY_PRINT = 0x1 << 11;
    ...
    /**
     * Constructs a new LSSerializer. The constructor turns on the namespace support in <code>
     * XMLSerializer</code> and initializes the following fields: fNSBinder, fLocalNSBinder,
     * fSymbolTable, fEmptySymbol, fXmlSymbol, fXmlnsSymbol, fNamespaceCounter, fFeatures.
     */
    ...
    public void setParameter(String name, Object value) throws DOMException {
        if (value instanceof Boolean) {
            ...
            if (value == null || value instanceof DOMErrorHandler) {
                ...
                || name.equalsIgnoreCase(Constants.DOM_NORMALIZE_CHARACTERS) && value != null) {
            String msg =
                    DOMMessageFormatter.formatMessage(
                            DOMMessageFormatter.DOM_DOMAIN,
                            "FEATURE_NOT_SUPPORTED",
    ...
    public boolean canSetParameter(String name, Object state) {
        ...
        } else if (name.equalsIgnoreCase(Constants.DOM_ERROR_HANDLER) && state == null
                || state instanceof DOMErrorHandler) {
            return true;
        }
     ...
     *
     * <p>The list of the parameters supported by this <code>DOMConfiguration</code> object and for
    ...
    /**
     * DOM L3 EXPERIMENTAL: Serialize the specified node as described above in the description of
     * <code>LSSerializer</code>. The result of serializing the node is returned as a string.
     * Writing a Document or Entity node produces a serialized form that is well formed XML. Writing
     ...
     * @exception DOMException DOMSTRING_SIZE_ERR: The resulting string is too long to fit in a
     *     <code>DOMString</code>.
     * @exception LSException SERIALIZE_ERR: Unable to serialize the node. DOM applications should
    ...
    public String writeToString(Node wnode) throws DOMException, LSException {
        ...
        Document doc =
                (wnode.getNodeType() == Node.DOCUMENT_NODE)
                        ? (Document) wnode
                        : wnode.getOwnerDocument();
        ...
        try {
            ...
            throw (LSException)
                    new LSException(LSException.SERIALIZE_ERR, e.toString()).initCause(e);
            ...
            throw (DOMException)
                    new DOMException(DOMException.DOMSTRING_SIZE_ERR, msg).initCause(ioe);
        }
    ...
    /**
     * DOM L3 EXPERIMENTAL: The end-of-line sequence of characters to be used in the XML being
     * written out. The only permitted values are these:
     *
     * <dl>
     *   <dt><code>null</code>
     *   <dd>Use a default end-of-line sequence. DOM implementations should choose the default to
     *       match the usual convention for text files in the environment being used.
     *       Implementations must choose a default sequence that matches one of those allowed by
     *       2.11 "End-of-Line Handling".
     *   <dt>CR
     *   <dd>The carriage-return character (#xD).
     *   <dt>CR-LF
     *   <dd>The carriage-return and line-feed characters (#xD #xA).
     *   <dt>LF
     *   <dd>The line-feed character (#xA).
     * </dl>
     *
     * <br>
     * The default value for this attribute is <code>null</code>.
     */
     ...
     * serializing each Node. Attribute nodes are never passed to the filter. The filter
     * implementation can choose to remove the node from the stream or to terminate the
     * serialization early.
     */
     ...
     *   <li><code>Document.actualEncoding</code>,
     ...
     * be used. <br>
     * If the specified encoding is not supported an "unsupported-encoding" error is raised. <br>
     * If no output is specified in the <code>LSOutput</code>, a "no-output-specified" error is
     * raised.
     *
     * @param node The node to serialize.
     * @param destination The destination for the serialized DOM.
     * @return Returns <code>true</code> if <code>node</code> was successfully serialized and <code>
     *     false</code> in case the node couldn't be serialized.
     */
    public boolean write(Node node, LSOutput destination) throws LSException {
        ...
        try {
            ...
            throw new LSException(
                    ...
                    DOMMessageFormatter.formatMessage(
                            DOMMessageFormatter.SERIALIZER_DOMAIN, "unsupported-encoding", null));
            // return false;
        } catch (LSException lse) {
            // Rethrow LSException.
            throw lse;
            ...
            throw (LSException)
                    DOMUtil.createLSException(LSException.SERIALIZE_ERR, e).fillInStackTrace();
        } catch (Exception e) {
            if (ser.fDOMErrorHandler != null) {
                DOMErrorImpl error = new DOMErrorImpl();
                error.fException = e;
    ...
    /**
     * Serialize the specified node as described above in the general description of the <code>
     * LSSerializer</code> interface. The output is written to the supplied URI. <br>
     * When writing to a URI, the encoding is found by looking at the encoding information that is
     * reachable through the item to be written (or its owner document) in this order:
     *
     * <ol>
    ...
    private void prepareForSerialization(XMLSerializer ser, Node node) {
        ...
        if ((features & WELLFORMED) != 0) {
            // REVISIT: this is inefficient implementation of well-formness. Instead, we should
            // check
            ...
            try {
                ...
                if (versionChanged != null) {
                    verifyNames =
                            ((Boolean) versionChanged.invoke(document, (Object[]) null))
                                    .booleanValue();
                }
            } catch (Exception e) {
                // no way to test the version...
            ...
            case Node.ELEMENT_NODE:
                {
                    if (verifyNames) {
                        if ((features & NAMESPACES) != 0) {
                            wellformed =
                                    CoreDocumentImpl.isValidQName(
                                            node.getPrefix(), node.getLocalName(), xml11Version);
                        } else {
                            wellformed =
            ...
            case Node.CDATA_SECTION_NODE:
                {
                    ...
                    DOMNormalizer.isXMLCharWF(
                            fErrorHandler, fError, fLocator, node.getNodeValue(), xml11Version);
            ...
            case Node.PROCESSING_INSTRUCTION_NODE:
                {
                    ...
                    DOMNormalizer.isXMLCharWF(
                            fErrorHandler, fError, fLocator, pinode.getData(), xml11Version);
                    break;
                }
        }
    }
