...
/**
 * Utility class for accessing information specific to HTML documents. The HTML DTD is expressed as
 * three utility function groups. Two methods allow for checking whether an element requires an open
 * tag on printing ({@link #isEmptyTag}) or on parsing ({@link #isOptionalClosing}).
 *
 * <p>Two other methods translate character references from name to value and from value to name. A
 * small entities resource is loaded into memory the first time any of these methods is called for
...
public final class HTMLdtd {
    ...
    /**
     * Table of reverse character reference mapping. Character codes are held as single-character
     * strings, mapped to their reference name.
     */
    private static Hashtable _byChar;

    /**
     * Table of entity name to value mapping. Entities are held as strings, character references as
     * <TT>Character</TT> objects.
     */
    ...
    /**
     * Locates the HTML entities file that is loaded upon initialization. This file is a resource
     * loaded with the default class loader.
     */
    ...
    /**
     * Returns true if element is declared to be empty. HTML elements are defines as empty in the
     * DTD, not by the document syntax.
     *
    ...
    /**
     * Returns true if element is declared to have element content. Whitespaces appearing inside
     * element content will be ignored, other text will simply report an error.
     *
     * @param tagName The element tag name (upper case)
    ...
    /**
     * Returns true if element's textual contents preserves spaces. This only applies to PRE and
     * TEXTAREA, all other HTML elements do not preserve space.
     *
     * @param tagName The element tag name (upper case)
    ...
    /**
     * Returns true if element's closing tag is optional and need not exist. An error will not be
     * reported for such elements if they are not closed. For example, <tt>LI</tt> is most often not
     * closed.
     *
     * @param tagName The element tag name (upper case)
    ...
    /**
     * Returns true if element's closing tag is generally not printed. For example, <tt>LI</tt>
     * should not print the closing tag.
     *
     * @param tagName The element tag name (upper case)
    ...
    /**
     * Returns true if the opening of one element (<tt>tagName</tt>) implies the closing of another
     * open element (<tt>openTag</tt>). For example, every opening <tt>LI</tt> will close the
     * previously open <tt>LI</tt>, and every opening <tt>BODY</tt> will close the previously open
     * <tt>HEAD</tt>.
     *
    ...
    public static boolean isClosing(String tagName, String openTag) {
        ...
        if (openTag.equalsIgnoreCase("THEAD")
                || openTag.equalsIgnoreCase("TFOOT")
                || openTag.equalsIgnoreCase("TBODY")
                || openTag.equalsIgnoreCase("TR")
                || openTag.equalsIgnoreCase("COLGROUP")) return isElement(tagName, CLOSE_TABLE);
    ...
    /**
     * Returns true if the specified attribute it a URI and should be escaped appropriately. In HTML
     * URIs are escaped differently than normal attributes.
     *
     ...
     * This applies to attributes that are true if they exist, such as selected (OPTION/INPUT).
     *
     * @param tagName The element's tag name
     * @param attrName The attribute's name
     */
    public static boolean isBoolean(String tagName, String attrName) {
        ...
        for (int i = 0; i < attrNames.length; ++i)
            if (attrNames[i].equalsIgnoreCase(attrName)) return true;
        return false;
    }

    /**
     * Returns the value of an HTML character reference by its name. If the reference is not found
     * or was not defined as a character reference, returns EOF (-1).
     *
    ...
    public static int charFromName(String name) {
        ...
        if (value != null && value instanceof Integer) return ((Integer) value).intValue();
        else return -1;
    }

    /**
     * Returns the name of an HTML character reference based on its character value. Only valid for
     * entities defined from character references. If no such character value was defined, return
     * null.
     *
    ...
    /**
     * Initialize upon first access. Will load all the HTML character references into a list that is
     * accessible by name or character value and is optimized for character substitution. This
     * method may be called any number of times but will execute only once.
     */
        ...
        try {
            ...
            throw new RuntimeException(
                    DOMMessageFormatter.formatMessage(
                            DOMMessageFormatter.SERIALIZER_DOMAIN,
                            "ResourceNotLoaded",
                            new Object[] {ENTITIES_RESOURCE, except.toString()}));
        } finally {
            if (is != null) {
                try {
                    is.close();
    ...
    private static boolean isElement(String name, int flag) {
        ...
        else return ((flags.intValue() & flag) == flag);
    }
