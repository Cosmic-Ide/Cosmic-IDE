...
/**
 * Specifies an output format to control the serializer. Based on the XSLT specification for output
 * format, plus additional parameters. Used to select the suitable serializer and determine how the
 * document should be formatted on output.
 *
 * <p>The two interesting constructors are:
 *
 * <ul>
 *   <li>{@link #OutputFormat(String,String,boolean)} creates a format for the specified method
 *       (XML, HTML, Text, etc), encoding and indentation
 *   <li>{@link #OutputFormat(Document,String,boolean)} creates a format compatible with the
 *       document type (XML, HTML, Text, etc), encoding and indentation
 * </ul>
 *
 * @author <a href="mailto:arkin@intalio.com">Assaf Arkin</a> <a
 *     href="mailto:visco@intalio.com">Keith Visco</a>
 * @see Serializer
 * @see Method
 * @see LineSeparator
 */
public class OutputFormat {
    ...
    /**
     * True if spaces should be preserved in elements that do not specify otherwise, or specify the
     * default behavior.
     */
    private boolean _preserve = false;
    /**
     * If true, an empty string valued attribute is output as "". If false and and we are using the
     * HTMLSerializer, then only the attribute name is serialized. Defaults to false for backwards
     * compatibility.
     */
     ...
     * If <tt>indent</tt> is true, the document will be pretty printed with the default indentation
     * level and default line wrapping.
     *
    ...
    /**
     * Constructs a new output format with the proper method, document type identifiers and media
     * type for the specified document.
     *
    ...
    /**
     * Constructs a new output format with the proper method, document type identifiers and media
     * type for the specified document, and with the specified encoding. If <tt>indent</tt> is true,
     * the document will be pretty printed with the default indentation level and default line
     * wrapping.
     *
     * @param doc The document to output
     * @param encoding The specified encoding
    ...
    /**
     * Returns the method specified for this output format. Typically the method will be
     * <tt>xml</tt>, <tt>html</tt> or <tt>text</tt>, but it might be other values. If no method was
     * specified, null will be returned and the most suitable method will be determined for the
     * document by calling {@link #whichMethod}.
     *
    ...
    /**
     * Returns the version for this output method. If no version was specified, will return null and
     * the default version number will be used. If the serializerr does not support that particular
    ...
    /**
     * Sets the version for this output method. For XML the value would be "1.0", for HTML it would
     * be "4.0".
     *
     * @see #getVersion
    ...
    /**
     * Returns the indentation specified. If no indentation was specified, zero is returned and the
     * document should not be indented.
     *
    ...
    /**
     * Sets the indentation. The document will not be indented if the indentation is set to zero.
     * Calling {@link #setIndenting} will reset this value to zero (off) or the default (on).
     *
    ...
    /**
     * Sets the indentation on and off. When set on, the default indentation level and default line
     * wrapping is used (see {@link Defaults#Indent} and {@link Defaults#LineWidth}). To specify a
     * different indentation level or line wrapping, use {@link #setIndent} and {@link
     * #setLineWidth}.
     *
    ...
    /**
     * Sets the encoding for this output method. If no encoding was specified, the default is always
     * "UTF-8". Make sure the encoding is compatible with the one used by the {@link
     * java.io.Writer}.
     *
     * @see #getEncoding
    ...
    /**
     * Returns the specified media type, or null. To determine the media type based on the document
     * type, use {@link #whichMediaType}.
     *
    ...
    /**
     * Sets the document type public and system identifiers. Required only if the DOM Document or
     * SAX events do not specify the document type, and one must be present in the serialized
     * document. Any document type specified by the DOM Document or SAX events will override these
     * values.
     *
    ...
    /**
     * Sets document DTD standalone. The public and system identifiers must be null for the document
     * to be serialized as standalone.
     *
    ...
    /**
     * Returns a list of all the elements whose text node children should be output as CDATA, or
     * null if no such elements were specified.
     */
    ...
    public boolean isCDataElement(String tagName) {
        ...
        for (i = 0; i < _cdataElements.length; ++i)
            if (_cdataElements[i].equals(tagName)) return true;
        return false;
    }
    ...
    /**
     * Returns a list of all the elements whose text node children should be output unescaped (no
     * character references), or null if no such elements were specified.
     */
    ...
    public boolean isNonEscapingElement(String tagName) {
        ...
        for (i = 0; i < _nonEscapingElements.length; ++i)
            if (_nonEscapingElements[i].equals(tagName)) return true;
        return false;
    }

    /**
     * Sets the list of elements for which text node children should be output unescaped (no
     ...
     * line separator can be obtained from the system property <tt>line.separator</tt>, but is only
     * useful if the document is edited on machines of the same type. For general documents, use the
     * Web line separator.
     *
    ...
    /**
     * Returns true if the default behavior for this format is to preserve spaces. All elements that
     * do not specify otherwise or specify the default behavior will be formatted based on this
     * rule. All elements that specify space preserving will always preserve space.
     */
    ...
    /**
     * Sets space preserving as the default behavior. The default is space stripping and all
     * elements that do not specify otherwise or use the default value will not preserve spaces.
     *
    ...
    /**
     * Return the selected line width for breaking up long lines. When indenting, and only when
     * indenting, long lines will be broken at space boundaries based on this line width. No line
     * wrapping occurs if this value is zero.
     */
    ...
    /**
     * Sets the line width. If zero then no line wrapping will occur. Calling {@link #setIndenting}
     * will reset this value to zero (off) or the default (on).
     *
    ...
    /**
     * Returns the preserveEmptyAttribute flag. If flag is false, then' attributes with empty string
     * values are output as the attribute name only (in HTML mode).
     *
    ...
    /**
     * Sets the preserveEmptyAttribute flag. If flag is false, then' attributes with empty string
     * values are output as the attribute name only (in HTML mode).
     *
    ...
    /**
     * Returns the last printable character based on the selected encoding. Control characters and
     * non-printable characters are always printed as character references.
     */
    public char getLastPrintable() {
        if (getEncoding() != null && (getEncoding().equalsIgnoreCase("ASCII"))) return 0xFF;
        else return 0xFFFF;
    }

    /**
     * Determine the output method for the specified document. If the document is an instance of
     * {@link org.w3c.dom.html.HTMLDocument} then the method is said to be <tt>html</tt>. If the
     * root element is 'html' and all text nodes preceding the root element are all whitespace, then
     * the method is said to be <tt>html</tt>. Otherwise the method is <tt>xml</tt>.
     *
            ...
            if (node.getNodeType() == Node.ELEMENT_NODE) {
                ...
                for (i = 0; i < value.length(); ++i)
                    if (value.charAt(i) != 0x20
                            && value.charAt(i) != 0x0A
                            && value.charAt(i) != 0x09
                            && value.charAt(i) != 0x0D) return Method.XML;
            }
