...
/**
 * Holds information about a given encoding, which is the Java name for the encoding, the equivalent
 * ISO name.
 *
 * <p>An object of this type has two useful methods
 *
 * <pre>
 ...
 *
 * <p>An EncodingInfo object is a node in a binary search tree. Such a node will answer if a
 * character is in the encoding, and do so for a given range of unicode values (<code>m_first</code>
 * to <code>m_last</code>). It will handle a certain range of values explicitly (<code>m_explFirst
 * </code> to <code>m_explLast</code>). If the unicode point is before that explicit range, that is
 * it is in the range <code>m_first <= value < m_explFirst</code>, then it will delegate to another
 * EncodingInfo object for The root of such a tree, m_before. Likewise for values in the range
 * <code>m_explLast < value <= m_last</code>, but delgating to <code>m_after</code>
 *
 * <p>Actually figuring out if a code point is in the encoding is expensive. So the purpose of this
 * tree is to cache such determinations, and not to build the entire tree of information at the
 * start, but only build up as much of the tree as is used during the transformation.
 *
 * <p>This Class is not a public API, and should only be used internally within the serializer.
 *
 * @xsl.usage internal
 */
public final class EncodingInfo extends Object {
    ...
    /**
     * A helper object that we can ask if a single char, or a surrogate UTF-16 pair of chars that
     * form a single character, is in this encoding.
     */
    ...
    /**
     * This is not a public API. It returns true if the character formed by the high/low pair is in
     * the encoding.
     *
    ...
    /**
     * Create an EncodingInfo object based on the ISO name and Java name. If both parameters are
     * null any character will be considered to be in the encoding. This is useful for when the
     * serializer is in temporary output state, and has no assciated encoding.
     *
    ...
    /**
     * A simple interface to isolate the implementation. We could also use some new JRE 1.4 methods
     * in another implementation provided we use reflection with them.
     *
     * <p>This interface is not a public API, and should only be used internally within the
     * serializer.
     *
     * @xsl.usage internal
     */
    ...
    private class EncodingImpl implements InEncoding {
        ...
        /**
         * m_first through m_last is the range of unicode values that this object will return an
         * answer on. It may delegate to a similar object with a different range
         */
        private final int m_first;

        /**
         * m_explFirst through m_explLast is the range of unicode value that this object handles
         * explicitly and does not delegate to a similar object.
         */
        ...
        /**
         * The object, of the same type as this one, that handles unicode values in a range before
         * the range explictly handled by this object, and to which this object may delegate.
         */
        private InEncoding m_before;
        /**
         * The object, of the same type as this one, that handles unicode values in a range after
         * the range explictly handled by this object, and to which this object may delegate.
         */
        private InEncoding m_after;

        /**
         * The number of unicode values explicitly handled by a single EncodingInfo object. This
         * value is tuneable, but is set to 128 because that covers the entire low range of ASCII
         * type chars within a single object.
         */
    ...
    /**
     * This is heart of the code that determines if a given character is in the given encoding. This
     * method is probably expensive, and the answer should be cached.
     *
     * <p>This method is not a public API, and should only be used internally within the serializer.
     *
     * @param ch the char in question, that is not a high char of a high/low surrogate pair.
    ...
    /**
     * This is heart of the code that determines if a given high/low surrogate pair forms a
     * character that is in the given encoding. This method is probably expensive, and the answer
     * should be cached.
     *
     * <p>This method is not a public API, and should only be used internally within the serializer.
     *
    ...
    /**
     * This method is the core of determining if character is in the encoding. The method is not
     * foolproof, because s.getBytes(encoding) has specified behavior only if the characters are in
     * the specified encoding. However this method tries it's best.
     *
     * @param ch the char that was converted using getBytes, or the first char of a high/low pair
     *     that was converted.
