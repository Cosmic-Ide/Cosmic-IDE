...
/**
 * This interface describes extensions to the SAX ContentHandler interface. It is intended to be
 * used by a serializer. The methods on this interface will implement SAX- like behavior. This
 * allows the gradual collection of information rather than having it all up front. For example the
 * call
 *
 * <pre>
 ...
 * </pre>
 *
 * If there are no attributes the second call can be dropped. If attributes are to be added one at a
 * time with calls to
 *
 * <pre>
...
abstract interface ExtendedContentHandler extends org.xml.sax.ContentHandler {
    ...
    /**
     * Add an attribute to the current element. The namespace URI of the attribute will be
     * calculated from the prefix of qName. The local name will be derived from qName and the type
     * will be assumed to be "CDATA".
     *
     * @param qName
     * @param value
     */
    ...
    /**
     * This method is used to notify of a character event, but passing the data as a character
    ...
    /**
     * This method is used to notify of a character event, but passing the data as a DOM Node rather
     * than the standard character array.
     *
     ...
     * </pre>
     *
     * only the last parameter is passed. If needed the serializer can derive the localName from the
     * qualified name and derive the namespaceURI from its implementation.
     *
    ...
    /**
     * This method is used to notify that an element is starting. This method is just like the
     * standard SAX method
     *
     * <pre>
    ...
    /**
     * This method is used to notify that a prefix mapping is to start, but after an element is
     * started. The SAX method call
     *
     * <pre>
     * startPrefixMapping(prefix,uri)
     * </pre>
     *
     * is used just before an element starts and applies to the element to come, not to the current
     * element. This method applies to the current element. For example one could make the calls in
     * this order:
     *
     * <pre>
    ...
    /**
     * This method is used to notify that a prefix maping is to start, which can be for the current
     * element, or for the one to come.
     *
     * @param prefix the prefix that maps to the given URI
     * @param uri the namespace URI of the given prefix
     * @param shouldFlush if true this call is like the SAX startPrefixMapping(prefix,uri) call and
     *     the mapping applies to the element to come. If false the mapping applies to the current
     *     element.
     * @return boolean false if the prefix mapping was already in effect (in other words we are just
     *     re-declaring), true if this is a new, never before seen mapping for the element.
     * @throws SAXException
     */
     ...
     *
     * @return NamespaceMappings an object that has the current namespace mappings in effect.
     */
     ...
     * The serializer can write it out immediately without saving it in a table first. The integer
     * flag contains information about the attribute, which helps the serializer to decide whether a
