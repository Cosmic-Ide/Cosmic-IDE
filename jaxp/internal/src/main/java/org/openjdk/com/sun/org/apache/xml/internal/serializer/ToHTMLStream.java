...
/**
 * This serializer takes a series of SAX or SAX-like events and writes its output to the given
 * stream.
 *
 * <p>This class is not a public API, it is public because it is used from another package.
 *
 * @xsl.usage internal
 */
public final class ToHTMLStream extends ToStream {
    ...
    /**
     * Map that tells which XML characters should have special treatment, and it provides character
     * to entity name lookup.
     */
    ...
    static void initTagReference(Trie m_elementFlags) {
        ...
        m_elementFlags.put(
                "ADDRESS",
                new ElemDesc(0 | ElemDesc.BLOCK | ElemDesc.BLOCKFORM | ElemDesc.BLOCKFORMFIELDSET));
        m_elementFlags.put(
                "DIV",
                new ElemDesc(0 | ElemDesc.BLOCK | ElemDesc.BLOCKFORM | ElemDesc.BLOCKFORMFIELDSET));
        m_elementFlags.put("A", new ElemDesc(0 | ElemDesc.SPECIAL));
        m_elementFlags.put(
                "MAP", new ElemDesc(0 | ElemDesc.SPECIAL | ElemDesc.ASPECIAL | ElemDesc.BLOCK));
        m_elementFlags.put("AREA", new ElemDesc(0 | ElemDesc.EMPTY | ElemDesc.BLOCK));
        m_elementFlags.put(
                "LINK", new ElemDesc(0 | ElemDesc.HEADMISC | ElemDesc.EMPTY | ElemDesc.BLOCK));
        m_elementFlags.put(
                "IMG",
                new ElemDesc(
                        0
        ...
        m_elementFlags.put(
                "INPUT",
                new ElemDesc(0 | ElemDesc.FORMCTRL | ElemDesc.INLINELABEL | ElemDesc.EMPTY));
        ...
        m_elementFlags.put(
                "META", new ElemDesc(0 | ElemDesc.HEADMISC | ElemDesc.EMPTY | ElemDesc.BLOCK));
        m_elementFlags.put(
                "STYLE", new ElemDesc(0 | ElemDesc.HEADMISC | ElemDesc.RAW | ElemDesc.BLOCK));
        m_elementFlags.put(
                "SCRIPT",
                new ElemDesc(
                        0
     ...
     *
     * <p>This method can be called multiple times before starting the serialization of a particular
     * result-tree. In principle all serialization parameters can be changed, with the exception of
     * method="html" (it must be method="html" otherwise we shouldn't even have a ToHTMLStream
     * object here!)
     *
     ...
     * @return non-null reference to ElemDesc, which may be m_dummy if no element description
    ...
    /**
     * A Trie that is just a copy of the "static" one. We need this one to be able to use the
     * faster, but not thread-safe method Trie.get2(name)
     */
    private Trie m_htmlInfo = new Trie(m_elementFlags);
    /**
     * Calls to this method could be replaced with calls to getElemDesc(name), but this one should
     * be faster.
     */
    ...
    public void startElement(String namespaceURI, String localName, String name, Attributes atts)
        ...
        try {
            ...
            if (m_doIndent) {
                ...
                else if ((null != elemContext.m_elementName)
    ...
    public final void endElement(
            final String namespaceURI, final String localName, final String name)
            throws org.xml.sax.SAXException {
    ...
    protected void processAttribute(
            java.io.Writer writer, String name, String value, ElemDesc elemDesc)
            throws IOException {
        writer.write(' ');
    ...
    /**
     * Make an integer into an HH hex value. Does no checking on the size of the input, since this
    ...
    /**
     * Write the specified <var>string</var> after substituting non ASCII characters, with <CODE>%HH
     * </CODE>, where HH is the hex of the byte value.
     *
     * @param string String to convert to XML format.
     * @param doURLEscaping True if we should try to encode as per
    ...
    public void writeAttrURI(final java.io.Writer writer, String string, boolean doURLEscaping)
        ...
        for (int i = 0; i < end; i++) {
            ...
            // identifier, we believe that it's OK to double escape quotes.
                ...
                // Dmitri Ilyin: to check if '%' number number is invalid. It must be checked if %xx
                // is a sign, that would be encoded
                // The encoded signes are in Hex form. So %xx my be in form %3C that is "<" sign. I
                // will try to change here a little.
                ...
                else writer.write("&quot;"); // we have to escape this, I guess.
    ...
    /**
     * Writes the specified <var>string</var> after substituting <VAR>specials</VAR>, and UTF-16
     * surrogates for character references <CODE>&amp;#xnn</CODE>.
     *
     * @param string String to convert to XML format.
    ...
    /**
     * This method is used to add an attribute to the currently open element. The caller has
     * guaranted that this attribute is unique, which means that it not been seen before and will
     * not be seen again.
     *
     * @param name the qualified name of the attribute
     * @param value the value of the attribute which can contain only ASCII printable characters
     *     characters in the range 32 to 127 inclusive.
    ...
    public void addUniqueAttribute(String name, String value, int flags) throws SAXException {
        try {
            ...
            if ((flags & NO_BAD_CHARS) > 0 && m_htmlcharInfo.onlyQuotAmpLtGt) {
                ...
                writer.write(value);
                writer.write('"');
            } else if ((flags & HTML_ATTREMPTY) > 0
    ...
    static class Trie {
        /**
         * A digital search trie for 7-bit ASCII text The API is a subset of java.util.Hashtable The
         * key must be a 7-bit ASCII string The value may be any Java Object One can get an object
         * stored in a trie from its key, but the search is either case sensitive or case
         * insensitive to the characters in the key, and this choice of sensitivity or insensitivity
         * is made when the Trie is created, before any objects are put in it.
         *
         * <p>This class is a copy of the one in com.sun.org.apache.xml.internal.utils. It exists to
         * cut the serializers dependancy on that package.
         *
         * @xsl.usage internal
         */
         ...
         *
         * @param lowerCaseOnly true if the search keys are to be loser case only, not case
         *     insensitive.
         */
        ...
        public Object get(final String key) {
             ...
             */
            if (m_charBuffer.length < len) return null;
                ...
                case 1:
                    {
                        ...
                        if (ch < ALPHA_SIZE) {
                            ...
                            if (node != null) return node.m_Value;
                        }
                        return null;
                    }
        ...
        /**
         * Construct the trie from another Trie. Both the existing Trie and this new one share the
         * same table for lookup, and it is assumed that the table is fully populated and not
         * changing anymore.
         *
        ...
        /**
         * Get an object that matches the key. This method is faster than get(), but is not
         * thread-safe.
         *
         * @param key must be a 7-bit ASCII string
         * @return The object that matches the key, or null.
