...
public abstract class ToStream extends SerializerBase {
     ...
     *
     * <p>Used to push/pop values used for the field m_ispreserve, but m_ispreserve is only relevant
     * if m_doIndent is true. If m_doIndent is false this field has no impact.
     */
     ...
     *
     * <p>Used only in shouldIndent() but only if m_doIndent is true. If m_doIndent is false this
     * flag has no impact.
     */
    ...
    public OutputStream getOutputStream() {
        ...
        if (m_writer instanceof WriterToASCI) return ((WriterToASCI) m_writer).getOutputStream();
        else return null;
    }
    ...
    /**
     * Initialize the serializer with the specified writer and output format. Must be called before
     * calling any of the serialize methods. This method can be called multiple times and the
     * xsl:output properties passed in the 'format' parameter are accumulated across calls.
     *
    ...
    private synchronized void init(
            Writer writer, Properties format, boolean defaultProperties, boolean shouldFlush) {

        m_shouldFlush = shouldFlush;

        // if we are tracing events we need to trace what
        // characters are written to the output writer.
        if (m_tracer != null && !(writer instanceof SerializerTraceWriter))
            m_writer = new SerializerTraceWriter(writer, m_tracer);
        else m_writer = writer;
        ...
        boolean shouldNotWriteXMLHeader =
                OutputPropertyUtils.getBooleanProperty(OutputKeys.OMIT_XML_DECLARATION, format);
        ...
        if (format.get(OutputKeys.STANDALONE) != null) {
            ...
            if (defaultProperties) setStandaloneInternal(val);
            else setStandalone(val);
        }

        setMediaType(format.getProperty(OutputKeys.MEDIA_TYPE));

        if (null != doctypePublic) {
            if (doctypePublic.startsWith("-//W3C//DTD XHTML")) m_spaceBeforeClose = true;
        }
    ...
    /**
     * Initialize the serializer with the specified writer and output format. Must be called before
     * calling any of the serialize methods.
     *
     * @param writer The writer to use
     * @param format The output format
     */
    ...
    /**
     * Initialize the serializer with the specified output stream and output format. Must be called
     * before calling any of the serialize methods.
     *
    ...
    protected synchronized void init(
            OutputStream output, Properties format, boolean defaultProperties)
        ...
        } else if (encoding.equals("WINDOWS-1250")
     ...
     *     character represented by the surrogate pair if an entity reference with that value was
     *     written out.
     * @throws IOException
    ...
    protected int writeUTF16Surrogate(char c, char ch[], int i, int end) throws IOException {
        ...
        if (i + 1 >= end) {
            throw new IOException(
                    org.openjdk.com.sun.org.apache.xml.internal.serializer.utils.Utils.messages
                            .createMessage(
    ...
    protected void cdata(char ch[], int start, final int length) throws org.xml.sax.SAXException {
        ...
        try {
            ...
            throw new org.xml.sax.SAXException(
                    org.openjdk.com.sun.org.apache.xml.internal.serializer.utils.Utils.messages
                            .createMessage(MsgKey.ER_OIERROR, null),
                    ioe);
            // "IO error", ioe);
        }
    }
    ...
    private int processDirty(
        ...
        if (CharInfo.S_LINEFEED == ch && fromTextNode) {
            ...
            startClean =
                    accumDefaultEscape(m_writer, (char) ch, i, chars, end, fromTextNode, false);
            i = startClean - 1;
        }
    ...
    protected int accumDefaultEscape(
        ...
        if (i == pos) {
            if (Encodings.isHighUTF16Surrogate(ch)) {
                ...
                if (i + 1 >= len) {
                    throw new IOException(
                            org.openjdk.com.sun.org.apache.xml.internal.serializer.utils.Utils
                                    .messages
                                    .createMessage(
                                            MsgKey.ER_INVALID_UTF16_SURROGATE,
    ...
    protected void closeStartTag() throws SAXException {
        if (m_elemContext.m_startTagOpen) {
             ...
             */
            if (m_cdataSectionElements != null) m_elemContext.m_isCdataSection = isCdataSection();
     ...
     *     <p>Sets the vector of local-name/URI pairs of the cdata section elements specified in the
     *     cdata-section-elements property.
     *     <p>This method is essentially a copy of getQNameProperties() from OutputProperties.
     *     Eventually this method should go away and a call to setCdataSectionElements(Vector v)
