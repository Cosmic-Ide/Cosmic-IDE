...
/**
 * This class writes unicode characters to a byte stream (java.io.OutputStream) as quickly as
 * possible. It buffers the output in an internal buffer which must be flushed to the OutputStream
 * when done. This flushing is done via the close() flush() or flushBuffer() method.
 *
 * <p>This class is only used internally within Xalan.
 *
 * @xsl.usage internal
 */
final class WriterToUTF8Buffered extends Writer implements WriterChain {
    ...
    /**
     * number of bytes that the byte buffer can hold. This is a fixed constant is used rather than
     * m_outputBytes.lenght for performance.
     */
    private static final int BYTES_MAX = 16 * 1024;
    /**
     * number of characters that the character buffer can hold. This is 1/3 of the number of bytes
     * because UTF-8 encoding can expand one unicode character by up to 3 bytes.
     */
     ...
     * <tt>m_outputBytes.length</tt>; elements <tt>m_outputBytes[0]</tt> through
     * <tt>m_outputBytes[count-1]</tt> contain valid byte data.
     */
    private int count;
    ...
    /**
     * Create an buffered UTF-8 writer to write data to the specified underlying output stream with
    ...
    public void write(final char chars[], final int start, final int length)
        ...
        for (; i < n; i++) {
            ...
            /**
             * The following else if condition is added to support XML 1.1 Characters for UTF-8:
             * [1111 0uuu] [10uu zzzz] [10yy yyyy] [10xx xxxx]* Unicode: [1101 10ww] [wwzz zzyy]
             * (high surrogate) [1101 11yy] [yyxx xxxx] (low surrogate) * uuuuu = wwww + 1
             */
