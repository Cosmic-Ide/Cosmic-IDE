...
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.DTMNodeProxy;
...
public class DOMHelper {
    ...
    /**
     * DOM Level 1 did not have a standard mechanism for creating a new Document object. This
     * function provides a DOM-implementation-independent abstraction for that for that concept.
     * It's typically used when outputting a new DOM as the result of an operation.
     *
     * <p>TODO: This isn't directly compatable with DOM Level 2. The Level 2 createDocument call
     * also creates the root element, and thus requires that you know what that element will be
     * before creating the Document. We should think about whether we want to change this code, and
     * the callers, so we can use the DOM's own method. (It's also possible that DOM Level 3 may
     * relax this sequence, but you may give up some intelligence in the DOM by doing so; the intent
     * was that knowing the document type and root element might let the DOM automatically switch to
     * a specialized subclass for particular kinds of documents.)
     *
     * @param isSecureProcessing state of the secure processing feature.
     * @return The newly created DOM Document object, with no children, or null if we can't find a
     *     DOM implementation that permits creating new empty Documents.
     */
        ...
        try {
            ...
            throw new RuntimeException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_CREATEDOCUMENT_NOT_SUPPORTED,
     ...
     * xsl:strip-space, xsl:preserve-space, and the xml:space attribute, whether or not extra
     * whitespace should be stripped from the node. Literal elements from template elements should
    ...
    /**
     * Supports the XPath function GenerateID by returning a unique identifier string for any given
     * DOM Node.
     *
     * <p>Warning: The base implementation uses the Node object's hashCode(), which is NOT
     * guaranteed to be unique. If that method hasn't been overridden in this DOM ipmlementation,
     * most Java implementions will derive it from the object's address and should be OK... but if
     * your DOM uses a different definition of hashCode (eg hashing the contents of the subtree), or
     * if your DOM may have multiple objects that represent a single Node in the data structure (eg
     * via proxying), you may need to find another way to assign a unique identifier.
     *
     * <p>Also, be aware that if nodes are destroyed and recreated, there is an open issue regarding
     * whether an ID may be reused. Currently we're assuming that the input document is stable for
     * the duration of the XPath/XSLT operation, so this shouldn't arise in this context.
     *
     * <p>(DOM Level 3 is investigating providing a unique node "key", but that won't help Level 1
     * and Level 2 implementations.)
     *
    ...
    /**
     * Given an XML Namespace prefix and a context in which the prefix is to be evaluated, return
     * the Namespace Name this prefix was bound to. Note that DOM Level 3 is expected to provide a
     * version of this which deals with the DOM's "early binding" behavior.
     *
     * <p>Default handling:
     *
     * @param prefix String containing namespace prefix to be resolved, without the ':' which
     *     separates it from the localname when used in a Node Name. The empty sting signifies the
     *     default namespace at this point in the document.
     * @param namespaceContext Element which provides context for resolution. (We could extend this
     *     to work for other nodes by first seeking their nearest Element ancestor.)
     * @return a String containing the Namespace URI which this prefix represents in the specified
     *     context.
     */
    ...
    /**
     * Object to put into the m_NSInfos table that tells that a node has not been processed, but has
     * xmlns namespace decls.
     */
    protected static final NSInfo m_NSInfoUnProcWithXMLNS = new NSInfo(false, true);

    /**
     * Object to put into the m_NSInfos table that tells that a node has not been processed, but has
     * no xmlns namespace decls.
     */
    protected static final NSInfo m_NSInfoUnProcWithoutXMLNS = new NSInfo(false, false);

    /**
     * Object to put into the m_NSInfos table that tells that a node has not been processed, and has
     * no xmlns namespace decls, and has no ancestor decls.
     */
    protected static final NSInfo m_NSInfoUnProcNoAncestorXMLNS =
            new NSInfo(false, false, NSInfo.ANCESTORNOXMLNS);

    /**
     * Object to put into the m_NSInfos table that tells that a node has been processed, and has
     * xmlns namespace decls.
     */
    protected static final NSInfo m_NSInfoNullWithXMLNS = new NSInfo(true, true);

    /**
     * Object to put into the m_NSInfos table that tells that a node has been processed, and has no
     * xmlns namespace decls.
     */
    ...
    /**
     * Object to put into the m_NSInfos table that tells that a node has been processed, and has no
     * xmlns namespace decls. and has no ancestor decls.
     */
    protected static final NSInfo m_NSInfoNullNoAncestorXMLNS =
            new NSInfo(true, false, NSInfo.ANCESTORNOXMLNS);

    /**
     * Vector of node (odd indexes) and NSInfos (even indexes) that tell if the given node is a
     * candidate for ancestor namespace processing.
     */
    protected Vector m_candidateNoAncestorXMLNS = new Vector();

    /**
     * Returns the namespace of the given node. Differs from simply getting the node's prefix and
     * using getNamespaceForPrefix in that it attempts to cache some of the data in NSINFO objects,
     * to avoid repeated lookup. TODO: Should we consider moving that logic into
     * getNamespaceForPrefix?
     *
     * @param n Node to be examined.
     * @return String containing the Namespace Name (uri) for this node. Note that this is undefined
     *     for any nodes other than Elements and Attributes.
     */
                    ...
                    if ((false == ancestorsHaveXMLNS) && (null == parent)) {
                        for (int i = 0; i < nCandidates; i += 2) {
                            ...
                            if (candidateInfo == m_NSInfoUnProcWithoutXMLNS) {
                                m_NSInfos.put(
                                        m_candidateNoAncestorXMLNS.elementAt(i),
            ...
            if (Node.ATTRIBUTE_NODE != ntype) {
                if (null == namespaceOfPrefix) {
                    if (ancestorsHaveXMLNS) {
                        if (nHasXMLNS) m_NSInfos.put(n, m_NSInfoNullWithXMLNS);
                        else m_NSInfos.put(n, m_NSInfoNullWithoutXMLNS);
                    } else {
    ...
    /**
     * Returns the element name with the namespace prefix (if any) replaced by the Namespace URI it
     * was bound to. This is not a standard representation of a node name, but it allows convenient
     * single-string comparison of the "universal" names of two nodes.
     *
     * @param elem Element to be examined.
     * @return String in the form "namespaceURI:localname" if the node belongs to a namespace, or
     *     simply "localname" if it doesn't.
    ...
    /**
     * Returns the attribute name with the namespace prefix (if any) replaced by the Namespace URI
     * it was bound to. This is not a standard representation of a node name, but it allows
     * convenient single-string comparison of the "universal" names of two nodes.
     *
    ...
    /**
     * Tell if the node is ignorable whitespace. Note that this can be determined only in the
     * context of a DTD or other Schema, and that DOM Level 2 has nostandardized DOM API which can
     * return that information.
     *
     * @deprecated
     * @param node Node to be examined
     * @return CURRENTLY HARDCODED TO FALSE, but should return true if and only if the node is of
     *     type Text, contains only whitespace, and does not appear as part of the #PCDATA content
     *     of an element. (Note that determining this last may require allowing for Entity
     *     References.)
     */
    ...
    /**
     * Get the root node of the document tree, regardless of whether or not the node passed in is a
     * document node.
     *
     * <p>TODO: This doesn't handle DocumentFragments or "orphaned" subtrees -- it's currently
     * returning ownerDocument even when the tree is not actually part of the main Document tree. We
     * should either rewrite the description to say that it finds the Document node, or change the
     * code to walk up the ancestor chain.
     *
     * @param n Node to be examined
     * @return the Document node. Note that this is not the correct answer if n was (or was a child
     *     of) a DocumentFragment or an orphaned node, as can arise if the DOM has been edited
     *     rather than being generated by a parser.
     */
    ...
    /**
     * Test whether the given node is a namespace decl node. In DOM Level 2 this can be done in a
     * namespace-aware manner, but in Level 1 DOMs it has to be done by testing the node name.
     *
     * @param n Node to be examined.
     * @return boolean -- true iff the node is an Attr whose name is "xmlns" or has the "xmlns:"
     *     prefix.
     */
    ...
    /**
     * Obtain the XPath-model parent of a DOM node -- ownerElement for Attrs, parent for other
     * nodes.
     *
     * <p>Background: The DOM believes that you must be your Parent's Child, and thus Attrs don't
     * have parents. XPath said that Attrs do have their owning Element as their parent. This
     * function bridges the difference, either by using the DOM Level 2 ownerElement function or by
     * using a "silly and expensive function" in Level 1 DOMs.
     *
     * <p>(There's some discussion of future DOMs generalizing ownerElement into ownerNode and
     * making it work on all types of nodes. This still wouldn't help the users of Level 1 or Level
     * 2 DOMs)
     *
     * <p>
     *
     ...
     *     is an orphan.
     * @throws RuntimeException if the Document has no root element. This can't arise if the
     *     Document was created via the DOM Level 2 factory methods, but is possible if other
    ...
    /**
     * Given an ID, return the element. This can work only if the document is interpreted in the
     * context of a DTD or Schema, since otherwise we don't know which attributes are or aren't IDs.
     *
     * <p>Note that DOM Level 1 had no ability to retrieve this information. DOM Level 2 introduced
     * it but does not promise that it will be supported in all DOMs; those which can't support it
     * will always return null.
     *
     * <p>TODO: getElementByID is currently unimplemented. Support DOM Level 2?
     *
     * @param id The unique identifier to be searched for.
     * @param doc The document to search within.
     * @return CURRENTLY HARDCODED TO NULL, but it should be: The node which has this unique
     *     identifier, or null if there is no such node or this DOM can't reliably recognize it.
     */
     ...
     * should be done here; currently that's pushed up to a higher levelof our application. (Note
     ...
     * seen and could recover the info from that source. Obviously that doesn't work very well if
     * the only information you've been presented with is the Attr. The DOM Level 2
     * getOwnerElement() method fixes that, but only for Level 2 and later DOMs.
     *
     * @param elem Element whose subtree is to be searched for this Attr
     * @param attr Attr whose owner is to be located.
     * @return the first Element whose attribute list includes the provided attr. In modern DOMs,
     *     this will also be the only such Element. (Early DOMs had some hope that Attrs might be
     *     sharable, but this idea has been abandoned.)
     */
    ...
    /**
     * Store the factory object required to create DOM nodes in the result tree. In fact, that's
     * just the result tree's Document node...
     *
    ...
    /**
     * Get the textual contents of the node. See getNodeData(Node,FastStringBuffer) for discussion
     * of how whitespace nodes are handled.
     *
     * @param node DOM Node to be examined
     * @return String containing a concatenation of all the textual content within that node.
    ...
    /**
     * Retrieve the text content of a DOM subtree, appending it into a user-supplied
     * FastStringBuffer object. Note that attributes are not considered part of the content of an
     * element.
     *
     * <p>There are open questions regarding whitespace stripping. Currently we make no special
     * effort in that regard, since the standard DOM doesn't yet provide DTD-based information to
     * distinguish whitespace-in-element-context from genuine #PCDATA. Note that we should probably
     * also consider xml:space if/when we address this. DOM Level 3 may solve the problem for us.
     *
