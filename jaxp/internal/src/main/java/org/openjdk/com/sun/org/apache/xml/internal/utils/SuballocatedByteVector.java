...
/**
 * A very simple table that stores a list of byte. Very similar API to our IntVector class (same
 ...
 *
 * <p>If an element has not been set (because we skipped it), its value will initially be 0.
 * Shortening the vector does not clear old storage; if you then skip values and setElementAt a
 * higher index again, you may see old data reappear in the truncated-and-restored section. Doing
 * anything else would have performance costs.
 *
 * @xsl.usage internal
 */
public class SuballocatedByteVector {
    ...
    /**
     * Append several slots onto the vector, but do not set the values. Note: "Not Set" means the
    ...
    private void addElements(int numberOfElements) {
        ...
        if (newlen > m_blocksize) {
            ...
            for (int i = index + 1; i <= newindex; ++i) m_map[i] = new byte[m_blocksize];
        }
        m_firstFree = newlen;
    }
     ...
     *
     * <p>Insertion may be an EXPENSIVE operation!
     *
     ...
     * @return value at given index. If that value wasn't previously set, the result is undefined
     *     for performance reasons. It may throw an exception (see below), may return zero, or (if
     *     setSize has previously been used) may return stale data.
     ...
     *     the highest index used) but has never had anything set into it. %REVIEW% Could add a
     *     catch to create the block in that case, or return 0. Try/Catch is _supposed_ to be nearly
     *     free when not thrown to. Do we believe that? Should we have a separate safeElementAt?
     */
        ...
        for (int index = m_firstFree / m_blocksize; index >= 0; --index) {
            ...
            if (block != null)
                for (int offset = boffset; offset >= 0; --offset)
                    if (block[offset] == elem) return offset + index * m_blocksize;
            boffset = 0; // after first
        }
        return -1;
    }
