...
public class TreeWalker {
     ...
     * @param systemId System identifier for the document. contentHandler operation (toXMLString,
     *     digest, ...)
     */
        ...
        if (systemId != null) m_locator.setSystemId(systemId);
            ...
            try {
                ...
                m_locator.setSystemId(
                        SecuritySupport.getSystemProperty("user.dir")
                                + File.separator
                                + "dummy.xsl");
     ...
     * Perform a pre-order traversal non-recursive style.
     *
     * <p>In contrast to the traverse() method this method will not issue startDocument() and
     * endDocument() events to the SAX listener.
     *
     * @param pos Node in the tree where to start traversal
     * @throws TransformerException
     */
        ...
        switch (node.getNodeType()) {
            ...
            case Node.ELEMENT_NODE:
                ...
                this.m_contentHandler.endElement(
                        ns, m_dh.getLocalNameOfNode(node), node.getNodeName());
