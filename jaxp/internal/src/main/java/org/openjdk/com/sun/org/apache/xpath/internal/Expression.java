 ...
 * to return a {@link XObject}, normally has a location within a document or DOM, can send error and
 * warning events, and normally do not hold state and are meant to be immutable once construction
 * has completed. An exception to the immutibility rule is iterators and walkers, which must be
 * cloned in order to be used -- the original must still be immutable.
 */
public abstract class Expression implements java.io.Serializable, ExpressionNode, XPathVisitable {
    ...
    /**
     * This function is used to fixup variables from QNames to stack frame indexes at stylesheet
     * build time.
     *
     * @param vars List of QNames that correspond to variables. This list should be searched
     *     backwards for the first qualified name that corresponds to the variable reference qname.
     *     The position of the QName in the vector from the start of the vector will be its position
     *     in the stack frame (but variables above the globalsTop value will need to be offset to
     *     the current stack frame). NEEDSDOC @param globalsSize
     */
    ...
    /**
     * This is a utility method to tell if the passed in class is the same class as this. It is to
     * be used by the deepEquals method. I'm bottlenecking it here because I'm not totally confident
     * that comparing the class objects is the best way to do this.
     *
     ...
     * external parsed entity where the markup that triggered the event appears.
     *
