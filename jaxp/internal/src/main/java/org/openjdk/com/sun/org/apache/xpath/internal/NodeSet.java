...
import org.openjdk.com.sun.org.apache.xml.internal.utils.DOM2Helper;
import org.openjdk.com.sun.org.apache.xpath.internal.axes.ContextNodeList;
...
public class NodeSet implements NodeList, NodeIterator, Cloneable, ContextNodeList {
     ...
     * of constants is defined in the <code>NodeFilter</code> interface. For NodeSets, the mask has
     * been hardcoded to show all nodes except EntityReference nodes, which have no equivalent in
     ...
     * restructure) the NodeIterator's view of the document. In our case, we will be using hardcoded
     * filters built into our iterators... but getFilter() is part of the DOM's NodeIterator
     ...
     *     nodes may be returned (though whatToShow says that the EntityReferences themselves are
     *     not shown.)
     */
     ...
     * @throws DOMException INVALID_STATE_ERR: Raised if this method is called after the <code>
     *     detach</code> method was invoked.
     * @throws RuntimeException thrown if this NodeSet is not of a cached type, and hence doesn't
     ...
     * references will be copied; nulls appearing in the source NodeSet will not be added to this
     * one.
     *
     ...
     * @throws RuntimeException thrown if this NodeSet is not of a mutable type.
     */
    private boolean addNodesInDocOrder(
            int start, int end, int testIndex, NodeList nodelist, XPathContext support) {
    ...
    public int addNodeInDocOrder(Node node, boolean test, XPathContext support) {
        ...
        if (!m_mutable)
            throw new RuntimeException(
                    XSLMessages.createXPATHMessage(
                            XPATHErrorResources.ER_NODESET_NOT_MUTABLE,
                            null)); // XSLMessages.createXPATHMessage(XPATHErrorResources.ER_NODESET_NOT_MUTABLE, null)); //"This NodeSet is not mutable!");
