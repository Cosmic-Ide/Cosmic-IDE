...
public class NodeSetDTM extends NodeVector
    ...
    /**
     * Set the environment in which this iterator operates, which should provide: a node (the
     * context node... same value as "root" defined below) a pair of non-zero positive integers (the
     * context position and the context size) a set of variable bindings a function library the set
     * of namespace declarations in scope for the expression.
     *
     * <p>At this time the exact implementation of this environment is application dependent.
     * Probably a proper interface will be created fairly soon.
     *
     ...
     *
     * @return a new NodeSetDTM of the same type, having the same state... though unless overridden
     *     in the subclasses, it may not copy all the state information.
     * @throws CloneNotSupportedException if this subclass of NodeSetDTM does not support the
     *     clone() operation.
     */
     ...
     * of constants is defined in the <code>DTMFilter</code> interface. For NodeSetDTMs, the mask
     * has been hardcoded to show all nodes except EntityReference nodes, which have no equivalent
     * in the XPath data model.
     *
     * @return integer used as a bit-array, containing flags defined in the DOM's DTMFilter class.
     *     The value will be <code>SHOW_ALL & ~SHOW_ENTITY_REFERENCE</code>, meaning that only
    ...
    /**
     * The filter object used to screen nodes. Filters are applied to further reduce (and
     * restructure) the DTMIterator's view of the document. In our case, we will be using hardcoded
     * filters built into our iterators... but getFilter() is part of the DOM's DTMIterator
     * interface, so we have to support it.
     *
     ...
     *     returning a placeholder object just to indicate that this is not going to return all
     *     nodes selected by whatToShow.
     */
     ...
     * to the iterator. If false, they will be skipped over. <br>
     ...
     *
     * @return true for all iterators based on NodeSetDTM, meaning that the contents of
     *     EntityRefrence nodes may be returned (though whatToShow says that the EntityReferences
    ...
    /**
     * Get an instance of a DTM that "owns" a node handle. Since a node iterator may be passed
     * without a DTMManager, this allows the caller to easily get the DTM using just the iterator.
     *
    ...
    /**
     * Returns the next node in the set and advances the position of the iterator in the set. After
     * a DTMIterator is created, the first call to nextNode() returns the first node in the set.
     *
     * @return The next <code>Node</code> in the set being iterated over, or <code>DTM.NULL</code>
     ...
     * By default this sets m_next to the index. If the index argument is -1, this signals that the
     ...
     * </code> inclusive. Note that this operation requires finding all the matching nodes, which
     * may defeat attempts to defer that work.
     *
    ...
    /**
     * Inserts the specified node in this vector at the specified index. Each component in this
     ...
     * default. This switch must be set before the first call to nextNode is made, to ensure that
