...
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.DTMNodeIterator;
import org.openjdk.com.sun.org.apache.xml.internal.dtm.ref.sax2dtm.SAX2RTFDTM;
...
import org.openjdk.com.sun.org.apache.xml.internal.utils.NodeVector;
import org.openjdk.com.sun.org.apache.xml.internal.utils.ObjectStack;
...
import org.openjdk.com.sun.org.apache.xpath.internal.axes.OneStepIteratorForward;
import org.openjdk.com.sun.org.apache.xpath.internal.axes.SubContextList;
import org.openjdk.com.sun.org.apache.xpath.internal.objects.DTMXRTreeFrag;
import org.openjdk.com.sun.org.apache.xpath.internal.objects.XMLStringFactoryImpl;
...
public class XPathContext extends DTMManager // implements ExpressionContext
{
     ...
     *
     * <p>%REVIEW% I'm using a Vector rather than Stack so we can reuse the DTMs if the problem
     * occurs multiple times. I'm not sure that's really a net win versus discarding the DTM and
     * starting a new one... but the retained RTF DTM will have been tail-pruned so should be small.
     */
    ...
    /**
     * Though XPathContext context extends the DTMManager, it really is a proxy for this object,
     * which is the real DTMManager.
     */
    ...
    /**
     * Get an instance of a DTM, loaded with the content from the specified source. If the unique
     * flag is true, a new instance will always be returned. Otherwise it is up to the DTMManager to
     * return a new instance or an instance that it already created and may be being used by someone
     * else. (I think more parameters will need to be added for error handling, and entity
     * resolution).
     *
    ...
    //
    /**
     * Release a DTM either to a lru pool, or completely remove reference. DTMs without system IDs
     * are always hard deleted. State: experimental.
     *
    ...
    //
    /**
     * Create a new <code>DTMIterator</code> based only on a whatToShow and a DTMFilter. The
     ...
     *     tree presented by the iterator. See the description of <code>NodeFilter</code> for the
     *     set of possible <code>SHOW_</code> values.These flags can be combined using <code>OR
     *     </code>.
    ...
    public DTMIterator createDTMIterator(int node) {
        ...
        DTMIterator iter = new OneStepIteratorForward(Axis.SELF);
    ...
    /**
     * *This constructor doesn't seem to be used anywhere -- huizhe wang** Create an XPathContext
     * instance.
     *
    ...
    /**
     * The owner context of this XPathContext. In the case of XSLT, this will be a Transformer
     * object.
     */
    ...
    /**
     * Get the "owner" context of this context, which should be, in the case of XSLT, the
     * Transformer object. This is needed so that XSLT functions can get the Transformer.
     *
    ...
    /**
     * A default ErrorListener in case our m_errorListener was not specified and our owner either
     * does not have an ErrorListener or has a null one.
     */
    ...
    public final ErrorListener getErrorListener() {
        ...
        if (null == retval) {
            if (null == m_defaultErrorListener) m_defaultErrorListener = new DefaultErrorHandler();
    ...
    /**
     * The TrAX URI Resolver for resolving URIs from the document(...) function to source tree
     * nodes.
     */
    ...
    /** The amount to use for stacks that record information during the recursive execution. */
     ...
     * Not to be confused with the current node list. %REVIEW% Note that there are no bounds check
    ...
    public SubContextList getCurrentNodeList() {
        return m_axesIteratorStack.isEmpty()
                ? null
                : (SubContextList) m_axesIteratorStack.elementAt(0);
    }
    // ==========================================================
    ...
    public class XPathExpressionContext implements ExpressionContext {
        /**
         * Return the XPathContext associated with this XPathExpressionContext. Extensions should
         * use this judiciously and only when special processing requirements cannot be met another
         * way. Consider requesting an enhancement to the ExpressionContext interface to avoid
        ...
        /**
         * Return the DTMManager object. Though XPathContext context extends the DTMManager, it
         * really is a proxy for the real DTMManager. If a caller needs to make a lot of calls to
         * the DTMManager, it is faster if it gets the real one from this function.
         */
        public DTMManager getDTMManager() {
            return m_dtmManager;
        }
     ...
     *
     * <p>Note: The distinction between this method and getRTFDTM() is that the latter allocates
     * space from the dynamic variable stack (m_rtfdtm_stack), which may be pruned away again as the
     * templates which defined those variables are exited. Global variables may be bound late (see
     * XUnresolvedVariable), and never want to be discarded, hence we need to allocate them
     * separately and don't actually need a stack to track them.
     *
     * @return a non-null DTM reference.
     */
    ...
    /**
     * Get a DTM to be used as a container for a dynamic Result Tree Fragment. This will always be
     * an instance of (derived from? equivalent to?) SAX2DTM, since each RTF is constructed by
     * temporarily redirecting our SAX output to it. It may be a single DTM containing for multiple
     * fragments, if the implementation supports that.
     *
     * @return a non-null DTM reference.
     ...
     * one of the weird produce-an-RTF-to-build-an-RTF cases arose. Basically: If pop says this DTM
     * is now empty, then return to the previous if one exists, in whatever state we left it in.
     * UGLY, but hopefully the situation which forces us to consider this will arise exceedingly
     * rarely.
     */
    ...
    /**
     * Gets DTMXRTreeFrag object if one has already been created. Creates new DTMXRTreeFrag object
     * and adds to m_DTMXRTreeFrags HashMap, otherwise.
     *
     * @param dtmIdentity
