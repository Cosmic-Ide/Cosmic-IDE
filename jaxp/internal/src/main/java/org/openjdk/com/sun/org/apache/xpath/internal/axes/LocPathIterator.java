...
public abstract class LocPathIterator extends PredicatedNodeTest
    ...
    /**
     * Execute this iterator, meaning create a clone that can store state, and initialize it for
     * fast execution from the current runtime state. When this is called, no actual query from the
     * current context node is performed.
     *
     ...
     * @throws org.openjdk.javax.xml.transform.TransformerException if a runtime exception occurs.
     * @throws org.xml.sax.SAXException
     */
    public void executeCharsToContentHandler(XPathContext xctxt, org.xml.sax.ContentHandler handler)
            throws org.openjdk.javax.xml.transform.TransformerException, org.xml.sax.SAXException {
    ...
    /**
     * Given an select expression and a context, evaluate the XPath and return the resulting
     * iterator.
     *
     ...
     * of constants is defined in the <code>NodeFilter</code> interface.
     *
     * <p>This is somewhat useless at this time, since it doesn't really return information that
     * tells what this iterator will show. It is here only to fullfill the DOM NodeIterator
     * interface.
     *
     ...
     * a NodeIterator is created, the first call to nextNode() returns the first node in the set.
     *
     ...
     *
     * @return XPath execution context this iterator is operating on, or null if setRoot has not
     *     been called.
     */
    ...
    /**
     * The node context from where the expression is being executed from (i.e. for current()
     * support).
     *
    ...
    /**
     * The pool for cloned iterators. Iterators need to be cloned because the hold running state,
     * and thus the original iterator expression from the stylesheet pool can not be used.
     */
    ...
    /**
     * The node context from where the expression is being executed from (i.e. for current()
     * support). Different from m_context in that this is the context for the entire expression,
     * rather than the context for the subexpression.
     */
