...
public class NodeSequence extends XObject implements DTMIterator, Cloneable, PathComponent {
    ...
    /**
     * If this NodeSequence has a cache, mark that it is complete. This method should be called
    ...
    /**
     * The DTMManager to use if we're using a NodeVector only. We may well want to do away with
     * this, and store it in the NodeVector.
     */
    ...
    private NodeSequence(
            DTMIterator iter, int context, XPathContext xctxt, boolean shouldCacheNodes) {
        setIter(iter);
    ...
    /**
     * It used to be that many locations in the code simply did an assignment to this.m_obj
     * directly, rather than calling the setObject(Object) method. The problem is that our
     * super-class would be updated on what the cache associated with this NodeSequence, but we
     * wouldn't know ourselves.
     *
     ...
     * obtains nodes from it. As those nodes are obtained they may be cached, making the next
     * walking of a copy or clone of the iterator faster. This field (m_cache) is a reference to
     * such a cache, which is populated as the iterator is walked.
     *
     * <p>Note that multiple NodeSequence objects may hold a reference to the same cache, and also
     * (and this is important) the same iterator. The iterator and its cache may be shared among
     ...
     * iterator itself, so the iterator is only ever walked once and everyone benefits from the
     * cache.
     *
     * <p>At some point the cache may be complete due to walking to the end of one of the copies of
     * the iterator, and the cache is then marked as "complete". and the cache will have no more
     * nodes added to it.
     *
    ...
    private static final class IteratorCache {
         ...
         * objects.
         *
         * <p>For example, consider three NodeSequence objects ns1, ns2 and ns3 doing such sharing,
         * and the nodes to be obtaind from the iterator being the sequence { 33, 11, 44, 22, 55 }.
         *
         * <p>If ns3.nextNode() is called 3 times the the underlying iterator will have walked
         * through 33, 11, 55 and these three nodes will have been put in the cache.
         *
         * <p>If ns2.nextNode() is called 2 times it will return 33 and 11 from the cache, leaving
         * the iterator alone.
         *
         * <p>If ns1.nextNode() is called 6 times it will return 33 and 11 from the cache, then get
         * 44, 22, 55 from the iterator, and appending 44, 22, 55 to the cache. On the sixth call it
         * is found that the iterator is exhausted and the cache is marked complete.
         *
         * <p>Should ns2 or ns3 have nextNode() called they will know that the cache is complete,
         * and they will obtain all subsequent nodes from the cache.
         *
        ...
        /**
         * true if the associated iterator is exhausted and all nodes obtained from it are in the
         * cache.
         */
        ...
        /**
         * This method is called when yet another NodeSequence object uses, or shares this same
         * cache.
         */
        ...
        /**
         * Sets the NodeVector that holds the growing list of nodes as they are appended to the
         * cached list.
         */
        ...
        /**
         * Call this method with 'true' if the iterator is exhausted and the cached list is
         * complete, or no longer growing.
         */
    ...
    /**
     * Get the cached list of nodes appended with values obtained from the iterator as a
     * NodeSequence is walked when its nextNode() method is called.
     */
