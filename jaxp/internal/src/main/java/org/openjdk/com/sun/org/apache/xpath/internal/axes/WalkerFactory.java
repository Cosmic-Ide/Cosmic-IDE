...
import org.openjdk.com.sun.org.apache.xpath.internal.compiler.FunctionTable;
import org.openjdk.com.sun.org.apache.xpath.internal.compiler.OpCodes;
import org.openjdk.com.sun.org.apache.xpath.internal.compiler.OpMap;
import org.openjdk.com.sun.org.apache.xpath.internal.objects.XNumber;
import org.openjdk.com.sun.org.apache.xpath.internal.patterns.ContextMatchStepPattern;
...
public class WalkerFactory {
    ...
    /**
     * Analyze the location path and return 32 bits that give information about the location path as
     * a whole. See the BIT_XXX constants for meaning about each of the bits.
     *
     * @param compiler non-null reference to compiler object that has processed the XPath operations
     *     into an opcode map.
     * @param stepOpCodePos The opcode position for the step.
     ...
     * match pattern. What this means is that the LocationPath is read backwards, as a test on a
     * given node, to see if it matches the criteria of the selection, and ends up at the context
     * node. Essentially, this is a backwards query from a given node, to find the context node.
     *
     * <p>So, the selection "foo/daz[2]" is, in non-abreviated expanded syntax,
     * "self::node()/following-sibling::foo/child::daz[position()=2]". Taking this as a match
     * pattern for a probable node, it works out to "self::daz/parent::foo[child::daz[position()=2
     * and isPrevStepNode()] precedingSibling::node()[isContextNodeOfLocationPath()]", adding magic
     * isPrevStepNode and isContextNodeOfLocationPath operations. Predicates in the location path
     * have to be executed by the following step, because they have to know the context of their
     * execution.
     *
        ...
        while (OpCodes.ENDOP != (stepType = compiler.getOp(stepOpCodePos))) {
            step =
                    createDefaultStepPattern(
                            compiler, stepOpCodePos, mpi, analysis, firstStep, prevStep);
    ...
    /**
     * Analyze a step and give information about it's predicates. Right now this just returns true
     * or false if the step has a predicate.
     *
     * @param compiler non-null reference to compiler object that has processed the XPath operations
     *     into an opcode map.
            ...
            if ((0
                            == (whatToShow
                                    & (DTMFilter.SHOW_ATTRIBUTE
                                            ...
                                            | DTMFilter.SHOW_ELEMENT
                                            | DTMFilter.SHOW_PROCESSING_INSTRUCTION)))
                    || (whatToShow == DTMFilter.SHOW_ALL)) ai.initNodeTest(whatToShow);
            else {
                ai.initNodeTest(
                        whatToShow, compiler.getStepNS(opPos), compiler.getStepLocalName(opPos));
            }
        }
    ...
    /**
     * Tell if the pattern can be 'walked' with the iteration steps in natural document order,
     * without duplicates.
     *
    ...
    public static boolean isNaturalDocOrder(int analysis) {
        if (canCrissCross(analysis)
                ...
                || walksFilteredList(analysis)) return false;
    ...
    /**
     * Bit is on if any of the walkers contain an node() test. This is really only useful if the
     * count is 1.
     */
