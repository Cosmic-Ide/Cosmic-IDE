...
class Lexer {
    ...
    private int mapNSTokens(String pat, int startSubstring, int posOfNSSep, int posOfScan)
        ...
        if ((null != uName) && (uName.length() > 0)) {
            ...
            m_processor.errorForDOM3(
                    XPATHErrorResources.ER_PREFIX_MUST_RESOLVE,
                    new String[] {prefix}); // "Prefix must resolve to a namespace: {0}";

            /**
             * old code commented out 17-Sep-2004 // error("Could not locate namespace for prefix:
             * "+prefix); // m_processor.error(XPATHErrorResources.ER_PREFIX_MUST_RESOLVE, // new
