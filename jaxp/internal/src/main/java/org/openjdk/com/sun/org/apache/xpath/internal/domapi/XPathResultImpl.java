...
/**
 * The class provides an implementation XPathResult according to the DOM L3 XPath Specification,
 ...
 *
 * <p>The <code>XPathResult</code> interface represents the result of the evaluation of an XPath
...
class XPathResultImpl implements XPathResult, EventListener {
    ...
    /**
     * This the type specified by the user during construction. Typically the constructor will be
     * called by com.sun.org.apache.xpath.internal.XPath.evaluate().
     */
    ...
    XPathResultImpl(short type, XObject result, Node contextNode, XPath xpath) {
        ...
        if (null == result) {
            ...
            throw new XPathException(
                    XPathException.INVALID_EXPRESSION_ERR, fmsg); // Empty XPath result object
        }
        ...
        if ((m_resultType != UNORDERED_NODE_SNAPSHOT_TYPE)
            ...
            String fmsg =
                    XPATHMessages.createXPATHMessage(
                            ...
                            new Object[] {m_xpath.getPatternString(), getTypeString(m_resultType)});
            throw new XPathException(XPathException.TYPE_ERR, fmsg);
            //                              "The method getSnapshotLength cannot be called on the
            // XPathResult of XPath expression {0} because its XPathResultType is {1}.
        }
    ...
    public Node iterateNext() throws XPathException, DOMException {
        if ((m_resultType != UNORDERED_NODE_ITERATOR_TYPE)
            ...
            // expression {0} because its XPathResultType is {1}.
            //                This method applies only to types UNORDERED_NODE_ITERATOR_TYPE and
        ...
        if (getInvalidIteratorState()) {
            ...
            throw new DOMException(
                    ...
                    fmsg); // Document mutated since result was returned. Iterator is invalid.
        }

        Node node = m_iterator.nextNode();
        if (null == node) removeEventListener(); // JIRA 1673
        // Wrap "namespace node" in an XPathNamespace
        if (isNamespaceNode(node)) {
            return new XPathNamespaceImpl(node);
        } else {
    ...
    public Node snapshotItem(int index) throws XPathException {
        ...
        if ((m_resultType != UNORDERED_NODE_SNAPSHOT_TYPE)
            ...
            // expression {0} because its XPathResultType is {1}.
            //              This method applies only to types UNORDERED_NODE_SNAPSHOT_TYPE and
    ...
    private short getTypeFromXObject(XObject object) {
        switch (object.getType()) {
            ...
            case XObject.CLASS_STRING:
                ...
                //          case XObject.CLASS_YMDURATION: return STRING_TYPE; // treat all date
                // types as strings?
