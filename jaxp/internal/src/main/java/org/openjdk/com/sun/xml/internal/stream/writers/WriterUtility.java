...
public class WriterUtility {
    ...
    /**
     * XXX: This feature is only used when writing element content values. default value is 'true'
     * however, if the feature is set to false characters wont be escaped. This feature has no
     * effect when writing Attribute values, character would still be escaped. I can't think of any
     * reason why this would be useful when writing attribute values. However, this can be
     * reconsidered if there is any usecase.
     */
    ...
    private void writeXMLContent(char[] content, int start, int length, boolean escapeCharacter)
        ...
        // define startWritePos to track the position from where the character array data needs to
        // be written
        ...
        for (index = start; index < end; index++) {
            ...
            switch (ch) {
                case '<':
                    {
                        if (escapeCharacter) {
                            // this character needs to be escaped, write the data from the last
                            // write pos
                            fWriter.write(content, startWritePos, index - startWritePos);
                            fWriter.write("&lt;");
                            if (DEBUG_XML_CONTENT) {
                                System.out.print(
                                        new String(content, startWritePos, index - startWritePos));
                                System.out.println("&lt;");
                            }
                            // increase the startWritePos by 1 indicating that next write should
                            // start from
                            // one position ahead
                            startWritePos = index + 1;
                        }
        ...
        if (DEBUG_XML_CONTENT) {
            System.out.println(
                    "out of the loop, writing "
                            + new String(content, startWritePos, end - startWritePos));
        }
