...
/**
 * XMLWriter <code>XMLWriter</code> is not thread safe.
 *
 * <p>For efficiency this writer buffers the input. Use <code>flush()</code> function to explicitly
 * write the data to underlying stream.
 *
 * <p>This writer is designed in such a way that it atleast buffers the input to the <code>size
 * </code> specified. Unless <code>flush</code> is called, it guarantees that data in chunks of size
 * equal to or more than <code>size</code> specified will be written. <code>XMLWriter</code>
 * instance can be reused. <code>setWriter()</code> internally clears the buffer and stores the
 * reference to newly supplied <code>Writer</code> instance.
 *
...
public class XMLWriter extends Writer {
     ...
     *
     * <p>atleast buffers the input to the <code>size</code> specified.
     */
            ...
            if (DEBUG) {
                System.out.println(
                        "internal buffer length "
                                ...
                                + size);
                System.out.println(
                        "Data: ('" + new String(buffer.ch, buffer.offset, buffer.length) + "')");
            }
            writeBufferedData();
        }
    }
    ...
    /**
     * Write the data present in the buffer to the writer. buffer is cleared after write operation.
     */
