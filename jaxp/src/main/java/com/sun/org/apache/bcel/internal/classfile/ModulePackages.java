...
/**
 * This class is derived from <em>Attribute</em> and represents the list of packages that are
 * exported or opened by the Module attribute. There may be at most one ModulePackages attribute in
...
public final class ModulePackages extends Attribute {
        ...
        for (int i = 0; i < packageIndexTable.length; i++) {
            names[i] =
                    super.getConstantPool()
                            .getConstantString(packageIndexTable[i], Const.CONSTANT_Package)
                            .replace('/', '.');
        }
        return names;
    }
    ...
    /**
     * @param packageIndexTable the list of package indexes Also redefines number_of_packages
    ...
    public Attribute copy(final ConstantPool _constant_pool) {
        ...
        if (packageIndexTable != null) {
            ...
            System.arraycopy(
                    packageIndexTable, 0, c.packageIndexTable, 0, packageIndexTable.length);
        }
