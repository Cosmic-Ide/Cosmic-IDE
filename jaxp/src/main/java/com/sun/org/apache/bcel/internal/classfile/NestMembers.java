...
/**
 * This class is derived from <em>Attribute</em> and records the classes and interfaces that are
 * authorized to claim membership in the nest hosted by the current class or interface. There may be
 * at most one NestMembers attribute in a ClassFile structure.
 *
 * @see Attribute
 */
public final class NestMembers extends Attribute {
        ...
        for (int i = 0; i < classes.length; i++) {
            names[i] =
                    super.getConstantPool()
                            .getConstantString(classes[i], Const.CONSTANT_Class)
                            .replace('/', '.');
        }
        return names;
    }
    ...
    /**
     * @param classes the list of class indexes Also redefines number_of_classes according to table
     *     length.
     */
