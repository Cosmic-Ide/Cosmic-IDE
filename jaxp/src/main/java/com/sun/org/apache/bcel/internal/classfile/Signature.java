...
public final class Signature extends Attribute {
    ...
    public String getSignature() {
        final ConstantUtf8 c =
                (ConstantUtf8)
                        super.getConstantPool().getConstant(signatureIndex, Const.CONSTANT_Utf8);
        return c.getBytes();
    }
    ...
    private static void matchIdent(final MyByteArrayInputStream in, final StringBuilder buf) {
        ...
        if ((ch = in.read()) == -1) {
            throw new IllegalArgumentException(
                    "Illegal signature: " + in.getData() + " no ident, reaching EOF");
        }
    ...
    private static void matchGJIdent(final MyByteArrayInputStream in, final StringBuilder buf) {
        ...
        if ((ch == '<') || ch == '(') { // Parameterized or method
            ...
            while (((ch = in.read()) != '>') && (ch != ')')) { // List of parameters
                if (ch == -1) {
                    throw new IllegalArgumentException(
                            "Illegal signature: " + in.getData() + " reaching EOF");
                }
        ...
        if (identStart(ch)) {
            ...
            throw new IllegalArgumentException(
                    "Illegal signature: " + in.getData() + " read " + (char) ch);
        }
    }
