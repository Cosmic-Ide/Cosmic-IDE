...
/**
 * Equality of instructions isn't clearly to be defined. You might wish, for example, to compare
 * whether instructions have the same meaning. E.g., whether two INVOKEVIRTUALs describe the same
 * call.
 *
 * <p>The DEFAULT comparator however, considers two instructions to be equal if they have same
 * opcode and point to the same indexes (if any) in the constant pool or the same local variable
 * index. Branch instructions must have the same target.
 *
 * @see Instruction
 */
            ...
            (i1, i2) -> {
                if (i1.getOpcode() == i2.getOpcode()) {
                    if (i1 instanceof BranchInstruction) {
                        ...
                        return ((ConstantPushInstruction) i1)
                                .getValue()
