...
/** Super class for the INVOKExxx family of instructions. @LastModified: Jan 2020 */
public abstract class InvokeInstruction extends FieldOrMethod
    ...
    public int consumeStack(final ConstantPoolGen cpg) {
        int sum;
        if ((super.getOpcode() == Const.INVOKESTATIC)
                || (super.getOpcode() == Const.INVOKEDYNAMIC)) {
            sum = 0;
        } else {
    ...
    /**
     * This overrides the deprecated version as we know here that the referenced class may legally
     * be an array.
     *
