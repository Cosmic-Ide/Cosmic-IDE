...
/**
 * Represents a local variable within a method. It contains its scope, name and type. The generated
 * LocalVariable object can be obtained with getLocalVariable which needs the instruction list and
 * the constant pool as parameters.
 *
 * @see LocalVariable
 * @see MethodGen
 */
public class LocalVariableGen implements InstructionTargeter, NamedAndTyped, Cloneable {
    ...
    /**
     * Generate a local variable that with index `index'. Note that double and long variables need
     * two indexs. Index indices have to be provided by the user.
     *
    ...
    /**
     * Generates a local variable that with index `index'. Note that double and long variables need
     * two indexs. Index indices have to be provided by the user.
     *
     * @param index index of local variable
     * @param name its name
     ...
     *
     * <p>Note that due to the conversion from byte code offset to InstructionHandle, it is
     * impossible to tell the difference between a live range that ends BEFORE the last insturction
     * of the method or a live range that ends AFTER the last instruction of the method. Hence the
     * liveToEnd flag to differentiate between these two cases.
     *
    ...
    /**
     * We consider to local variables to be equal, if the use the same index and are valid in the
     * same range.
     */
    @Override
