...
/**
 * Template class for building up a method. This is done by defining exception handlers, adding
 * thrown exceptions, local variables and attributes, whereas the `LocalVariableTable' and
 * `LineNumberTable' attributes will be set automatically for the code. Use stripAttributes() if you
 * don't like this.
 *
 * <p>While generating code it may be necessary to insert NOP operations. You can use the
 * `removeNOPs' method to get rid off them. The resulting method object can be obtained via the
 * `getMethod()' method.
 *
 * @see InstructionList
 * @see Method @LastModified: May 2021
 */
public class MethodGen extends FieldGenOrMethodGen {
    ...
    private List<AnnotationEntryGen>[]
            paramAnnotations; // Array of lists containing AnnotationGen objects
    ...
    /**
     * Declare method. If the method is non-static the constructor automatically declares a local
     * variable `$this' in slot 0. The actual code is contained in the `il' parameter, which may
     * further manipulated by the user. But he must take care not to remove any instruction
     * (handles) that are still referenced from this object.
     *
     * <p>For example one may not add a local variable and later remove the instructions it refers
     ...
     * @param il instruction list associated with this method, may be null only for abstract or
     *     native methods
                        ...
                        if (type > 0) {
                            final String cen =
                                    method.getConstantPool()
                                            .getConstantString(type, Const.CONSTANT_Class);
                ...
                for (final Attribute c_attribute : c_attributes) {
                    ...
                    if (a instanceof LineNumberTable) {
                        ...
                        this.localVariableTypeTable =
                                (LocalVariableTypeTable) a.copy(cp.getConstantPool());
                    } else {
                        addCodeAttribute(a);
                    }
                }
    ...
    public LocalVariableGen addLocalVariable(
        ...
        if (t != Const.T_ADDRESS) {
            ...
            final LocalVariableGen l =
                    new LocalVariableGen(slot, name, type, start, end, orig_index);
            int i;
    ...
    /**
     * Remove a local variable, its slot will not be reused, if you do not use addLocalVariable with
     * an explicit index argument.
     */
    ...
    public LocalVariableTable getLocalVariableTable(final ConstantPoolGen cp) {
        ...
        return new LocalVariableTable(
                cp.addUtf8("LocalVariableTable"), 2 + lv.length * 10, lv, cp.getConstantPool());
    }
    ...
    public LineNumberTable getLineNumberTable(final ConstantPoolGen cp) {
        ...
        return new LineNumberTable(
                cp.addUtf8("LineNumberTable"), 2 + ln.length * 4, ln, cp.getConstantPool());
    }

    /**
     * Add an exception handler, i.e., specify region where a handler is active and an instruction
    ...
    public CodeExceptionGen addExceptionHandler(
            final InstructionHandle start_pc,
            final InstructionHandle end_pc,
            final InstructionHandle handler_pc,
     ...
     * LocalVariableTable and StackMap attributes, where the former two will be generated
     * automatically and the latter is used for the MIDP only. Other attributes will be ignored by
     * the JVM but do no harm.
     *
    ...
    /**
     * Would prefer to make this private, but need a way to test if client is using BCEL version
     * 6.5.0 or later that contains fix for BCEL-329.
     *
     * @since 6.5.0
     */
    ...
    public Method getMethod() {
        ...
        if (localVariableTypeTable != null) {
            // LocalVariable length in LocalVariableTypeTable is not updated automatically. It's a
            // difference with LocalVariableTable.
            if (lvt != null) {
    ...
    private void adjustLocalVariableTypeTable(final LocalVariableTable lvt) {
        ...
        for (final LocalVariable element : lvg) {
            for (final LocalVariable l : lv) {
                if (element.getName().equals(l.getName())
                        && element.getIndex() == l.getOrigIndex()) {
    ...
    /**
     * Remove all NOPs from the instruction list (if possible) and update every object referring to
     * them, i.e., branch instructions, local variables and exception handlers.
     */
    ...
    public void
            setMaxStack() { // TODO could be package-protected? (some tests would need repackaging)
        if (il != null) {
    ...
    public void
            setMaxLocals() { // TODO could be package-protected? (some tests would need repackaging)
        if (il != null) {
                ...
                if ((ins instanceof LocalVariableInstruction)
                    ...
                    final int index =
                            ((IndexedInstruction) ins).getIndex()
                                    + ((TypedInstruction) ins)
                                            .getType(super.getConstantPool())
                                            .getSize();
                    if (index > max) {
                        max = index;
                    }
    ...
    /**
     * Do not/Do produce attributes code attributesLineNumberTable and LocalVariableTable, like
     * javac -O
     */
    ...
    /**
     * Call notify() method on all observers. This method is not called automatically whenever the
     * state has changed, but has to be called by the user after he has finished editing the object.
     */
    public void update() {
        if (observers != null) {
     ...
     * RuntimeParameterAnnotations, extracting their contents and storing them as parameter
     * annotations. There are two kinds of parameter annotation - visible and invisible. Once they
     * have been unpacked, these attributes are deleted. (The annotations will be rebuilt as
     * attributes when someone builds a Method object out of this MethodGen object).
     */
    ...
    public void addParameterAnnotation(
            final int parameterIndex, final AnnotationEntryGen annotation) {
        ensureExistingParameterAnnotationsUnpacked();
        if (!hasParameterAnnotations) {
    ...
    /**
     * Return value as defined by given BCELComparator strategy. By default two MethodGen objects
     * are said to be equal when their names and signatures are equal.
     *
     ...
     * the method's name XOR signature.
     *
