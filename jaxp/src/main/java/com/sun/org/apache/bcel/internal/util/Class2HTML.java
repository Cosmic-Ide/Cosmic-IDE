 ...
 * </OL>
 *
 * All subfiles reference each other appropriately, e.g. clicking on a method in the Method's frame
 * will jump to the appropriate method in the Code frame. @LastModified: Jan 2020
 */
public class Class2HTML {
    ...
    public Class2HTML(final JavaClass java_class, final String dir) throws IOException {
         ...
         */
        final AttributeHTML attribute_html =
                new AttributeHTML(dir, class_name, constant_pool, constant_html);
        new MethodHTML(
                dir, class_name, methods, java_class.getFields(), constant_html, attribute_html);
                ...
                if (argv[i].equals("-d")) { // Specify target directory, default '.'
                    ...
                    if (!store.isDirectory()) {
                        final boolean created =
                                store.mkdirs(); // Create target directory if necessary
                        if (!created) {
                            if (!store.isDirectory()) {
                                System.out.println(
                                        "Tried to create the directory " + dir + " but failed");
                            }
                        }
                    }
        ...
        if (files == 0) {
            ...
            for (int i = 0; i < files; i++) {
                ...
                if (zip_file == null) {
                    ...
                    parser =
                            new ClassParser(
                                    zip_file, file_name[i]); // Create parser object from zip file
                }
