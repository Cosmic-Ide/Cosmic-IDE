 ...
 * @author Santiago Pericas-Geertsen @LastModified: May 2021
 */
public final class TemplatesImpl implements Templates, Serializable {
    ...
    /**
     * Name of the superclass of all translets. This is needed to determine which, among all classes
     * comprising a translet, is the main one.
     */
    ...
    /**
     * Contains the translet class definition(s). These are created when this Templates is created
     * or when it is read back from disk.
     */
    ...
    /**
     * Cache the DTM for the stylesheet in a thread local variable, which is used by the
     * document('') function. Use ThreadLocal because a DTM cannot be shared between multiple
     * threads. Declaring it transient to fix bug 22438
     */
    ...
    /**
     * Create an XSLTC template object from the bytecodes. The bytecodes for the translet and
     * auxiliary classes, plus the name of the main translet class, must be supplied.
     */
     ...
     * defined class for URIResolver also implemented Serializable if yes then we need to
     * deserialize the URIResolver Fix for bugzilla bug 22438
     */
    ...
    /**
     * The TransformerFactory must pass us the translet bytecodes using this method before we can
     * create any translet instances
     *
     * <p>Note: This method is private for security reasons. See CR 6537898. When merging with
     * Apache, we must ensure that the privateness of this method is maintained (that is why it
     * wasn't removed).
     */
    ...
    /**
     * Defines the translet class and auxiliary classes. Returns a reference to the Class object
     * that defines the main class
     */
    private void defineTransletClasses() throws TransformerConfigurationException {
        ...
        TransletClassLoader loader =
                AccessController.doPrivileged(
                        new PrivilegedAction<TransletClassLoader>() {
        ...
        try {
            ...
            PermissionCollection perms = new RuntimePermission("*").newPermissionCollection();
            Arrays.asList(Constants.PKGS_USED_BY_TRANSLET_CLASSES)
                    .forEach(
                            p -> {
            ...
            ProtectionDomain pd = new ProtectionDomain(codeSource, perms, loader, null);
     ...
     * The translet instance will later be wrapped inside a Transformer object.
     */
    ...
    public synchronized Transformer newTransformer() throws TransformerConfigurationException {
        ...
        transformer =
                new TransformerImpl(
                        getTransletInstance(), _outputProperties, _indentNumber, _tfactory);
    ...
    /**
     * Implements JAXP's Templates.getOutputProperties(). We need to instanciate a translet to get
     * the output settings, so we might as well just instanciate a Transformer and use its
