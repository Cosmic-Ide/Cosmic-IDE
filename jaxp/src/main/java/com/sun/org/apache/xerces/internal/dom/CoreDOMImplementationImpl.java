...
/**
 * The DOMImplementation class is description of a particular implementation of the Document Object
 * Model. As such its data is static, shared by all instances of this implementation.
 *
 * <p>The DOM API requires that it be a real object rather than static methods. However, there's
 * nothing that says it can't be a singleton, so that's how I've implemented it.
 *
 * <p>This particular class, along with CoreDocumentImpl, supports the DOM Core and Load/Save
 * (Experimental). Optional modules are supported by the more complete DOMImplementation class along
 * with DocumentImpl.
 *
 * @xerces.internal
...
public class CoreDOMImplementationImpl implements DOMImplementation, DOMImplementationLS {
    ...
    //
    /**
     * Test if the DOM implementation supports a specific "feature" -- currently meaning language
     * and level thereof.
     *
     * @param feature The package name of the feature to test. In Level 1, supported values are
     *     "HTML" and "XML" (case-insensitive). At this writing,
     *     com.sun.org.apache.xerces.internal.dom supports only XML.
     * @param version The version number of the feature being tested. This is interpreted as
     *     "Version of the DOM API supported for the specified Feature", and in Level 1 should be
     *     "1.0"
     * @return true if this implementation is compatible with the specified feature and version.
     */
     ...
     *
     * <p>Creates an empty DocumentType node.
     *
     ...
     *     <p>When doctype is not null, its Node.ownerDocument attribute is set to the document
     *     being created.
    ...
    /**
     * DOM Level 3 LS CR - Experimental. Create a new <code>LSParser</code>. The newly constructed
     * parser may then be configured by means of its <code>DOMConfiguration</code> object, and used
     * to parse documents by means of its <code>parse</code> method.
     *
     * @param mode The <code>mode</code> argument is either <code>MODE_SYNCHRONOUS</code> or <code>
     *     MODE_ASYNCHRONOUS</code>, if <code>mode</code> is <code>MODE_SYNCHRONOUS</code> then the
     *     <code>LSParser</code> that is created will operate in synchronous mode, if it's <code>
     *     MODE_ASYNCHRONOUS</code> then the <code>LSParser</code> that is created will operate in
     *     asynchronous mode.
     * @param schemaType An absolute URI representing the type of the schema language used during
     *     the load of a <code>Document</code> using the newly created <code>LSParser</code>. Note
     *     that no lexical checking is done on the absolute URI. In order to create a <code>LSParser
     *     </code> for any kind of schema types (i.e. the LSParser will be free to use any schema
     *     found), use the value <code>null</code>.
     ...
     *     DTD [<a href='http://www.w3.org/TR/2000/REC-xml-20001006'>XML 1.0</a>], applications must
     *     use the value <code>"http://www.w3.org/TR/REC-xml"</code>. Other Schema languages are
     *     outside the scope of the W3C and therefore should recommend an absolute URI in order to
     *     use this method.
     * @return The newly created <code>LSParser</code> object. This <code>LSParser</code> is either
     *     synchronous or asynchronous depending on the value of the <code>mode</code> argument.
     *     <p><b>Note:</b> By default, the newly created <code>LSParser</code> does not contain a
     *     <code>DOMErrorHandler</code>, i.e. the value of the "<a
     ...
     *     may provide a default error handler at creation time. In that case, the initial value of
     *     the <code>"error-handler"</code> configuration parameter on the new created <code>
     *     LSParser</code> contains a reference to the default error handler.
     * @exception DOMException NOT_SUPPORTED_ERR: Raised if the requested mode or schema type is not
     *     supported.
     */
    public LSParser createLSParser(short mode, String schemaType) throws DOMException {
        ...
        if (schemaType != null && schemaType.equals("http://www.w3.org/TR/REC-xml")) {
            return new DOMParserImpl(new XML11DTDConfiguration(), schemaType);
        } else {
     ...
     *     <p><b>Note:</b> By default, the newly created <code>LSSerializer</code> has no <code>
     *     DOMErrorHandler</code>, i.e. the value of the <code>"error-handler"</code> configuration
     *     parameter is <code>null</code>. However, implementations may provide a default error
     *     handler at creation time. In that case, the initial value of the <code>"error-handler"
     *     </code> configuration parameter on the new created <code>LSSerializer</code> contains a
    ...
    synchronized void releaseValidator(
            String schemaType, String xmlVersion, RevalidationHandler validator) {
        if (schemaType == XMLGrammarDescription.XML_SCHEMA) {
            ++freeSchemaValidatorIndex;
            ...
            schemaValidators[freeSchemaValidatorIndex] =
            ...
            if ("1.1".equals(xmlVersion)) {
                ...
                xml11DTDValidators[freeXML11DTDValidatorIndex] =
                        new SoftReference(new RevalidationHandlerHolder(validator));
            }
            // release an instance of XMLDTDValidator
                ...
                xml10DTDValidators[freeXML10DTDValidatorIndex] =
                        new SoftReference(new RevalidationHandlerHolder(validator));
            }
        }
    }
    ...
    final synchronized void releaseDTDLoader(String xmlVersion, XMLDTDLoader loader) {
        ...
        if ("1.1".equals(xmlVersion)) {
            ...
            xml11DTDLoaders[freeXML11DTDLoaderIndex] =
                    new SoftReference(new XMLDTDLoaderHolder(loader));
        }
        // release an instance of XMLDTDLoader
        else {
            ++freeXML10DTDLoaderIndex;
            ...
            xml10DTDLoaders[freeXML10DTDLoaderIndex] =
                    new SoftReference(new XMLDTDLoaderHolder(loader));
        }
    }
    ...
    /**
     * A holder for RevalidationHandlers. This allows us to reuse SoftReferences which haven't yet
    ...
    /**
     * A holder for XMLDTDLoaders. This allows us to reuse SoftReferences which haven't yet been
