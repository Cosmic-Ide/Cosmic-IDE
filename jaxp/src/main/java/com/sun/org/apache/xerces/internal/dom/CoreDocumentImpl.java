 ...
 * these objects. The Node objects created have a ownerDocument attribute which associates them with
 * the Document within whose context they were created.
 *
 * <p>The CoreDocumentImpl class only implements the DOM Core. Additional modules are supported by
 * the more complete DocumentImpl subclass.
 *
 * <p><b>Note:</b> When any node in the document is serialized, the entire document is serialized
 * along with it.
 *
 * @xerces.internal
 ...
 * @since PR-DOM-Level-1-19980818. @LastModified: Sept 2019
 */
public class CoreDocumentImpl extends ParentNode implements Document {

    /**
     * TODO:: 1. Change XML11Char method names similar to XMLChar. That will prevent lot of dirty
     * version checking code.
     *
     * <p>2. IMO during cloneNode qname/isXMLName check should not be made.
     */
    //
    // Constants
    //
    ...
    /**
     * Number of alterations made to this document since its creation. Serves as a "dirty bit" so
     * that live objects such as NodeList can recognize when an alteration has been made and discard
     * its cached state information.
     *
     * <p>Any method that alters the tree structure MUST cause or be accompanied by a call to
     * changed(), to inform it that any outstanding NodeLists may have to be updated.
     *
     * <p>(Required because NodeList is simultaneously "live" and integer- indexed -- a bad decision
     * in the DOM's design.)
     *
     * <p>Note that changes which do not affect the tree's structure -- changing the node's name,
     * for example -- do _not_ have to call changed().
     *
     * <p>Alternative implementation would be to use a cryptographic Digest value rather than a
     * count. This would have the advantage that "harmless" changes (those producing equal() trees)
     * would not force NodeList to resynchronize. Disadvantage is that it's slightly more prone to
     * "false negatives", though that's the difference between "wildly unlikely" and "absurdly
     * unlikely". IF we start maintaining digests, we should consider taking advantage of them.
     *
     * <p>Note: This used to be done a node basis, so that we knew what subtree changed. But since
     * only DeepNodeList really use this today, the gain appears to be really small compared to the
     * cost of having an int on every (parent) node plus having to walk up the tree all the way to
     * the root to mark the branch as changed everytime a node is changed. So we now have a single
     * counter global to the document. It means that some objects may flush their cache more often
     * than necessary, but this makes nodes smaller and only the document needs to be marked as
     * changed.
     */
    ...
    static {
        ...
        kidOK[DOCUMENT_TYPE_NODE] =
                kidOK[PROCESSING_INSTRUCTION_NODE] =
                        kidOK[COMMENT_NODE] =
                                kidOK[TEXT_NODE] =
                                        kidOK[CDATA_SECTION_NODE] = kidOK[NOTATION_NODE] = 0;
    } // static
     ...
     * @serialField userData Hashtable user data attached to the nodes. Note that it was original
     *     called "userData". It has been changed to nodeUserData to avoid confusion with those that
     *     are actually values of the map.
    ...
    /**
     * NON-DOM: Actually creating a Document is outside the DOM's spec, since it has to operate in
    ...
    /**
     * Since a Document may contain at most one top-level Element child, and at most one
     * DocumentType declaraction, we need to subclass our add-children methods to implement this
     * constraint. Since appendChild() is implemented as insertBefore(,null), altering the latter
     * fixes both.
     *
     * <p>While I'm doing so, I've taken advantage of the opportunity to cache documentElement and
     * docType so we don't have to search for them.
     *
     * <p>REVISIT: According to the spec it is not allowed to alter neither the document element nor
     * the document type in any way
     */
    ...
    /**
     * Since insertBefore caches the docElement (and, currently, docType), removeChild has to know
     * how to undo the cache
     *
     * <p>REVISIT: According to the spec it is not allowed to alter neither the document element nor
     * the document type in any way
     */
     ...
     *
     * @param name The name of the attribute. Note that the attribute's value is _not_ established
     *     at the factory; remember to set it!
     ...
     * @throws DOMException(NOT_SUPPORTED_ERR) for HTML documents, where nonstandard entities are
     *     not permitted. (HTML not yet implemented.)
     */
    ...
    /**
     * For XML, this provides access to the Document Type Definition. For HTML documents, and XML
     * documents which don't specify a DTD, it will be null.
     */
    ...
    /**
     * Convenience method, allowing direct access to the child node which is considered the root of
     * the actual document content. For HTML, where it is legal to have more than one Element at the
     * top level of the document, we pick the one with the tagName "HTML". For XML there should be
     * only one top-level
     *
     * <p>(HTML not yet supported.)
     */
    ...
    /**
     * Return a <em>live</em> collection of all descendent Elements (not just immediate children)
     * having the specified tag name.
     *
     * @param tagname The type of Element we want to gather. "*" will be taken as a wildcard,
     *     meaning "all elements in the document."
    ...
    /**
     * Sets whether the DOM implementation performs error checking upon operations. Turning off
     * error checking only affects the following DOM checks:
     *
     * <ul>
     ...
     * </ul>
     *
     * <p>Turning off error checking does <em>not</em> turn off the following checks:
     *
     * <ul>
     *   <li>Read only checks
     ...
     *
     * <p>An attribute specifying the encoding used for this document at the time of the parsing.
     * This is <code>null</code> when it is not known, such as when the <code>Document</code> was
     * created in memory.
     *
     * @since DOM Level 3
     */
     ...
     *
     * <p>An attribute specifying the actual encoding of this document. This is <code>null</code>
     * otherwise. <br>
     ...
     *
     * <p>An attribute specifying, as part of the XML declaration, the encoding of this document.
    ...
    /**
     * @deprecated This method is internal and only exists for compatibility with older
     *     applications. New applications should never call this method.
     */
    @Deprecated
    ...
    /**
     * DOM Level 3 CR - Experimental. version - An attribute specifying, as part of the XML
     ...
     *
     * <p>Xmlstandalone - An attribute specifying, as part of the XML declaration, whether this
    ...
    /**
     * DOM Level 3 WD - Experimental. standalone that specifies whether this document is standalone
    ...
    /**
     * DOM Level 3 WD - Experimental. The location of the document or <code>null</code> if
     * undefined. <br>
     * Beware that when the <code>Document</code> supports the feature "HTML" , the href attribute
     * of the HTML BASE element takes precedence over this attribute.
     *
     * @since DOM Level 3
     */
     ...
     * DOM Level 3 CR - Experimental
     *
     * <p>The configuration used when <code>Document.normalizeDocument</code> is invoked.
     *
     * @since DOM Level 3
     */
    ...
    /**
     * Returns the absolute base URI of this node or null if the implementation wasn't able to
     * obtain an absolute URI. Note: If the URI is malformed, a null is returned.
     *
    ...
    //
    /**
     * DOM Level 3 WD - Experimental. Indicates whether the method load should be synchronous or
     * asynchronous. When the async attribute is set to <code>true</code> the load method returns
     * control to the caller before the document has completed loading. The default value of this
     * property is <code>false</code>. <br>
     * Setting the value of this attribute might throw NOT_SUPPORTED_ERR if the implementation
     * doesn't support the mode the attribute is being set to. Should the DOM spec define the
     * default value of this property? What if implementing both async and sync IO is impractical in
     * some systems? 2001-09-14. default is <code>false</code> but we need to check with Mozilla and
     * IE.
     */
    ...
    /**
     * DOM Level 3 WD - Experimental. If the document is currently being loaded as a result of the
     * method <code>load</code> being invoked the loading and parsing is immediately aborted. The
     * possibly partial result of parsing the document is discarded and the document is cleared.
     */
    public void abort() {}
     ...
     * DOM Level 3 WD - Experimental.
     *
     * <p>Replaces the content of the document with the result of parsing the given URI. Invoking
     * this method will either block the caller or return to the caller immediately depending on the
     * value of the async attribute. Once the document is fully loaded a "load" event (as defined in
     * [<a href='http://www.w3.org/TR/2003/WD-DOM-Level-3-Events-20030331'>DOM Level 3 Events</a>] ,
     * except that the <code>Event.targetNode</code> will be the document, not an element) will be
     * dispatched on the document. If an error occurs, an implementation dependent "error" event
     * will be dispatched on the document. If this method is called on a document that is currently
     * loading, the current load is interrupted and the new URI load is initiated. <br>
     * When invoking this method the parameters used in the <code>DOMParser</code> interface are
     * assumed to have their default values with the exception that the parameters <code>"entities"
     * </code> , <code>"normalize-characters"</code>, <code>"check-character-normalization"</code>
     * are set to <code>"false"</code>. <br>
     * The result of a call to this method is the same the result of a call to <code>
     * DOMParser.parseWithContext</code> with an input stream referencing the URI that was passed to
     * this call, the document as the context node, and the action <code>ACTION_REPLACE_CHILDREN
     * </code>.
     *
     * @param uri The URI reference for the XML file to be loaded. If this is a relative URI, the
     *     base URI used by the implementation is implementation dependent.
     * @return If async is set to <code>true</code> <code>load</code> returns <code>true</code> if
     *     the document load was successfully initiated. If an error occurred when initiating the
     *     document load, <code>load</code> returns <code>false</code>.If async is set to <code>
     *     false</code> <code>load</code> returns <code>true</code> if the document was successfully
     *     loaded and parsed. If an error occurred when either loading or parsing the URI, <code>
     *     load</code> returns <code>false</code>.
     */
    ...
    /**
     * DOM Level 3 WD - Experimental. Replace the content of the document with the result of parsing
     * the input string, this method is always synchronous.
     *
    ...
    /**
     * DOM Level 3 WD - Experimental. Save the document or the given node and all its descendants to
     * a string (i.e. serialize the document or node). <br>
     * The parameters used in the <code>LSSerializer</code> interface are assumed to have their
     * default values when invoking this method. <br>
     * The result of a call to this method is the same the result of a call to <code>
     * LSSerializer.writeToString</code> with the document as the node to write.
     *
     ...
     * @exception DOMException WRONG_DOCUMENT_ERR: Raised if the node passed in as the node
     ...
     * (REC-DOM-Level-1-19981001 left the process of building DTD information unspecified.)
     *
     * @param name The name of the Entity we wish to provide a value for.
     * @throws DOMException(NOT_SUPPORTED_ERR) for HTML documents, where DTDs are not permitted.
     ...
     * @throws DOMException(NOT_SUPPORTED_ERR) for HTML documents, where notations are not
    ...
    /**
     * NON-DOM Factory method: creates an element definition. Element definitions hold default
     * attribute values.
     */
    ...
    /**
     * NON-DOM: Get the number associated with this document. Used to order documents in the
     * implementation.
     */
    ...
    /**
     * NON-DOM: Get a number associated with a node created with respect to this document. Needed
     * for compareDocumentPosition when nodes are disconnected. This is only used on demand.
     */
    ...
    /**
     * Copies a node from another document to this document. The new nodes are created using this
     * document's factory methods and are populated with the data from the source's accessor methods
     * defined by the DOM interfaces. Its behavior is otherwise similar to that of cloneNode.
     *
     * <p>According to the DOM specifications, document nodes cannot be imported and a
    ...
    /**
     * Overloaded implementation of DOM's importNode method. This method provides the core
     * functionality for the public importNode and cloneNode methods.
     *
     * <p>The reversedIdentifiers parameter is provided for cloneNode to preserve the document's
     * identifiers. The Map has Elements as the keys and their identifiers as the values. When an
     * element is being imported, a check is done for an associated identifier. If one exists, the
     * identifier is registered with the new, imported element. If reversedIdentifiers is null, the
     * parameter is not applied.
     */
    private Node importNode(
            Node source, boolean deep, boolean cloningDoc, Map<Node, String> reversedIdentifiers)
            throws DOMException {
        Node newnode = null;
            ...
            case ELEMENT_NODE:
                {
                    ...
                    if (domLevel20 == false || source.getLocalName() == null)
                        ...
                        newElement =
                                createElementNS(source.getNamespaceURI(), source.getNodeName());
            ...
            case ATTRIBUTE_NODE:
                {
                    if (source.getOwnerDocument().getImplementation().hasFeature("XML", "2.0")) {
                        if (source.getLocalName() == null) {
                            ...
                            newnode =
                                    createAttributeNS(
                                            source.getNamespaceURI(), source.getNodeName());
                        }
                    } else {
                        newnode = createAttribute(source.getNodeName());
                    }
            ...
            case DOCUMENT_TYPE_NODE:
                {
                    ...
                    if (smap != null) {
                        for (int i = 0; i < smap.getLength(); i++) {
                            tmap.setNamedItem(
                                    importNode(smap.item(i), true, true, reversedIdentifiers));
                        }
                    }
    ...
    /**
     * DOM Level 3 WD - Experimental Change the node's ownerDocument, and its subtree, to this
     * Document
     *
    ...
    /**
     * Introduced in DOM Level 2 Returns the Element whose ID is given by elementId. If no such
     * element exists, returns null. Behavior is not defined if more than one element has this ID.
     *
     * <p>Note: The DOM implementation must have information that says which attributes are of type
     * ID. Attributes with the name "ID" are not of type ID unless so defined. Implementations that
     * do not know whether attributes are of type ID or not are expected to return null.
     *
     * @see #getIdentifier
     */
    ...
    /**
     * Registers an identifier name with a specified element node. If the identifier is already
     * registered, the new element node replaces the previous node. If the specified element node is
     * null, removeIdentifier() is called.
     *
     * @see #getIdentifier
    ...
    /**
     * Returns a previously registered element with the specified identifier name, or null if no
     ...
     *
     * <p>Creates an element of the given qualified name and namespace URI. If the given
     * namespaceURI is null or an empty string and the qualifiedName has a prefix that is "xml", the
     * created element is bound to the predefined namespace "http://www.w3.org/XML/1998/namespace"
     * [Namespaces].
     *
     ...
     *     character.
     * @throws DOMException NAMESPACE_ERR: Raised if the qualifiedName has a prefix that is "xml"
     *     and the namespaceURI is neither null nor an empty string nor
     *     "http://www.w3.org/XML/1998/namespace", or if the qualifiedName has a prefix different
     ...
     * @return Element A new Element object with the following attributes:
     * @exception DOMException INVALID_CHARACTER_ERR: Raised if the specified name contains an
     *     invalid character.
     */
     ...
     * Introduced in DOM Level 2.
     *
     * <p>Creates an attribute of the given qualified name and namespace URI. If the given
     * namespaceURI is null or an empty string and the qualifiedName has a prefix that is "xml", the
     * created element is bound to the predefined namespace "http://www.w3.org/XML/1998/namespace"
     * [Namespaces].
     *
     ...
     * Introduced in DOM Level 2.
     *
     * <p>Returns a NodeList of all the Elements with a given local name and namespace URI in the
    ...
    /**
     * Check the string against XML's definition of acceptable names for elements and attributes and
     * so on using the XMLCharacterProperties utility class
     */
    ...
    /**
     * Checks if the given qualified name is legal with respect to the version of XML to which this
    ...
    /**
     * Puts the given NodeListCache in the free list. Note: The owner node can keep using it until
     * we reuse it
     */
     ...
     * by calling <code>getUserData</code> with the same key.
     *
     ...
     * @return Returns the <code>DOMObject</code> previously associated to the given key on this
     *     node, or <code>null</code> if there was none.
     * @since DOM Level 3
     *     <p>REVISIT: we could use a free list of UserDataRecord here
     */
     ...
     * to this node by calling <code>setUserData</code> with the same key.
     *
     ...
     */
    void callUserDataHandlers(
            Node n, Node c, short operation, Map<String, UserDataRecord> userData) {
    ...
    /**
     * Call user data handlers to let them know the nodes they are related to are being deleted. The
     * alternative would be to do that on Node but because the nodes are used as the keys we have a
     * reference to them that prevents them from being gc'ed until the document is. At the same
     * time, doing it here has the advantage of avoiding a finalize() method on Node, which would
     * affect all nodes and not just the ones that have a user data.
     */
    ...
    /**
     * We could have more xml versions in future , but for now we could do with this to handle XML
     * 1.0 and 1.1
     */
    ...
    /**
     * NON-DOM: kept for backward compatibility Store user data related to a given node This is a
     * place where we could use weak references! Indeed, the node here won't be GC'ed as long as
     * some user data is attached to it, since the userData table will have a reference to the node.
     */
    ...
    protected void addEventListener(
            NodeImpl node, String type, EventListener listener, boolean useCapture) {
        // does nothing by default - overidden in subclass
    }
    ...
    /**
     * A method to be called when some text was changed in a text node, so that live objects can be
     * notified.
     */
    void replacedText(NodeImpl node) {}

    /**
     * A method to be called when some text was deleted from a text node, so that live objects can
     * be notified.
     */
    void deletedText(NodeImpl node, int offset, int count) {}

    /**
     * A method to be called when some text was inserted into a text node, so that live objects can
     * be notified.
     */
    ...
    /**
     * @serialData Serialized fields. Convert Maps to Hashtables for backward compatibility.
     */
    private void writeObject(ObjectOutputStream out) throws IOException {
        ...
        // userData is the original name. It has been changed to nodeUserData, refer to the
        // corrsponding @serialField
        pf.put("userData", nud);
