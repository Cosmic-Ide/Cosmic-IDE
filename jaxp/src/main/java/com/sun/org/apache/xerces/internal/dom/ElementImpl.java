 ...
 * terms of Strings.
 *
 * <p>ElementImpl does not support Namespaces. ElementNSImpl, which inherits from it, does.
 *
 * @see ElementNSImpl
 * @xerces.internal
...
public class ElementImpl extends ParentNode implements Element, ElementTraversal, TypeInfo {
    ...
    /**
     * Retrieve all the Attributes as a set. Note that this API is inherited from Node rather than
     * specified on Element; in fact only Elements will ever have Attributes, but they want to allow
     * folks to "blindly" operate on the tree as a set of Nodes.
     *
    ...
    /**
     * Return a duplicate copy of this Element. Note that its children will not be copied unless the
     * "deep" flag is true, but Attributes are {@code always} replicated.
     *
            ...
            if (attrNode != null) {
                ...
                if (uri.length() != 0) { // attribute value is always empty string
                    try {
                        ...
                        // If the URI is already absolute return it; otherwise it's relative and we
                        // need to resolve it.
                        ...
                        String parentBaseURI =
                                (this.ownerNode != null) ? this.ownerNode.getBaseURI() : null;
    ...
    //
    /**
     * Look up a single Attribute by name. Returns the Attribute's string value, or an empty string
     * (NOT null!) to indicate that the name did not map to a currently defined attribute.
     *
     * <p>Note: Attributes may contain complex node trees. This method returns the "flattened"
     * string obtained from Attribute.getValue(). If you need the structure information, see
     * getAttributeNode().
     */
    ...
    /**
     * Look up a single Attribute by name. Returns the Attribute Node, so its complete child tree is
     * available. This could be important in XML, where the string rendering may not be sufficient
     * information.
     *
     * <p>If no matching attribute is available, returns null.
     */
    ...
    /**
     * Returns a NodeList of all descendent nodes (children, grandchildren, and so on) which are
     * Elements and which have the specified tag name.
     *
     * <p>Note: NodeList is a "live" view of the DOM. Its contents will change as the DOM changes,
     * and alterations made to the NodeList will be reflected in the DOM.
     *
    ...
    /**
     * Returns the name of the Element. Note that Element.nodeName() is defined to also return the
     * tag name.
     *
     * <p>This is case-preserving in XML. HTML should uppercasify it on the way in.
     */
    ...
    /**
     * In "normal form" (as read from a source file), there will never be two Text children in
     * succession. But DOM users may create successive Text nodes in the course of manipulating the
     * document. Normalize walks the sub-tree and merges adjacent Texts, as if the DOM had been
     * written out and read back in again. This simplifies implementation of higher-level functions
     * that may want to assume that the document is in standard form.
     *
     * <p>To normalize a Document, normalize its top-level Element child.
     *
     * <p>As of PR-DOM-Level-1-19980818, CDATA -- despite being a subclass of Text -- is considered
     * "markup" and will _not_ be merged either with normal Text or with other CDATASections.
     */
    ...
    /**
     * Remove the named attribute from this Element. If the removed Attribute has a default value,
     * it is immediately replaced thereby.
     *
     * <p>The default logic is actually implemented in NamedNodeMapImpl. PR-DOM-Level-1-19980818
     * doesn't fully address the DTD, so some of this behavior is likely to change in future
     * versions. ?????
     *
     * <p>Note that this call "succeeds" even if no attribute by this name existed -- unlike
     * removeAttributeNode, which will throw a not-found exception in that case.
     *
    ...
    /**
     * Remove the specified attribute/value pair. If the removed Attribute has a default value, it
     * is immediately replaced.
     *
     * <p>NOTE: Specifically removes THIS NODE -- not the node with this name, nor the node with
     * these contents. If the specific Attribute object passed in is not stored in this Element, we
     * throw a DOMException. If you really want to remove an attribute by name, use
     * removeAttribute().
     *
     ...
     *
     * <p>Note: this method supports only the simplest kind of Attribute, one whose value is a
     * string contained in a single Text node. If you want to assert a more complex value (which XML
     * permits, though HTML doesn't), see setAttributeNode().
     *
     * <p>The attribute is created with specified=true, meaning it's an explicit value rather than
     * inherited from the DTD as a default. Again, setAttributeNode can be used to achieve other
     * results.
     *
    ...
    /**
     * Add a new attribute/value pair, or replace the value of the existing attribute with that
     * name.
     *
     * <p>This method allows you to add an Attribute that has already been constructed, and hence
     * avoids the limitations of the simple setAttribute() call. It can handle attribute values that
     * have arbitrarily complex tree structure -- in particular, those which had entity references
     * mixed into their text.
     *
     * @throws DOMException(INUSE_ATTRIBUTE_ERR) if the Attribute object has already been assigned
     *     to another Element.
     */
     ...
     *
     * <p>Retrieves an attribute value by local name and namespace URI.
     *
     * @param namespaceURI The namespace URI of the attribute to retrieve.
     * @param localName The local name of the attribute to retrieve.
     * @return String The Attr value as a string, or empty string if that attribute does not have a
     ...
     * Introduced in DOM Level 2.
     *
     * <p>Adds a new attribute. If the given namespaceURI is null or an empty string and the
     * qualifiedName has a prefix that is "xml", the new attribute is bound to the predefined
     * namespace "http://www.w3.org/XML/1998/namespace" [Namespaces]. If an attribute with the same
     * local name and namespace URI is already present on the element, its prefix is changed to be
     * the prefix part of the qualifiedName, and its value is changed to be the value parameter.
     ...
     * attribute value that contains entity references, the user must create an Attr node plus any
     * Text and EntityReference nodes, build the appropriate subtree, and use setAttributeNodeNS or
     ...
     *     namespaceURI is neither null nor an empty string nor
     *     "http://www.w3.org/XML/1998/namespace", or if the qualifiedName has a prefix that is
     *     "xmlns" but the namespaceURI is neither null nor an empty string, or if if the
     *     qualifiedName has a prefix different from "xml" and "xmlns" and the namespaceURI is null
     *     or an empty string.
     * @since WD-DOM-Level-2-19990923
     */
        ...
        if (newAttr == null) {
            ...
            if (newAttr instanceof AttrNSImpl) {
                ...
                if (!newName.equals(origNodeName)) {
                    // Note: we can't just change the name of the attribute. Names have to be in
                    // sorted
                    // order in the attributes vector because a binary search is used to locate
                    // them.
                ...
                newAttr =
                        ((CoreDocumentImpl) getOwnerDocument())
                                .createAttributeNS(namespaceURI, qualifiedName, localName);
                attributes.setNamedItemNS(newAttr);
            }
     ...
     * Introduced in DOM Level 2.
     *
     * <p>Removes an attribute by local name and namespace URI. If the removed attribute has a
     * default value it is immediately replaced. The replacing attribute has the same namespace URI
     ...
     * @param namespaceURI The namespace URI of the attribute to retrieve.
     * @param localName The local name of the attribute to retrieve.
     * @return Attr The Attr node with the specified attribute local name and namespace URI or null
     ...
     * Introduced in DOM Level 2.
     *
     * <p>Adds a new attribute. If an attribute with that local name and namespace URI is already
     * present in the element, it is replaced by the new one.
     *
     * @param newAttr The Attr node to add to the attribute list. When the Node has no namespaceURI,
     *     this method behaves like setAttributeNode.
     * @return Attr If the newAttr attribute replaces an existing attribute with the same local name
     *     and namespace URI, the * previously existing Attr node is returned, otherwise null is
     *     returned.
    ...
    /**
     * DOM Level 3 WD- Experimental. Override inherited behavior from NodeImpl and ParentNode to
     * check on attributes
     */
    ...
    protected NamedNodeMapImpl getDefaultAttributes() {
        ...
        ElementDefinitionImpl eldef =
                (ElementDefinitionImpl) doctype.getElements().getNamedItem(getNodeName());
        if (eldef == null) {
            return null;
        }
