...
/**
 * NamedNodeMaps represent collections of Nodes that can be accessed by name. Entity and Notation
 * nodes are stored in NamedNodeMaps attached to the DocumentType. Attributes are placed in a
 * NamedNodeMap attached to the elem they're related too. However, because attributes require more
 * work, such as firing mutation events, they are stored in a subclass of NamedNodeMapImpl.
 *
 * <p>Only one Node may be stored per name; attempting to store another will replace the previous
 * value.
 *
 * <p>NOTE: The "primary" storage key is taken from the NodeName attribute of the node. The
 * "secondary" storage key is the namespaceURI and localName, when accessed by DOM level 2 nodes.
 * All nodes, even DOM Level 2 nodes are stored in a single Vector sorted by the primary "nodename"
 * key.
 *
 * <p>NOTE: item()'s integer index does _not_ imply that the named nodes must be stored in an array;
 * that's only an access method. Note too that these indices are "live"; if someone changes the
 * map's contents, the indices associated with nodes may change.
 *
 * <p>
 *
 * @xerces.internal
 * @since PR-DOM-Level-1-19980818. @LastModified: Jan 2018
 */
public class NamedNodeMapImpl implements NamedNodeMap, Serializable {
    ...
    /**
     * Report how many nodes are currently stored in this NamedNodeMap. Caveat: This is a count
     * rather than an index, so the highest-numbered node at any time can be accessed via
     * item(getLength()-1).
     */
     ...
     *
     * @param index Which item to retrieve. Note that indices are just an enumeration of the current
     *     contents; they aren't guaranteed to be stable, nor do they imply any promises about the
     *     order of the NamedNodeMap's contents. In other words, DO NOT assume either that index(i)
     *     will always refer to the same entry, or that there is any stable ordering of entries...
     *     and be prepared for double-reporting or skips as insertion and deletion occur.
     * @return the node which currenly has the specified index, or null if index is greater than or
     *     equal to getLength().
     */
     ...
     * @return the Node (of unspecified sub-class) stored with that name, or null if no value has
     ...
     *
     * <p>Retrieves a node specified by local name and namespace URI.
     *
     ...
     * @return Node A Node (of any type) with the specified name, or null if the specified name did
     *     not identify any node in the map.
     */
     ...
     * @return If the new Node replaces an existing node the replaced Node is returned, otherwise
     *     null is returned.
     ...
     *     namespace URI and local name is already present in the map, it is replaced by the new
     *     one.
    ...
    /**
     * Cloning a NamedNodeMap is a DEEP OPERATION; it always clones all the nodes contained in the
     * map.
     */
    ...
    /**
     * Internal subroutine to allow read-only Nodes to make their contained NamedNodeMaps readonly
     * too. I expect that in fact the shallow version of this operation will never be
     *
     * @param readOnly boolean true to make read-only, false to permit editing.
     * @param deep boolean true to pass this request along to the contained nodes, false to only
     *     toggle the NamedNodeMap itself. I expect that the shallow version of this operation will
     *     never be used, but I want to design it in now, while I'm thinking about it.
     */
     ...
     *     reconvertable by subtracting it from -1. (Encoding because I don't want to recompare the
     *     strings but don't want to burn bytes on a datatype to hold a flagged value.)
     */
            ...
            if (namespaceURI == null) {
                if (aNamespaceURI == null
                        && (name.equals(aLocalName)
                                || (aLocalName == null && name.equals(a.getNodeName())))) return i;
            } else {
