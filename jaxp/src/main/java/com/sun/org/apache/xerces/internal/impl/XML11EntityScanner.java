 ...
 * @author Neil Graham, IBM @LastModified: Aug 2021
 */
public class XML11EntityScanner extends XMLEntityScanner {
    ...
    protected int scanChar(NameType nt) throws IOException {
        ...
        if (c == '\n'
                || ((c == '\r' || c == 0x85 || c == 0x2028)
                        && (external = fCurrentEntity.isExternal()))) {
    ...
    /**
     * Returns a string matching the NCName production appearing immediately on the input as a
     * symbol, or null if no NCName string is present.
     *
     * <p><strong>Note:</strong> The NCName characters are consumed.
     *
     * <p><strong>Note:</strong> The string returned must be a symbol. The SymbolTable can be used
     * for this purpose.
     *
    ...
    /**
     * Scans a range of parsed character data, setting the fields of the XMLString structure,
     * appropriately.
     *
        ...
        if (external) {
            ...
            while (fCurrentEntity.position < fCurrentEntity.count) {
                ...
                if ((c == quote && !fCurrentEntity.literal)
                        || c == '%'
                        || !XML11Char.isXML11InternalEntityContent(c)
                        || c == '\r') {
                    fCurrentEntity.position--;
                    break;
                }
        ...
        do {
            ...
            while (fCurrentEntity.position < fCurrentEntity.count) {
                ...
                // as literals so do not skip over them.
                        ...
                        // Control characters are allowed to appear as literals in internal
                        // entities.
        ...
        if (fCurrentEntity.isExternal()) {
            if (XML11Char.isXML11Space(c)) {
                do {
                    ...
                    checkEntityLimit(
                            null, fCurrentEntity, offset, fCurrentEntity.position - offset);
