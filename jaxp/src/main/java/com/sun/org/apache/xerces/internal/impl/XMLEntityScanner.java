 ...
 * @author K.Venugopal Sun Microsystems @LastModified: Sep 2021
 */
public class XMLEntityScanner implements XMLLocator {
    ...
    /**
     * Debug printing of buffer. This debugging flag works best when you resize the
     * DEFAULT_BUFFER_SIZE down to something reasonable like 64 characters.
     */
    ...
    /** Default constructor. */
    public XMLEntityScanner() {} // <init>()

    /**
     * private constructor, this class can only be instantiated within this class. Instance of this
     * class should be obtained using getEntityScanner() or getEntityScanner(ScannedEntity
     * scannedEntity)
     *
    ...
    public final void reset(
            SymbolTable symbolTable, XMLEntityManager entityManager, XMLErrorReporter reporter) {
        fCurrentEntity = null;
    ...
    /**
     * Returns the XML version of the current entity. This will normally be the value from the XML
     * or text declaration or defaulted by the parser. Note that that this value may be different
     * than the version of the processing rules applied to the current entity. For instance, an XML
     * 1.1 document may refer to XML 1.0 entities. In such a case the rules of XML 1.1 are applied
     * to the entire document. Also note that, for a given entity, this value can only be considered
     * final once the XML or text declaration has been read or once it has been determined that
    ...
    /**
     * Sets the XML version. This method is used by the scanners to report the value of the version
     * pseudo-attribute in an XML or text declaration.
     *
    ...
    public final void setCurrentEntity(Entity.ScannedEntity scannedEntity) {
        ...
        if (fCurrentEntity != null) {
            ...
            if (DEBUG_BUFFER) System.out.println("Current Entity is " + scannedEntity.name);
        }
    }
    ...
    /**
     * Returns the base system identifier of the currently scanned entity, or null if none is
     * available.
     */
    ...
    public final int getCharacterOffset() {
        return fCurrentEntity != null
                ? fCurrentEntity.fTotalCountTillLastLoad + fCurrentEntity.position
                : -1;
    }
    ...
    public final String getLiteralSystemId() {
        return (fCurrentEntity != null && fCurrentEntity.entityLocation != null)
                ? fCurrentEntity.entityLocation.getLiteralSystemId()
                : null;
    }
    ...
    /**
     * Returns the encoding of the current entity. Note that, for a given entity, this value can
     * only be considered final once the encoding declaration has been read (or once it has been
     * determined that there is no such declaration) since, no encoding having been specified on the
     * XMLInputSource, the parser will make an initial "guess" which could be in error.
     */
    ...
    /**
     * Sets the encoding of the scanner. This method is used by the scanners if the XMLDecl or
     * TextDecl line contains an encoding pseudo-attribute.
     *
     * <p><strong>Note:</strong> The underlying character reader on the current entity will be
     * changed to accomodate the new encoding. However, the new encoding is ignored if the current
     * reader was not constructed from an input stream (e.g. an external entity that is resolved
     * directly to the appropriate java.io.Reader object).
     *
    ...
    public final void setEncoding(String encoding) throws IOException {
        ...
        if (fCurrentEntity.stream != null) {
            ...
            //       a single char! -Ac
            if (fCurrentEntity.encoding == null || !fCurrentEntity.encoding.equals(encoding)) {
                ...
                if (fCurrentEntity.encoding != null
                        && fCurrentEntity.encoding.startsWith("UTF-16")) {
                    String ENCODING = encoding.toUpperCase(Locale.ENGLISH);
                    if (ENCODING.equals("UTF-16")) return;
                    if (ENCODING.equals("ISO-10646-UCS-4")) {
                        if (fCurrentEntity.encoding.equals("UTF-16BE")) {
                            fCurrentEntity.reader =
                                    new UCSReader(fCurrentEntity.stream, UCSReader.UCS4BE);
                        } else {
                            fCurrentEntity.reader =
                                    new UCSReader(fCurrentEntity.stream, UCSReader.UCS4LE);
                        }
                        return;
                    }
                    if (ENCODING.equals("ISO-10646-UCS-2")) {
                        if (fCurrentEntity.encoding.equals("UTF-16BE")) {
                            fCurrentEntity.reader =
                                    new UCSReader(fCurrentEntity.stream, UCSReader.UCS2BE);
                        } else {
                            fCurrentEntity.reader =
                                    new UCSReader(fCurrentEntity.stream, UCSReader.UCS2LE);
                        }
                        return;
                    }
                }
                ...
                if (DEBUG_ENCODINGS) {
                    System.out.println(
                            "$$$ creating new reader from stream: " + fCurrentEntity.stream);
                }
                ...
                if (DEBUG_ENCODINGS) System.out.println("$$$ reusing old reader on stream");
            }
        }
    } // setEncoding(String)
     ...
     *
     * <p><strong>Note:</strong> The character is <em>not</em> consumed.
     *
     ...
     * Returns the next character on the input.
     *
     * <p><strong>Note:</strong> The character is consumed.
     *
    ...
    /**
     * Returns a string matching the NMTOKEN production appearing immediately on the input as a
     * symbol, or null if NMTOKEN Name string is present.
     *
     * <p><strong>Note:</strong> The NMTOKEN characters are consumed.
     *
     * <p><strong>Note:</strong> The string returned must be a symbol. The SymbolTable can be used
     * for this purpose.
     *
     * @throws IOException Thrown if i/o error occurs.
     * @throws EOFException Thrown on end of file.
    ...
    /**
     * Returns a string matching the Name production appearing immediately on the input as a symbol,
     * or null if no Name string is present.
     *
     * <p><strong>Note:</strong> The Name characters are consumed.
     *
     * <p><strong>Note:</strong> The string returned must be a symbol. The SymbolTable can be used
     * for this purpose.
     *
    ...
    /**
     * Scans a qualified name from the input, setting the fields of the QName structure
     * appropriately.
     *
     * <p><strong>Note:</strong> The qualified name characters are consumed.
     *
     * <p><strong>Note:</strong> The strings used to set the values of the QName structure must be
     * symbols. The SymbolTable can be used for this purpose.
     *
     * @param qname The qualified name structure to fill.
     * @param nt The type of the name (element or attribute)
     * @return Returns true if a qualified name appeared immediately on the input and was scanned,
     *     false otherwise.
     * @throws IOException Thrown if i/o error occurs.
     * @throws EOFException Thrown on end of file.
     * @see com.sun.org.apache.xerces.internal.util.SymbolTable
    ...
    /**
     * Checks whether the end of the entity buffer has been reached. If yes, checks against the
     * limit and buffer size before loading more characters.
     *
     ...
     * @return the length of characters scanned before the end of the buffer, zero if there is more
     *     to be read in the buffer
     */
    protected int checkBeforeLoad(Entity.ScannedEntity entity, int offset, int nameOffset)
            throws IOException {
        int length = 0;
    ...
    /**
     * If the current entity is an Entity reference, counts the total nodes in the entity and checks
     * the accumulated value against the limit.
     *
     * @param entity The current entity
     */
    ...
    protected void checkLimit(Limit limit, ScannedEntity entity, int offset, int length) {
        ...
        if (fSecurityManager.isOverLimit(limit, fLimitAnalyzer)) {
            ...
            Object[] e =
                    (limit == Limit.ENTITY_REPLACEMENT_LIMIT)
                            ? new Object[] {
                                fLimitAnalyzer.getValue(limit),
    ...
    /**
     * CHANGED: Scans a range of parsed character data, This function appends the character data to
     * the supplied buffer.
     *
     * <p><strong>Note:</strong> The characters are consumed.
     *
     * <p><strong>Note:</strong> This method does not guarantee to return the longest run of parsed
     * character data. This method may return before markup due to reaching the end of the input
     * buffer or any other reason.
     *
     * <p>
     *
     * @param content The content structure to fill.
     * @return Returns the next character on the input, if known. This value may be -1 but this does
     *     <em>note</em> designate end of file.
     * @throws IOException Thrown if i/o error occurs.
     * @throws EOFException Thrown on end of file.
     */
    ...
    /**
     * Scans a range of attribute value data, setting the fields of the XMLString structure,
     * appropriately.
     *
     * <p><strong>Note:</strong> The characters are consumed.
     *
     * <p><strong>Note:</strong> This method does not guarantee to return the longest run of
     * attribute value data. This method may return before the quote character due to reaching the
     * end of the input buffer or any other reason.
     *
     * <p><strong>Note:</strong> The fields contained in the XMLString structure are not guaranteed
     * to remain valid upon subsequent calls to the entity scanner. Therefore, the caller is
     * responsible for immediately using the returned character data or making a copy of the
     * character data.
     *
     * @param quote The quote character that signifies the end of the attribute value data.
     * @param content The content structure to fill.
    ...
    protected int scanLiteral(int quote, XMLString content, boolean isNSURI) throws IOException {
        ...
        // normalize newlines
        if (whiteSpaceInfoNeeded) whiteSpaceLen = 0;
        ...
        for (; fCurrentEntity.position < fCurrentEntity.count; fCurrentEntity.position++) {
            c = fCurrentEntity.ch[fCurrentEntity.position];
            if ((c == quote && (!fCurrentEntity.literal || isExternal))
                    || c == '%'
                    || !XMLChar.isContent(c)
                    || c == '\r' && !isExternal) {
                break;
    ...
    /**
     * Save whitespace information. Increase the whitespace buffer by 100 when needed.
     *
     * <p>For XML 1.0, legal characters below 0x20 are 0x09 (TAB), 0x0A (LF) and 0x0D (CR).
     *
    ...
    // CHANGED:
    /**
     * Scans a range of character data up to the specified delimiter, setting the fields of the
     * XMLString structure, appropriately.
     *
     * <p><strong>Note:</strong> The characters are consumed.
     *
     * <p><strong>Note:</strong> This assumes that the delimiter contains at least one character.
     *
     * <p><strong>Note:</strong> This method does not guarantee to return the longest run of
     * character data. This method may return before the delimiter due to reaching the end of the
     * input buffer or any other reason.
     *
     * <p>
     *
     * @param delimiter The string that signifies the end of the character data to be scanned.
     * @param buffer The XMLStringBuffer to fill.
     * @param chunkLimit the size limit of the data to be scanned. Zero by default indicating no
     *     limit.
    ...
    protected boolean scanData(String delimiter, XMLStringBuffer buffer, int chunkLimit)
        ...
        do {
            ...
            if (fCurrentEntity.position > fCurrentEntity.count - delimLen) {
                // something must be wrong with the input:  e.g., file ends in an unterminated
                // comment
     ...
     *
     * <p><strong>Note:</strong> The character is consumed only if it matches the specified
     * character.
     *
     ...
     *
     * <p><strong>Note:</strong> The characters are consumed only if they are space characters.
     *
    ...
    /**
     * @param length This function checks that following number of characters are available. to the
     *     underlying buffer.
     ...
     *
     * <p><strong>Note:</strong> The characters are consumed only if all the characters are skipped.
     *
    ...
    protected boolean skipString(String s) throws IOException {
        ...
        if (arrangeCapacity(length, false)) {
            ...
            if (DEBUG_SKIP_STRING) {
                ...
                System.out.println(
                        "Buffer string to be skipped = "
                                + new String(fCurrentEntity.ch, beforeSkip, length));
            }
     ...
     *
     * @param offset The offset into the character buffer to read the next batch of characters.
     * @param changeEntity True if the load should change entities at the end of the entity,
     *     otherwise leave the current entity in place and the entity boundary will be signaled by
     *     the return value.
     * @param notify Determine whether to notify listeners of the event
     * @returns Returns true if the entity changed as a result of this load operation.
     */
     ...
     * @param encoding The encoding name that the input stream is encoded using. If the user has
     *     specified that Java encoding names are allowed, then the encoding name may be a Java
     *     encoding name; otherwise, it is an ianaEncoding name.
     * @param isBigEndian For encodings (like uCS-4), whose names cannot specify a byte order, this
     *     tells whether the order is bigEndian. null menas unknown or not relevant.
     ...
     * @return a 2-element array: the first element, an IANA-encoding string, the second element a
     *     Boolean which is true iff the document is big endian, false if it's little-endian, and
     *     null if the distinction isn't relevant.
     */
    ...
    /**
     * xxx not removing endEntity() so that i remember that we need to implement it. Ends an entity.
     *
     ...
     *
     * @param listener listener to which call back should be provided when scanner buffer is being
     *     changed.
     */
    ...
    /**
     * Skips space characters appearing immediately on the input that would match non-terminal S
     * (0x09, 0x0A, 0x0D, 0x20) before end of line normalization is performed. This is useful when
     * scanning structures such as the XMLDecl and TextDecl that can only contain US-ASCII
     * characters.
     *
     * <p><strong>Note:</strong> The characters are consumed only if they would match non-terminal S
     * before end of line normalization is performed.
     *
     * @return Returns true if at least one space character was skipped.
     * @throws IOException Thrown if i/o error occurs.
     * @throws EOFException Thrown on end of file.
    ...
    /**
     * Normalizes newlines. As specified in XML specification, this method converts newlines, '\n',
     * '\r' and '\r\n' to '\n' as 2.11 End-of-Line Handling. Further, it may put them in a cache for
     * later process as needed, for example as specified in 3.3.3 Attribute-Value
     * Normalization. @ImplNote this method does not limit to processing external parsed entities as
     * 2.11 required. It handles all cases where newlines need to be processed.
     *
     * @param buffer the current content buffer
     * @param append a flag indicating whether to append to the buffer
     * @param storeWS a flag indicating whether the whitespaces need to be stored for later
     *     processing
     * @param nt the type of the entity
     * @return true if the cursor is at the end of the current entity, false otherwise.
     * @throws IOException
     */
    protected boolean normalizeNewlines(
            short version, XMLString buffer, boolean append, boolean storeWS, NameType nt)
            throws IOException {
        // normalize newlines
        offset = fCurrentEntity.position;
