 ...
 * @author Neeraj Bajaj, Sun Microsystems, inc. @LastModified: May 2021
 */
public class XMLSchemaValidator
    ...
    public void reset(XMLComponentManager componentManager) throws XMLConfigurationException {
        ...
        try {
            fValidationState.setUnparsedEntityChecking(
                    componentManager.getFeature(UNPARSED_ENTITY_CHECKING));
        } catch (XMLConfigurationException e) {
    ...
    public XSElementDecl getGlobalElementDecl(QName element) {
        final SchemaGrammar sGrammar =
                findSchemaGrammar(XSDDescription.CONTEXT_ELEMENT, element.uri, null, element, null);
        if (sGrammar != null) {
    ...
    Augmentations handleStartElement(QName element, XMLAttributes attributes, Augmentations augs) {
        ...
        if (!fUseGrammarPoolOnly) {
            ...
            // store the location hints..  we need to do it so that we can defer the loading of
            // grammar until
            // there is a reference to a component from that namespace. To provide location hints to
            // the
        ...
        if (fCurrentCM != null) {
            ...
            if (fCurrCMState[0] == XSCMValidator.FIRST_ERROR) {
                ...
                if (ctype.fParticle != null
                    ...
                    String elemExpandedQname =
                            (element.uri != null)
                                    ? "{" + '"' + element.uri + '"' + ":" + element.localpart + "}"
                                    : element.localpart;
                    if (occurenceInfo != null) {
                        ...
                        if (count < minOccurs) {
                            ...
                            if (required > 1) {
                                reportSchemaError(
                                        ...
                                        new Object[] {
                                            element.rawname,
                                            fCurrentCM.getTermName(occurenceInfo[3]),
                                            Integer.toString(minOccurs),
                        ...
                        // Check if this is a violation of maxOccurs
                            ...
                            reportSchemaError(
                                    ...
                                    new Object[] {elemExpandedQname, expected});
                        }
                    } else {
                        reportSchemaError(
                                "cvc-complex-type.2.4.a",
                    ...
                    if (occurenceInfo != null) {
                        ...
                        if (count >= maxOccurs && maxOccurs != SchemaSymbols.OCCURRENCE_UNBOUNDED) {
                            ...
                            reportSchemaError(
                                    "cvc-complex-type.2.4.d", new Object[] {element.rawname});
                        }
                    } else {
            ...
            for (int i = oldCount - 1; i >= newCount; i--) {
                ...
                if (matcher instanceof Selector.Matcher) {
                    ...
                    if ((id = selMatcher.getIdentityConstraint()) != null
                        ...
                        // fields are present.
                        if (values != null && values.fHasValue) values.endDocumentFragment();
                    }
                }
            }
    ...
    void storeLocations(String sLocation, String nsLocation) {
        if (sLocation != null) {
            if (!XMLSchemaLoader.tokenizeSchemaLocationStr(
                    sLocation,
                    fLocationPairs,
                    fLocator == null ? null : fLocator.getExpandedSystemId())) {
                // error!
                ...
                try {
                    nsLocation =
                            XMLEntityManager.expandSystemId(
                                    nsLocation, fLocator.getExpandedSystemId(), false);
    ...
    SchemaGrammar findSchemaGrammar(
        ...
        if (!fUseGrammarPoolOnly
                && (grammar == null
                        || (fNamespaceGrowth
                                && !hasSchemaComponent(
                                        grammar, contextType, triggeringComponent)))) {
    ...
    private boolean hasSchemaComponent(
            SchemaGrammar grammar, short contextType, QName triggeringComponent) {
        ...
        if (fCurrentType != null) {
            ...
            if (!XSConstraints.checkTypeDerivationOk(type, fCurrentType, block)) {
                reportSchemaError(
                        "cvc-elt.4.3",
                        new Object[] {
                            element.rawname, xsiType, XS10TypeHelper.getSchemaTypeName(fCurrentType)
                        });
            }
        }
    ...
    void processOneAttribute(
        ...
        if (actualValue != null && currDecl.getConstraintType() == XSConstants.VC_FIXED) {
            if (!ValidatedInfo.isComparable(fValidatedInfo, currDecl.fDefault)
        ...
        if (actualValue != null
            ...
            if (!ValidatedInfo.isComparable(fValidatedInfo, currUse.fDefault)
                ...
                if (!isSpecified) {
                    if (currDecl.fTargetNamespace != null) {
                        reportSchemaError(
                                ...
                                new Object[] {
                                    element.rawname, currDecl.fName, currDecl.fTargetNamespace
                                });
                    } else {
                        reportSchemaError(
                                "cvc-complex-type.4",
                    ...
                    // lexical representation of the {value constraint} value.
                        ...
                        if (actualValue != null
                                && (!ValidatedInfo.isComparable(
                                                fValidatedInfo, fCurrentElemDecl.fDefault)
                                        || !actualValue.equals(
                                                fCurrentElemDecl.fDefault.actualValue))) {
                            reportSchemaError(
                                    "cvc-elt.5.2.2.2.2",
    ...
    void processRootTypeQName(final javax.xml.namespace.QName rootTypeQName) {
        ...
        if (SchemaSymbols.URI_SCHEMAFORSCHEMA.equals(rootTypeNamespace)) {
            ...
            final SchemaGrammar grammarForRootType =
                    findSchemaGrammar(
    ...
    void processRootElementDeclQName(
            final javax.xml.namespace.QName rootElementDeclQName, final QName element) {
        ...
        final SchemaGrammar grammarForRootElement =
                findSchemaGrammar(
        ...
        if (fCurrentElemDecl == null) {
            String declName =
                    (rootElementDeclQName.getPrefix().equals(XMLConstants.DEFAULT_NS_PREFIX))
                            ...
                            : rootElementDeclQName.getPrefix()
                                    + ":"
                                    + rootElementDeclQName.getLocalPart();
    ...
    void checkElementMatchesRootElementDecl(
            final XSElementDecl rootElementDecl, final QName element) {
    ...
    protected abstract class ValueStoreBase implements ValueStore {
         ...
         * @param itemValueType If the value is a list, a list of types for each of the values in
         *     the list.
         */
    ...
    protected class ValueStoreCache {
        ...
        /**
         * Values stores associated to specific identity constraints. This map maps
        ...
        public void startElement() {
            ...
            if (fGlobalIDConstraintMap.size() > 0)
                fGlobalMapStack.push(
                        (Map<IdentityConstraint, ValueStoreBase>)
                                ((HashMap<IdentityConstraint, ValueStoreBase>)
                                                fGlobalIDConstraintMap)
                                        .clone());
