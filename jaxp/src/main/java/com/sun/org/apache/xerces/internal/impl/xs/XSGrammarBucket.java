...
public class XSGrammarBucket {
    ...
    /**
     * Put a schema grammar into the registry This method is for internal use only: it assumes that
     * a grammar with the same target namespace is not already in the bucket.
     *
    ...
    /**
     * put a schema grammar and any grammars imported by it (directly or inderectly) into the
     * registry. when a grammar with the same target namespace is already in the bucket, and
     * different from the one being added, it's an error, and no grammar will be added into the
     * bucket.
     *
     * @param grammar the grammar to put in the registry
    ...
    public boolean putGrammar(SchemaGrammar grammar, boolean deep) {
        ...
        // to the same vector
        ArrayList<SchemaGrammar> currGrammars =
                (ArrayList<SchemaGrammar>) grammar.getImportedGrammars();
        ...
        for (int i = 0; i < grammars.size(); i++) {
            ...
            if (sg2 == null) {
                ...
                for (int j = gs.size() - 1; j >= 0; j--) {
                    sg2 = gs.get(j);
                    if (!grammars.contains(sg2)) grammars.add(sg2);
                }
            }
        ...
        putGrammar(grammar);
        for (int i = grammars.size() - 1; i >= 0; i--) putGrammar(grammars.get(i));
     ...
     * put a schema grammar and any grammars imported by it (directly or inderectly) into the
     * registry. when a grammar with the same target namespace is already in the bucket, and
     * different from the one being added, no grammar will be added into the bucket.
     *
     * @param grammar the grammar to put in the registry
     * @param deep whether to add imported grammars
     * @param ignoreConflict whether to ignore grammars that already exist in the grammar bucket or
     *     not - including 'grammar' parameter.
     * @return whether the process succeeded
     */
