 ...
 *
 * @xerces.internal
 * @author Andy Clark, IBM @LastModified: July 2019
 */
public class XPathMatcher {
    ...
    protected static final boolean DEBUG_ANY =
            DEBUG_METHODS || DEBUG_METHODS2 || DEBUG_METHODS3 || DEBUG_MATCH || DEBUG_STACK;
    ...
    /**
     * No match depth. The value of this field will be zero while matching is successful for the
    ...
    /**
     * This method is called when the XPath handler matches the XPath expression. Subclasses can
     * override this method to provide default handling upon a match.
     */
                            ...
                            if (fCurrentStep[i] == steps.length) {
                                ...
                                if (j == i) {
                                    AttributePSVI attrPSVI =
                                            (AttributePSVI)
                                                    attributes
                                                            .getAugmentations(aIndex)
                                                            .getItem(Constants.ATTRIBUTE_PSVI);
                                    fMatchedString = attrPSVI.getSchemaValue().getActualValue();
                                    matched(
                                            fMatchedString,
                                            attrPSVI.getSchemaValue().getActualValueType(),
                                            attrPSVI.getSchemaValue().getListValueTypes(),
                                            false);
                                }
                            }
                            break;
     ...
     * @param type content type of this element. IOW, the XML schema type of the <tt>value</tt>.
     *     Note that this may not be the type declared in the element declaration, but it is "the
     *     actual type". For example, if the XML is &lt;foo xsi:type="xs:string">aaa&lt;/foo>, this
     *     parameter will be "xs:string".
     * @param nillable - nillable true if the element declaration is nillable.
     * @param value - actual value the typed value of the content of this element.
     */
