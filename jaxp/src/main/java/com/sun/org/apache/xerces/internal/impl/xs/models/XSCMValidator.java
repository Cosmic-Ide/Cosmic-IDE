...
public interface XSCMValidator {
     ...
     * @return element decl or wildcard decl that corresponds to the element from the Schema grammar
     */
    public Object oneTransition(
            QName elementName, int[] state, SubstitutionGroupHandler subGroupHandler);
     ...
     * check the n and m bounds.
     *
     * @return <code>null</code> if validation of bounds is successful. Returns a list of strings
     *     with error info if not. Even entries in list returned are error codes (used to look up
     *     properties) and odd entries are parameters to be passed when formatting error message.
     *     Each parameter is associated with the error code that proceeds it in the list.
     */
    public List<String> checkMinMaxBounds();

    /**
     * Returns an array containing information about the current repeating term or <code>null</code>
     * if no occurrence counting was being performed at the current state.
     *
     ...
     *   <li>a[0] :: min occurs
     *   <li>a[1] :: max occurs
     *   <li>a[2] :: current value of the counter
     *   <li>a[3] :: identifier for the repeating term
     * </ul>
     *
     * @param state the current state
    ...
    /**
     * Checks if this content model has had its min/maxOccurs values reduced for purposes of
     * speeding up UPA. If so, this content model should not be used for any purpose other than
     * checking unique particle attribution
     *
