 ...
 * navigation and type, while leaving data management and construction to the derived
 * classes. @LastModified: Oct 2017
 */
public abstract class DTMDefaultBase implements DTM {
     ...
     * namespace. The second lookup is the local name, and the last array contains the the first
     * free element at the start, and the list of element handles following.
     */
    ...
    /**
     * Find the first index that occurs in the list that is greater than or equal to the given
     * value.
     *
     * @param list A list of integers.
     * @param start The start index to begin the search.
     * @param len The number of items to search.
     * @param value Find the slot that has a value that is greater than or identical to this
     *     argument.
     * @return The index in the list of the slot that is higher or identical to the identity
     *     argument, or -1 if no node is higher or equal.
     */
     ...
     *
     * <p>%REVIEW% KNOWN GLITCH: If you pass it a node index rather than a node handle, it works
     * just fine... but the displayed identity number before the colon is different, which
     * complicates comparing it with nodes printed the other way. We could always OR the DTM ID into
     * the value, to suppress that distinction...
     *
    ...
    /**
     * Given a node identity, return a node handle. If extended addressing has been used (multiple
     * DTM IDs), we need to map the high bits of the identity into the proper DTM ID.
     *
     ...
     *
     * <p>%REVIEW% Is it worth trying to specialcase small documents? %REVIEW% Should this be
     * exposed at the package/public layers?
     *
    ...
    /**
     * Given a node handle, return a node identity. If extended addressing has been used (multiple
     * DTM IDs), we need to map the high bits of the identity into the proper DTM ID and thence find
     * the proper offset to add to the low bits of the identity
     *
     * <p>This has been made FINAL to facilitate inlining, since we do not expect any subclass of
     * DTMDefaultBase to ever change the algorithm. (I don't really like doing so, and would love to
     * have an excuse not to...)
    ...
    /**
     * Given a node handle, advance to its next sibling. If not yet resolved, waits for more nodes
    ...
    /**
     * Given a node handle, advance to the next attribute. If an attr, we advance to the next attr
     * on the same node. If not an attribute, we return NULL.
     *
     * @param nodeHandle int Handle of the node.
    ...
    /**
     * Build table of namespace declaration locations during DTM construction. Table is
     * aArrayList<>of SuballocatedIntVectors containing the namespace node HANDLES declared at that
     * ID, plus an SuballocatedIntVector of the element node INDEXES at which these declarations
     * appeared.
     *
     * <p>NOTE: Since this occurs during model build, nodes will be encountered in doucment order
     * and thus the table will be ordered by element, permitting binary-search as a possible
     * retrieval optimization.
     *
     * <p>%REVIEW% Directly managed arrays rather than vectors? %REVIEW% Handles or IDs? Given
     * usage, I think handles.
     */
     ...
     *     which it would have appeared, encoded as -1-index and hence reconvertable by subtracting
     *     it from -1. (Encoding because I don't want to recompare the strings but don't want to
     *     burn bytes on a datatype to hold a flagged value.)
     */
    ...
    /**
     * Append a child to the end of the document. Please note that the node is always cloned if it
     * is owned by another document.
     *
