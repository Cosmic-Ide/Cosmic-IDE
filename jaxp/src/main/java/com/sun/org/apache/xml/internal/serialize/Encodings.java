...
class Encodings {
    ...
    static EncodingInfo getEncodingInfo(String encoding, boolean allowJavaNames)
        ...
        if (encoding == null) {
            if ((eInfo = _encodings.get(DEFAULT_ENCODING)) != null) return eInfo;
            eInfo =
                    new EncodingInfo(
        ...
        if (jName == null) {
            ...
            if (allowJavaNames) {
                ...
                if ((eInfo = _encodings.get(encoding)) != null) return eInfo;
        ...
        if ((eInfo = _encodings.get(jName)) != null) return eInfo;
        // have to create one...
        // is it known to be unicode-compliant?
        int i = 0;
        for (; i < UNICODE_ENCODINGS.length; i++) {
