...
/**
 * Implements an XML serializer supporting both DOM and SAX pretty serializing. For usage
 * instructions see {@link Serializer}.
 *
 ...
 *
 * <p>The serializer supports both DOM and SAX. SAX serializing is done by firing SAX events and
 * using the serializer as a document handler. DOM serializing is done by calling {@link
 * #serialize(Document)} or by using DOM Level 3 {@link org.w3c.dom.ls.LSSerializer} and serializing
 * with {@link org.w3c.dom.ls.LSSerializer#write}, {@link
...
public class XMLSerializer extends BaseMarkupSerializer {
    ...
    /**
     * Controls whether namespace fixup should be performed during the serialization. NOTE: if this
     * field is set to true the following fields need to be initialized: fNSBinder, fLocalNSBinder,
     * fSymbolTable, XMLSymbols.EMPTY_STRING, fXmlSymbol, fXmlnsSymbol
     */
    ...
    public void startElement(
        ...
        if (DEBUG) {
            System.out.println(
                    "==>startElement(" + namespaceURI + "," + localName + "," + rawName + ")");
        }
     ...
     * must name the root element, but the root element is only known when that element is
     * serialized, and not at the start of the document.
     *
        ...
        if (!fNamespaces) { // no namespace fixup should be performed
            ...
            if (uri != null) { // Element has a namespace
                ...
                if (fNSBinder.getURI(prefix) == uri) {
                    ...
                    // Thus the case where element was declared with uri="" (with or without a
                    // prefix)
                ...
                if (uri != null) { // attribute has namespace !=null
                    ...
                    if (uri != null && uri.equals(NamespaceContext.XMLNS_URI)) {
                        ...
                        if (prefix == XMLSymbols.PREFIX_XMLNS) { // xmlns:prefix
                            ...
                            if (value.length() != 0) {
                                if (localUri == null) {
                                    // declaration was not printed while fixing element namespace
                                    // binding
                                    ...
                                    // redeclare "xx" prefix. Instead we would pick up different
                                    // prefix
                                    // for the attribute.
                    ...
                    printAttribute(
                            name,
                            (value == null) ? XMLSymbols.EMPTY_STRING : value,
                            attr.getSpecified(),
                            attr);
    ...
    protected void checkUnboundNamespacePrefixedNode(Node node) throws IOException {
        ...
        if (fNamespaces) {
            ...
            if (DEBUG) {
                ...
                System.out.println(
                        "==>Declared Prefix Count: " + fNSBinder.getDeclaredPrefixCount());
                System.out.println("==>Node Name: " + node.getNodeName());
                System.out.println(
                        "==>First Child Node Name: " + node.getFirstChild().getNodeName());
                System.out.println(
                        "==>First Child Node Prefix: " + node.getFirstChild().getPrefix());
                System.out.println(
                        "==>First Child Node NamespaceURI: "
            ...
            for (child = node.getFirstChild(); child != null; child = next) {
                ...
                if (DEBUG) {
                    System.out.println(
                            "==>serializeNode(" + child.getNodeName() + ") [Child Node]");
                    System.out.println(
                            "==>serializeNode(" + child.getPrefix() + ") [Child Node Prefix]");
                }
