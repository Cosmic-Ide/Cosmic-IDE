...
/**
 * This class extends org.xml.sax.helpers.AttributesImpl which implements org. xml.sax.Attributes.
 * But for optimization this class adds a Map for faster lookup of an index by qName, which is
 * commonly done in the stream serializer.
 *
...
public final class AttributesImplSerializer extends AttributesImpl {
    /**
     * Hash table of qName/index values to quickly lookup the index of an attributes qName. qNames
     * are in uppercase in the hash table to make the search case insensitive.
     *
     * <p>The keys to the hashtable to find the index are either "prefix:localName" or
     * "{uri}localName".
     */
    ...
    /**
     * This is the number of attributes before switching to the hash table, and can be tuned, but 12
     * seems good for now - Brian M.
     */
    private static final int MAX = 12;

    /** One less than the number of attributes before switching to the Map. */
    ...
    /**
     * This method adds the attribute, but also records its qName/index pair in the hashtable for
     * fast lookup by getIndex(qName).
     *
    ...
    /**
     * We are switching over to having a hash table for quick look up of attributes, but up until
     * now we haven't kept any information in the Map, so we now update the Map. Future additional
     * attributes will update the Map as they are added.
     *
     * @param numAtts
     */
    ...
    /**
     * This method sets the attributes, previous attributes are cleared, it also keeps the hashtable
     * up to date for quick lookup via getIndex(qName).
     *
