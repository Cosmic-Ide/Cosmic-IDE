...
/**
 * This class acts as a base class for the XML "serializers" and the stream serializers. It contains
 * a number of common fields and methods.
 *
...
public abstract class SerializerBase implements SerializationHandler, SerializerConstants {
    ...
    protected void fireEndElem(String name) throws org.xml.sax.SAXException {
        if (m_tracer != null) {
            ...
            m_tracer.fireGenerateEvent(
                    SerializerTrace.EVENTTYPE_ENDELEMENT, name, (Attributes) null);
        }
    }
    ...
    /**
     * True if a trailing "]]>" still needs to be written to be written out. Used to merge adjacent
     * CDATA sections
     */
    protected boolean m_cdataTagOpen = false;

    /**
     * All the attributes of the current element, collected from startPrefixMapping() calls, or
     * addAddtribute() calls, or from the SAX attributes in a startElement() call.
     */
    protected AttributesImplSerializer m_attributes = new AttributesImplSerializer();

    /**
     * Tells if we're in an EntityRef event, true if it's greater than 0. Use integer type to handle
     * nested entity reference, increase m_inEntityRef in startEntity, decrease m_inEntityRef in
     * endEntity.
     */
    ...
    /**
     * Flag to tell that we need to add the doctype decl, which we can't do until the first element
     * is encountered.
     */
    ...
    /**
     * Namespace support, that keeps track of currently defined prefix/uri mappings. As processed
     * elements come and go, so do the associated mappings for that element.
     */
    protected NamespaceMappings m_prefixMap;

    /**
     * Handle for firing generate events. This interface may be implemented by the referenced
     * transformer object.
     */
    ...
    /**
     * The writer to send output to. This field is only used in the ToStream serializers, but exists
     * here just so that the fireStartDoc() and other fire... methods can flush this writer when
     * tracing.
     */
    protected java.io.Writer m_writer = null;

    /**
     * A reference to "stack frame" corresponding to the current element. Such a frame is pushed at
     * a startElement() and popped at an endElement(). This frame contains information about the
     * element, such as its namespace URI.
     */
     ...
     * "ns?:otherprefix:name", this function patches the qname by simply ignoring "otherprefix".
     * TODO: This method is a HACK! We do not have access to the XML file, it sometimes generates a
     * NS prefix of the form "ns?" for an attribute.
     */
    ...
    /**
     * Returns the local name of a qualified name. If the name has no prefix, then it works as the
     * identity (SAX2).
     *
     ...
     *     <p>Receive an object for locating the origin of SAX document events.
     *     <p>SAX parsers are strongly encouraged (though not absolutely required) to supply a
     *     locator: if it does so, it must supply the locator to the application by invoking this
     *     method before invoking any of the other methods in the DocumentHandler interface.
     ...
     *     content that does not match an application's business rules). The information returned by
     *     the locator is probably not sufficient for use with a search engine.
     *     <p>Note that the locator will return correct information only during the invocation of
     *     the events in this interface. The application should not attempt to use it at any other
     *     time.
     */
    ...
    /**
     * Adds the given attribute to the set of collected attributes , but only if there is a
     * currently open element.
     *
     * <p>An element is currently open if a startElement() notification has occured but the start of
     * the element has not yet been written to the output. In the stream case this means that we
     * have not yet been forced to close the elements opening tag by another notification, such as a
     ...
     * @return true if the attribute was added, false if an existing value was replaced.
     */
    ...
    /**
     * Adds the given attribute to the set of collected attributes, but only if there is a currently
     * open element.
     *
    ...
    /**
     * Adds the given xsl:attribute to the set of collected attributes, but only if there is a
     * currently open element.
     *
    ...
    /**
     * Add the given attributes to the currently collected ones. These attributes are always added,
     * regardless of whether on not an element is currently open.
     *
    ...
    /**
     * Return a {@link ContentHandler} interface into this serializer. If the serializer does not
     * support the {@link ContentHandler} interface, it should return null.
     *
    ...
    public void endEntity(String name) throws org.xml.sax.SAXException {
        if (name.equals("[dtd]")) m_inExternalDTD = false;

        if (!m_inExternalDTD) m_inEntityRef--;

        if (m_tracer != null) this.fireEndEntity(name);
    }
    ...
    /**
     * Flush and close the underlying java.io.Writer. This method applies to ToStream serializers,
     * not ToSAXHandler serializers.
     *
     * @see ToStream
     */
    public void close() {
    ...
    /**
     * Returns the previously set value of the value to be used as the public identifier in the
     ...
     *
     * @param doctypePublic the public identifier to be used in the DOCTYPE declaration in the
     *     output document.
     */
    ...
    /**
     * Returns the previously set value of the value to be used as the system identifier in the
     * document type declaration (DTD).
     *
     ...
     *
     * @param doctypeSystem the system identifier to be used in the DOCTYPE declaration in the
     *     output document.
     */
     ...
     *
     * @param standalone a value of "yes" indicates that the <code>standalone</code> delaration is
     *     to be included in the output document. This method remembers if the value was explicitly
     *     set using this method, verses if the value is the default value.
     */
    ...
    /**
     * Sets the XSL standalone attribute, but does not remember if this is a default or explicite
     * setting.
     *
     ...
     *
     * @return a value of "yes" if the <code>standalone</code> delaration is to be included in the
     *     output document.
     ...
     *
     * @param doIndent true if the output document should be indented to visually indicate its
     *     structure.
    ...
    /**
     * This method is used when a prefix/uri namespace mapping is indicated after the element was
     * started with a startElement() and before and endElement(). startPrefixMapping(prefix,uri)
     * would be used before the startElement() call.
     *
     ...
     * @return returns the prefix of the qualified name, or null if there is no prefix.
     */
    ...
    /**
     * Returns the URI of an element or attribute. Note that default namespaces do not apply
    ...
    /**
     * This method gets the nodes value as a String and uses that String as if it were an input
     * character notification.
     *
    ...
    /**
     * This method is only used internally when flushing the writer from the various fire...() trace
     * events. Due to the writer being wrapped with SerializerTraceWriter it may cause the flush of
     * these trace events: EVENTTYPE_OUTPUT_PSEUDO_CHARACTERS EVENTTYPE_OUTPUT_CHARACTERS which
     * trace the output written to the output stream.
     */
    ...
    protected void fireCommentEvent(char[] chars, int start, int length)
        ...
        if (m_tracer != null) {
            flushMyWriter();
            m_tracer.fireGenerateEvent(
                    SerializerTrace.EVENTTYPE_COMMENT, new String(chars, start, length));
        }
    }
    ...
    /**
     * Report the start element trace event. This trace method needs to be called just before the
    ...
    protected void fireStartElem(String elemName) throws org.xml.sax.SAXException {
        if (m_tracer != null) {
            flushMyWriter();
            m_tracer.fireGenerateEvent(
                    SerializerTrace.EVENTTYPE_STARTELEMENT, elemName, m_attributes);
        }
    }
    ...
    protected void fireEntityReference(String name) throws org.xml.sax.SAXException {
        if (m_tracer != null) {
            flushMyWriter();
            m_tracer.fireGenerateEvent(
                    SerializerTrace.EVENTTYPE_ENTITYREF, name, (Attributes) null);
        }
    }
    ...
    /**
     * This method handles what needs to be done at a startDocument() call, whether from an external
     * caller, or internally called in the serializer. For historical reasons the serializer is
     * flexible to startDocument() not always being called. Even if no external call is made into
     * startDocument() this method will always be called as a self generated internal startDocument,
     * it handles what needs to be done at a startDocument() call.
     *
     * <p>This method exists just to make sure that startDocument() is only ever called from an
     * external caller, which in principle is just a matter of style.
     *
     * @throws SAXException
     */
    ...
    protected void setDocumentInfo() {
        ...
        try {
            ...
            if (strVersion != null) setVersion(strVersion);
    ...
    /**
     * This method is used to set the source locator, which might be used to generated an error
     * message.
     *
     ...
     *
     * <p>This concept is made clear in the XSLT 2.0 draft.
     */
    ...
    /**
     * This method adds an attribute the the current element, but should not be used for an
     * xsl:attribute child.
     *
    ...
    /**
     * The CDATA section names stored in a whitespace separateed list with each element being a word
     * of the form "{uri}localName" This list comes from the cdata-section-elements attribute.
     *
     * <p>This field replaces m_cdataSectionElements Vector.
     */
            ...
            for (int i = 0; i < max; i++) {
                ...
                if (Character.isWhitespace(c)) {
                    if (!inCurly) {
                        ...
                        continue;
                    } else buf.append(c); // add whitespace to the URI
    ...
    /**
     * Return true if the current element in m_elemContext is a CDATA section. CDATA sections are
     * specified in the <xsl:output> attribute cdata-section-names or in the JAXP equivalent
     * property. In any case the format of the value of such a property is:
     *
     * <pre>
     * "{uri1}localName1 {uri2}localName2 . . . "
     * </pre>
     *
     * <p>This method is not a public API, but is only used internally by the serializer.
     */
    protected boolean isCdataSection() {
        ...
        if (null != m_StringOfCDATASections) {
            ...
            if (m_elemContext.m_elementURI == null) {
                ...
                } else if (m_elemContext.m_elementLocalName.length()
                        < m_elemContext.m_elementName.length()) {
                    // We were told the URI was "", yet the name has a prefix since the name is
                    // longer than the localname.
     ...
     * After this call it is non-null, but possibly "" meaning that it is in the default namespace.
     *
    ...
    /**
     * Get the value of an output property, the explicit value, if any, otherwise the default value,
     * if any, otherwise null.
     */
    public String getOutputProperty(String name) {
        String val = getOutputPropertyNonDefault(name);
        // If no explicit value, try to get the default value
        if (val == null) val = getOutputPropertyDefault(name);
        return val;
    }
    /**
     * Get the value of an output property, not the default value. If there is a default value, but
     * no non-default value this method will return null.
     *
     * <p>
     */
    ...
    /**
     * Return a {@link DOM3Serializer} interface into this serializer. If the serializer does not
     * support the {@link DOM3Serializer} interface, it should return null.
     *
     * @return A {@link DOM3Serializer} interface into this serializer, or null if the serializer is
     *     not DOM capable
     * @throws IOException An I/O exception occured
    ...
    /**
     * Get the default value of an xsl:output property, which would be null only if no default value
    ...
    /**
     * Set the value for the output property, typically from an xsl:output element, but this does
     * not change what the default value is.
     */
    ...
    /**
     * Set the default value for an output property, but this does not impact any explicitly set
     * value.
     */
    ...
    /**
     * A mapping of keys to explicitly set values, for example if and <xsl:output/> has an
     * "encoding" attribute, this map will have what that attribute maps to.
     */
    private HashMap<String, String> m_OutputProps;

    /**
     * A mapping of keys to default values, for example if the default value of the encoding is
     * "UTF-8" then this map will have that "encoding" maps to "UTF-8".
     */
    ...
    private String getProp(String name, boolean defaultVal) {
        ...
        String val;
        if (defaultVal) val = m_OutputPropsDefault.get(name);
        else val = m_OutputProps.get(name);
     ...
     * @param defaultVal true if this is a default value being set for the property as opposed to a
     *     user define on, set say explicitly in the stylesheet or via JAXP
     */
