...
/**
 * This class wraps another SerializationHandler. The wrapped object will either handler XML or
 * HTML, which is not known until a little later when the first XML tag is seen. If the first tag is
 * <html> then the wrapped object is an HTML handler, otherwise it is an XML handler.
 *
 * <p>This class effectively caches the first few calls to it then passes them on to the wrapped
 * handler (once it exists). After that subsequent calls a simply passed directly to the wrapped
 * handler.
 *
 * <p>The user of this class doesn't know if the output is ultimatley XML or HTML.
 *
 * <p>This class is not a public API, it is public because it is used within Xalan.
 *
 * @xsl.usage internal @LastModified: Feb 2021
 */
public final class ToUnknownStream extends SerializerBase {
    ...
    /**
     * A collection of namespace URI's (only for first element). _namespacePrefix has the matching
     * prefix for these URI's
     */
    private List<String> m_namespaceURI = null;

    /**
     * A collection of namespace Prefix (only for first element) _namespaceURI has the matching URIs
     * for these prefix'
     */
    ...
    /**
     * Default constructor. Initially this object wraps an XML Stream object, so _handler is never
     * null. That may change later to an HTML Stream object.
     */
        ...
        if (m_firstTagNotEmitted) {
            if (m_firstElementName != null && shouldFlush) {
                ...
                if (m_firstElementURI == null) {
                    if (prefix.equals(m_firstElementPrefix)) m_firstElementURI = uri;
                }
            }
        } else {
            pushed = m_handler.startPrefixMapping(prefix, uri, shouldFlush);
    ...
    /**
     * This method cannot be cached because default is different in HTML and XML (we need more than
     * a boolean).
     */
    ...
    public void startElement(
            String namespaceURI, String localName, String elementName, Attributes atts)
            throws SAXException {
        ...
        if (m_firstTagNotEmitted) {
            ...
            if (m_firstElementName != null) {
                ...
                if (m_tracer != null) firePseudoElement(elementName);
                 ...
                 */
                if (atts != null) super.addAttributes(atts);
    ...
    /**
     * Set the doctype in the underlying XML handler. Remember that this method was called, just in
     * case we need to transfer this doctype to an HTML handler
     *
    ...
    /**
     * Initialize the wrapped output stream (XML or HTML). If the stream handler should be HTML,
     * then replace the XML handler with an HTML handler. After than send the starting method calls
     * that were cached to the wrapped handler.
     */
     ...
     *
     * <p>Don't want to override static function on SerializerBase So added Unknown suffix to method
     * name.
     */
    ...
    /**
     * Determine if the firts element in the document is <html> or <HTML> This uses the cached first
     * element name, first element prefix and the cached namespaces from previous method calls
     *
    ...
    /**
     * @param URI_and_localNames a list of pairs of URI/localName specified in the
