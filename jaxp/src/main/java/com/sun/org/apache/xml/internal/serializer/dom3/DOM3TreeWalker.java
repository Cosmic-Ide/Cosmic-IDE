...
/**
 * Built on org.apache.xml.serializer.TreeWalker and adds functionality to traverse and serialize a
 * DOM Node (Level 2 or Level 3) as specified in the DOM Level 3 LS Recommedation by evaluating and
 * applying DOMConfiguration parameters and filters if any during serialization.
 *
...
final class DOM3TreeWalker {
    ...
    /**
     * The SerializationHandler, it extends ContentHandler and when this class is instantiated via
     * the constructor provided, a SerializationHandler object is passed to it.
     */
    ...
    DOM3TreeWalker(
        ...
        // fErrorHandler = errHandler == null ? new DOMErrorHandlerImpl() : errHandler; // Should we
     ...
     *
     * <p>Note that TreeWalker assumes that the subtree is intended to represent a complete (though
     * not necessarily well-formed) document and, during a traversal, startDocument and endDocument
     * will always be issued to the SAX listener.
     *
    ...
    public void traverse(Node pos) throws org.xml.sax.SAXException {
        ...
        if (fFilter != null) fWhatToShowFilter = fFilter.getWhatToShow();
     ...
     * @return True if the node is to be serialized else false if the node is to be rejected or
     *     skipped.
     */
            ...
            if (bStart) {
                try {
                    ...
                    throw new SAXException(
                            Utils.messages.createMessage(MsgKey.ER_WRITING_INTERNAL_SUBSET, null),
                            e);
                }
    ...
    protected void serializeElement(Element node, boolean bStart) throws SAXException {
        if (bStart) {
            ...
            fSerializer.startElement(
                    node.getNamespaceURI(), node.getLocalName(), node.getNodeName());
    ...
    protected void serializeAttList(Element node) throws SAXException {
        ...
        for (int i = 0; i < nAttrs; i++) {
                    ...
                    if (localName == null) {
                        ...
                        String msg =
                                Utils.messages.createMessage(
                                        MsgKey.ER_NULL_LOCAL_ELEMENT_NAME, new Object[] {attrName});
            ...
            } else if (addAttr && !xmlnsAttr) { // if the node is not a namespace node
                // If namespace-declarations=true, add the node with the Attr nodes namespaceURI
                // else add the node setting it's namespace to null or else the serializer will
                // later
            ...
            if (xmlnsAttr && ((fFeatures & NAMESPACEDECLS) != 0)) {
                ...
                String prefix =
                        (index = attrName.indexOf(":")) < 0 ? "" : attrName.substring(index + 1);
     ...
     *
     * <p>Check the string against XML's definition of acceptable names for elements and attributes
     * and so on using the XMLCharacterProperties utility class
     */
     ...
     * Taken from org.apache.xerces.dom.CoreDocumentImpl
     *
     * <p>Checks if the given qualified name is legal with respect to the version of XML to which
     * this document must conform.
     *
     ...
     * @param refInvalidChar A reference to the character to be returned that was determined
     *     invalid.
     */
    ...
    /**
     * Checks if a XML character is well-formed. If there is a problem with the character a non-null
     * Character is returned else null is returned.
     *
     * @param characters A String of characters to be checked for Well-Formedness
        ...
        if (fIsXMLVersion11) {
            ...
            while (i < datalength) {
                ...
                if (XML11Char.isXML11Invalid(c)) {
                    ...
                    String msg =
                            Utils.messages.createMessage(
                                    MsgKey.ER_WF_INVALID_CHARACTER_IN_COMMENT, new Object[] {c});
        ...
        if (value.indexOf('<') >= 0) {
            String msg =
                    Utils.messages.createMessage(
                            ...
                            new Object[] {
                                ((Attr) node).getOwnerElement().getNodeName(), node.getNodeName()
                            });
        ...
        if (invalidChar != null) {
            String msg =
                    Utils.messages.createMessage(
                            ...
                            new Object[] {
                                Integer.toHexString(
                                        Character.getNumericValue(invalidChar.charValue()))
                            });
     ...
     * Note that the presence of a CDATA termination mark in the contents of a CDATASection is
     * handled by the parameter spli-cdata-sections
     *
     * @param data The contents of the comment node
     */
     ...
     * on whether it is referenced in Element content or in an Attr Node, checks if the
     * EntityReference references an unparsed entity or a external entity and if so throws raises
     * the appropriate well-formedness error.
     *
     * @param data The contents of the comment node
    ...
    /**
     * If the configuration parameter "namespaces" is set to true, this methods checks if an entity
     * whose replacement text contains unbound namespace prefixes is referenced in a location where
     * there are no bindings for the namespace prefixes and if so raises a LSException with the
     * error-type "unbound-prefix-in-entity-reference"
     *
                ...
                if (prefix != null && fNSBinder.getURI(prefix) == null) {
                    String msg =
                            Utils.messages.createMessage(
                                    ...
                                    new Object[] {node.getNodeName(), child.getNodeName(), prefix});
                    ...
                    if (attrPrefix != null && fNSBinder.getURI(attrPrefix) == null) {
                        String msg =
                                Utils.messages.createMessage(
                                        ...
                                        new Object[] {
                                            node.getNodeName(), child.getNodeName(), attrs.item(i)
                                        });
            ...
            if ((inScopeNamespaceURI != null && inScopeNamespaceURI.equals(namespaceURI))) {
                ...
                if ((fFeatures & NAMESPACEDECLS) != 0) {
                    if ("".equals(prefix) || "".equals(namespaceURI)) {
                        ...
                        ((Element) node)
                                .setAttributeNS(
                                        XMLNS_URI, XMLNS_PREFIX + ":" + prefix, namespaceURI);
                    }
                }
    ...
    /**
     * This table is a quick lookup of a property key (String) to the integer that is the bit to
     * flip in the fFeatures field, so the integers should have values 1,2,4,8,16...
     */
    ...
    static {
        ...
        // cdata-sections
        featureMap.put(DOMConstants.S_DOM3_PROPERTIES_NS + DOMConstants.DOM_CDATA_SECTIONS, CDATA);

        // comments
        featureMap.put(DOMConstants.S_DOM3_PROPERTIES_NS + DOMConstants.DOM_COMMENTS, COMMENTS);
        ...
        // entities
        featureMap.put(DOMConstants.S_DOM3_PROPERTIES_NS + DOMConstants.DOM_ENTITIES, ENTITIES);

        // namespaces
        featureMap.put(DOMConstants.S_DOM3_PROPERTIES_NS + DOMConstants.DOM_NAMESPACES, NAMESPACES);
        ...
        featureMap.put(
                DOMConstants.S_DOM3_PROPERTIES_NS + DOMConstants.DOM_SPLIT_CDATA, SPLITCDATA);

        // discard-default-content
        featureMap.put(DOMConstants.S_DOM3_PROPERTIES_NS + DOMConstants.DOM_WELLFORMED, WELLFORMED);
     ...
     *
     * @param properties DOMConfiguraiton properties that were set and which are to be used while
     *     serializing the DOM.
     */
    protected void initProperties(Properties properties) {
        for (String key : properties.stringPropertyNames()) {
            ...
            if (bitFlag != null) {
                ...
                /**
                 * Other properties that have a bit more complex value than the features in the
                 * above map.
                 */
                if ((DOMConstants.S_DOM3_PROPERTIES_NS + DOMConstants.DOM_FORMAT_PRETTY_PRINT)
                        .equals(key)) {
                    // format-pretty-print; set internally on the serializers via xsl:output
                    // properties in LSSerializer
                    if ((properties.getProperty(key).endsWith("yes"))) {
                    ...
                    // omit-xml-declaration; set internally on the serializers via xsl:output
                    // properties in LSSerializer
                    if ((properties.getProperty(key).endsWith("yes"))) {
                ...
                } else if ((DOMConstants.S_XERCES_PROPERTIES_NS + DOMConstants.S_XML_VERSION)
                        .equals(key)) {
