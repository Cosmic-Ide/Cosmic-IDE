 ...
 *
 * <p>This is a generic utility for resolving URIs, other than the fact that it's declared to throw
 * TransformerException. Please see code comments for details on how resolution is performed. This
 * class is a copy of the one in com.sun.org.apache.xml.internal.utils. It exists to cut the
...
public final class SystemIDResolver {
     ...
     *
     * <p>The relative URI is a local filesystem path. The path can be absolute or relative. If it
     * is a relative path, it is resolved relative to the system property "user.dir" if it is
     * available; if not (i.e. in an Applet perhaps which throws SecurityException) then we just
     * return the relative path. The space and backslash characters are also replaced to generate a
     * good absolute URI.
     *
    ...
    public static boolean isAbsoluteURI(String systemId) {
        /**
         * http://www.ietf.org/rfc/rfc2396.txt Authors should be aware that a path segment which
         * contains a colon character cannot be used as the first segment of a relative URI path
         * (e.g., "this:that"), because it would be mistaken for a scheme name.
         */
        /**
         * %REVIEW% Can we assume here that systemId is a valid URI? It looks like we cannot ( See
         * discussion of this common problem in Bugzilla Bug 22777 ).
         */
    ...
    private static boolean isWindowsAbsolutePath(String systemId) {
        if (!isAbsolutePath(systemId)) return false;
        // On Windows, an absolute path starts with "[drive_letter]:\".
        if (systemId.length() > 2
                && systemId.charAt(1) == ':'
                && Character.isLetter(systemId.charAt(0))
                && (systemId.charAt(2) == '\\' || systemId.charAt(2) == '/')) return true;
        else return false;
    }

    /**
     * Replace spaces with "%20" and backslashes with forward slashes in the input string to
     * generate a well-formed URI string.
     *
                    ...
                    if (str.startsWith("///") || !str.startsWith("//")) {
                        ...
                        if (secondColonIndex > 0) {
                            ...
                            try {
                                if (!isAbsolutePath(localPath))
                                    absoluteURI =
                                            systemId.substring(0, secondColonIndex - 1)
    ...
    public static String getAbsoluteURI(String urlString, String base) throws TransformerException {
        if (base == null || base.length() == 0) return getAbsoluteURI(urlString);
