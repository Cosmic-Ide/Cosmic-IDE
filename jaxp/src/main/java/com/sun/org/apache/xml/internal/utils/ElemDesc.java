...
/**
 * This class is in support of SerializerToHTML, and acts as a sort of element representative for
 * HTML elements.
 *
 * @xsl.usage internal
 */
class ElemDesc {
     ...
     * This is used as a bitvector; more than one flag may be set at a time, bitwise-ORed together.
     * Mnemonic and bits have been assigned to the flag values. NOTE: Some bits are currently
     * assigned multiple mnemonics; it is the caller's responsibility to disambiguate these if
     * necessary.
     */
    int m_flags;
     ...
     *
     * <p>This might more properly be called areFlagsSet(). It accepts an integer (being used as a
     * bitvector) and checks whether all the corresponding bits are set in our internal flags. Note
     * that this test is performed as a bitwise AND, not an equality test, so a 0 bit in the input
     * means "don't test", not "must be set false".
     *
     * @param flags Vector of flags to compare against this element's flags
     * @return true if the flags set in the parameter are also set in the element's stored flags.
     * @see m_flags
     * @see isAttrFlagSet
     */
    ...
    void setAttr(String name, int flags) {
        ...
        if (null == m_attrs) m_attrs = new HashMap<>();
     ...
     * @return True if the flag is set in the attribute. Returns false if the attribute is not found
     * @see m_flags
     */
