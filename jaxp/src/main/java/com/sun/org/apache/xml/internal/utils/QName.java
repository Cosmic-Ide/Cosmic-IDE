 ...
 * template (see [7 Named Templates]), a mode (see [6.7 Modes]), an attribute set (see [8.1.4 Named
 * Attribute Sets]), a key (see [14.2 Keys]), a locale (see [14.3 Number Formatting]), a variable or
 * a parameter (see [12 Variables and Parameters]) is specified as a QName. If it has a prefix, then
 * the prefix is expanded into a URI reference using the namespace declarations in effect on the
 * attribute in which the name occurs. The expanded name consisting of the local part of the name
...
public class QName implements java.io.Serializable {
        ...
        if (localName == null)
            throw new IllegalArgumentException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_ARG_LOCALNAME_NULL,
                            null)); // "Argument 'localName' is null");
    ...
    public QName(String namespaceURI, String prefix, String localName, boolean validate) {
        ...
        if (validate) {
            ...
            if ((null != prefix) && (!XML11Char.isXML11ValidNCName(prefix))) {
                throw new IllegalArgumentException(
                        XMLMessages.createXMLMessage(
                                XMLErrorResources.ER_ARG_PREFIX_INVALID,
                                null)); // "Argument 'prefix' not a valid NCName");
            }
        }
        _namespaceURI = namespaceURI;
        _prefix = prefix;
            ...
            if (null == namespace) {
                throw new RuntimeException(
                        XMLMessages.createXMLMessage(
                                ...
                                new Object[] {
                                    prefix
                                })); // "Prefix must resolve to a namespace: "+prefix);
            }
        }
    ...
    /**
     * Construct a QName from a string, resolving the prefix using the given namespace context and
     ...
     */
    public QName(
            String qname, Element namespaceContext, PrefixResolver resolver, boolean validate) {
     ...
     * '{ns}foo' notation if not. Performs string concatenation, so beware of performance issues.
     *
     ...
     * Performs string concatenation, so beware of performance issues.
     *
     * @return the string representation of the namespace
     */
