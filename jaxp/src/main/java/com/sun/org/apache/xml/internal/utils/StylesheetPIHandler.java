 ...
 *
 * @see <a href="http://www.w3.org/TR/xml-stylesheet/">Associating Style Sheets with XML documents,
 *     Version 1.0</a> @LastModified: Oct 2017
 */
public class StylesheetPIHandler extends DefaultHandler {
    ...
    /**
     * Get the object that will be used to resolve URIs in href in xml-stylesheet processing
     * instruction.
     *
    ...
    /**
     * Construct a StylesheetPIHandler instance that will search for xml-stylesheet PIs based on the
     * given criteria.
     *
     ...
     *
     * @return Source object that references the last stylesheet reference that matches the
     *     constraints.
     */
        ...
        if (target.equals("xml-stylesheet")) {
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                if (tokenizer.hasMoreTokens()
                        && (token.equals(" ") || token.equals("\t") || token.equals("="))) continue;
    ...
    /**
     * The spec notes that "The xml-stylesheet processing instruction is allowed only in the prolog
     * of an XML document.", so, at least for right now, I'm going to go ahead an throw a
     * TransformerException in order to stop the parse.
     *
     * @param namespaceURI The Namespace URI, or an empty string.
     * @param localName The local name (without prefix), or empty string if not namespace
     *     processing.
     * @param qName The qualified name (with prefix).
     * @param atts The specified or defaulted attributes.
     * @throws StopParseException since there can be no valid xml-stylesheet processing instructions
