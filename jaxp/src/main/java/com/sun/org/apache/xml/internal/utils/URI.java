 ...
 * parsing of URIs and provide access to the various components (scheme, host, port, userinfo, path,
 * query string and fragment) that may constitute a URI.
 *
 * <p>Parsing of a URI specification is done according to the URI syntax described in RFC 2396
 * <http://www.ietf.org/rfc/rfc2396.txt?number=2396>. Every URI consists of a scheme, followed by a
 * colon (':'), followed by a scheme-specific part. For URIs that follow the "generic URI" syntax,
 * the scheme- specific part begins with two slashes ("//") and may be followed by an authority
 * segment (comprised of user information, host, and port), path segment, query segment and
 * fragment. Note that RFC 2396 no longer specifies the use of the parameters segment and excludes
 * the "user:password" syntax as part of the authority segment. If "user:password" appears in a URI,
 * the entire user/password string is stored as userinfo.
 *
 * <p>For URIs that do not follow the "generic URI" syntax (e.g. mailto), the entire scheme-specific
 * part is treated as the "path" portion of the URI.
 *
 * <p>Note that, unlike the java.net.URL class, this class does not provide any built-in network
 ...
 * of operations that can be applied to a URI. @LastModified: Oct 2017
 */
public class URI implements Serializable {
    ...
    /**
     * URI punctuation mark characters - these, combined with alphanumerics, constitute the
    ...
    /**
     * Construct a new URI from a URI specification string. If the specification follows the
     * "generic URI" syntax, (two slashes following the first colon), the specification will be
     * parsed accordingly - setting the scheme, userinfo, host,port, path, query string and fragment
     * fields as necessary. If the specification does not follow the "generic URI" syntax, the
     ...
     *     fragment will be set from the path; however, if the query and fragment are specified both
     *     in the path and as separate parameters, an exception is thrown
     ...
     * @throws MalformedURIException if any of the parameters violates syntax rules or semantic
     *     rules
     */
    public URI(
            String p_scheme, String p_host, String p_path, String p_queryString, String p_fragment)
    ...
    public URI(
        ...
        if (p_scheme == null || p_scheme.trim().length() == 0) {
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_SCHEME_REQUIRED, null)); // "Scheme is required!");
        }
        ...
        if (p_host == null) {
            ...
            if (p_port != -1) {
                throw new MalformedURIException(
                        XMLMessages.createXMLMessage(
                                XMLErrorResources.ER_NO_PORT_IF_NO_HOST,
                                null)); // "Port may not be specified if host is not specified!");
            }
        }
    ...
    /**
     * Initializes this URI from a base URI and a URI specification string. See RFC 2396 Section 4
     * and Appendix B for specifications on parsing the URI and Section 5 for specifications on
     * resolving relative URIs and relative paths.
     *
    ...
    private void initialize(URI p_base, String p_uriSpec) throws MalformedURIException {
        ...
        if (p_base == null && (p_uriSpec == null || p_uriSpec.trim().length() == 0)) {
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_CANNOT_INIT_URI_EMPTY_PARMS,
        ...
        if (colonIndex < 0) {
            if (p_base == null) {
                throw new MalformedURIException(
                        XMLMessages.createXMLMessage(
                                XMLErrorResources.ER_NO_SCHEME_IN_URI,
            ...
            if (m_scheme != null && p_base != null) {
                ...
                //
                // b) Similarily, according to RFC 2396, uri is resolved for <uriSpec> relative to
                // <p_base>
                //    if scheme in <uriSpec> is same as scheme in <p_base>, else p_base can be
                // ignored.
                //
                // c) if <p_base> is not hierarchical, it can be ignored.
                //
                if (uriSpec.startsWith("/")
                        || !m_scheme.equals(p_base.m_scheme)
    ...
    private void initializeScheme(String p_uriSpec) throws MalformedURIException {
        ...
        if (scheme.length() == 0) {
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_NO_SCHEME_INURI,
        ...
        while (index < end) {
            ...
            if (testChar == '%') {
                if (index + 2 >= end
                    ...
                    throw new MalformedURIException(
                            XMLMessages.createXMLMessage(
                                    XMLErrorResources.ER_PATH_CONTAINS_INVALID_ESCAPE_SEQUENCE,
                ...
                if ('\\' != testChar)
                    throw new MalformedURIException(
                            XMLMessages.createXMLMessage(
                                    XMLErrorResources.ER_PATH_INVALID_CHAR,
                                    new Object[] {
     ...
     *
     * @param p_includeQueryString if true (and query string is not null), then a "?" followed by
     ...
     *
     * @return the fragment for this URI. Null is returned if there was no "#" in the URI spec,
     *     empty string if there was a "#" but no fragment following it.
     */
    ...
    public void setScheme(String p_scheme) throws MalformedURIException {
        ...
        if (p_scheme == null) {
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_SCHEME_FROM_NULL_STRING,
        ...
        if (!isConformantSchemeName(p_scheme)) {
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_SCHEME_NOT_CONFORMANT,
    ...
    /**
     * Set the host for this URI. If null is passed in, the userinfo field is also set to null and
    ...
    public void setHost(String p_host) throws MalformedURIException {
        ...
        if (p_host == null || p_host.trim().length() == 0) {
            ...
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_HOST_ADDRESS_NOT_WELLFORMED,
    ...
    public void setPort(int p_port) throws MalformedURIException {
        ...
        if (p_port >= 0 && p_port <= 65535) {
            if (m_host == null) {
                throw new MalformedURIException(
                        XMLMessages.createXMLMessage(
                                XMLErrorResources.ER_PORT_WHEN_HOST_NULL,
                                null)); // "Port cannot be set when host is null!");
            }
        } else if (p_port != -1) {
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_INVALID_PORT, null)); // "Invalid port number!");
        }
    ...
    /**
     * Set the path for this URI. If the supplied path is null, then the query string and fragment
     * are set to null as well. If the supplied path includes a query string and/or fragment, these
     * fields will be parsed and set as well. Note that, for URIs following the "generic URI"
     * syntax, the path specified should start with a slash. For URIs that do not follow the generic
    ...
    /**
     * Append to the end of the path of this URI. If the current path does not end in a slash and
     * the path to be appended does not begin with a slash, a slash will be appended to the current
     * path before the new segment is added. Also, if the current path ends in a slash and the new
     * segment begins with a slash, the extra slash will be removed before the new segment is
     * appended.
     *
    ...
    /**
     * Set the query string for this URI. A non-null value is valid only if this is an URI
     * conforming to the generic URI syntax and the path value is not null.
     *
     * @param p_queryString the query string for this URI
     * @throws MalformedURIException if p_queryString is not null and this URI does not conform to
    ...
    public void setFragment(String p_fragment) throws MalformedURIException {
        ...
        if (p_fragment == null) {
            ...
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_FRAG_FOR_GENERIC_URI,
                            null)); // "Fragment can only be set for a generic URI!");
        } else if (getPath() == null) {
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_FRAG_WHEN_PATH_NULL,
                            null)); // "Fragment cannot be set when path is null!");
        } else if (!isURIString(p_fragment)) {
            throw new MalformedURIException(
                    XMLMessages.createXMLMessage(
                            XMLErrorResources.ER_FRAG_INVALID_CHAR,
     ...
     * the domain name of a network host. A valid IPv4 address consists of four decimal digit groups
     * separated by a '.'. A hostname consists of domain labels (each of which must begin and end
     * with an alphanumeric but may contain '-') separated & by a '.'. See RFC 2396 Section 3.2.2.
     *
     ...
     * 2396). uric consist of all reserved characters, unreserved characters and escaped characters.
     *
