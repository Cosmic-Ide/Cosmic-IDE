...
/**
 * Serves as common interface for axes Walkers, and stores common state variables. @LastModified:
 * Oct 2017
 */
public class AxesWalker extends PredicatedNodeTest
    ...
    /**
     * Do a deep clone of this walker, including next and previous walkers. If the this AxesWalker
     * is on the clone list, don't clone but return the already cloned version.
     *
     ...
     *     even vectors.
     * @return non-null clone, which may be a new clone, or may be a clone contained on the
     *     cloneList.
     */
    ...
    /**
     * The root node of the TreeWalker, as specified in setRoot(int root). Note that this may
     * actually be below the current node.
     *
     ...
     * The value must not be null. Alterations to the DOM tree may cause the current node to no
     * longer be accepted by the TreeWalker's associated filter. currentNode may also be explicitly
     * set to any node, whether or not it is within the subtree specified by the root node or would
     * be accepted by the filter and whatToShow flags. Further traversal occurs relative to
     * currentNode even if it is not part of the current view by applying the filters in the
     * requested direction (not changing currentNode where no traversal is possible).
     *
     * @return The node at which the TreeWalker is currently positioned, only null if setRoot has
     *     not yet been called.
     */
     ...
     * search for nextNode attempts to step upward from the TreeWalker's root node, returns <code>
