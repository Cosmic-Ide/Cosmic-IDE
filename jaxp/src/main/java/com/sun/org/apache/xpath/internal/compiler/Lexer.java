...
/**
 * This class is in charge of lexical processing of the XPath expression into tokens. @LastModified:
 * June 2022
 */
class Lexer {
     ...
     * backwards. Each element that is a 'target', (right-most top level element name) has
    ...
    Lexer(
        ...
        /**
         * No limits if XML Security Manager is null. Applications using XPath through the public
         * API always have a XMLSecurityManager. Applications invoking the internal XPath API shall
         * consider using the public API instead.
         */
    ...
    void tokenize(String pat, List<String> targetStrings) throws TransformerException {
        ...
        if (0 == m_compiler.getTokenQueueSize()) {
            m_processor.error(
                    XPATHErrorResources.ER_EMPTY_EXPRESSION, null); // "Empty expression!");
    ...
    /**
     * Record the current position on the token queue as long as this is a top-level element. Must
     * be called before the next token is added to the m_tokenQueue.
     *
