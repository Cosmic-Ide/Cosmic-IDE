...
class FactoryFinder {
    ...
    /** Flag indicating if properties from java.home/conf/jaxp.properties have been cached. */
    ...
    static <T> T newInstance(
        ...
        try {
            ...
            throw new DatatypeConfigurationException("Provider " + className + " not found", x);
        } catch (Exception x) {
        ...
        try {
            if (firstTime) {
                synchronized (cacheProps) {
                    if (firstTime) {
                        String configFile =
                                SecuritySupport.getSystemProperty("java.home")
                                        + File.separator
                                        + "conf"
                                        + File.separator
                                        + "jaxp.properties";
            ...
            if (factoryClassName != null) {
                dPrint(
                        () ->
                                "found in ${java.home}/conf/jaxp.properties, value="
                                        + factoryClassName);
