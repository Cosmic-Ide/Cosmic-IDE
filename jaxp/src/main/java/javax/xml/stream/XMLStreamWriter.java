...
/**
 * The XMLStreamWriter interface specifies how to write XML. The XMLStreamWriter does not perform
 * well formedness checking on its input. However the writeCharacters method is required to escape
 * {@literal &, < and >} For attribute values the writeAttribute method will escape the above
 * characters plus {@literal "} to ensure that all character content and attribute values are well
 * formed.
 *
 * <p>Each NAMESPACE and ATTRIBUTE must be individually written.
 *
 * <table class="striped">
 ...
 *
 * <ul>
 *   <li>[1] if namespaceURI == default Namespace URI, then no prefix is written
 *   <li>[2] if prefix == "" || null {@literal &&} namespaceURI == "", then no prefix or Namespace
 *       declaration is generated or written
 *   <li>[3] if prefix == "" || null, then a prefix is randomly generated
 *   <li>[4] if prefix == "" || null, then it is treated as the default Namespace and no prefix is
 *       generated or written, an xmlns declaration is generated and written if the namespaceURI is
 *       unbound
 *   <li>[5] if prefix == "" || null, then it is treated as an invalid attempt to define the default
 *       Namespace and an XMLStreamException is thrown
 * </ul>
 *
 * @version 1.0
...
public interface XMLStreamWriter {
    ...
    /**
     * Writes a start tag to the output. All writeStartElement methods open a new scope in the
     * internal namespace context. Writing the corresponding EndElement causes the scope to be
     * closed.
     *
    ...
    /**
     * Writes string data to the output without checking for well formedness. The data is opaque to
     * the XMLStreamWriter, i.e. the characters are written blindly to the underlying output. If the
     * method cannot be supported in the currrent writing context the implementation may throw a
     * UnsupportedOperationException. For example note that any namespace declarations, end tags,
     * etc. will be ignored and could interfere with proper maintanence of the writers internal
     * state.
     *
    ...
    /**
     * Writes an end tag to the output relying on the internal state of the writer to determine the
     * prefix and local name of the event.
     *
     * @throws XMLStreamException if an error occurs
     */
    ...
    /**
     * Close this writer and free any resources associated with the writer. This must not close the
    ...
    /**
     * Writes a namespace to the output stream If the prefix argument to this method is the empty
     * string, "xmlns", or null this method will delegate to writeDefaultNamespace
     *
    ...
    /**
     * Write a DTD section. This string represents the entire doctypedecl production from the XML
     * 1.0 specification.
     *
     ...
     * of the underlying output. That must be set when the instance of the XMLStreamWriter is
     * created using the XMLOutputFactory
     *
     ...
     * calls to setPrefix and setDefaultNamespace will bind namespaces using the context passed to
     * the method as the root context for resolving namespaces. This method may only be called once
     * at the start of the document. It does not cause the namespaces to be declared. If a namespace
     * URI to prefix mapping is found in the namespace context it is treated as declared and the
     * prefix may be used by the StreamWriter.
     *
