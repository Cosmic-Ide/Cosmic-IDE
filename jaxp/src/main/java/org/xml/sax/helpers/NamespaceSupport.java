 ...
 *
 * <p>This class encapsulates the logic of Namespace processing: it tracks the declarations
 * currently in force for each context and automatically processes qualified XML names into their
 * Namespace parts; it can also be used in reverse for generating XML qnames from Namespaces.
 *
 * <p>Namespace support objects are reusable, but the reset method must be invoked between each
 * session.
 *
 ...
 * </pre>
 *
 * <p>Note that this class is optimized for the use case where most elements do not contain
 * Namespace declarations: if the same prefix/URI mapping is repeated for each context (for
 * example), this class will be somewhat less efficient.
 *
 * <p>Although SAX drivers (parsers) may choose to use this class to implement namespace handling,
 * they are not required to do so. Applications must track namespace information themselves if they
...
public class NamespaceSupport {
    ...
    /**
     * The XML Namespace URI as a constant. The value is <code>http://www.w3.org/XML/1998/namespace
     * </code> as defined in the "Namespaces in XML" * recommendation.
     *
     * <p>This is the Namespace URI that is automatically mapped to the "xml" prefix.
     */
    public static final String XMLNS = "http://www.w3.org/XML/1998/namespace";

    /**
     * The namespace declaration URI as a constant. The value is <code>http://www.w3.org/xmlns/2000/
     * </code>, as defined in a backwards-incompatible erratum to the "Namespaces in XML"
     * recommendation. Because that erratum postdated SAX2, SAX2 defaults to the original
     * recommendation, and does not normally use this URI.
     *
     * <p>This is the Namespace URI that is optionally applied to <em>xmlns</em> and
     * <em>xmlns:*</em> attributes, which are used to declare namespaces.
     *
     ...
     *
     * <p>It is necessary to invoke this method before reusing the Namespace support object for a
     * new session. If namespace declaration URIs are to be supported, that flag must also be set to
     * a non-default value.
     *
     * @see #setNamespaceDeclUris
     */
    ...
    /**
     * Start a new Namespace context. The new context will automatically inherit the declarations of
     * its parent context, but it will also keep track of which declarations were made within this
     * context.
     *
     * <p>Event callback code should start a new context once per element. This means being ready to
     ...
     *
     * <p>Normally, SAX drivers would push a new context at the beginning of each XML element. Then
    ...
    /**
     * Declare a Namespace prefix. All prefixes must be declared before they are referenced. For
     * example, a SAX driver (parser) would scan an element's attributes in two passes: first for
     * namespace declarations, then a second pass using {@link #processName processName()} to
     ...
     *
     * <p>To declare the default element Namespace, use the empty string as the prefix.
     *
     * <p>Note that there is an asymmetry in this library: {@link #getPrefix getPrefix} will not
     * return the "" prefix, even if you have declared a default element namespace. To check for a
     * default namespace, you have to look it up explicitly using {@link #getURI getURI}. This
     * asymmetry exists to make it easier to look up prefixes for attribute names, where the default
    ...
    /**
     * Process a raw XML qualified name, after all declarations in the current context have been
     * handled by {@link #declarePrefix declarePrefix()}.
     *
     * <p>This method processes a raw XML qualified name in the current context by removing the
     * prefix and looking it up among the prefixes currently declared. The return value will be the
     ...
     * </dl>
     *
     * <p>All of the strings in the array will be internalized. If the raw name has a prefix that
     * has not been declared, then the return value will be null.
     *
     * <p>Note that attribute names are processed differently than element names: an unprefixed
     * element name will receive the default Namespace (if any), while an unprefixed attribute name
     * will not.
     *
     ...
     *
     * <p><strong>Note:</strong> if there is a default prefix, it will not be returned in this
     * enumeration; check for the default prefix using the {@link #getURI getURI} with an argument
     * of "".
     *
     ...
     * @return one of the prefixes currently mapped to the URI supplied, or null if none is mapped
     *     or if the URI is assigned to the default namespace
    ...
    /**
     * Return an enumeration of all prefixes for a given URI whose declarations are active in the
     * current context. This includes declarations from parent contexts that have not been
     * overridden.
     *
     * <p>This method returns prefixes mapped to a specific Namespace URI. The xml: prefix will be
     * included. If you want only one prefix that's mapped to the Namespace URI, and you don't care
     * which one you get, use the {@link #getPrefix getPrefix} method instead.
     *
     * <p><strong>Note:</strong> the empty (default) prefix is <em>never</em> included in this
    ...
    /**
     * Controls whether namespace declaration attributes are placed into the {@link #NSDECL NSDECL}
     * namespace by {@link #processName processName()}. This may only be changed before any contexts
     * have been pushed.
     *
     * @param value a flag indicating whether namespace declaration attributes are placed into the
     *     {@link #NSDECL NSDECL} namespace
     * @since 1.5, SAX 2.1alpha
    ...
    public void setNamespaceDeclUris(boolean value) {
        ...
        if (value) currentContext.declarePrefix("xmlns", NSDECL);
        else {
            contexts[contextPos] = currentContext = new Context();
            currentContext.declarePrefix("xml", XMLNS);
        }
    ...
    /**
     * Returns true if namespace declaration attributes are placed into a namespace. This behavior
     * is not the default.
     *
     * @return true if namespace declaration attributes are placed into a namespace, false otherwise
     * @since 1.5, SAX 2.1alpha
     */
    ...
    final class Context {
        ...
        /**
         * (Re)set the parent of this Namespace context. The context must either have been freshly
         * constructed, or must have been cleared.
         *
        ...
        /**
         * Makes associated state become collectible, invalidating this context. {@link #setParent}
         * must be called before this context may be used again.
         */
        void clear() {
            parent = null;
            prefixTable = null;
         ...
         * @return An array of three strings containing the URI part (or empty string), the local
         *     part, and the raw name, all internalized, or null if there is an undeclared prefix.
         ...
         *
         * <p>Since many prefixes may be mapped to the same URI, the return value may be unreliable.
         *
         ...
         *
         * <p>The default prefix, if in force, is <em>not</em> returned, and will have to be checked
         * for separately.
         *
         * @return An enumeration of prefixes (never empty).
         ...
         *
         * <p>This class is optimized for the normal case where most elements do not contain
