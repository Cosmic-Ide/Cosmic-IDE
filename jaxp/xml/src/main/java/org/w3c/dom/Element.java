...
/**
 * The <code>Element</code> interface represents an element in an HTML or XML document. Elements may
 * have attributes associated with them; since the <code>Element</code> interface inherits from
 * <code>Node</code>, the generic <code>Node</code> interface attribute <code>attributes</code> may
 * be used to retrieve the set of all attributes for an element. There are methods on the <code>
 * Element</code> interface to retrieve either an <code>Attr</code> object by name or an attribute
 * value by name. In XML, where an attribute value may contain entity references, an <code>Attr
 * </code> object should be retrieved to examine the possibly fairly complex sub-tree representing
 ...
 *
 * <p><b>Note:</b> In DOM Level 2, the method <code>normalize</code> is inherited from the <code>
 * Node</code> interface where it was moved.
 *
...
public interface Element extends Node {
     ...
     * and <code>EntityReference</code> nodes, build the appropriate subtree, and use <code>
     * setAttributeNode</code> to assign it as the value of an attribute. <br>
     * To set an attribute with a qualified name and namespace URI, use the <code>setAttributeNS
     * </code> method.
     *
     ...
     * values from other schemas similarly but applications should use <code>
     * Document.normalizeDocument()</code> to guarantee this information is up-to-date. <br>
     * If no attribute with this name is found, this method has no effect. <br>
     * To remove an attribute by local name and namespace URI, use the <code>removeAttributeNS
     * </code> method.
     *
     ...
     * To retrieve an attribute node by qualified name and namespace URI, use the <code>
     * getAttributeNodeNS</code> method.
     *
     * @param name The name (<code>nodeName</code>) of the attribute to retrieve.
     * @return The <code>Attr</code> node with the specified name ( <code>nodeName</code>) or <code>
     *     null</code> if there is no such attribute.
     */
     ...
     * has no effect. <br>
     * To add a new attribute node with a qualified name and namespace URI, use the <code>
     * setAttributeNodeNS</code> method.
     *
     ...
     * @exception org.w3c.dom.DOMException WRONG_DOCUMENT_ERR: Raised if <code>newAttr</code> was
     *     created from a different document than the one that created the element. <br>
     ...
     *     <code>Element</code> object. The DOM user must explicitly clone <code>Attr</code> nodes
     ...
     * may handle default values from other schemas similarly but applications should use <code>
     * Document.normalizeDocument()</code> to guarantee this information is up-to-date.
     *
     ...
     * @return The <code>Attr</code> value as a string, or the empty string if that attribute does
     *     not have a specified or default value.
     * @exception org.w3c.dom.DOMException NOT_SUPPORTED_ERR: May be raised if the implementation
     *     does not support the feature <code>"XML"</code> and the language exposed through the
     ...
     * present on the element, its prefix is changed to be the prefix part of the <code>
     * qualifiedName</code>, and its value is changed to be the <code>value</code> parameter. This
     * value is a simple string; it is not parsed as it is being set. So any markup (such as syntax
     * to be recognized as an entity reference) is treated as literal text, and needs to be
     ...
     * @exception org.w3c.dom.DOMException INVALID_CHARACTER_ERR: Raised if the specified qualified
     *     name is not an XML name according to the XML version in use specified in the <code>
     *     Document.xmlVersion</code> attribute. <br>
     *     NO_MODIFICATION_ALLOWED_ERR: Raised if this node is readonly. <br>
     *     NAMESPACE_ERR: Raised if the <code>qualifiedName</code> is malformed per the Namespaces
     *     in XML specification, if the <code>qualifiedName</code> has a prefix and the <code>
     *     namespaceURI</code> is <code>null</code>, if the <code>qualifiedName</code> has a prefix
     *     that is "xml" and the <code>namespaceURI</code> is different from "<a
     *     href='http://www.w3.org/XML/1998/namespace'>http://www.w3.org/XML/1998/namespace</a>", if
     *     the <code>qualifiedName</code> or its prefix is "xmlns" and the <code>namespaceURI</code>
     *     is different from "<a
     ...
     *     <code>"XML"</code> and the language exposed through the Document does not support XML
     *     Namespaces (such as [<a href='http://www.w3.org/TR/1999/REC-html401-19991224/'>HTML
     *     4.01</a>]).
     * @since DOM Level 2
     */
     ...
     * use <code>Document.normalizeDocument()</code> to guarantee this information is up-to-date.
     * <br>
     ...
     *     local name and namespace URI, the replaced <code>Attr</code> node is returned, otherwise
     ...
     *     INUSE_ATTRIBUTE_ERR: Raised if <code>newAttr</code> is already an attribute of another
     *     <code>Element</code> object. The DOM user must explicitly clone <code>Attr</code> nodes
     *     to re-use them in other elements. <br>
     *     NOT_SUPPORTED_ERR: May be raised if the implementation does not support the feature
     *     <code>"XML"</code> and the language exposed through the Document does not support XML
     *     Namespaces (such as [<a href='http://www.w3.org/TR/1999/REC-html401-19991224/'>HTML
     *     4.01</a>]).
     ...
     * attribute to be a user-determined ID attribute . This affects the value of <code>Attr.isId
     * </code> and the behavior of <code>Document.getElementById</code>, but does not change any
     * schema that may be in use, in particular this does not affect the <code>Attr.schemaTypeInfo
     * </code> of the specified <code>Attr</code> node. Use the value <code>false</code> for the
     * parameter <code>isId</code> to undeclare an attribute for being a user-determined ID
     * attribute. <br>
     * To specify an attribute by local name and namespace URI, use the <code>setIdAttributeNS
     * </code> method.
     *
