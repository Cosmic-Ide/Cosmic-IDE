...
/**
 * The <code>CSSRule</code> interface is the abstract base interface for any type of CSS statement.
 * This includes both rule sets and at-rules. An implementation is expected to preserve all rules
 * specified in a CSS style sheet, even if the rule is not recognized by the parser. Unrecognized
...
public interface CSSRule {
     ...
     * methods can be used to cast down from an instance of the <code>CSSRule</code> interface to
     * the specific derived interface implied by the <code>type</code>.
     */
     ...
     *     type of rule than the current one. <br>
     *     HIERARCHY_REQUEST_ERR: Raised if the rule cannot be inserted at this point in the style
     *     sheet. <br>
