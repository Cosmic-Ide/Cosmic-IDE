 ...
 * one is in the class path, even when nothing bound its class name to <code>org.xml.sax.driver
 * </code> so those configuration mechanisms would see it.
 *
 * @since SAX 2.0
...
public final class XMLReaderFactory {
    ...
    /**
     * Attempt to create an XMLReader from system defaults. In environments which can support it,
     * the name of the XMLReader class is determined by trying each these options in order, and
     ...
     *       <em>META-INF/services/org.xml.sax.driver</em> file in jarfiles available to the
     *       runtime.
     *   <li>SAX parser distributions are strongly encouraged to provide a default XMLReader class
     *       name that will take effect only when previous options (on this list) are not
     *       successful.
     *   <li>Finally, if {@link org.xml.sax.helpers.ParserFactory#makeParser()} can return a system
     *       default SAX1 parser, that parser is wrapped in a {@link
     *       org.xml.sax.helpers.ParserAdapter}. (This is a migration aid for SAX1 environments,
     *       where the <code>org.xml.sax.parser</code> system property will often be usable.)
     * </ul>
     *
     ...
     * that calls to this method will succeed, except when security policies intervene. This will
     * also maximize application portability to older SAX environments, with less robust
    ...
    public static XMLReader createXMLReader() throws SAXException {
        ...
        if (className != null) return loadClass(cl, className);
        ...
        try {
            ...
            throw new SAXException(
                    "Can't create default XMLReader; "
