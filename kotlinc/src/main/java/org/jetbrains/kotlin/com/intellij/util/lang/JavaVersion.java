...
/**
 * A class representing a version of some Java platform - e.g. the runtime the class is loaded into,
 * or some installed JRE.
 *
 * <p>Based on <a href="http://openjdk.java.net/jeps/322">JEP 322 "Time-Based Release
 * Versioning"</a> (Java 10+), but also supports JEP 223 "New Version-String Scheme" (Java 9), as
 * well as earlier version's formats.
 *
...
public final class JavaVersion implements Comparable<JavaVersion> {
    /**
     * The major version. Corresponds to the first number of 9+ format (<b>9</b>.0.1) / the second
     * number of 1.x format (1.<b>8</b>.0_60).
     */
    public final int feature;

    /**
     * The minor version. Corresponds to the second number of 9+ format (9.<b>0</b>.1) / the third
     * number of 1.x format (1.8.<b>0</b>_60). Was used in version strings prior to 1.5, in newer
     * strings is always {@code 0}.
     */
    public final int minor;

    /**
     * The patch version. Corresponds to the third number of 9+ format (9.0.<b>1</b>) / the number
     * after an underscore of 1.x format (1.8.0_<b>60</b>).
     */
    public final int update;

    /**
     * The build number. Corresponds to a number prefixed by the "plus" sign in 9+ format
     * (9.0.1+<b>7</b>) / by "-b" string in 1.x format (1.8.0_60-b<b>12</b>).
     */
     ...
     */
    public static @NonNull JavaVersion compose(
            int feature, int minor, int update, int build, boolean ea)
     ...
     * {@code "java.runtime.version"} system property first (usually, it is more complete), and
     * falls back to {@code "java.version"} if the former is invalid or differs in {@link #feature}
     * or {@link #minor} numbers.
     */
     ...
     * - a second line of the above command (something like to "Java(TM) SE Runtime Environment
     * (build $VERSION)")<br>
     ...
     */
    public static @NonNull JavaVersion parse(@NonNull String versionString)
            throws IllegalArgumentException {
        // trimming
            ...
            try {
                ...
                if (feature >= 5 && feature < MAX_ACCEPTED_VERSION) {
                    ...
                    if (feature <= MAX_ACCEPTED_VERSION) {
                        if (numbers.size() > 2
                                && separators.size() > 2
                                && ".".equals(separators.get(2))) {
                            minor = Integer.parseInt(numbers.get(2));
                            if (numbers.size() > 3
                                    && separators.size() > 3
                                    && "_".equals(separators.get(3))) {
                                ...
                                if (separators.size() > 4) {
                                    ...
                                    if (s != null && s.length() != 0 && s.charAt(0) == '-') {
                                        ea =
                                                startsWithWord(s, "-ea")
                                                        || startsWithWord(s, "-internal");
                                    }
                                    p = 4;
                                    while (p < separators.size()
                                            && !separators.get(p).endsWith("-b")) p++;
                                    if (p < numbers.size()) {
                                        build = Integer.parseInt(numbers.get(p));
                                    }
                                }
