 ...
 */
public final class BasicBlocker implements com.pranav.ide.dx.cf.code.BytecodeArray.Visitor {
    /**
     * {@code non-null;} method being converted
     */
    private final com.pranav.ide.dx.cf.code.ConcreteMethod method;
     ...
     */
    private final com.pranav.ide.dx.util.IntList[] targetLists;
     ...
     */
    private final com.pranav.ide.dx.cf.code.ByteCatchList[] catchLists;
    ...
    private BasicBlocker(ConcreteMethod method) {
        ...
        workSet = com.pranav.ide.dx.util.Bits.makeBitSet(sz);
        liveSet = com.pranav.ide.dx.util.Bits.makeBitSet(sz);
        blockSet = com.pranav.ide.dx.util.Bits.makeBitSet(sz);
        targetLists = new com.pranav.ide.dx.util.IntList[sz];
        catchLists = new com.pranav.ide.dx.cf.code.ByteCatchList[sz];
        previousOffset = -1;
    }
     ...
     */
    public static ByteBlockList identifyBlocks(com.pranav.ide.dx.cf.code.ConcreteMethod method) {
    ...
    public void visitNoArgs(int opcode, int offset, int length, com.pranav.ide.dx.rop.type.Type type) {
        switch (opcode) {
            ...
            case com.pranav.ide.dx.cf.code.ByteOps.RETURN: {
                ...
                targetLists[offset] = com.pranav.ide.dx.util.IntList.EMPTY;
                break;
            }
            ...
            case com.pranav.ide.dx.cf.code.ByteOps.IREM: {
                 ...
                 */
                visitCommon(offset, length, true);
                if ((type == com.pranav.ide.dx.rop.type.Type.INT) || (type == com.pranav.ide.dx.rop.type.Type.LONG)) {
                    visitThrowing(offset, length, true);
                }
                break;
            }
    ...
    public void visitLocal(int opcode, int offset, int length,
        ...
        if (opcode == com.pranav.ide.dx.cf.code.ByteOps.RET) {
            visitCommon(offset, length, false);
            targetLists[offset] = com.pranav.ide.dx.util.IntList.EMPTY;
        } else {
            visitCommon(offset, length, true);
    ...
    public void visitConstant(int opcode, int offset, int length,
        ...
        if (cst instanceof CstMemberRef || cst instanceof com.pranav.ide.dx.rop.cst.CstType ||
    ...
    public void visitBranch(int opcode, int offset, int length,
        ...
        switch (opcode) {
            case com.pranav.ide.dx.cf.code.ByteOps.GOTO: {
                visitCommon(offset, length, false);
                targetLists[offset] = com.pranav.ide.dx.util.IntList.makeImmutable(target);
                break;
            }
            case ByteOps.JSR: {
                /*
            ...
            default: {
                ...
                targetLists[offset] = com.pranav.ide.dx.util.IntList.makeImmutable(next, target);
                break;
            }
        }
    ...
    private ByteBlockList getBlockList() {
        com.pranav.ide.dx.cf.code.BytecodeArray bytes = method.getCode();
        com.pranav.ide.dx.cf.code.ByteBlock[] bbs = new com.pranav.ide.dx.cf.code.ByteBlock[bytes.size()];
        int count = 0;

        for (int at = 0, next; /*at*/ ; at = next) {
            next = com.pranav.ide.dx.util.Bits.findFirst(blockSet, at + 1);
            if (next < 0) {
                break;
            }

            if (com.pranav.ide.dx.util.Bits.get(liveSet, at)) {
                /*
                 ...
                 */
                com.pranav.ide.dx.util.IntList targets = null;
                int targetsAt = -1;
                com.pranav.ide.dx.cf.code.ByteCatchList blockCatches;
                ...
                if (targets == null) {
                    ...
                    blockCatches = com.pranav.ide.dx.cf.code.ByteCatchList.EMPTY;
                } else {
    ...
    private void doit() {
        ...
        com.pranav.ide.dx.cf.code.ByteCatchList catches = method.getCatches();
        ...
        com.pranav.ide.dx.util.Bits.set(blockSet, 0);
         ...
         */
        while (!com.pranav.ide.dx.util.Bits.isEmpty(workSet)) {
            try {
            ...
            for (int i = 0; i < catchSz; i++) {
                com.pranav.ide.dx.cf.code.ByteCatchList.Item item = catches.get(i);
                int start = item.getStartPc();
                int end = item.getEndPc();
                if (com.pranav.ide.dx.util.Bits.anyInRange(liveSet, start, end)) {
                    com.pranav.ide.dx.util.Bits.set(blockSet, start);
                    com.pranav.ide.dx.util.Bits.set(blockSet, end);
    ...
    private void addWorkIfNecessary(int offset, boolean blockStart) {
        if (!com.pranav.ide.dx.util.Bits.get(liveSet, offset)) {
            com.pranav.ide.dx.util.Bits.set(workSet, offset);
        }

        if (blockStart) {
            com.pranav.ide.dx.util.Bits.set(blockSet, offset);
        }
    }
    ...
    private void visitCommon(int offset, int length, boolean nextIsLive) {
        com.pranav.ide.dx.util.Bits.set(liveSet, offset);
