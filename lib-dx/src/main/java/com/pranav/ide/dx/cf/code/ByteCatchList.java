...
public final class ByteCatchList extends FixedSizeList {
    ...
    private static boolean typeNotFound(Item item, Item[] arr, int count) {
        com.pranav.ide.dx.rop.cst.CstType type = item.getExceptionClass();

        for (int i = 0; i < count; i++) {
            com.pranav.ide.dx.rop.cst.CstType one = arr[i].getExceptionClass();
            if ((one == type) || (one == com.pranav.ide.dx.rop.cst.CstType.OBJECT)) {
                return false;
            }
        }
    ...
    public void set(int n, int startPc, int endPc, int handlerPc,
                    com.pranav.ide.dx.rop.cst.CstType exceptionClass) {
     ...
     */
    public com.pranav.ide.dx.util.IntList toTargetList(int noException) {
        if (noException < -1) {
        ...
        if (sz == 0) {
            if (hasDefault) {
                 ...
                 */
                return com.pranav.ide.dx.util.IntList.makeImmutable(noException);
            }
            /*
             * The list is empty and there isn't even a no-exception
             * address.
             */
            return com.pranav.ide.dx.util.IntList.EMPTY;
        }

        com.pranav.ide.dx.util.IntList result = new IntList(sz + (hasDefault ? 1 : 0));
    ...
    public TypeList toRopCatchList() {
        ...
        com.pranav.ide.dx.rop.type.StdTypeList result = new StdTypeList(sz);
    ...
    public static class Item {
         ...
         */
        private final com.pranav.ide.dx.rop.cst.CstType exceptionClass;
         ...
         *
         * @return {@code non-null;} the exception class; {@link com.pranav.ide.dx.rop.cst.CstType#OBJECT}
         * if this entry handles all possible exceptions
         */
        public com.pranav.ide.dx.rop.cst.CstType getExceptionClass() {
