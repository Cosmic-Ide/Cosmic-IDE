...
import com.pranav.ide.dx.rop.code.BasicBlock;
import com.pranav.ide.dx.rop.code.BasicBlockList;
import com.pranav.ide.dx.rop.code.Insn;
import com.pranav.ide.dx.rop.code.InsnList;
import com.pranav.ide.dx.rop.code.PlainCstInsn;
...
import com.pranav.ide.dx.rop.code.ThrowingInsn;
...
import com.pranav.ide.dx.util.IntList;
...
public final class Ropper {
     ...
     */
    private final com.pranav.ide.dx.cf.code.Simulator sim;
     ...
     */
    private final ArrayList<com.pranav.ide.dx.rop.code.BasicBlock> result;
     ...
     */
    private final ArrayList<com.pranav.ide.dx.util.IntList> resultSubroutines;
    ...
    private Ropper(ConcreteMethod method, TranslationAdvice advice, MethodList methods,
         ...
         */
        this.result = new ArrayList<com.pranav.ide.dx.rop.code.BasicBlock>(blocks.size() * 2 + 10);
        this.resultSubroutines =
                new ArrayList<com.pranav.ide.dx.util.IntList>(blocks.size() * 2 + 10);
    ...
    /**
     * Converts a {@link ConcreteMethod} to a {@link com.pranav.ide.dx.rop.code.RopMethod}.
     *
     * @param method  {@code non-null;} method to convert
     * @param advice  {@code non-null;} translation advice to use
     * @param methods {@code non-null;} list of methods defined by the class
     ...
     */
    public static com.pranav.ide.dx.rop.code.RopMethod convert(ConcreteMethod method,
                                                               com.pranav.ide.dx.rop.code.TranslationAdvice advice, MethodList methods, com.pranav.ide.dx.dex.DexOptions dexOptions) {
        try {
    ...
    private int getAvailableLabel() {
        ...
        for (com.pranav.ide.dx.rop.code.BasicBlock bb : result) {
    ...
    private boolean isSynchronized() {
        ...
        return (accessFlags & com.pranav.ide.dx.rop.code.AccessFlags.ACC_SYNCHRONIZED) != 0;
    }
     ...
     */
    private com.pranav.ide.dx.rop.code.RegisterSpec getSynchReg() {
        /*
         * We use the register that is just past the deepest possible
        ...
        return com.pranav.ide.dx.rop.code.RegisterSpec.make((reg < 1) ? 1 : reg, com.pranav.ide.dx.rop.type.Type.OBJECT);
    }
    ...
    private int labelToResultIndex(int label) {
        int sz = result.size();
        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.code.BasicBlock one = result.get(i);
     ...
     */
    private com.pranav.ide.dx.rop.code.BasicBlock labelToBlock(int label) {
        int idx = labelToResultIndex(label);

        if (idx < 0) {
            throw new IllegalArgumentException("no such label " +
                    com.pranav.ide.dx.util.Hex.u2(label));
        }
     ...
     */
    private void addBlock(com.pranav.ide.dx.rop.code.BasicBlock block, com.pranav.ide.dx.util.IntList subroutines) {
        if (block == null) {
     ...
     */
    private boolean addOrReplaceBlock(com.pranav.ide.dx.rop.code.BasicBlock block, com.pranav.ide.dx.util.IntList subroutines) {
        if (block == null) {
            throw new NullPointerException("block == null");
        }
     ...
     * {@code false} if it was added for the first time
     */
    private boolean addOrReplaceBlockNoDelete(com.pranav.ide.dx.rop.code.BasicBlock block,
                                              com.pranav.ide.dx.util.IntList subroutines) {
        if (block == null) {
            throw new NullPointerException("block == null");
        }
    ...
    private void removeBlockAndSpecialSuccessors(int idx) {
        ...
        com.pranav.ide.dx.rop.code.BasicBlock block = result.get(idx);
        com.pranav.ide.dx.util.IntList successors = block.getSuccessors();
    ...
    /**
     * Extracts the resulting {@link com.pranav.ide.dx.rop.code.RopMethod} from the instance.
     *
     * @return {@code non-null;} the method object
     */
    private com.pranav.ide.dx.rop.code.RopMethod getRopMethod() {

        // Construct the final list of blocks.

        int sz = result.size();
        com.pranav.ide.dx.rop.code.BasicBlockList bbl = new BasicBlockList(sz);
        for (int i = 0; i < sz; i++) {
     ...
     */
    private void doit() {
        int[] workSet = com.pranav.ide.dx.util.Bits.makeBitSet(maxLabel);

        com.pranav.ide.dx.util.Bits.set(workSet, 0);
        addSetupBlocks();
        setFirstFrame();
        ...
        for (; ; ) {
            ...
            com.pranav.ide.dx.util.Bits.clear(workSet, offset);
            ByteBlock block = blocks.labelToBlock(offset);
            Frame frame = startFrames[offset];
            try {
                processBlock(block, frame, workSet);
            } catch (SimException ex) {
                ex.addContext("...while working on block " + com.pranav.ide.dx.util.Hex.u2(offset));
                throw ex;
            }
        }
    ...
    private void setFirstFrame() {
        com.pranav.ide.dx.rop.type.Prototype desc = method.getEffectiveDescriptor();
    ...
    private void processBlock(ByteBlock block, Frame frame, int[] workSet) {
        ...
        com.pranav.ide.dx.cf.code.ByteCatchList catches = block.getCatches();
        ...
        ArrayList<com.pranav.ide.dx.rop.code.Insn> insns = machine.getInsns();
        ...
        for (int i = startSuccessorIndex; i < succSz;
            ...
            try {
                ...
                ex.addContext("...while merging to block " + com.pranav.ide.dx.util.Hex.u2(succ));
                throw ex;
            }
        }
        ...
        if ((succSz == 0) && machine.returns()) {
             ...
             */
            successors = com.pranav.ide.dx.util.IntList.makeImmutable(getSpecialLabel(RETURN));
            succSz = 1;
        }
        ...
        if (synch || (catchSz != 0)) {
            ...
            com.pranav.ide.dx.util.IntList newSucc = new com.pranav.ide.dx.util.IntList(succSz);
            for (int i = 0; i < catchSz; i++) {
                ByteCatchList.Item one = catches.get(i);
                com.pranav.ide.dx.rop.cst.CstType exceptionClass = one.getExceptionClass();
                ...
                try {
                    ...
                    ex.addContext("...while merging exception to block " +
                            com.pranav.ide.dx.util.Hex.u2(targ));
                    throw ex;
                }
            ...
            if (synch && !catchesAny) {
                ...
                for (int i = insnSz - extraBlockCount - 1; i < insnSz; i++) {
                    com.pranav.ide.dx.rop.code.Insn insn = insns.get(i);
                    if (insn.canThrow()) {
                        insn = insn.withAddedCatch(com.pranav.ide.dx.rop.type.Type.OBJECT);
                        insns.set(i, insn);
                    }
                }
            }
        ...
        for (/*extraBlockCount*/; extraBlockCount > 0; extraBlockCount--) {
             ...
             */
            com.pranav.ide.dx.rop.code.Insn extraInsn = insns.get(--insnSz);
            boolean needsGoto
                    = extraInsn.getOpcode().getBranchingness()
                    == com.pranav.ide.dx.rop.code.Rop.BRANCH_NONE;
            com.pranav.ide.dx.rop.code.InsnList il = new com.pranav.ide.dx.rop.code.InsnList(needsGoto ? 2 : 1);
            com.pranav.ide.dx.util.IntList extraBlockSuccessors = successors;

            il.set(0, extraInsn);

            if (needsGoto) {
                il.set(1, new com.pranav.ide.dx.rop.code.PlainInsn(com.pranav.ide.dx.rop.code.Rops.GOTO,
                        extraInsn.getPosition(), null,
                        com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY));
                /*
                 * Obviously, this block won't be throwing an exception
                 * so it should only have one successor.
                 */
                extraBlockSuccessors = com.pranav.ide.dx.util.IntList.makeImmutable(primarySucc);
            }
            il.setImmutable();

            int label = getAvailableLabel();
            com.pranav.ide.dx.rop.code.BasicBlock bb = new com.pranav.ide.dx.rop.code.BasicBlock(label, il, extraBlockSuccessors,
                    primarySucc);
        ...
        com.pranav.ide.dx.rop.code.Insn lastInsn = (insnSz == 0) ? null : insns.get(insnSz - 1);
         ...
         */
        if ((lastInsn == null) ||
                (lastInsn.getOpcode().getBranchingness() == com.pranav.ide.dx.rop.code.Rop.BRANCH_NONE)) {
            com.pranav.ide.dx.rop.code.SourcePosition pos = (lastInsn == null) ? com.pranav.ide.dx.rop.code.SourcePosition.NO_INFO :
                    lastInsn.getPosition();
            insns.add(new com.pranav.ide.dx.rop.code.PlainInsn(com.pranav.ide.dx.rop.code.Rops.GOTO, pos, null,
                    com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY));
            insnSz++;
        }
        ...
        com.pranav.ide.dx.rop.code.InsnList il = new com.pranav.ide.dx.rop.code.InsnList(insnSz);
        for (int i = 0; i < insnSz; i++) {
            il.set(i, insns.get(i));
        }
        il.setImmutable();

        com.pranav.ide.dx.rop.code.BasicBlock bb =
                new com.pranav.ide.dx.rop.code.BasicBlock(block.getLabel(), il, successors, primarySucc);
    ...
    private void mergeAndWorkAsNecessary(int label, int pred,
        ...
        if (existing != null) {
            ...
            if (merged != existing) {
                ...
                com.pranav.ide.dx.util.Bits.set(workSet, label);
            }
        } else {
    ...
    private void addSetupBlocks() {
        ...
        com.pranav.ide.dx.rop.code.SourcePosition pos = method.makeSourcePosistion(0);
        Prototype desc = method.getEffectiveDescriptor();
        com.pranav.ide.dx.rop.type.StdTypeList params = desc.getParameterTypes();
        int sz = params.size();
        com.pranav.ide.dx.rop.code.InsnList insns = new com.pranav.ide.dx.rop.code.InsnList(sz + 1);
        int at = 0;

        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.type.Type one = params.get(i);
            LocalVariableList.Item local =
                    localVariables.pcAndIndexToLocal(0, at);
            com.pranav.ide.dx.rop.code.RegisterSpec result = (local == null) ?
                    com.pranav.ide.dx.rop.code.RegisterSpec.make(at, one) :
                    com.pranav.ide.dx.rop.code.RegisterSpec.makeLocalOptional(at, one, local.getLocalItem());

            com.pranav.ide.dx.rop.code.Insn insn = new com.pranav.ide.dx.rop.code.PlainCstInsn(com.pranav.ide.dx.rop.code.Rops.opMoveParam(one), pos, result,
                    com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY,
                    com.pranav.ide.dx.rop.cst.CstInteger.make(at));
            insns.set(i, insn);
            at += one.getCategory();
        }

        insns.set(sz, new com.pranav.ide.dx.rop.code.PlainInsn(com.pranav.ide.dx.rop.code.Rops.GOTO, pos, null,
                com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY));
        insns.setImmutable();
        ...
        com.pranav.ide.dx.rop.code.BasicBlock bb =
                new com.pranav.ide.dx.rop.code.BasicBlock(getSpecialLabel(PARAM_ASSIGNMENT), insns,
                        com.pranav.ide.dx.util.IntList.makeImmutable(label), label);
        addBlock(bb, com.pranav.ide.dx.util.IntList.EMPTY);

        if (synch) {
            com.pranav.ide.dx.rop.code.RegisterSpec synchReg = getSynchReg();
            com.pranav.ide.dx.rop.code.Insn insn;
            if (isStatic()) {
                insn = new ThrowingCstInsn(com.pranav.ide.dx.rop.code.Rops.CONST_OBJECT, pos,
                        com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY,
                        com.pranav.ide.dx.rop.type.StdTypeList.EMPTY,
                        method.getDefiningClass());
                insns = new com.pranav.ide.dx.rop.code.InsnList(1);
                insns.set(0, insn);
            } else {
                insns = new com.pranav.ide.dx.rop.code.InsnList(2);
                insn = new PlainCstInsn(com.pranav.ide.dx.rop.code.Rops.MOVE_PARAM_OBJECT, pos,
                        synchReg, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY,
                        CstInteger.VALUE_0);
                insns.set(0, insn);
                insns.set(1, new com.pranav.ide.dx.rop.code.PlainInsn(com.pranav.ide.dx.rop.code.Rops.GOTO, pos, null,
                        com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY));
            }

            int label2 = getSpecialLabel(SYNCH_SETUP_2);
            insns.setImmutable();
            bb = new com.pranav.ide.dx.rop.code.BasicBlock(label, insns,
                    com.pranav.ide.dx.util.IntList.makeImmutable(label2), label2);
            addBlock(bb, com.pranav.ide.dx.util.IntList.EMPTY);

            insns = new com.pranav.ide.dx.rop.code.InsnList(isStatic() ? 2 : 1);

            if (isStatic()) {
                insns.set(0, new com.pranav.ide.dx.rop.code.PlainInsn(com.pranav.ide.dx.rop.code.Rops.opMoveResultPseudo(synchReg),
                        pos, synchReg, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY));
            }

            insn = new com.pranav.ide.dx.rop.code.ThrowingInsn(com.pranav.ide.dx.rop.code.Rops.MONITOR_ENTER, pos,
                    com.pranav.ide.dx.rop.code.RegisterSpecList.make(synchReg),
                    com.pranav.ide.dx.rop.type.StdTypeList.EMPTY);
            insns.set(isStatic() ? 1 : 0, insn);
            insns.setImmutable();
            bb = new com.pranav.ide.dx.rop.code.BasicBlock(label2, insns, com.pranav.ide.dx.util.IntList.makeImmutable(0), 0);
            addBlock(bb, com.pranav.ide.dx.util.IntList.EMPTY);
        }
    }
    ...
    private void addReturnBlock() {
        ...
        com.pranav.ide.dx.rop.code.SourcePosition returnPos = machine.getReturnPosition();
        int label = getSpecialLabel(RETURN);

        if (isSynchronized()) {
            com.pranav.ide.dx.rop.code.InsnList insns = new com.pranav.ide.dx.rop.code.InsnList(1);
            com.pranav.ide.dx.rop.code.Insn insn = new com.pranav.ide.dx.rop.code.ThrowingInsn(com.pranav.ide.dx.rop.code.Rops.MONITOR_EXIT, returnPos,
                    com.pranav.ide.dx.rop.code.RegisterSpecList.make(getSynchReg()),
                    com.pranav.ide.dx.rop.type.StdTypeList.EMPTY);
            insns.set(0, insn);
            insns.setImmutable();
            ...
            com.pranav.ide.dx.rop.code.BasicBlock bb =
                    new com.pranav.ide.dx.rop.code.BasicBlock(label, insns,
                            com.pranav.ide.dx.util.IntList.makeImmutable(nextLabel), nextLabel);
            addBlock(bb, com.pranav.ide.dx.util.IntList.EMPTY);
        ...
        com.pranav.ide.dx.rop.code.RegisterSpecList sources;

        if (sourceTypes.size() == 0) {
            sources = com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY;
        } else {
            com.pranav.ide.dx.rop.code.RegisterSpec source = com.pranav.ide.dx.rop.code.RegisterSpec.make(0, sourceTypes.getType(0));
            sources = com.pranav.ide.dx.rop.code.RegisterSpecList.make(source);
        }

        com.pranav.ide.dx.rop.code.Insn insn = new com.pranav.ide.dx.rop.code.PlainInsn(returnOp, returnPos, null, sources);
        insns.set(0, insn);
        insns.setImmutable();

        com.pranav.ide.dx.rop.code.BasicBlock bb = new com.pranav.ide.dx.rop.code.BasicBlock(label, insns, com.pranav.ide.dx.util.IntList.EMPTY, -1);
        addBlock(bb, com.pranav.ide.dx.util.IntList.EMPTY);
    }
    ...
    private void addSynchExceptionHandlerBlock() {
        ...
        com.pranav.ide.dx.rop.code.SourcePosition pos = method.makeSourcePosistion(0);
        com.pranav.ide.dx.rop.code.RegisterSpec exReg = com.pranav.ide.dx.rop.code.RegisterSpec.make(0, com.pranav.ide.dx.rop.type.Type.THROWABLE);
        com.pranav.ide.dx.rop.code.BasicBlock bb;
        com.pranav.ide.dx.rop.code.Insn insn;

        com.pranav.ide.dx.rop.code.InsnList insns = new com.pranav.ide.dx.rop.code.InsnList(2);
        insn = new com.pranav.ide.dx.rop.code.PlainInsn(com.pranav.ide.dx.rop.code.Rops.opMoveException(Type.THROWABLE), pos,
                exReg, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY);
        insns.set(0, insn);
        insn = new com.pranav.ide.dx.rop.code.ThrowingInsn(com.pranav.ide.dx.rop.code.Rops.MONITOR_EXIT, pos,
                com.pranav.ide.dx.rop.code.RegisterSpecList.make(getSynchReg()),
                com.pranav.ide.dx.rop.type.StdTypeList.EMPTY);
        insns.set(1, insn);
        insns.setImmutable();
        ...
        bb = new com.pranav.ide.dx.rop.code.BasicBlock(getSpecialLabel(SYNCH_CATCH_1), insns,
                com.pranav.ide.dx.util.IntList.makeImmutable(label2), label2);
        addBlock(bb, com.pranav.ide.dx.util.IntList.EMPTY);

        insns = new com.pranav.ide.dx.rop.code.InsnList(1);
        insn = new ThrowingInsn(com.pranav.ide.dx.rop.code.Rops.THROW, pos,
                com.pranav.ide.dx.rop.code.RegisterSpecList.make(exReg),
                StdTypeList.EMPTY);
        insns.set(0, insn);
        insns.setImmutable();

        bb = new com.pranav.ide.dx.rop.code.BasicBlock(label2, insns, com.pranav.ide.dx.util.IntList.EMPTY, -1);
        addBlock(bb, com.pranav.ide.dx.util.IntList.EMPTY);
    }
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (catches != null) {
                for (ExceptionHandlerSetup one : catches.getSetups()) {
                    com.pranav.ide.dx.rop.code.Insn proto = labelToBlock(i).getFirstInsn();
                    SourcePosition pos = proto.getPosition();
                    com.pranav.ide.dx.rop.code.InsnList il = new com.pranav.ide.dx.rop.code.InsnList(2);

                    com.pranav.ide.dx.rop.code.Insn insn = new com.pranav.ide.dx.rop.code.PlainInsn(com.pranav.ide.dx.rop.code.Rops.opMoveException(one.getCaughtType()),
                            pos,
                            RegisterSpec.make(maxLocals, one.getCaughtType()),
                            com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY);
                    il.set(0, insn);

                    insn = new PlainInsn(com.pranav.ide.dx.rop.code.Rops.GOTO, pos, null,
                            RegisterSpecList.EMPTY);
                    il.set(1, insn);
                    il.setImmutable();

                    com.pranav.ide.dx.rop.code.BasicBlock bb = new com.pranav.ide.dx.rop.code.BasicBlock(one.getLabel(),
                            il,
                            com.pranav.ide.dx.util.IntList.makeImmutable(i),
                            i);
     ...
     */
    private boolean isSubroutineCaller(com.pranav.ide.dx.rop.code.BasicBlock bb) {
        com.pranav.ide.dx.util.IntList successors = bb.getSuccessors();
    ...
    private void inlineSubroutines() {
        final com.pranav.ide.dx.util.IntList reachableSubroutineCallerLabels = new com.pranav.ide.dx.util.IntList(4);
         ...
         */
        forEachNonSubBlockDepthFirst(0, new com.pranav.ide.dx.rop.code.BasicBlock.Visitor() {
            @Override
            public void visitBlock(com.pranav.ide.dx.rop.code.BasicBlock b) {
        ...
        ArrayList<com.pranav.ide.dx.util.IntList> labelToSubroutines
                = new ArrayList<com.pranav.ide.dx.util.IntList>(largestAllocedLabel);
        for (int i = 0; i < largestAllocedLabel; i++) {
            labelToSubroutines.add(null);
        }

        for (int i = 0; i < result.size(); i++) {
            com.pranav.ide.dx.rop.code.BasicBlock b = result.get(i);
            if (b == null) {
                continue;
            }
            com.pranav.ide.dx.util.IntList subroutineList = resultSubroutines.get(i);
    ...
    private void deleteUnreachableBlocks() {
        final com.pranav.ide.dx.util.IntList reachableLabels = new com.pranav.ide.dx.util.IntList(result.size());
        ...
        forEachNonSubBlockDepthFirst(getSpecialLabel(PARAM_ASSIGNMENT),
                new com.pranav.ide.dx.rop.code.BasicBlock.Visitor() {
     ...
     */
    private com.pranav.ide.dx.rop.code.InsnList filterMoveReturnAddressInsns(com.pranav.ide.dx.rop.code.InsnList insns) {
        int sz;
        int newSz = 0;
        ...
        sz = insns.size();
        for (int i = 0; i < sz; i++) {
            if (insns.get(i).getOpcode() != com.pranav.ide.dx.rop.code.Rops.MOVE_RETURN_ADDRESS) {
                newSz++;
            }
        }
        ...
        com.pranav.ide.dx.rop.code.InsnList newInsns = new InsnList(newSz);
    ...
    private void forEachNonSubBlockDepthFirst(int firstLabel,
                                              com.pranav.ide.dx.rop.code.BasicBlock.Visitor v) {
    ...
    private void forEachNonSubBlockDepthFirst0(
            com.pranav.ide.dx.rop.code.BasicBlock next, BasicBlock.Visitor v, BitSet visited) {
        v.visitBlock(next);
    ...
    private static class ExceptionHandlerSetup {
         ...
         */
        private final com.pranav.ide.dx.rop.type.Type caughtType;
        /**
         ...
         */
        ExceptionHandlerSetup(com.pranav.ide.dx.rop.type.Type caughtType, int label) {
         ...
         */
        com.pranav.ide.dx.rop.type.Type getCaughtType() {
            return caughtType;
        }
    ...
    private class CatchInfo {
         ...
         */
        private final Map<com.pranav.ide.dx.rop.type.Type, ExceptionHandlerSetup> setups =
                new HashMap<com.pranav.ide.dx.rop.type.Type, ExceptionHandlerSetup>();
         ...
         */
        ExceptionHandlerSetup getSetup(com.pranav.ide.dx.rop.type.Type caughtType) {
    ...
    private class Subroutine {
         ...
         */
        com.pranav.ide.dx.util.IntList getSuccessors() {
            com.pranav.ide.dx.util.IntList successors = new com.pranav.ide.dx.util.IntList(callerBlocks.size());
            ...
            for (int label = callerBlocks.nextSetBit(0); label >= 0;
                ...
                com.pranav.ide.dx.rop.code.BasicBlock subCaller = labelToBlock(label);
    ...
    private class SubroutineInliner {
         ...
         */
        private final ArrayList<com.pranav.ide.dx.util.IntList> labelToSubroutines;
        /**
         ...
         */
        void inlineSubroutineCalledFrom(final com.pranav.ide.dx.rop.code.BasicBlock b) {
            /*
            ...
            addOrReplaceBlockNoDelete(
                    new com.pranav.ide.dx.rop.code.BasicBlock(b.getLabel(), b.getInsns(),
                            com.pranav.ide.dx.util.IntList.makeImmutable(newSubStartLabel),
                            newSubStartLabel),
        ...
        private void copyBlock(int origLabel, int newLabel) {
            ...
            com.pranav.ide.dx.rop.code.BasicBlock origBlock = labelToBlock(origLabel);

            final com.pranav.ide.dx.util.IntList origSuccessors = origBlock.getSuccessors();
            com.pranav.ide.dx.util.IntList successors;
            ...
            if (isSubroutineCaller(origBlock)) {
                ...
                successors = com.pranav.ide.dx.util.IntList.makeImmutable(
                ...
                if (subroutine.startBlock != subroutineStart) {
                    throw new RuntimeException(
                            "ret instruction returns to label "
                                    + com.pranav.ide.dx.util.Hex.u2(subroutine.startBlock)
                                    + " expected: " + Hex.u2(subroutineStart));
                }

                successors = com.pranav.ide.dx.util.IntList.makeImmutable(subroutineSuccessor);
                ...
                successors = new com.pranav.ide.dx.util.IntList(sz);
            ...
            addBlock(
                    new com.pranav.ide.dx.rop.code.BasicBlock(newLabel,
        ...
        private boolean involvedInSubroutine(int label, int subroutineStart) {
            com.pranav.ide.dx.util.IntList subroutinesList = labelToSubroutines.get(label);
