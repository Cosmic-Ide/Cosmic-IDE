...
public final class LocalList extends FixedSizeList {
    ...
    public static LocalList make(DalvInsnList insns) {
        ...
        for (int i = 0; i < sz; i++) {
            ...
            if (insn instanceof com.pranav.ide.dx.dex.code.LocalSnapshot) {
                com.pranav.ide.dx.rop.code.RegisterSpecSet snapshot =
                        ((LocalSnapshot) insn).getLocals();
                state.snapshot(insn.getAddress(), snapshot);
            } else if (insn instanceof LocalStart) {
                com.pranav.ide.dx.rop.code.RegisterSpec local = ((LocalStart) insn).getLocal();
    ...
    public static class Entry implements Comparable<Entry> {
         ...
         */
        private final com.pranav.ide.dx.rop.code.RegisterSpec spec;

        /**
         * {@code non-null;} variable type (derived from {@code spec})
         */
        private final com.pranav.ide.dx.rop.cst.CstType type;
         ...
         *                    the variable
         */
        public Entry(int address, Disposition disposition, com.pranav.ide.dx.rop.code.RegisterSpec spec) {
            if (address < 0) {
            ...
            this.spec = spec;
            this.type = com.pranav.ide.dx.rop.cst.CstType.intern(spec.getType());
        }
         ...
         */
        public com.pranav.ide.dx.rop.cst.CstString getName() {
         ...
         */
        public com.pranav.ide.dx.rop.code.RegisterSpec getRegisterSpec() {
            return spec;
        }
         ...
         * {@code spec}
         */
        public boolean matches(com.pranav.ide.dx.rop.code.RegisterSpec otherSpec) {
    ...
    public static class MakeState {
         ...
         */
        private com.pranav.ide.dx.rop.code.RegisterSpecSet regs;
        /**
         ...
         */
        private static com.pranav.ide.dx.rop.code.RegisterSpec filterSpec(com.pranav.ide.dx.rop.code.RegisterSpec orig) {
            if ((orig != null) && (orig.getType() == com.pranav.ide.dx.rop.type.Type.KNOWN_NULL)) {
        ...
        private void aboutToProcess(int address, int reg) {
            ...
            if (first || (reg >= endIndices.length)) {
                 ...
                 */
                int newSz = reg + 1;
                com.pranav.ide.dx.rop.code.RegisterSpecSet newRegs = new com.pranav.ide.dx.rop.code.RegisterSpecSet(newSz);
        ...
        public void snapshot(int address, RegisterSpecSet specs) {
            ...
            for (int i = 0; i < sz; i++) {
                com.pranav.ide.dx.rop.code.RegisterSpec oldSpec = regs.get(i);
                com.pranav.ide.dx.rop.code.RegisterSpec newSpec = filterSpec(specs.get(i));
        ...
        public void startLocal(int address, com.pranav.ide.dx.rop.code.RegisterSpec startedLocal) {
            ...
            com.pranav.ide.dx.rop.code.RegisterSpec existingLocal = regs.get(regNum);
            ...
            com.pranav.ide.dx.rop.code.RegisterSpec movedLocal = regs.findMatchingLocal(startedLocal);
            if (movedLocal != null) {
                /*
             ...
             */
            if (regNum > 0) {
                com.pranav.ide.dx.rop.code.RegisterSpec justBelow = regs.get(regNum - 1);
            ...
            if (startedLocal.isCategory2()) {
                com.pranav.ide.dx.rop.code.RegisterSpec justAbove = regs.get(regNum + 1);
                if (justAbove != null) {
                    addOrUpdateEnd(address,
         ...
         *                   local being ended
         */
        public void endLocal(int address, com.pranav.ide.dx.rop.code.RegisterSpec endedLocal) {
        ...
        private boolean checkForEmptyRange(int address,
                                           com.pranav.ide.dx.rop.code.RegisterSpec endedLocal) {
