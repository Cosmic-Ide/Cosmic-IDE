...
import com.pranav.ide.dx.rop.code.RegisterSpecSet;
...
/**
 * Translator from {@link com.pranav.ide.dx.rop.code.RopMethod} to {@link DalvCode}. The {@link
...
public final class RopTranslator {
     ...
     */
    private final com.pranav.ide.dx.dex.DexOptions dexOptions;

    /**
     * {@code non-null;} method to translate
     */
    private final com.pranav.ide.dx.rop.code.RopMethod method;
     ...
     */
    private final com.pranav.ide.dx.rop.code.LocalVariableInfo locals;
     ...
     */
    private RopTranslator(com.pranav.ide.dx.rop.code.RopMethod method, int positionInfo, com.pranav.ide.dx.rop.code.LocalVariableInfo locals,
        ...
        com.pranav.ide.dx.rop.code.BasicBlockList blocks = method.getBlocks();
        int bsz = blocks.size();
    ...
    /**
     * Translates a {@link com.pranav.ide.dx.rop.code.RopMethod}. This may modify the given
     * input.
     *
     * @param method       {@code non-null;} the original method
     * @param positionInfo how much position info to preserve; one of the
     ...
     */
    public static DalvCode translate(com.pranav.ide.dx.rop.code.RopMethod method, int positionInfo,
                                     com.pranav.ide.dx.rop.code.LocalVariableInfo locals, int paramSize, com.pranav.ide.dx.dex.DexOptions dexOptions) {
    ...
    private static boolean calculateParamsAreInOrder(RopMethod method,
         ...
         */
        method.getBlocks().forEachInsn(new com.pranav.ide.dx.rop.code.Insn.BaseVisitor() {
            @Override
            public void visitPlainCstInsn(com.pranav.ide.dx.rop.code.PlainCstInsn insn) {
                if (insn.getOpcode().getOpcode() == com.pranav.ide.dx.rop.code.RegOps.MOVE_PARAM) {
                    int param =
                            ((com.pranav.ide.dx.rop.cst.CstInteger) insn.getConstant()).getValue();
     ...
     */
    private static com.pranav.ide.dx.rop.code.RegisterSpecList getRegs(com.pranav.ide.dx.rop.code.Insn insn) {
     ...
     */
    private static com.pranav.ide.dx.rop.code.RegisterSpecList getRegs(com.pranav.ide.dx.rop.code.Insn insn,
                                                                       com.pranav.ide.dx.rop.code.RegisterSpec resultReg) {
        com.pranav.ide.dx.rop.code.RegisterSpecList regs = insn.getSources();
        ...
        if (insn.getOpcode().isCommutative()
            ...
            regs = com.pranav.ide.dx.rop.code.RegisterSpecList.make(regs.get(1), regs.get(0));
        }
     ...
     */
    private void outputBlock(com.pranav.ide.dx.rop.code.BasicBlock block, int nextLabel) {
        ...
        com.pranav.ide.dx.rop.code.Insn lastInsn = block.getLastInsn();
        ...
        if ((succ >= 0) && (succ != nextLabel)) {
             ...
             */
            com.pranav.ide.dx.rop.code.Rop lastRop = lastInsn.getOpcode();
            if ((lastRop.getBranchingness() == com.pranav.ide.dx.rop.code.Rop.BRANCH_IF) &&
    ...
    private void pickOrder() {
        ...
        int[] tracebackSet = com.pranav.ide.dx.util.Bits.makeBitSet(maxLabel);

        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.code.BasicBlock one = blocks.get(i);
            com.pranav.ide.dx.util.Bits.set(workSet, one.getLabel());
        }
        ...
        for (int label = method.getFirstLabel();
             label != -1;
             label = com.pranav.ide.dx.util.Bits.findFirst(workSet, 0)) {
            ...
            traceBack:
            for (; ; ) {
                com.pranav.ide.dx.util.IntList preds = method.labelToPredecessors(label);
                int psz = preds.size();

                for (int i = 0; i < psz; i++) {
                    int predLabel = preds.get(i);

                    if (com.pranav.ide.dx.util.Bits.get(tracebackSet, predLabel)) {
                        /*
                    ...
                    if (!com.pranav.ide.dx.util.Bits.get(workSet, predLabel)) {
                        // This one's already ordered.
                        continue;
                    }

                    com.pranav.ide.dx.rop.code.BasicBlock pred = blocks.labelToBlock(predLabel);
                    if (pred.getPrimarySuccessor() == label) {
                        // Found one!
                        label = predLabel;
                        com.pranav.ide.dx.util.Bits.set(tracebackSet, label);
                        continue traceBack;
                    }
                }
             ...
             */
            while (label != -1) {
                com.pranav.ide.dx.util.Bits.clear(workSet, label);
                com.pranav.ide.dx.util.Bits.clear(tracebackSet, label);
                order[at] = label;
                at++;

                com.pranav.ide.dx.rop.code.BasicBlock one = blocks.labelToBlock(label);
                com.pranav.ide.dx.rop.code.BasicBlock preferredBlock = blocks.preferredSuccessorOf(one);
                ...
                if (com.pranav.ide.dx.util.Bits.get(workSet, preferred)) {
                    /*
                    ...
                    label = preferred;
                        ...
                        && com.pranav.ide.dx.util.Bits.get(workSet, primary)) {
                    /*
                     ...
                     */
                    com.pranav.ide.dx.util.IntList successors = one.getSuccessors();
     ...
     */
    private class TranslationVisitor implements com.pranav.ide.dx.rop.code.Insn.Visitor {
        /**
         * {@code non-null;} list of output instructions in-progress
         */
        private final OutputCollector output;
         ...
         */
        private com.pranav.ide.dx.rop.code.BasicBlock block;
         ...
         */
        @Override
        public void visitPlainInsn(com.pranav.ide.dx.rop.code.PlainInsn insn) {
            com.pranav.ide.dx.rop.code.Rop rop = insn.getOpcode();
            if (rop.getOpcode() == com.pranav.ide.dx.rop.code.RegOps.MARK_LOCAL) {
                /*
            ...
            if (rop.getOpcode() == com.pranav.ide.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO) {
                // These get skipped
                return;
            }

            com.pranav.ide.dx.rop.code.SourcePosition pos = insn.getPosition();
            Dop opcode = RopToDop.dopFor(insn);
            DalvInsn di;

            switch (rop.getBranchingness()) {
                case com.pranav.ide.dx.rop.code.Rop.BRANCH_NONE:
                case com.pranav.ide.dx.rop.code.Rop.BRANCH_RETURN:
                case com.pranav.ide.dx.rop.code.Rop.BRANCH_THROW: {
                    di = new SimpleInsn(opcode, pos, getRegs(insn));
                    break;
                }
                case com.pranav.ide.dx.rop.code.Rop.BRANCH_GOTO: {
                    /*
                ...
                case com.pranav.ide.dx.rop.code.Rop.BRANCH_IF: {
        ...
        public void visitPlainCstInsn(com.pranav.ide.dx.rop.code.PlainCstInsn insn) {
            ...
            if (rop.getBranchingness() != com.pranav.ide.dx.rop.code.Rop.BRANCH_NONE) {
                throw new RuntimeException("shouldn't happen");
            }

            if (ropOpcode == com.pranav.ide.dx.rop.code.RegOps.MOVE_PARAM) {
                if (!paramsAreInOrder) {
                    /*
                    ...
                    com.pranav.ide.dx.rop.code.RegisterSpec dest = insn.getResult();
                    int param =
                            ((CstInteger) insn.getConstant()).getValue();
                    com.pranav.ide.dx.rop.code.RegisterSpec source =
                            com.pranav.ide.dx.rop.code.RegisterSpec.make(regCount - paramSize + param,
                                    dest.getType());
                    di = new SimpleInsn(opcode, pos,
                            com.pranav.ide.dx.rop.code.RegisterSpecList.make(dest, source));
                    addOutput(di);
                }
            } else {
                // No moves required for the parameters
                com.pranav.ide.dx.rop.code.RegisterSpecList regs = getRegs(insn);
         ...
         */
        @Override
        public void visitSwitchInsn(com.pranav.ide.dx.rop.code.SwitchInsn insn) {
            com.pranav.ide.dx.rop.code.SourcePosition pos = insn.getPosition();
            com.pranav.ide.dx.util.IntList cases = insn.getCases();
            ...
            addOutputSuffix(new com.pranav.ide.dx.dex.code.OddSpacer(pos));
         ...
         * primary successor
         */
        private com.pranav.ide.dx.rop.code.RegisterSpec getNextMoveResultPseudo() {
            ...
            if (insn.getOpcode().getOpcode() != com.pranav.ide.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO) {
                return null;
            } else {
        ...
        public void visitInvokePolymorphicInsn(InvokePolymorphicInsn insn) {
            ...
            if (rop.getBranchingness() != com.pranav.ide.dx.rop.code.Rop.BRANCH_THROW) {
            ...
            com.pranav.ide.dx.rop.code.RegisterSpecList regs = insn.getSources();
            com.pranav.ide.dx.rop.cst.Constant[] constants = new com.pranav.ide.dx.rop.cst.Constant[]{
         ...
         */
        @Override
        public void visitThrowingCstInsn(com.pranav.ide.dx.rop.code.ThrowingCstInsn insn) {
            com.pranav.ide.dx.rop.code.SourcePosition pos = insn.getPosition();
            Dop opcode = RopToDop.dopFor(insn);
            com.pranav.ide.dx.rop.code.Rop rop = insn.getOpcode();
            com.pranav.ide.dx.rop.cst.Constant cst = insn.getConstant();
            ...
            if (rop.isCallLike()) {
                ...
                com.pranav.ide.dx.rop.code.RegisterSpec realResult = getNextMoveResultPseudo();

                com.pranav.ide.dx.rop.code.RegisterSpecList regs = getRegs(insn, realResult);
                DalvInsn di;

                boolean hasResult = opcode.hasResult()
                        || (rop.getOpcode() == com.pranav.ide.dx.rop.code.RegOps.CHECK_CAST);
                ...
                if ((rop.getOpcode() == RegOps.NEW_ARRAY) &&
                        (opcode.getOpcode() != com.pranav.ide.dx.io.Opcodes.NEW_ARRAY)) {
                    /*
                ...
                if (opcode.getOpcode() == com.pranav.ide.dx.io.Opcodes.INSTANCE_OF && previousDi != null) {
                    int prevOpcode = previousDi.getOpcode().getOpcode();
                    if (prevOpcode == com.pranav.ide.dx.io.Opcodes.MOVE_OBJECT
                            || prevOpcode == com.pranav.ide.dx.io.Opcodes.MOVE_OBJECT_FROM16
                            || prevOpcode == com.pranav.ide.dx.io.Opcodes.MOVE_OBJECT_16) {
         ...
         */
        @Override
        public void visitThrowingInsn(com.pranav.ide.dx.rop.code.ThrowingInsn insn) {
            com.pranav.ide.dx.rop.code.SourcePosition pos = insn.getPosition();
            Dop opcode = RopToDop.dopFor(insn);
            com.pranav.ide.dx.rop.code.Rop rop = insn.getOpcode();
            com.pranav.ide.dx.rop.code.RegisterSpec realResult;
