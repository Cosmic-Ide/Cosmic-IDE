...
import com.pranav.ide.dx.dex.code.MultiCstInsn;
...
public final class Form45cc extends InsnFormat {
     ...
     */
    private static int wordCount(com.pranav.ide.dx.rop.code.RegisterSpecList regs) {
        int sz = regs.size();
        ...
        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.code.RegisterSpec one = regs.get(i);
     ...
     */
    private static com.pranav.ide.dx.rop.code.RegisterSpecList explicitize(com.pranav.ide.dx.rop.code.RegisterSpecList orig) {
        ...
        com.pranav.ide.dx.rop.code.RegisterSpecList result = new RegisterSpecList(wordCount);
        int wordAt = 0;

        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.code.RegisterSpec one = orig.get(i);
    ...
    public String insnArgString(DalvInsn insn) {
        com.pranav.ide.dx.rop.code.RegisterSpecList regs = explicitize(insn.getRegisters());
    ...
    public boolean isCompatible(DalvInsn insn) {
        ...
        com.pranav.ide.dx.rop.cst.Constant methodRef = mci.getConstant(0);
        ...
        com.pranav.ide.dx.rop.code.RegisterSpecList regs = mci.getRegisters();
    ...
    public BitSet compatibleRegs(DalvInsn insn) {
        ...
        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.code.RegisterSpec reg = regs.get(i);
            /*
             * The check below adds (category - 1) to the register, to
             * account for the fact that the second half of a
             * category-2 register has to be represented explicitly in
