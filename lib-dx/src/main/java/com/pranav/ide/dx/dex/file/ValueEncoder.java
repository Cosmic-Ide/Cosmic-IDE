...
import com.pranav.ide.dex.EncodedValueCodec;
...
public final class ValueEncoder {
     ...
     */
    private final com.pranav.ide.dx.util.AnnotatedOutput out;
     ...
     */
    private static int constantToValueType(com.pranav.ide.dx.rop.cst.Constant cst) {
        /*
        ...
        if (cst instanceof CstByte) {
        ...
        } else if (cst instanceof com.pranav.ide.dx.rop.cst.CstString) {
            return VALUE_STRING;
            ...
            return VALUE_ENUM;
        } else if (cst instanceof com.pranav.ide.dx.rop.cst.CstArray) {
            return VALUE_ARRAY;
        } else if (cst instanceof com.pranav.ide.dx.rop.cst.CstAnnotation) {
     ...
     */
    public static String constantToHuman(com.pranav.ide.dx.rop.cst.Constant cst) {
     ...
     * contents for a particular {@link com.pranav.ide.dx.rop.annotation.Annotation}, calling itself
     * recursively should it encounter a nested annotation.
     *
     * @param file       {@code non-null;} the file to add to
     * @param annotation {@code non-null;} the annotation to add contents for
     */
    public static void addContents(DexFile file, com.pranav.ide.dx.rop.annotation.Annotation annotation) {
     ...
     * should it encounter a {@link com.pranav.ide.dx.rop.cst.CstArray} and calling {@link
     * #addContents(DexFile, Annotation)} recursively should it
     * encounter a {@link com.pranav.ide.dx.rop.cst.CstAnnotation}.
     *
     * @param file {@code non-null;} the file to add to
     * @param cst  {@code non-null;} the constant to add contents for
     */
    public static void addContents(DexFile file, Constant cst) {
        if (cst instanceof com.pranav.ide.dx.rop.cst.CstAnnotation) {
            addContents(file, ((CstAnnotation) cst).getAnnotation());
        } else if (cst instanceof com.pranav.ide.dx.rop.cst.CstArray) {
            com.pranav.ide.dx.rop.cst.CstArray.List list = ((CstArray) cst).getList();
            int size = list.size();
     ...
     */
    public void writeConstant(com.pranav.ide.dx.rop.cst.Constant cst) {
        int type = constantToValueType(cst);
        int arg;
        ...
        switch (type) {
            ...
            case VALUE_INT:
            case VALUE_LONG: {
                long value = ((com.pranav.ide.dx.rop.cst.CstLiteralBits) cst).getLongBits();
            ...
            case VALUE_FLOAT: {
                ...
                long value = ((com.pranav.ide.dx.rop.cst.CstFloat) cst).getLongBits() << 32;
                EncodedValueCodec.writeRightZeroExtendedValue(out, type, value);
                break;
            }
            case VALUE_DOUBLE: {
                long value = ((com.pranav.ide.dx.rop.cst.CstDouble) cst).getLongBits();
                EncodedValueCodec.writeRightZeroExtendedValue(out, type, value);
                break;
            }
            case VALUE_METHOD_TYPE: {
                int index = file.getProtoIds().indexOf(((com.pranav.ide.dx.rop.cst.CstProtoRef) cst).getPrototype());
                EncodedValueCodec.writeUnsignedIntegralValue(out, type, index);
                break;
            }
            case VALUE_METHOD_HANDLE: {
                int index = file.getMethodHandles().indexOf((com.pranav.ide.dx.rop.cst.CstMethodHandle) cst);
                EncodedValueCodec.writeUnsignedIntegralValue(out, type, index);
                break;
            }
            case VALUE_STRING: {
                int index = file.getStringIds().indexOf((com.pranav.ide.dx.rop.cst.CstString) cst);
                EncodedValueCodec.writeUnsignedIntegralValue(out, type, index);
                break;
            }
            case VALUE_TYPE: {
                int index = file.getTypeIds().indexOf((com.pranav.ide.dx.rop.cst.CstType) cst);
                EncodedValueCodec.writeUnsignedIntegralValue(out, type, index);
                break;
            }
            case VALUE_FIELD: {
                int index = file.getFieldIds().indexOf((com.pranav.ide.dx.rop.cst.CstFieldRef) cst);
                EncodedValueCodec.writeUnsignedIntegralValue(out, type, index);
                break;
            }
            case VALUE_METHOD: {
                int index = file.getMethodIds().indexOf((com.pranav.ide.dx.rop.cst.CstMethodRef) cst);
                EncodedValueCodec.writeUnsignedIntegralValue(out, type, index);
                break;
            }
            case VALUE_ENUM: {
                com.pranav.ide.dx.rop.cst.CstFieldRef fieldRef = ((com.pranav.ide.dx.rop.cst.CstEnumRef) cst).getFieldRef();
            ...
            case VALUE_ARRAY: {
                out.writeByte(type);
                writeArray((com.pranav.ide.dx.rop.cst.CstArray) cst, false);
                break;
            }
            case VALUE_ANNOTATION: {
                out.writeByte(type);
                writeAnnotation(((com.pranav.ide.dx.rop.cst.CstAnnotation) cst).getAnnotation(),
                        false);
                break;
            }
            case VALUE_NULL: {
            ...
            case VALUE_BOOLEAN: {
                int value = ((com.pranav.ide.dx.rop.cst.CstBoolean) cst).getIntBits();
     ...
     */
    public void writeArray(com.pranav.ide.dx.rop.cst.CstArray array, boolean topLevel) {
        boolean annotates = topLevel && out.annotates();
        com.pranav.ide.dx.rop.cst.CstArray.List list = array.getList();
        int size = list.size();

        if (annotates) {
            out.annotate("  size: " + com.pranav.ide.dx.util.Hex.u4(size));
        }

        out.writeUleb128(size);

        for (int i = 0; i < size; i++) {
            com.pranav.ide.dx.rop.cst.Constant cst = list.get(i);
            if (annotates) {
     ...
     *                   of some other annotation
     */
    public void writeAnnotation(com.pranav.ide.dx.rop.annotation.Annotation annotation, boolean topLevel) {
        boolean annotates = topLevel && out.annotates();
        com.pranav.ide.dx.dex.file.StringIdsSection stringIds = file.getStringIds();
        TypeIdsSection typeIds = file.getTypeIds();
        ...
        if (annotates) {
            out.annotate("  type_idx: " + com.pranav.ide.dx.util.Hex.u4(typeIdx) + " // " +
                    type.toHuman());
        }

        out.writeUleb128(typeIds.indexOf(annotation.getType()));

        Collection<com.pranav.ide.dx.rop.annotation.NameValuePair> pairs = annotation.getNameValuePairs();
        int size = pairs.size();
        ...
        int at = 0;
        for (com.pranav.ide.dx.rop.annotation.NameValuePair pair : pairs) {
            CstString name = pair.getName();
            int nameIdx = stringIds.indexOf(name);
            com.pranav.ide.dx.rop.cst.Constant value = pair.getValue();
