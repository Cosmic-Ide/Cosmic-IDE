...
package com.pranav.ide.dx.merge;

import com.pranav.ide.dex.Annotation;
import com.pranav.ide.dex.CallSiteId;
import com.pranav.ide.dex.ClassData;
import com.pranav.ide.dex.ClassDef;
import com.pranav.ide.dex.Code;
import com.pranav.ide.dex.Dex;
import com.pranav.ide.dex.DexException;
import com.pranav.ide.dex.DexIndexOverflowException;
import com.pranav.ide.dex.FieldId;
import com.pranav.ide.dex.MethodHandle;
import com.pranav.ide.dex.MethodId;
import com.pranav.ide.dex.ProtoId;
import com.pranav.ide.dex.SizeOf;
import com.pranav.ide.dex.TableOfContents;
import com.pranav.ide.dex.TypeList;
...
public final class DexMerger {
    ...
    private final com.pranav.ide.dx.merge.IndexMap[] indexMaps;
    private final CollisionPolicy collisionPolicy;
    private final com.pranav.ide.dx.command.dexer.DxContext context;
    ...
    public DexMerger(Dex[] dexes, CollisionPolicy collisionPolicy, com.pranav.ide.dx.command.dexer.DxContext context)
            throws IOException {
        this(dexes, collisionPolicy, context, new WriterSizes(dexes));
    }

    private DexMerger(Dex[] dexes, CollisionPolicy collisionPolicy, com.pranav.ide.dx.command.dexer.DxContext context,
        ...
        indexMaps = new com.pranav.ide.dx.merge.IndexMap[dexes.length];
        for (int i = 0; i < dexes.length; i++) {
            indexMaps[i] = new com.pranav.ide.dx.merge.IndexMap(dexOut, dexes[i].getTableOfContents());
        }
        ...
        contentsOut.classData.off = dexOut.getNextSectionStart();
        ...
        contentsOut.stringData.off = dexOut.getNextSectionStart();
        stringDataOut = dexOut.appendSection(writerSizes.stringData, "string data");

        contentsOut.debugInfo.off = dexOut.getNextSectionStart();
    ...
    private void mergeStringIds() {
        new IdMerger<String>(idsDefsOut) {
            ...
            @Override
            String read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index) {
                return in.readString();
            }

            @Override
            void updateIndex(int offset, com.pranav.ide.dx.merge.IndexMap indexMap, int oldIndex, int newIndex) {
                indexMap.stringIds[oldIndex] = newIndex;
            }

            @Override
            void write(String value) {
                contentsOut.stringData.size++;
    ...
    private void mergeTypeIds() {
        new IdMerger<Integer>(idsDefsOut) {
            ...
            @Override
            Integer read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index) {
    ...
    private void mergeTypeLists() {
        new IdMerger<TypeList>(typeListOut) {
            ...
            @Override
            TypeList read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index) {
    ...
    private void mergeProtoIds() {
        new IdMerger<ProtoId>(idsDefsOut) {
            ...
            @Override
            ProtoId read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index) {
    ...
    private void mergeCallSiteIds() {
        new IdMerger<CallSiteId>(idsDefsOut) {
            ...
            @Override
            CallSiteId read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index) {
    ...
    private void mergeMethodHandles() {
        new IdMerger<MethodHandle>(idsDefsOut) {
            ...
            @Override
            MethodHandle read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index) {
    ...
    private void mergeFieldIds() {
        new IdMerger<FieldId>(idsDefsOut) {
            ...
            @Override
            FieldId read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index) {
    ...
    private void mergeMethodIds() {
        new IdMerger<MethodId>(idsDefsOut) {
            ...
            @Override
            MethodId read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index) {
    ...
    private void mergeAnnotations() {
        new IdMerger<Annotation>(annotationOut) {
            ...
            @Override
            Annotation read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index) {
    ...
    private void readSortableTypes(SortableType[] sortableTypes, Dex buffer,
                                   com.pranav.ide.dx.merge.IndexMap indexMap) {
    ...
    private void transformAnnotationSets(Dex in, com.pranav.ide.dx.merge.IndexMap indexMap) {
    ...
    private void transformAnnotationSetRefLists(Dex in, com.pranav.ide.dx.merge.IndexMap indexMap) {
    ...
    private void transformAnnotationDirectories(Dex in, com.pranav.ide.dx.merge.IndexMap indexMap) {
    ...
    private void transformStaticValues(Dex in, com.pranav.ide.dx.merge.IndexMap indexMap) {
     ...
     * data.
     */
    private void transformClassDef(Dex in, ClassDef classDef, com.pranav.ide.dx.merge.IndexMap indexMap) {
    ...
    private void transformAnnotationDirectory(
            Dex.Section directoryIn, com.pranav.ide.dx.merge.IndexMap indexMap) {
     ...
     */
    private void transformAnnotationSet(com.pranav.ide.dx.merge.IndexMap indexMap, Dex.Section setIn) {
     ...
     */
    private void transformAnnotationSetRefList(com.pranav.ide.dx.merge.IndexMap indexMap, Dex.Section refListIn) {
    ...
    private void transformClassData(Dex in, ClassData classData, com.pranav.ide.dx.merge.IndexMap indexMap) {
        contentsOut.classData.size++;
    ...
    private void transformFields(com.pranav.ide.dx.merge.IndexMap indexMap, ClassData.Field[] fields) {
    ...
    private void transformMethods(Dex in, com.pranav.ide.dx.merge.IndexMap indexMap, ClassData.Method[] methods) {
    ...
    private void transformCode(Dex in, Code code, com.pranav.ide.dx.merge.IndexMap indexMap) {
     ...
     */
    private int[] transformCatchHandlers(com.pranav.ide.dx.merge.IndexMap indexMap, Code.CatchHandler[] catchHandlers) {
    ...
    private void transformDebugInfoItem(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap) {
        contentsOut.debugInfo.size++;
    ...
    private void transformEncodedCatchHandler(Code.CatchHandler catchHandler, com.pranav.ide.dx.merge.IndexMap indexMap) {
    ...
    private static class WriterSizes {
        ...
        private void plus(TableOfContents contents, boolean exact) {
            ...
            stringData += contents.stringData.byteCount;
            ...
            if (exact) {
                ...
                classData += (int) Math.ceil(contents.classData.byteCount * 1.67);
                ...
                debugInfo += contents.debugInfo.byteCount * 2 + 8;
            }
        }
    ...
    abstract class IdMerger<T extends Comparable<T>> {
        ...
        private int readIntoMap(Dex.Section in, TableOfContents.Section section, com.pranav.ide.dx.merge.IndexMap indexMap,
        ...
        private List<UnsortedValue> readUnsortedValues(Dex source, com.pranav.ide.dx.merge.IndexMap indexMap) {
        ...
        abstract T read(Dex.Section in, com.pranav.ide.dx.merge.IndexMap indexMap, int index);

        abstract void updateIndex(int offset, com.pranav.ide.dx.merge.IndexMap indexMap, int oldIndex, int newIndex);

        abstract void write(T value);

        class UnsortedValue implements Comparable<UnsortedValue> {
            final Dex source;
            final com.pranav.ide.dx.merge.IndexMap indexMap;
            final T value;
            final int index;
            final int offset;

            UnsortedValue(Dex source, com.pranav.ide.dx.merge.IndexMap indexMap, T value, int index, int offset) {
                this.source = source;
