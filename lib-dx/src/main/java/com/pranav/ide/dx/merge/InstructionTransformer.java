...
import com.pranav.ide.dx.io.CodeReader;
import com.pranav.ide.dx.io.Opcodes;
import com.pranav.ide.dx.io.instructions.DecodedInstruction;
import com.pranav.ide.dx.io.instructions.ShortArrayCodeOutput;

final class InstructionTransformer {
    private final com.pranav.ide.dx.io.CodeReader reader;

    private DecodedInstruction[] mappedInstructions;
    private int mappedAt;
    private com.pranav.ide.dx.merge.IndexMap indexMap;

    public InstructionTransformer() {
        this.reader = new com.pranav.ide.dx.io.CodeReader();
    ...
    private class GenericVisitor implements com.pranav.ide.dx.io.CodeReader.Visitor {
        @Override
    ...
    private class StringVisitor implements com.pranav.ide.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
            int stringId = one.getIndex();
            int mappedId = indexMap.adjustString(stringId);
            boolean isJumbo = (one.getOpcode() == com.pranav.ide.dx.io.Opcodes.CONST_STRING_JUMBO);
            jumboCheck(isJumbo, mappedId);
            mappedInstructions[mappedAt++] = one.withIndex(mappedId);
        }
    }

    private class FieldVisitor implements com.pranav.ide.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
    ...
    private class TypeVisitor implements com.pranav.ide.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
    ...
    private class MethodVisitor implements com.pranav.ide.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
    ...
    private class MethodAndProtoVisitor implements com.pranav.ide.dx.io.CodeReader.Visitor {
        @Override
        public void visit(DecodedInstruction[] all, DecodedInstruction one) {
            int methodId = one.getIndex();
