...
import com.pranav.ide.dx.util.LabeledList;

/**
 * List of {@link com.pranav.ide.dx.rop.code.BasicBlock} instances.
 */
public final class BasicBlockList extends LabeledList {
     ...
     */
    public com.pranav.ide.dx.rop.code.BasicBlock get(int n) {
        return (com.pranav.ide.dx.rop.code.BasicBlock) get0(n);
    }
     ...
     */
    public void set(int n, com.pranav.ide.dx.rop.code.BasicBlock bb) {
        super.set(n, bb);
     ...
     * instance's instructions (indirectly through {@link com.pranav.ide.dx.rop.code.BasicBlock}
     * instances).
     *
    ...
    public int getInstructionCount() {
        ...
        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.code.BasicBlock one = (com.pranav.ide.dx.rop.code.BasicBlock) getOrNull0(i);
            if (one != null) {
    ...
    public int getEffectiveInstructionCount() {
        ...
        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.code.BasicBlock one = (com.pranav.ide.dx.rop.code.BasicBlock) getOrNull0(i);
            if (one != null) {
                com.pranav.ide.dx.rop.code.InsnList insns = one.getInsns();
                int insnsSz = insns.size();

                for (int j = 0; j < insnsSz; j++) {
                    com.pranav.ide.dx.rop.code.Insn insn = insns.get(j);
     ...
     */
    public com.pranav.ide.dx.rop.code.BasicBlock labelToBlock(int label) {
     ...
     */
    public void forEachInsn(com.pranav.ide.dx.rop.code.Insn.Visitor visitor) {
        int sz = size();

        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.code.BasicBlock one = get(i);
    ...
    public BasicBlockList withRegisterOffset(int delta) {
        ...
        for (int i = 0; i < sz; i++) {
            com.pranav.ide.dx.rop.code.BasicBlock one = (com.pranav.ide.dx.rop.code.BasicBlock) get0(i);
            if (one != null) {
     ...
     */
    public com.pranav.ide.dx.rop.code.BasicBlock preferredSuccessorOf(com.pranav.ide.dx.rop.code.BasicBlock block) {
     ...
     * are identical
     */
    public boolean catchesEqual(com.pranav.ide.dx.rop.code.BasicBlock block1, BasicBlock block2) {
    ...
    private static class RegCountVisitor
            implements com.pranav.ide.dx.rop.code.Insn.Visitor {
        /**
