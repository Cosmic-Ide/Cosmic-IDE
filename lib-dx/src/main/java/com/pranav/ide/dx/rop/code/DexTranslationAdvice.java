...
public final class DexTranslationAdvice
     ...
     */
    @Override
    public boolean hasConstantOperation(com.pranav.ide.dx.rop.code.Rop opcode,
        ...
        if (!(sourceB.getTypeBearer() instanceof com.pranav.ide.dx.rop.cst.CstInteger)) {
            // Except for rsub-int (reverse sub) where first source is constant
            if (sourceA.getTypeBearer() instanceof com.pranav.ide.dx.rop.cst.CstInteger &&
                    opcode.getOpcode() == RegOps.SUB) {
                com.pranav.ide.dx.rop.cst.CstInteger cst = (com.pranav.ide.dx.rop.cst.CstInteger) sourceA.getTypeBearer();
        ...
        com.pranav.ide.dx.rop.cst.CstInteger cst = (com.pranav.ide.dx.rop.cst.CstInteger) sourceB.getTypeBearer();
        ...
        switch (opcode.getOpcode()) {
            ...
            case RegOps.SUB:
                com.pranav.ide.dx.rop.cst.CstInteger cst2 = CstInteger.make(-cst.getValue());
