...
public final class PlainInsn
    ...
    public PlainInsn(Rop opcode, SourcePosition position,
                     RegisterSpec result, com.pranav.ide.dx.rop.code.RegisterSpecList sources) {
    ...
    public PlainInsn(Rop opcode, SourcePosition position, RegisterSpec result,
                     RegisterSpec source) {
        this(opcode, position, result, com.pranav.ide.dx.rop.code.RegisterSpecList.make(source));
    }
    ...
    public Insn withSourceLiteral() {
        com.pranav.ide.dx.rop.code.RegisterSpecList sources = getSources();
        ...
        if (!lastType.isConstant()) {
            ...
            if (szSources == 2 && firstType.isConstant()) {
                com.pranav.ide.dx.rop.cst.Constant cst = (com.pranav.ide.dx.rop.cst.Constant) firstType;
                com.pranav.ide.dx.rop.code.RegisterSpecList newSources = sources.withoutFirst();
                Rop newRop = Rops.ropFor(getOpcode().getOpcode(), getResult(),
                        newSources, cst);
                return new com.pranav.ide.dx.rop.code.PlainCstInsn(newRop, getPosition(), getResult(),
                        newSources, cst);
            }
            return this;
        } else {

            com.pranav.ide.dx.rop.cst.Constant cst = (Constant) lastType;

            com.pranav.ide.dx.rop.code.RegisterSpecList newSources = sources.withoutLast();

            Rop newRop;
            try {
                // Check for constant subtraction and flip it to be addition
                int opcode = getOpcode().getOpcode();
                if (opcode == RegOps.SUB && cst instanceof com.pranav.ide.dx.rop.cst.CstInteger) {
                    opcode = RegOps.ADD;
                    cst = com.pranav.ide.dx.rop.cst.CstInteger.make(-((CstInteger) cst).getValue());
                }
