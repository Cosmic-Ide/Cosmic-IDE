...
public class ConstCollector {
    ...
    private void run() {
        ...
        ArrayList<com.pranav.ide.dx.rop.cst.TypedConstant> constantList
                = getConstsSortedByCountUse();

        int toCollect = Math.min(constantList.size(), MAX_COLLECTED_CONSTANTS);

        com.pranav.ide.dx.ssa.SsaBasicBlock start = ssaMeth.getEntryBlock();

        // Constant to new register containing the constant
        HashMap<com.pranav.ide.dx.rop.cst.TypedConstant, com.pranav.ide.dx.rop.code.RegisterSpec> newRegs
                = new HashMap<com.pranav.ide.dx.rop.cst.TypedConstant, com.pranav.ide.dx.rop.code.RegisterSpec>(toCollect);

        for (int i = 0; i < toCollect; i++) {
            com.pranav.ide.dx.rop.cst.TypedConstant cst = constantList.get(i);
            com.pranav.ide.dx.rop.code.RegisterSpec result
                    = com.pranav.ide.dx.rop.code.RegisterSpec.make(ssaMeth.makeNewSsaReg(), cst);

            com.pranav.ide.dx.rop.code.Rop constRop = com.pranav.ide.dx.rop.code.Rops.opConst(cst);

            if (constRop.getBranchingness() == Rop.BRANCH_NONE) {
                start.addInsnToHead(
                        new PlainCstInsn(com.pranav.ide.dx.rop.code.Rops.opConst(cst),
                                com.pranav.ide.dx.rop.code.SourcePosition.NO_INFO, result,
                                com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY, cst));
            } else {
                // We need two new basic blocks along with the new insn
                com.pranav.ide.dx.ssa.SsaBasicBlock entryBlock = ssaMeth.getEntryBlock();
                com.pranav.ide.dx.ssa.SsaBasicBlock successorBlock
                        = entryBlock.getPrimarySuccessor();

                // Insert a block containing the const insn.
                com.pranav.ide.dx.ssa.SsaBasicBlock constBlock
                        = entryBlock.insertNewSuccessor(successorBlock);

                constBlock.replaceLastInsn(
                        new ThrowingCstInsn(constRop, com.pranav.ide.dx.rop.code.SourcePosition.NO_INFO,
                ...
                com.pranav.ide.dx.rop.code.PlainInsn insn
                        = new com.pranav.ide.dx.rop.code.PlainInsn(
                        com.pranav.ide.dx.rop.code.Rops.opMoveResultPseudo(result.getTypeBearer()),
                        com.pranav.ide.dx.rop.code.SourcePosition.NO_INFO,
                        result, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY);
     ...
     */
    private ArrayList<com.pranav.ide.dx.rop.cst.TypedConstant> getConstsSortedByCountUse() {
        int regSz = ssaMeth.getRegCount();

        final HashMap<com.pranav.ide.dx.rop.cst.TypedConstant, Integer> countUses
                = new HashMap<com.pranav.ide.dx.rop.cst.TypedConstant, Integer>();

        /*
         * Each collected constant can be used by just one local
         * (used only if COLLECT_ONE_LOCAL is true).
         */
        final HashSet<com.pranav.ide.dx.rop.cst.TypedConstant> usedByLocal
                = new HashSet<com.pranav.ide.dx.rop.cst.TypedConstant>();
        ...
        for (int i = 0; i < regSz; i++) {
            ...
            com.pranav.ide.dx.rop.type.TypeBearer typeBearer = result.getTypeBearer();

            if (!typeBearer.isConstant()) continue;

            com.pranav.ide.dx.rop.cst.TypedConstant cst = (com.pranav.ide.dx.rop.cst.TypedConstant) typeBearer;

            // Find defining instruction for move-result-pseudo instructions
            if (insn.getOpcode().getOpcode() == RegOps.MOVE_RESULT_PSEUDO) {
                int pred = insn.getBlock().getPredecessors().nextSetBit(0);
                ArrayList<com.pranav.ide.dx.ssa.SsaInsn> predInsns;
        ...
        ArrayList<com.pranav.ide.dx.rop.cst.TypedConstant> constantList = new ArrayList<com.pranav.ide.dx.rop.cst.TypedConstant>();
        for (Map.Entry<com.pranav.ide.dx.rop.cst.TypedConstant, Integer> entry : countUses.entrySet()) {
            if (entry.getValue() > 1) {
        ...
        Collections.sort(constantList, new Comparator<com.pranav.ide.dx.rop.cst.Constant>() {
            @Override
            public int compare(com.pranav.ide.dx.rop.cst.Constant a, Constant b) {
                int ret;
     ...
     *                {@code origReg}
     */
    private void fixLocalAssignment(com.pranav.ide.dx.rop.code.RegisterSpec origReg,
                                    com.pranav.ide.dx.rop.code.RegisterSpec newReg) {
        for (com.pranav.ide.dx.ssa.SsaInsn use : ssaMeth.getUseListForRegister(origReg.getReg())) {
            com.pranav.ide.dx.rop.code.RegisterSpec localAssignment = use.getLocalAssignment();
            ...
            com.pranav.ide.dx.ssa.SsaInsn newInsn
                    = com.pranav.ide.dx.ssa.SsaInsn.makeFromRop(
                    new PlainInsn(Rops.opMarkLocal(newReg),
                            SourcePosition.NO_INFO, null,
                            RegisterSpecList.make(newReg)),
                    use.getBlock());

            ArrayList<com.pranav.ide.dx.ssa.SsaInsn> insns = use.getBlock().getInsns();
     ...
     */
    private void updateConstUses(HashMap<com.pranav.ide.dx.rop.cst.TypedConstant, com.pranav.ide.dx.rop.code.RegisterSpec> newRegs,
                                 int origRegCount) {
        ...
        for (int i = 0; i < origRegCount; i++) {
            ...
            final com.pranav.ide.dx.rop.code.RegisterSpec origReg = insn.getResult();
            TypeBearer typeBearer = insn.getResult().getTypeBearer();

            if (!typeBearer.isConstant()) continue;

            com.pranav.ide.dx.rop.cst.TypedConstant cst = (TypedConstant) typeBearer;
            final com.pranav.ide.dx.rop.code.RegisterSpec newReg = newRegs.get(cst);
