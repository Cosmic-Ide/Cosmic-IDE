...
public class DeadCodeRemover {
     ...
     */
    private final ArrayList<com.pranav.ide.dx.ssa.SsaInsn>[] useList;
     ...
     */
    private static boolean hasSideEffect(com.pranav.ide.dx.ssa.SsaInsn insn) {
        if (insn == null) {
    ...
    private void run() {
        ...
        while (0 <= (regV = worklist.nextSetBit(0))) {
            ...
            if (useList[regV].size() == 0
                ...
                com.pranav.ide.dx.ssa.SsaInsn insnS = ssaMeth.getDefinitionForRegister(regV);
                ...
                com.pranav.ide.dx.rop.code.RegisterSpecList sources = insnS.getSources();
    ...
    private void pruneDeadInstructions() {
        ...
        while ((blockIndex = reachable.nextClearBit(blockIndex)) < blocks.size()) {
            ...
            for (int i = 0; i < block.getInsns().size(); i++) {
                com.pranav.ide.dx.ssa.SsaInsn insn = block.getInsns().get(i);
                ...
                for (int j = 0; j < sourcesSize; j++) {
                    com.pranav.ide.dx.rop.code.RegisterSpec source = sources.get(j);
                ...
                for (com.pranav.ide.dx.ssa.SsaInsn use : useList[result.getReg()]) {
                    if (use instanceof com.pranav.ide.dx.ssa.PhiInsn) {
                        com.pranav.ide.dx.ssa.PhiInsn phiUse = (com.pranav.ide.dx.ssa.PhiInsn) use;
    ...
    private boolean isCircularNoSideEffect(int regV, BitSet set) {
        ...
        for (com.pranav.ide.dx.ssa.SsaInsn use : useList[regV]) {
        ...
        for (com.pranav.ide.dx.ssa.SsaInsn use : useList[regV]) {
            com.pranav.ide.dx.rop.code.RegisterSpec result = use.getResult();
