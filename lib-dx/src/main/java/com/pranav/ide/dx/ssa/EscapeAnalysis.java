...
import com.pranav.ide.dx.rop.code.Exceptions;
...
public class EscapeAnalysis {
     ...
     */
    private EscapeAnalysis(com.pranav.ide.dx.ssa.SsaMethod ssaMeth) {
     ...
     */
    private int findSetIndex(com.pranav.ide.dx.rop.code.RegisterSpec reg) {
        int i;
     ...
     * the move
     */
    private com.pranav.ide.dx.ssa.SsaInsn getInsnForMove(com.pranav.ide.dx.ssa.SsaInsn moveInsn) {
        int pred = moveInsn.getBlock().getPredecessors().nextSetBit(0);
        ArrayList<com.pranav.ide.dx.ssa.SsaInsn> predInsns = ssaMeth.getBlocks().get(pred).getInsns();
     ...
     */
    private com.pranav.ide.dx.ssa.SsaInsn getMoveForInsn(com.pranav.ide.dx.ssa.SsaInsn insn) {
        int succ = insn.getBlock().getSuccessors().nextSetBit(0);
        ArrayList<com.pranav.ide.dx.ssa.SsaInsn> succInsns = ssaMeth.getBlocks().get(succ).getInsns();
     ...
     */
    private void processInsn(com.pranav.ide.dx.ssa.SsaInsn insn) {
        int op = insn.getOpcode().getOpcode();
        com.pranav.ide.dx.rop.code.RegisterSpec result = insn.getResult();
        EscapeSet escSet;

        // Identify new objects
        if (op == com.pranav.ide.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO &&
                result.getTypeBearer().getBasicType() == com.pranav.ide.dx.rop.type.Type.BT_OBJECT) {
            // Handle objects generated through move_result_pseudo
            escSet = processMoveResultPseudoInsn(insn);
            processRegister(result, escSet);
        } else if (op == com.pranav.ide.dx.rop.code.RegOps.MOVE_PARAM &&
                result.getTypeBearer().getBasicType() == com.pranav.ide.dx.rop.type.Type.BT_OBJECT) {
            ...
            processRegister(result, escSet);
        } else if (op == com.pranav.ide.dx.rop.code.RegOps.MOVE_RESULT &&
                result.getTypeBearer().getBasicType() == com.pranav.ide.dx.rop.type.Type.BT_OBJECT) {
     ...
     */
    private EscapeSet processMoveResultPseudoInsn(com.pranav.ide.dx.ssa.SsaInsn insn) {
        com.pranav.ide.dx.rop.code.RegisterSpec result = insn.getResult();
        com.pranav.ide.dx.ssa.SsaInsn prevSsaInsn = getInsnForMove(insn);
        int prevOpcode = prevSsaInsn.getOpcode().getOpcode();
        EscapeSet escSet;
        com.pranav.ide.dx.rop.code.RegisterSpec prevSource;

        switch (prevOpcode) {
            // New instance / Constant
            case com.pranav.ide.dx.rop.code.RegOps.NEW_INSTANCE:
            case com.pranav.ide.dx.rop.code.RegOps.CONST:
            ...
            case com.pranav.ide.dx.rop.code.RegOps.CHECK_CAST:
            case com.pranav.ide.dx.rop.code.RegOps.GET_FIELD:
            case com.pranav.ide.dx.rop.code.RegOps.AGET:
                prevSource = prevSsaInsn.getSources().get(0);
                ...
                if (prevSource.getType() == com.pranav.ide.dx.rop.type.Type.KNOWN_NULL) {
                    escSet = new EscapeSet(result.getReg(), regCount,
                            EscapeState.NONE);
                } else {
                    escSet = new EscapeSet(result.getReg(), regCount,
     ...
     */
    private void processRegister(com.pranav.ide.dx.rop.code.RegisterSpec result, EscapeSet escSet) {
        ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> regWorklist = new ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec>();
        regWorklist.add(result);

        // Go through the worklist
        while (!regWorklist.isEmpty()) {
            int listSize = regWorklist.size() - 1;
            com.pranav.ide.dx.rop.code.RegisterSpec def = regWorklist.remove(listSize);
            List<com.pranav.ide.dx.ssa.SsaInsn> useList = ssaMeth.getUseListForRegister(def.getReg());

            // Handle all the uses of this register
            for (com.pranav.ide.dx.ssa.SsaInsn use : useList) {
                com.pranav.ide.dx.rop.code.Rop useOpcode = use.getOpcode();
     ...
     */
    private void processPhiUse(com.pranav.ide.dx.ssa.SsaInsn use, EscapeSet escSet,
                               ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> regWorklist) {
     ...
     *                    process for this object
     */
    private void processUse(com.pranav.ide.dx.rop.code.RegisterSpec def, com.pranav.ide.dx.ssa.SsaInsn use, EscapeSet escSet,
                            ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> regWorklist) {
        int useOpcode = use.getOpcode().getOpcode();
        switch (useOpcode) {
            case com.pranav.ide.dx.rop.code.RegOps.MOVE:
                // Follow uses of the move by adding it to the worklist
                escSet.regSet.set(use.getResult().getReg());
                regWorklist.add(use.getResult());
                break;
            case com.pranav.ide.dx.rop.code.RegOps.IF_EQ:
            case com.pranav.ide.dx.rop.code.RegOps.IF_NE:
            case com.pranav.ide.dx.rop.code.RegOps.CHECK_CAST:
                ...
                break;
            case com.pranav.ide.dx.rop.code.RegOps.APUT:
                // For array puts, check for a constant array index
                com.pranav.ide.dx.rop.code.RegisterSpec putIndex = use.getSources().get(2);
            ...
            case com.pranav.ide.dx.rop.code.RegOps.PUT_FIELD:
                ...
                com.pranav.ide.dx.rop.code.RegisterSpec putValue = use.getSources().get(0);
                if (putValue.getTypeBearer().getBasicType() != com.pranav.ide.dx.rop.type.Type.BT_OBJECT) {
                    break;
                }
                escSet.replaceableArray = false;

                // Raise 1st object's escape state to 2nd if 2nd is higher
                com.pranav.ide.dx.rop.code.RegisterSpecList sources = use.getSources();
            ...
            case com.pranav.ide.dx.rop.code.RegOps.AGET:
                ...
                com.pranav.ide.dx.rop.code.RegisterSpec getIndex = use.getSources().get(1);
            ...
            case com.pranav.ide.dx.rop.code.RegOps.RETURN:
            case com.pranav.ide.dx.rop.code.RegOps.THROW:
    ...
    private void scalarReplacement() {
        ...
        for (EscapeSet escSet : latticeValues) {
            ...
            com.pranav.ide.dx.ssa.SsaInsn def = ssaMeth.getDefinitionForRegister(e);
            com.pranav.ide.dx.ssa.SsaInsn prev = getInsnForMove(def);

            // Create a map for the new registers that will be created
            com.pranav.ide.dx.rop.type.TypeBearer lengthReg = prev.getSources().get(0).getTypeBearer();
            int length = ((com.pranav.ide.dx.rop.cst.CstLiteralBits) lengthReg).getIntBits();
            ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> newRegs =
                    new ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec>(length);
            HashSet<com.pranav.ide.dx.ssa.SsaInsn> deletedInsns = new HashSet<com.pranav.ide.dx.ssa.SsaInsn>();
            ...
            List<com.pranav.ide.dx.ssa.SsaInsn> useList = ssaMeth.getUseListForRegister(e);
            for (com.pranav.ide.dx.ssa.SsaInsn use : useList) {
     ...
     */
    private void replaceDef(com.pranav.ide.dx.ssa.SsaInsn def, com.pranav.ide.dx.ssa.SsaInsn prev, int length,
                            ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> newRegs) {
        Type resultType = def.getResult().getType();

        // Create new zeroed out registers for each element in the array
        for (int i = 0; i < length; i++) {
            com.pranav.ide.dx.rop.cst.Constant newZero = Zeroes.zeroFor(resultType.getComponentType());
            com.pranav.ide.dx.rop.cst.TypedConstant typedZero = (TypedConstant) newZero;
            com.pranav.ide.dx.rop.code.RegisterSpec newReg =
                    com.pranav.ide.dx.rop.code.RegisterSpec.make(ssaMeth.makeNewSsaReg(), typedZero);
            newRegs.add(newReg);
            insertPlainInsnBefore(def, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY, newReg,
                    com.pranav.ide.dx.rop.code.RegOps.CONST, newZero);
        }
    }
     ...
     *                     deletion
     */
    private void replaceUse(com.pranav.ide.dx.ssa.SsaInsn use, com.pranav.ide.dx.ssa.SsaInsn prev,
                            ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> newRegs,
                            HashSet<com.pranav.ide.dx.ssa.SsaInsn> deletedInsns) {
        int index;
        int length = newRegs.size();
        com.pranav.ide.dx.ssa.SsaInsn next;
        com.pranav.ide.dx.rop.code.RegisterSpecList sources;
        com.pranav.ide.dx.rop.code.RegisterSpec source, result;
        com.pranav.ide.dx.rop.cst.CstLiteralBits indexReg;
        ...
        switch (use.getOpcode().getOpcode()) {
            case com.pranav.ide.dx.rop.code.RegOps.AGET:
                ...
                indexReg = ((com.pranav.ide.dx.rop.cst.CstLiteralBits) sources.get(1).getTypeBearer());
                index = indexReg.getIntBits();
                if (index < length) {
                    source = newRegs.get(index);
                    result = source.withReg(next.getResult().getReg());
                    insertPlainInsnBefore(next, com.pranav.ide.dx.rop.code.RegisterSpecList.make(source),
                            result, com.pranav.ide.dx.rop.code.RegOps.MOVE, null);
                } else {
            ...
            case com.pranav.ide.dx.rop.code.RegOps.APUT:
                ...
                if (index < length) {
                    ...
                    insertPlainInsnBefore(use, com.pranav.ide.dx.rop.code.RegisterSpecList.make(source),
                            result, com.pranav.ide.dx.rop.code.RegOps.MOVE, null);
                ...
                break;
            case com.pranav.ide.dx.rop.code.RegOps.ARRAY_LENGTH:
                // Replace array lengths with const instructions
                com.pranav.ide.dx.rop.type.TypeBearer lengthReg = prev.getSources().get(0).getTypeBearer();
                //CstInteger lengthReg = CstInteger.make(length);
                next = getMoveForInsn(use);
                insertPlainInsnBefore(next, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY,
                        next.getResult(), com.pranav.ide.dx.rop.code.RegOps.CONST,
                        (com.pranav.ide.dx.rop.cst.Constant) lengthReg);
                deletedInsns.add(next);
                break;
            case com.pranav.ide.dx.rop.code.RegOps.MARK_LOCAL:
            ...
            case com.pranav.ide.dx.rop.code.RegOps.FILL_ARRAY_DATA:
                ...
                com.pranav.ide.dx.rop.code.Insn ropUse = use.getOriginalRopInsn();
                com.pranav.ide.dx.rop.code.FillArrayDataInsn fill = (FillArrayDataInsn) ropUse;
                ArrayList<com.pranav.ide.dx.rop.cst.Constant> constList = fill.getInitValues();
                for (int i = 0; i < length; i++) {
                    com.pranav.ide.dx.rop.code.RegisterSpec newFill =
                            com.pranav.ide.dx.rop.code.RegisterSpec.make(newRegs.get(i).getReg(),
                                    (TypeBearer) constList.get(i));
                    insertPlainInsnBefore(use, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY, newFill,
                            com.pranav.ide.dx.rop.code.RegOps.CONST, constList.get(i));
    ...
    private void movePropagate() {
        for (int i = 0; i < ssaMeth.getRegCount(); i++) {
            com.pranav.ide.dx.ssa.SsaInsn insn = ssaMeth.getDefinitionForRegister(i);

            // Look for move instructions only
            if (insn == null || insn.getOpcode() == null ||
                    insn.getOpcode().getOpcode() != com.pranav.ide.dx.rop.code.RegOps.MOVE) {
                continue;
            }

            final ArrayList<com.pranav.ide.dx.ssa.SsaInsn>[] useList = ssaMeth.getUseListCopy();
            final com.pranav.ide.dx.rop.code.RegisterSpec source = insn.getSources().get(0);
            final com.pranav.ide.dx.rop.code.RegisterSpec result = insn.getResult();
            ...
            com.pranav.ide.dx.ssa.RegisterMapper mapper = new RegisterMapper() {
                ...
                @Override
                public com.pranav.ide.dx.rop.code.RegisterSpec map(com.pranav.ide.dx.rop.code.RegisterSpec registerSpec) {
            ...
            for (com.pranav.ide.dx.ssa.SsaInsn use : useList[result.getReg()]) {
     ...
     */
    private void run() {
        ssaMeth.forEachBlockDepthFirstDom(new com.pranav.ide.dx.ssa.SsaBasicBlock.Visitor() {
            @Override
            public void visitBlock(com.pranav.ide.dx.ssa.SsaBasicBlock block,
                                   com.pranav.ide.dx.ssa.SsaBasicBlock unused) {
                block.forEachInsn(new com.pranav.ide.dx.ssa.SsaInsn.Visitor() {
                    @Override
     ...
     *                     deletion
     */
    private void insertExceptionThrow(com.pranav.ide.dx.ssa.SsaInsn insn, com.pranav.ide.dx.rop.code.RegisterSpec index,
                                      HashSet<com.pranav.ide.dx.ssa.SsaInsn> deletedInsns) {
        // Create a new ArrayIndexOutOfBoundsException
        com.pranav.ide.dx.rop.cst.CstType exception =
                new CstType(Exceptions.TYPE_ArrayIndexOutOfBoundsException);
        insertThrowingInsnBefore(insn, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY, null,
                com.pranav.ide.dx.rop.code.RegOps.NEW_INSTANCE, exception);

        // Add a successor block with a move result pseudo for the exception
        com.pranav.ide.dx.ssa.SsaBasicBlock currBlock = insn.getBlock();
        com.pranav.ide.dx.ssa.SsaBasicBlock newBlock =
                currBlock.insertNewSuccessor(currBlock.getPrimarySuccessor());
        com.pranav.ide.dx.ssa.SsaInsn newInsn = newBlock.getInsns().get(0);
        com.pranav.ide.dx.rop.code.RegisterSpec newReg =
                com.pranav.ide.dx.rop.code.RegisterSpec.make(ssaMeth.makeNewSsaReg(), exception);
        insertPlainInsnBefore(newInsn, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY, newReg,
                com.pranav.ide.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO, null);

        // Add another successor block to initialize the exception
        com.pranav.ide.dx.ssa.SsaBasicBlock newBlock2 =
                newBlock.insertNewSuccessor(newBlock.getPrimarySuccessor());
        com.pranav.ide.dx.ssa.SsaInsn newInsn2 = newBlock2.getInsns().get(0);
        com.pranav.ide.dx.rop.cst.CstNat newNat = new CstNat(new com.pranav.ide.dx.rop.cst.CstString("<init>"), new CstString("(I)V"));
        com.pranav.ide.dx.rop.cst.CstMethodRef newRef = new CstMethodRef(exception, newNat);
        insertThrowingInsnBefore(newInsn2, com.pranav.ide.dx.rop.code.RegisterSpecList.make(newReg, index),
                null, com.pranav.ide.dx.rop.code.RegOps.INVOKE_DIRECT, newRef);
        ...
        com.pranav.ide.dx.ssa.SsaInsn newInsn3 = newBlock3.getInsns().get(0);
        insertThrowingInsnBefore(newInsn3, com.pranav.ide.dx.rop.code.RegisterSpecList.make(newReg), null,
                com.pranav.ide.dx.rop.code.RegOps.THROW, null);
     ...
     */
    private void insertPlainInsnBefore(com.pranav.ide.dx.ssa.SsaInsn insn,
                                       com.pranav.ide.dx.rop.code.RegisterSpecList newSources, com.pranav.ide.dx.rop.code.RegisterSpec newResult, int newOpcode,
                                       com.pranav.ide.dx.rop.cst.Constant cst) {

        com.pranav.ide.dx.rop.code.Insn originalRopInsn = insn.getOriginalRopInsn();
        com.pranav.ide.dx.rop.code.Rop newRop;
        if (newOpcode == RegOps.MOVE_RESULT_PSEUDO) {
            newRop = com.pranav.ide.dx.rop.code.Rops.opMoveResultPseudo(newResult.getType());
        } else {
            newRop = com.pranav.ide.dx.rop.code.Rops.ropFor(newOpcode, newResult, newSources, cst);
        }

        com.pranav.ide.dx.rop.code.Insn newRopInsn;
        if (cst == null) {
        ...
        List<com.pranav.ide.dx.ssa.SsaInsn> insns = insn.getBlock().getInsns();
     ...
     * @param cst        {@code null-ok;} constant for new instruction, if any
     */
    private void insertThrowingInsnBefore(com.pranav.ide.dx.ssa.SsaInsn insn,
                                          RegisterSpecList newSources, RegisterSpec newResult, int newOpcode,
                                          Constant cst) {

        com.pranav.ide.dx.rop.code.Insn origRopInsn = insn.getOriginalRopInsn();
        Rop newRop = Rops.ropFor(newOpcode, newResult, newSources, cst);
        Insn newRopInsn;
        if (cst == null) {
            newRopInsn = new ThrowingInsn(newRop,
                    origRopInsn.getPosition(), newSources, com.pranav.ide.dx.rop.type.StdTypeList.EMPTY);
        } else {
