...
public class LiteralOpUpgrader {
     ...
     */
    private static boolean isConstIntZeroOrKnownNull(com.pranav.ide.dx.rop.code.RegisterSpec spec) {
        com.pranav.ide.dx.rop.type.TypeBearer tb = spec.getTypeBearer();
        if (tb instanceof com.pranav.ide.dx.rop.cst.CstLiteralBits) {
            com.pranav.ide.dx.rop.cst.CstLiteralBits clb = (CstLiteralBits) tb;
    ...
    private void run() {
        ...
        ssaMeth.forEachInsn(new com.pranav.ide.dx.ssa.SsaInsn.Visitor() {
            @Override
            public void visitMoveInsn(com.pranav.ide.dx.ssa.NormalSsaInsn insn) {
                // do nothing
            }
            ...
            @Override
            public void visitNonMoveInsn(com.pranav.ide.dx.ssa.NormalSsaInsn insn) {

                com.pranav.ide.dx.rop.code.Insn originalRopInsn = insn.getOriginalRopInsn();
                com.pranav.ide.dx.rop.code.Rop opcode = originalRopInsn.getOpcode();
                ...
                if (opcode.getBranchingness() == com.pranav.ide.dx.rop.code.Rop.BRANCH_IF) {
                    ...
                    if (isConstIntZeroOrKnownNull(sources.get(0))) {
                        replacePlainInsn(insn, sources.withoutFirst(),
                                com.pranav.ide.dx.rop.code.RegOps.flippedIfOpcode(opcode.getOpcode()), null);
                    ...
                    insn.setNewSources(
                            com.pranav.ide.dx.rop.code.RegisterSpecList.make(
     ...
     */
    private boolean tryReplacingWithConstant(com.pranav.ide.dx.ssa.NormalSsaInsn insn) {
        com.pranav.ide.dx.rop.code.Insn originalRopInsn = insn.getOriginalRopInsn();
        com.pranav.ide.dx.rop.code.Rop opcode = originalRopInsn.getOpcode();
        RegisterSpec result = insn.getResult();

        if (result != null && !ssaMeth.isRegALocal(result) &&
                opcode.getOpcode() != com.pranav.ide.dx.rop.code.RegOps.CONST) {
            TypeBearer type = insn.getResult().getTypeBearer();
            if (type.isConstant() && type.getBasicType() == Type.BT_INT) {
                // Replace the instruction with a constant
                replacePlainInsn(insn, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY,
                        com.pranav.ide.dx.rop.code.RegOps.CONST, (com.pranav.ide.dx.rop.cst.Constant) type);

                // Remove the source as well if this is a move-result-pseudo
                if (opcode.getOpcode() == com.pranav.ide.dx.rop.code.RegOps.MOVE_RESULT_PSEUDO) {
                    int pred = insn.getBlock().getPredecessors().nextSetBit(0);
                    ArrayList<com.pranav.ide.dx.ssa.SsaInsn> predInsns =
                            ssaMeth.getBlocks().get(pred).getInsns();
                    com.pranav.ide.dx.ssa.NormalSsaInsn sourceInsn =
                            (com.pranav.ide.dx.ssa.NormalSsaInsn) predInsns.get(predInsns.size() - 1);
                    replacePlainInsn(sourceInsn, com.pranav.ide.dx.rop.code.RegisterSpecList.EMPTY,
                            com.pranav.ide.dx.rop.code.RegOps.GOTO, null);
                }
                return true;
            }
        }
     ...
     */
    private void replacePlainInsn(com.pranav.ide.dx.ssa.NormalSsaInsn insn,
        ...
        com.pranav.ide.dx.ssa.NormalSsaInsn newInsn = new NormalSsaInsn(newRopInsn, insn.getBlock());
