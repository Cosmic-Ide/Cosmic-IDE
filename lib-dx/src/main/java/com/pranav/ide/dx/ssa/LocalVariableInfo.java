...
import com.pranav.ide.dx.util.MutabilityControl;
 ...
 * com.pranav.ide.dx.ssa.SsaMethod}.
 * Stolen from {@link com.pranav.ide.dx.rop.code.LocalVariableInfo}.
 */
public class LocalVariableInfo extends MutabilityControl {
    ...
    /**
     * {@code non-null;} {@link com.pranav.ide.dx.rop.code.RegisterSpecSet} to use when indicating a block
     * that has no locals; it is empty and immutable but has an appropriate
     * max size for the method
     */
    private final com.pranav.ide.dx.rop.code.RegisterSpecSet emptySet;
     ...
     */
    private final com.pranav.ide.dx.rop.code.RegisterSpecSet[] blockStarts;

    /**
     * {@code non-null;} map from instructions to the variable each assigns
     */
    private final HashMap<com.pranav.ide.dx.ssa.SsaInsn, com.pranav.ide.dx.rop.code.RegisterSpec> insnAssignments;
    ...
    public LocalVariableInfo(SsaMethod method) {
        ...
        List<com.pranav.ide.dx.ssa.SsaBasicBlock> blocks = method.getBlocks();

        this.regCount = method.getRegCount();
        this.emptySet = new com.pranav.ide.dx.rop.code.RegisterSpecSet(regCount);
        this.blockStarts = new com.pranav.ide.dx.rop.code.RegisterSpecSet[blocks.size()];
        this.insnAssignments =
                new HashMap<com.pranav.ide.dx.ssa.SsaInsn, com.pranav.ide.dx.rop.code.RegisterSpec>(/*hint here*/);
     ...
     */
    public void setStarts(int index, com.pranav.ide.dx.rop.code.RegisterSpecSet specs) {
        throwIfImmutable();
     ...
     */
    public boolean mergeStarts(int index, com.pranav.ide.dx.rop.code.RegisterSpecSet specs) {
        com.pranav.ide.dx.rop.code.RegisterSpecSet start = getStarts0(index);
        ...
        com.pranav.ide.dx.rop.code.RegisterSpecSet newStart = start.mutableCopy();
     ...
     */
    public com.pranav.ide.dx.rop.code.RegisterSpecSet getStarts(int index) {
        com.pranav.ide.dx.rop.code.RegisterSpecSet result = getStarts0(index);
     ...
     * @return {@code non-null;} the associated register set
     */
    public com.pranav.ide.dx.rop.code.RegisterSpecSet getStarts(SsaBasicBlock block) {
     ...
     * newly-allocated empty {@link com.pranav.ide.dx.rop.code.RegisterSpecSet} of appropriate
     * max size if there is not yet any set associated with the block.
     *
     * @param index {@code >= 0;} the block index
     * @return {@code non-null;} the associated register set
     */
    public com.pranav.ide.dx.rop.code.RegisterSpecSet mutableCopyOfStarts(int index) {
        com.pranav.ide.dx.rop.code.RegisterSpecSet result = getStarts0(index);

        return (result != null) ?
                result.mutableCopy() : new com.pranav.ide.dx.rop.code.RegisterSpecSet(regCount);
    }
     ...
     */
    public void addAssignment(com.pranav.ide.dx.ssa.SsaInsn insn, com.pranav.ide.dx.rop.code.RegisterSpec spec) {
        throwIfImmutable();
