...
import com.pranav.ide.dx.cf.code.Merger;
...
public class PhiTypeResolver {
     ...
     */
    public static void process(com.pranav.ide.dx.ssa.SsaMethod ssaMeth) {
     ...
     */
    private static boolean equalsHandlesNulls(com.pranav.ide.dx.rop.code.LocalItem a, com.pranav.ide.dx.rop.code.LocalItem b) {
    ...
    private void run() {
        ...
        for (int reg = 0; reg < regCount; reg++) {
            com.pranav.ide.dx.ssa.SsaInsn definsn = ssaMeth.getDefinitionForRegister(reg);

            if (definsn != null
                    && (definsn.getResult().getBasicType() == com.pranav.ide.dx.rop.type.Type.BT_VOID)) {
                worklist.set(reg);
            }
        }
        ...
        while (0 <= (reg = worklist.nextSetBit(0))) {
            ...
            if (resolveResultType(definsn)) {
                ...
                List<com.pranav.ide.dx.ssa.SsaInsn> useList = ssaMeth.getUseListForRegister(reg);

                int sz = useList.size();
                for (int i = 0; i < sz; i++) {
                    SsaInsn useInsn = useList.get(i);
                    com.pranav.ide.dx.rop.code.RegisterSpec resultReg = useInsn.getResult();
    ...
    boolean resolveResultType(PhiInsn insn) {
        ...
        com.pranav.ide.dx.rop.code.RegisterSpec first = null;
        int firstIndex = -1;

        int szSources = sources.size();
        for (int i = 0; i < szSources; i++) {
            com.pranav.ide.dx.rop.code.RegisterSpec rs = sources.get(i);

            if (rs.getBasicType() != com.pranav.ide.dx.rop.type.Type.BT_VOID) {
                first = rs;
                firstIndex = i;
            }
        }
        ...
        com.pranav.ide.dx.rop.code.LocalItem firstLocal = first.getLocalItem();
        com.pranav.ide.dx.rop.type.TypeBearer mergedType = first.getType();
