...
public class SCCP {
     ...
     */
    private final com.pranav.ide.dx.rop.cst.Constant[] latticeConstants;
    /**
     * Worklist of basic blocks to be processed
     */
    private final ArrayList<com.pranav.ide.dx.ssa.SsaBasicBlock> cfgWorklist;
    /**
     * Worklist of executed basic blocks with phis to be processed
     */
    private final ArrayList<com.pranav.ide.dx.ssa.SsaBasicBlock> cfgPhiWorklist;
    /**
     ...
     */
    private final ArrayList<com.pranav.ide.dx.ssa.SsaInsn> ssaWorklist;
    /**
     ...
     * possible.
     */
    private final ArrayList<com.pranav.ide.dx.ssa.SsaInsn> varyingWorklist;
    /**
     * Worklist of potential branches to convert to gotos
     */
    private final ArrayList<com.pranav.ide.dx.ssa.SsaInsn> branchWorklist;

    private SCCP(com.pranav.ide.dx.ssa.SsaMethod ssaMeth) {
        this.ssaMeth = ssaMeth;
        this.regCount = ssaMeth.getRegCount();
        this.latticeValues = new int[this.regCount];
        this.latticeConstants = new com.pranav.ide.dx.rop.cst.Constant[this.regCount];
        this.cfgWorklist = new ArrayList<com.pranav.ide.dx.ssa.SsaBasicBlock>();
        this.cfgPhiWorklist = new ArrayList<com.pranav.ide.dx.ssa.SsaBasicBlock>();
        this.executableBlocks = new BitSet(ssaMeth.getBlocks().size());
        this.ssaWorklist = new ArrayList<com.pranav.ide.dx.ssa.SsaInsn>();
        this.varyingWorklist = new ArrayList<com.pranav.ide.dx.ssa.SsaInsn>();
        this.branchWorklist = new ArrayList<com.pranav.ide.dx.ssa.SsaInsn>();
     ...
     */
    private void addBlockToWorklist(com.pranav.ide.dx.ssa.SsaBasicBlock ssaBlock) {
    ...
    private void addUsersToWorklist(int reg, int latticeValue) {
        if (latticeValue == VARYING) {
            for (com.pranav.ide.dx.ssa.SsaInsn insn : ssaMeth.getUseListForRegister(reg)) {
     ...
     */
    private boolean setLatticeValueTo(int reg, int value, com.pranav.ide.dx.rop.cst.Constant cst) {
        if (value != CONSTANT) {
    ...
    private void simulatePhi(PhiInsn insn) {
        ...
        com.pranav.ide.dx.rop.cst.Constant phiConstant = null;
     ...
     */
    private void simulateBlock(com.pranav.ide.dx.ssa.SsaBasicBlock block) {
     ...
     */
    private void simulateBranch(com.pranav.ide.dx.ssa.SsaInsn insn) {
        com.pranav.ide.dx.rop.code.Rop opcode = insn.getOpcode();
        com.pranav.ide.dx.rop.code.RegisterSpecList sources = insn.getSources();
        ...
        if (opcode.getBranchingness() == com.pranav.ide.dx.rop.code.Rop.BRANCH_IF) {
            com.pranav.ide.dx.rop.cst.Constant cA = null;
            com.pranav.ide.dx.rop.cst.Constant cB = null;

            com.pranav.ide.dx.rop.code.RegisterSpec specA = sources.get(0);
            ...
            if (sources.size() == 2) {
                com.pranav.ide.dx.rop.code.RegisterSpec specB = sources.get(1);
            ...
            if (cA != null && sources.size() == 1) {
                switch (((com.pranav.ide.dx.rop.cst.TypedConstant) cA).getBasicType()) {
                    case com.pranav.ide.dx.rop.type.Type.BT_INT:
                        constantBranch = true;
                        int vA = ((com.pranav.ide.dx.rop.cst.CstInteger) cA).getValue();
                        switch (opcode.getOpcode()) {
                            ...
                            case com.pranav.ide.dx.rop.code.RegOps.IF_NE:
                                constantSuccessor = (vA != 0);
                                break;
                            case com.pranav.ide.dx.rop.code.RegOps.IF_LT:
                                constantSuccessor = (vA < 0);
                                break;
                            case com.pranav.ide.dx.rop.code.RegOps.IF_GE:
                                constantSuccessor = (vA >= 0);
                                break;
                            case com.pranav.ide.dx.rop.code.RegOps.IF_LE:
                                constantSuccessor = (vA <= 0);
                                break;
                            case com.pranav.ide.dx.rop.code.RegOps.IF_GT:
                                constantSuccessor = (vA > 0);
                                break;
                            default:
                ...
                switch (((com.pranav.ide.dx.rop.cst.TypedConstant) cA).getBasicType()) {
                    case com.pranav.ide.dx.rop.type.Type.BT_INT:
                        constantBranch = true;
                        int vA = ((com.pranav.ide.dx.rop.cst.CstInteger) cA).getValue();
                        int vB = ((com.pranav.ide.dx.rop.cst.CstInteger) cB).getValue();
                        switch (opcode.getOpcode()) {
                            case com.pranav.ide.dx.rop.code.RegOps.IF_EQ:
         ...
         */
        com.pranav.ide.dx.ssa.SsaBasicBlock block = insn.getBlock();
     ...
     */
    private com.pranav.ide.dx.rop.cst.Constant simulateMath(com.pranav.ide.dx.ssa.SsaInsn insn, int resultType) {
        com.pranav.ide.dx.rop.code.Insn ropInsn = insn.getOriginalRopInsn();
        int opcode = insn.getOpcode().getOpcode();
        com.pranav.ide.dx.rop.code.RegisterSpecList sources = insn.getSources();
        int regA = sources.get(0).getReg();
        com.pranav.ide.dx.rop.cst.Constant cA;
        com.pranav.ide.dx.rop.cst.Constant cB;
        ...
        if (sources.size() == 1) {
            com.pranav.ide.dx.rop.code.CstInsn cstInsn = (com.pranav.ide.dx.rop.code.CstInsn) ropInsn;
        ...
        switch (resultType) {
            case Type.BT_INT:
                ...
                switch (opcode) {
                    case com.pranav.ide.dx.rop.code.RegOps.ADD:
                        vR = vA + vB;
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.SUB:
                        ...
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.MUL:
                        vR = vA * vB;
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.DIV:
                        if (vB == 0) {
                            skip = true;
                        ...
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.AND:
                        vR = vA & vB;
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.OR:
                        vR = vA | vB;
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.XOR:
                        vR = vA ^ vB;
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.SHL:
                        vR = vA << vB;
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.SHR:
                        vR = vA >> vB;
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.USHR:
                        vR = vA >>> vB;
                        break;
                    case com.pranav.ide.dx.rop.code.RegOps.REM:
                        if (vB == 0) {
                            skip = true;
                            vR = 0; // just to hide a warning
                        } else {
     ...
     */
    private void simulateStmt(com.pranav.ide.dx.ssa.SsaInsn insn) {
        com.pranav.ide.dx.rop.code.Insn ropInsn = insn.getOriginalRopInsn();
        ...
        if (result == null) {
            ...
            if (opcode == com.pranav.ide.dx.rop.code.RegOps.DIV || opcode == com.pranav.ide.dx.rop.code.RegOps.REM) {
                com.pranav.ide.dx.ssa.SsaBasicBlock succ = insn.getBlock().getPrimarySuccessor();
        ...
        switch (opcode) {
            case com.pranav.ide.dx.rop.code.RegOps.CONST: {
                com.pranav.ide.dx.rop.code.CstInsn cstInsn = (CstInsn) ropInsn;
    ...
    private void run() {
        com.pranav.ide.dx.ssa.SsaBasicBlock firstBlock = ssaMeth.getEntryBlock();
        ...
        while (!cfgWorklist.isEmpty()
            ...
            while (!cfgWorklist.isEmpty()) {
                ...
                com.pranav.ide.dx.ssa.SsaBasicBlock block = cfgWorklist.remove(listSize);
                simulateBlock(block);
            }

            while (!cfgPhiWorklist.isEmpty()) {
                int listSize = cfgPhiWorklist.size() - 1;
                com.pranav.ide.dx.ssa.SsaBasicBlock block = cfgPhiWorklist.remove(listSize);
                simulatePhiBlock(block);
            }

            while (!varyingWorklist.isEmpty()) {
                int listSize = varyingWorklist.size() - 1;
                com.pranav.ide.dx.ssa.SsaInsn insn = varyingWorklist.remove(listSize);
            ...
            while (!ssaWorklist.isEmpty()) {
                ...
                com.pranav.ide.dx.ssa.SsaInsn insn = ssaWorklist.remove(listSize);
    ...
    private void replaceConstants() {
        for (int reg = 0; reg < regCount; reg++) {
            ...
            if (!(latticeConstants[reg] instanceof com.pranav.ide.dx.rop.cst.TypedConstant)) {
                // We can't do much with these
                continue;
            }

            com.pranav.ide.dx.ssa.SsaInsn defn = ssaMeth.getDefinitionForRegister(reg);
            ...
            com.pranav.ide.dx.rop.code.RegisterSpec dest = defn.getResult();
            com.pranav.ide.dx.rop.code.RegisterSpec newDest
                    = dest.withType((com.pranav.ide.dx.rop.cst.TypedConstant) latticeConstants[reg]);
            ...
            for (com.pranav.ide.dx.ssa.SsaInsn insn : ssaMeth.getUseListForRegister(reg)) {
                ...
                com.pranav.ide.dx.rop.code.RegisterSpec spec = sources.get(index);
