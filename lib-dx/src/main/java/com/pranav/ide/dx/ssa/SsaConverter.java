...
public class SsaConverter {
     ...
     */
    public static com.pranav.ide.dx.ssa.SsaMethod convertToSsaMethod(com.pranav.ide.dx.rop.code.RopMethod rmeth,
                                                                     int paramWidth, boolean isStatic) {
        com.pranav.ide.dx.ssa.SsaMethod result
                = com.pranav.ide.dx.ssa.SsaMethod.newFromRopMethod(rmeth, paramWidth, isStatic);
        ...
        new com.pranav.ide.dx.ssa.SsaRenamer(result).run();
     ...
     */
    public static void updateSsaMethod(com.pranav.ide.dx.ssa.SsaMethod ssaMeth, int threshold) {
     ...
     */
    private static void edgeSplit(com.pranav.ide.dx.ssa.SsaMethod result) {
     ...
     */
    private static void edgeSplitPredecessors(com.pranav.ide.dx.ssa.SsaMethod result) {
        ArrayList<com.pranav.ide.dx.ssa.SsaBasicBlock> blocks = result.getBlocks();
     ...
     */
    private static boolean nodeNeedsUniquePredecessor(com.pranav.ide.dx.ssa.SsaBasicBlock block) {
        /*
     ...
     */
    private static void edgeSplitMoveExceptionsAndResults(com.pranav.ide.dx.ssa.SsaMethod ssaMeth) {
        ArrayList<com.pranav.ide.dx.ssa.SsaBasicBlock> blocks = ssaMeth.getBlocks();
        ...
        for (int i = blocks.size() - 1; i >= 0; i--) {
            ...
            if (!block.isExitBlock()
                ...
                for (int j = preds.nextSetBit(0); j >= 0;
                    ...
                    com.pranav.ide.dx.ssa.SsaBasicBlock predecessor = blocks.get(j);
                    com.pranav.ide.dx.ssa.SsaBasicBlock zNode
     ...
     * @param result {@code non-null;} method to process
     */
    private static void edgeSplitSuccessors(com.pranav.ide.dx.ssa.SsaMethod result) {
        ArrayList<com.pranav.ide.dx.ssa.SsaBasicBlock> blocks = result.getBlocks();
     ...
     */
    private static boolean needsNewSuccessor(com.pranav.ide.dx.ssa.SsaBasicBlock block,
                                             com.pranav.ide.dx.ssa.SsaBasicBlock succ) {
        ArrayList<com.pranav.ide.dx.ssa.SsaInsn> insns = block.getInsns();
        com.pranav.ide.dx.ssa.SsaInsn lastInsn = insns.get(insns.size() - 1);
    ...
    private static void placePhiFunctions(SsaMethod ssaMeth,
        ...
        ArrayList<com.pranav.ide.dx.ssa.SsaBasicBlock> ssaBlocks;
        int regCount;
        int blockCount;
        ...
        com.pranav.ide.dx.ssa.DomFront df = new com.pranav.ide.dx.ssa.DomFront(ssaMeth);
        ...
        for (int bi = 0, s = ssaBlocks.size(); bi < s; bi++) {
            ...
            for (SsaInsn insn : b.getInsns()) {
                com.pranav.ide.dx.rop.code.RegisterSpec rs = insn.getResult();
