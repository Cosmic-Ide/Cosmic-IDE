...
public class SsaRenamer implements Runnable {
     ...
     */
    private final com.pranav.ide.dx.rop.code.RegisterSpec[][] startsForBlocks;
    /**
     * map of SSA register number to debug (local var names) or null of n/a
     */
    private final ArrayList<com.pranav.ide.dx.rop.code.LocalItem> ssaRegToLocalItems;
    /**
     ...
     *                be renamed.
     */
    public SsaRenamer(com.pranav.ide.dx.ssa.SsaMethod ssaMeth) {
        ...
        threshold = 0;
        startsForBlocks = new com.pranav.ide.dx.rop.code.RegisterSpec[ssaMeth.getBlocks().size()][];

        ssaRegToLocalItems = new ArrayList<com.pranav.ide.dx.rop.code.LocalItem>();
        ...
        com.pranav.ide.dx.rop.code.RegisterSpec[] initialRegMapping = new com.pranav.ide.dx.rop.code.RegisterSpec[ropRegCount];
        for (int i = 0; i < ropRegCount; i++) {
            // everyone starts with a version 0 register
            initialRegMapping[i] = com.pranav.ide.dx.rop.code.RegisterSpec.make(i, com.pranav.ide.dx.rop.type.Type.VOID);
     ...
     */
    private static com.pranav.ide.dx.rop.code.RegisterSpec[] dupArray(com.pranav.ide.dx.rop.code.RegisterSpec[] orig) {
        com.pranav.ide.dx.rop.code.RegisterSpec[] copy = new com.pranav.ide.dx.rop.code.RegisterSpec[orig.length];
     ...
     */
    private com.pranav.ide.dx.rop.code.LocalItem getLocalForNewReg(int ssaReg) {
     ...
     */
    private void setNameForSsaReg(com.pranav.ide.dx.rop.code.RegisterSpec ssaReg) {
        int reg = ssaReg.getReg();
        com.pranav.ide.dx.rop.code.LocalItem local = ssaReg.getLocalItem();
     ...
     * as appropriate.
     */
    private class BlockRenamer implements com.pranav.ide.dx.ssa.SsaInsn.Visitor {
        /**
         * {@code non-null;} block we're processing.
         */
        private final com.pranav.ide.dx.ssa.SsaBasicBlock block;
         ...
         */
        private final com.pranav.ide.dx.rop.code.RegisterSpec[] currentMapping;

        /**
         * contains the set of moves we need to keep to preserve local
         * var info. All other moves will be deleted.
         */
        private final HashSet<com.pranav.ide.dx.ssa.SsaInsn> movesToKeep;

        /**
         * maps the set of insns to replace after renaming is finished
         * on the block.
         */
        private final HashMap<com.pranav.ide.dx.ssa.SsaInsn, com.pranav.ide.dx.ssa.SsaInsn> insnsToReplace;
         ...
         */
        BlockRenamer(final com.pranav.ide.dx.ssa.SsaBasicBlock block) {
            this.block = block;
            currentMapping = startsForBlocks[block.getIndex()];
            movesToKeep = new HashSet<com.pranav.ide.dx.ssa.SsaInsn>();
            insnsToReplace = new HashMap<com.pranav.ide.dx.ssa.SsaInsn, com.pranav.ide.dx.ssa.SsaInsn>();
        ...
        public void process() {
            ...
            for (int i = szInsns - 1; i >= 0; i--) {
                ...
                com.pranav.ide.dx.ssa.SsaInsn replaceInsn;
            ...
            boolean first = true;
            for (com.pranav.ide.dx.ssa.SsaBasicBlock child : block.getDomChildren()) {
                if (child != block) {
                    // Don't bother duplicating the array for the first child.
                    com.pranav.ide.dx.rop.code.RegisterSpec[] childStart = first ? currentMapping
         ...
         */
        private void addMapping(int ropReg, com.pranav.ide.dx.rop.code.RegisterSpec ssaReg) {
            int ssaRegNum = ssaReg.getReg();
            com.pranav.ide.dx.rop.code.LocalItem ssaRegLocal = ssaReg.getLocalItem();
            ...
            for (int i = currentMapping.length - 1; i >= 0; i--) {
                com.pranav.ide.dx.rop.code.RegisterSpec cur = currentMapping[i];
         ...
         */
        @Override
        public void visitPhiInsn(com.pranav.ide.dx.ssa.PhiInsn phi) {
        ...
        public void visitMoveInsn(NormalSsaInsn insn) {
            ...
            com.pranav.ide.dx.rop.code.RegisterSpec ropResult = insn.getResult();
            ...
            com.pranav.ide.dx.rop.code.LocalItem sourceLocal
                    = currentMapping[ropSourceReg].getLocalItem();
            com.pranav.ide.dx.rop.code.LocalItem resultLocal = ropResult.getLocalItem();
            ...
            com.pranav.ide.dx.rop.code.LocalItem newLocal
             ...
             */
            com.pranav.ide.dx.rop.code.RegisterSpec ssaReg
                    = com.pranav.ide.dx.rop.code.RegisterSpec.makeLocalOptional(
            ...
            if (!Optimizer.getPreserveLocals() || (onlyOneAssociatedLocal
                ...
                com.pranav.ide.dx.rop.code.RegisterSpecList ssaSources = RegisterSpecList.make(
                        com.pranav.ide.dx.rop.code.RegisterSpec.make(ssaReg.getReg(),
        ...
        private void updateSuccessorPhis() {
            com.pranav.ide.dx.ssa.PhiInsn.Visitor visitor = new com.pranav.ide.dx.ssa.PhiInsn.Visitor() {
                @Override
        ...
        private class RenamingMapper extends RegisterMapper {
            ...
            public com.pranav.ide.dx.rop.code.RegisterSpec map(com.pranav.ide.dx.rop.code.RegisterSpec registerSpec) {
                ...
                if (DEBUG) {
                    com.pranav.ide.dx.rop.code.RegisterSpec newVersion = currentMapping[reg];
