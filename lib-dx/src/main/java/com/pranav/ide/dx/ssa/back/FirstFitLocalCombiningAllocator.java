...
import com.pranav.ide.dx.ssa.InterferenceRegisterMapper;
import com.pranav.ide.dx.ssa.NormalSsaInsn;
import com.pranav.ide.dx.ssa.Optimizer;
import com.pranav.ide.dx.ssa.PhiInsn;
...
public class FirstFitLocalCombiningAllocator extends RegisterAllocator {
     ...
     */
    private final Map<com.pranav.ide.dx.rop.code.LocalItem, ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec>> localVariables;
    /**
    ...
    public FirstFitLocalCombiningAllocator(
        ...
        localVariables = new TreeMap<com.pranav.ide.dx.rop.code.LocalItem, ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec>>();
    ...
    private void printLocalVars() {
        ...
        for (Map.Entry<com.pranav.ide.dx.rop.code.LocalItem, ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec>> e :
                localVariables.entrySet()) {
            StringBuilder regs = new StringBuilder();

            regs.append('{');
            regs.append(' ');
            for (com.pranav.ide.dx.rop.code.RegisterSpec reg : e.getValue()) {
                regs.append('v');
    ...
    private void handleLocalAssociatedParams() {
        for (ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> ssaRegs : localVariables.values()) {
            int sz = ssaRegs.size();
            int paramIndex = -1;
            int paramCategory = 0;
            ...
            for (int i = 0; i < sz; i++) {
                com.pranav.ide.dx.rop.code.RegisterSpec ssaSpec = ssaRegs.get(i);
    ...
    private int getParameterIndexForReg(int ssaReg) {
        ...
        if (opcode != null && opcode.getOpcode() == com.pranav.ide.dx.rop.code.RegOps.MOVE_PARAM) {
            com.pranav.ide.dx.rop.code.CstInsn origInsn = (CstInsn) defInsn.getOriginalRopInsn();
    ...
    private void handleLocalAssociatedOther() {
        for (ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> specs : localVariables.values()) {
            ...
            do {
                ...
                int sz = specs.size();
                for (int i = 0; i < sz; i++) {
                    com.pranav.ide.dx.rop.code.RegisterSpec ssaSpec = specs.get(i);
    ...
    private boolean tryMapRegs(
            ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> specs, int ropReg,
            int maxAllowedCategory, boolean markReserved) {
        boolean remaining = false;
        for (com.pranav.ide.dx.rop.code.RegisterSpec spec : specs) {
     ...
     */
    private boolean tryMapReg(com.pranav.ide.dx.rop.code.RegisterSpec ssaSpec, int ropReg,
    ...
    private void handleUnassociatedParameters() {
        ...
        for (int ssaReg = 0; ssaReg < szSsaRegs; ssaReg++) {
            ...
            com.pranav.ide.dx.rop.code.RegisterSpec ssaSpec = getDefinitionSpecForSsaReg(ssaReg);
            if (paramIndex >= 0) {
                addMapping(ssaSpec, paramIndex);
            }
        }
    ...
    private void handleCheckCastResults() {
        for (NormalSsaInsn insn : moveResultPseudoInsns) {
            com.pranav.ide.dx.rop.code.RegisterSpec moveRegSpec = insn.getResult();
            ...
            if (checkCastInsn.getOpcode().getOpcode() != com.pranav.ide.dx.rop.code.RegOps.CHECK_CAST) {
                continue;
            }

            com.pranav.ide.dx.rop.code.RegisterSpec checkRegSpec = checkCastInsn.getSources().get(0);
            ...
            if (!moveMapped || !checkMapped) {
                ...
                ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> ssaRegs =
                        new ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec>(2);
     ...
     */
    private boolean canMapRegs(ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> specs, int ropReg) {
        for (com.pranav.ide.dx.rop.code.RegisterSpec spec : specs) {
     ...
     */
    private boolean canMapReg(com.pranav.ide.dx.rop.code.RegisterSpec ssaSpec, int ropReg) {
        int category = ssaSpec.getCategory();
    ...
    private void analyzeInstructions() {
        ssaMeth.forEachInsn(new SsaInsn.Visitor() {
            ...
            private void processInsn(SsaInsn insn) {
                com.pranav.ide.dx.rop.code.RegisterSpec assignment;
                assignment = insn.getLocalAssignment();

                if (assignment != null) {
                    com.pranav.ide.dx.rop.code.LocalItem local = assignment.getLocalItem();

                    ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> regList
                            = localVariables.get(local);

                    if (regList == null) {
                        regList = new ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec>();
     ...
     * @param ropReg  {@code >=0;} rop register to map to
     */
    private void addMapping(com.pranav.ide.dx.rop.code.RegisterSpec ssaSpec, int ropReg) {
        int ssaReg = ssaSpec.getReg();
    ...
    private void adjustAndMapSourceRangeRange(NormalSsaInsn insn) {
        ...
        for (int i = 0; i < szSources; i++) {
            com.pranav.ide.dx.rop.code.RegisterSpec source = sources.get(i);
            ...
            com.pranav.ide.dx.rop.code.LocalItem localItem = getLocalItemForReg(sourceReg);
            addMapping(source, curRopReg);

            if (localItem != null) {
                markReserved(curRopReg, category);
                ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> similarRegisters
                ...
                for (int j = 0; j < szSimilar; j++) {
                    com.pranav.ide.dx.rop.code.RegisterSpec similarSpec = similarRegisters.get(j);
    ...
    private int fitPlanForRange(int ropReg, NormalSsaInsn insn,
        ...
        com.pranav.ide.dx.rop.code.RegisterSpecList liveOutSpecs = ssaSetToSpecs(liveOut);

        // An SSA reg may only be mapped into a range once.
        BitSet seen = new BitSet(ssaMeth.getRegCount());

        for (int i = 0; i < szSources; i++) {
            com.pranav.ide.dx.rop.code.RegisterSpec ssaSpec = sources.get(i);
            int ssaReg = ssaSpec.getReg();
     ...
     */
    com.pranav.ide.dx.rop.code.RegisterSpecList ssaSetToSpecs(IntSet ssaSet) {
        com.pranav.ide.dx.rop.code.RegisterSpecList result = new com.pranav.ide.dx.rop.code.RegisterSpecList(ssaSet.elements());
     ...
     */
    private com.pranav.ide.dx.rop.code.LocalItem getLocalItemForReg(int ssaReg) {
        for (Map.Entry<LocalItem, ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec>> entry :
                localVariables.entrySet()) {
            for (com.pranav.ide.dx.rop.code.RegisterSpec spec : entry.getValue()) {
    ...
    private void processPhiInsn(PhiInsn insn) {
        ...
        ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec> ssaRegs = new ArrayList<com.pranav.ide.dx.rop.code.RegisterSpec>();
