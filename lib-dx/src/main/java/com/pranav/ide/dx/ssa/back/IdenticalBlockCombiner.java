...
public class IdenticalBlockCombiner {
    private final com.pranav.ide.dx.rop.code.RopMethod ropMethod;
    private final com.pranav.ide.dx.rop.code.BasicBlockList blocks;
     ...
     */
    public IdenticalBlockCombiner(com.pranav.ide.dx.rop.code.RopMethod rm) {
        ropMethod = rm;
     ...
     */
    private static boolean compareInsns(com.pranav.ide.dx.rop.code.BasicBlock a, com.pranav.ide.dx.rop.code.BasicBlock b) {
     ...
     */
    public com.pranav.ide.dx.rop.code.RopMethod process() {
        ...
        for (int bindex = 0; bindex < szBlocks; bindex++) {
            com.pranav.ide.dx.rop.code.BasicBlock b = blocks.get(bindex);
            ...
            for (int i = 0; i < szPreds; i++) {
                ...
                com.pranav.ide.dx.rop.code.BasicBlock iBlock = blocks.labelToBlock(iLabel);
                ...
                for (int j = i + 1; j < szPreds; j++) {
                    ...
                    com.pranav.ide.dx.rop.code.BasicBlock jBlock = blocks.labelToBlock(jLabel);
    ...
    private void combineBlocks(int alphaLabel, IntList betaLabels) {
        ...
        for (int i = 0; i < szBetas; i++) {
            ...
            com.pranav.ide.dx.rop.code.BasicBlock bb = blocks.labelToBlock(betaLabel);
            IntList preds = ropMethod.labelToPredecessors(bb.getLabel());
            int szPreds = preds.size();

            for (int j = 0; j < szPreds; j++) {
                com.pranav.ide.dx.rop.code.BasicBlock predBlock = newBlocks.labelToBlock(preds.get(j));
     ...
     */
    private void replaceSucc(com.pranav.ide.dx.rop.code.BasicBlock block, int oldLabel, int newLabel) {
        ...
        com.pranav.ide.dx.rop.code.BasicBlock newBB = new BasicBlock(block.getLabel(),
