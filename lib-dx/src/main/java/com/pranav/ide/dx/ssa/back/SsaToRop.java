...
public class SsaToRop {
     ...
     */
    public static com.pranav.ide.dx.rop.code.RopMethod convertToRopMethod(SsaMethod ssaMeth,
     ...
     * @return {@code non-null;} rop-form output
     */
    private com.pranav.ide.dx.rop.code.RopMethod convert() {
        if (DEBUG) {
        ...
        com.pranav.ide.dx.rop.code.RopMethod ropMethod = new RopMethod(convertBasicBlocks(),
    ...
    private void removeEmptyGotos() {
        ...
        ssaMeth.forEachBlockDepthFirst(false, new SsaBasicBlock.Visitor() {
            ...
            public void visitBlock(SsaBasicBlock b, SsaBasicBlock parent) {
                ...
                if ((insns.size() == 1)
                        && (insns.get(0).getOpcode() == com.pranav.ide.dx.rop.code.Rops.GOTO)) {
     ...
     */
    private com.pranav.ide.dx.rop.code.BasicBlockList convertBasicBlocks() {
        ArrayList<SsaBasicBlock> blocks = ssaMeth.getBlocks();
        ...
        com.pranav.ide.dx.rop.code.BasicBlockList result = new BasicBlockList(ropBlockCount);
    ...
    private void verifyValidExitPredecessor(SsaBasicBlock b) {
        ...
        com.pranav.ide.dx.rop.code.Rop opcode = lastInsn.getOpcode();
     ...
     */
    private com.pranav.ide.dx.rop.code.BasicBlock convertBasicBlock(SsaBasicBlock block) {
        ...
        com.pranav.ide.dx.rop.code.BasicBlock result = new BasicBlock(
     ...
     */
    private com.pranav.ide.dx.rop.code.InsnList convertInsns(ArrayList<SsaInsn> ssaInsns) {
        int insnCount = ssaInsns.size();
        com.pranav.ide.dx.rop.code.InsnList result = new InsnList(insnCount);
