...
public class Java14InputAstVisitor extends JavaInputAstVisitor {
    ...
    public Void visitBindingPattern(BindingPatternTree node, Void unused) {
        ...
        if (BINDING_PATTERN_TREE_GET_VARIABLE != null) {
            VariableTree variableTree =
                    (VariableTree) invoke(BINDING_PATTERN_TREE_GET_VARIABLE, node);
            visitBindingPattern(
                    variableTree.getModifiers(), variableTree.getType(), variableTree.getName());
        } else if (BINDING_PATTERN_TREE_GET_TYPE != null
                && BINDING_PATTERN_TREE_GET_BINDING != null) {
    ...
    public void visitRecordDeclaration(ClassTree node) {
        ...
        builder.open(plusFour);
        {
            if (!node.getTypeParameters().isEmpty()) {
                typeParametersRest(
                        node.getTypeParameters(), hasSuperInterfaceTypes ? plusFour : ZERO);
            }
        ...
        switch (node.getCaseKind()) {
            ...
            case RULE:
                ...
                if (node.getBody().getKind() == Tree.Kind.BLOCK) {
                    // Explicit call with {@link CollapseEmptyOrNot.YES} to handle empty case
                    // blocks.
                    visitBlock(
