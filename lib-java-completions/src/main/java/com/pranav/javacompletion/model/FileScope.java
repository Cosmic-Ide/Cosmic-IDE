...
public class FileScope implements EntityScope {
    ...
    public enum FileType {
        /**
         * The {@link FileScope} is created from a Java source code file. The filename can be
         * accessed from the file sytstem.
         */
        SOURCE_CODE,
        /**
         * The {@link FileScope} is created from a type index JSON file generated by the Indexer
         * tool. There is no actual file available on the file system.
         */
        TYPE_INDEX,
        /**
         * The {@link FileScope} is created from a compiled .class file. The file name may point to
         * an existing file, or a file in the JAR archive.
         */
        CLASS_FILE,
    ...
    public static FileScope createFromClassFile(
            Path classFilePath, List<String> packageQualifiers) {
        return new FileScope(
     ...
     *
     * @param qualifiers the imported package qualifiers without *. For example, if the import
     *     statment is {@code import static foo.Bar.*}, then the qualifiers are {@code ['foo',
     *     'Bar']}
     */
    ...
    public void addChildScope(EntityScope entityScope) {
        throw new UnsupportedOperationException(
                "Only classes can be added to a file. Found "
     ...
     * SOURCE_CODE}.
     *
     * <p>Note: use this method instead of {@code getCompilationUnit().getLineMap()}. The line map
     * may need adjustment if the source code is fixed by {@code FileContentFixer}.
     *
     * @throws IllegalStateException thrown if called when the return value of {@link #getFileType}
     *     is not {@code SOURCE_CODE}.
     */
    ...
    public List<EntityScope> getChildScopes() {
        return entities.values().stream().map(Entity::getScope).collect(Collectors.toList());
    }
