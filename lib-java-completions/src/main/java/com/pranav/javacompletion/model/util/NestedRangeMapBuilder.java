package com.pranav.javacompletion.model.util;
...
public class NestedRangeMapBuilder<V> {
    private static class Entry<V> implements Comparable<Entry<V>> {
         ...
         *
         * <p>Range A is less than range B if: a) the lower endpoint of A is less than that of B, or
         * b) the lower endpoint of A is equal to that of B and the upper endpoint is greater than
         * that of B. Otherwise, if the A and B are not equal, then A is greater than B.
         */
    ...
    public ImmutableRangeMap<Integer, V> build() {
        ...
        // A stack of disconnected range and value entries. Ranges are in ascending order, e.g the
        // first
                        ...
                        stack.addFirst(
                                new Entry<>(
                                        Range.closedOpen(
                                                ...
                                                topEntry.range.upperEndpoint()),
                                        topEntry.value));
                    }
                    break;
                } else {
                    // Disconnected with top entry, add the whole range of the top entry to the
                    // builder
