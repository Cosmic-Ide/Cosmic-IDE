...
/**
 * This event happens when {@link CodeEditor#setText(CharSequence)} is called or user edited the
 * displaying content.
 *
 * <p>Note that you should not update the content at this time. Otherwise, there might be some
 * exceptions causing the editor framework to crash. If you do need to update the content, you
 * should post your actions to the main thread so that the user's modification will be successful.
 */
public class ContentChangeEvent extends Event {
    ...
    public ContentChangeEvent(
            CodeEditor editor,
            int action,
            CharPosition changeStart,
            CharPosition changeEnd,
            CharSequence textChanged) {
        super(editor);
        mAction = action;
        mStart = changeStart;
        mEnd = changeEnd;
     ...
     *
     * <p>Note that you can not modify the values in the returned instance.
     */
    ...
    /**
     * Return the changed text in this modification. If action is {@link #ACTION_SET_NEW_TEXT},
     * Content instance is returned. If action is {@link #ACTION_INSERT}, inserted text is the
     * result. If action is {@link #ACTION_DELETE}, deleted text is the result.
     */
