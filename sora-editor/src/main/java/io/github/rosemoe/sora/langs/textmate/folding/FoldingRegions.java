...
public class FoldingRegions {
    ...
    public FoldingRegions(SparseIntArray startIndexes, SparseIntArray endIndexes) throws Exception {
        if (startIndexes.size() != endIndexes.size()
                || startIndexes.size() > IndentRange.MAX_FOLDING_REGIONS) {
    ...
    private boolean isInsideLast(
            Stack<Integer> parentIndexes, int startLineNumber, int endLineNumber) {
                ...
                if (startLineNumber > IndentRange.MAX_LINE_NUMBER
                    ...
                    throw new Exception(
                            "startLineNumber or endLineNumber must not exceed "
                                    + IndentRange.MAX_LINE_NUMBER);
                }
                while (parentIndexes.size() > 0
                        && !isInsideLast(parentIndexes, startLineNumber, endLineNumber)) {
                    parentIndexes.pop();
                }
                int parentIndex =
                        parentIndexes.size() > 0 ? parentIndexes.get(parentIndexes.size() - 1) : -1;
    ...
    public int getParentIndex(int index) throws Exception {
        ...
        int parent =
                ((this._startIndexes.get(index) & IndentRange.MASK_INDENT) >>> 24)
                        + ((this._endIndexes.get(index) & IndentRange.MASK_INDENT) >>> 16);
