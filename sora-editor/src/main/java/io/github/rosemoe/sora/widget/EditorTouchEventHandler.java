    ...
    public void updateMagnifier(MotionEvent e) {
        ...
        if (mMagnifier.isEnabled()) {
            var height =
                    Math.max(
                            Math.max(
                                    mEditor.getInsertHandleDescriptor().position.height(),
                                    mEditor.getLeftHandleDescriptor().position.height()),
                            mEditor.getRightHandleDescriptor().position.height());
        ...
        if (!mEditor.getVerticalEdgeEffect().isFinished()
            ...
            if (distance < -0.001) {
                ...
                mEditor.getVerticalEdgeEffect()
                        .onPull(distance, !topOrBottom ? displacement : 1 - displacement);
            }
            notifyY = false;
        }
        if (!mEditor.getHorizontalEdgeEffect().isFinished()
            ...
            if (distance < -0.001) {
                ...
                mEditor.getHorizontalEdgeEffect()
                        .onPull(distance, !leftOrRight ? 1 - displacement : displacement);
            }
            notifyX = false;
        }
        mScroller.startScroll(
