...
public class SymbolPairMatch {
    ...
    /**
     * Put a pair of symbol completion When the user types the {@param singleCharacter}, it will be
     * replaced by {@param symbolPair} SymbolPair maybe null to disable completion for this
     * character.
     *
     * @see SymbolPair
     */
    ...
    /**
     * Put a pair of symbol completion When the user types the {@param charArray}, it will be
     * replaced by {@param symbolPair} SymbolPair maybe null to disable completion for this
     * character.
     *
     * @see SymbolPair
    ...
    /**
     * Put a pair of symbol completion When the user types the {@param openString}, it will be
     * replaced by {@param symbolPair} SymbolPair maybe null to disable completion for this
     * character.
     *
    ...
    @Nullable
    public final SymbolPair matchBestPair(
            Content content, CharPosition cursorPosition, char[] inputCharArray, char firstChar) {
        ...
        for (var pair : matchList) {
            ...
            if (inputCharArray == null) {
                ...
                // so if the length of the entered string is greater than the length of the symbol
                // pair,
                ...
                for (int charIndex = inputCharArray.length - 1;
                        charIndex > 0;
                        charIndex--, pairIndex--) {
    ...
    public static class SymbolPair {
        ...
        /**
         * If your {@param open} string and {@param close} string are both ', it makes a pair of
         * single quotes. This will replace the entered character with a pair of single quotes, and
         * will move the cursor to the middle of the pair. This class defines these symbol pairs
         */
        ...
        protected boolean shouldNotReplace(CodeEditor editor) {
            ...
            return !symbolPairEx.shouldDoReplace(
                    editor, currentLine, content.getCursor().getLeftColumn());
        }
        ...
        public interface SymbolPairEx {
             ...
             *
             * @param editor The current edit content, sometimes you may need to get the analyzed
             *     data from {@link AnalyzeManager} (e.g. token with tags) and use editor to get
             *     more information, such as the line of the cursor.
             ...
             */
            default boolean shouldDoReplace(
                    CodeEditor editor, ContentLine currentLine, int leftColumn) {
                return true;
            }

            /**
             * when before the replaced and select a range,surrounds the selected content with
             * return ture. If not implemented, always return false also see <a
