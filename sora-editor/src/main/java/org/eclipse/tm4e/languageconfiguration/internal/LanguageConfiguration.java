...
public class LanguageConfiguration implements ILanguageConfiguration {
    ...
    public static LanguageConfiguration load(Reader reader) {
        return new GsonBuilder()
                        ...
                        (JsonDeserializer<OnEnterRule>)
                                (json, typeOfT, context) -> {
                                    ...
                                    if (json.isJsonObject()) {
                                        ...
                                        beforeText =
                                                getAsString(
                                                        object.get("beforeText")); // $NON-NLS-1$
                                        afterText =
                                                getAsString(object.get("afterText")); // $NON-NLS-1$
                                        JsonElement actionElement =
                                                object.get("action"); // $NON-NLS-1$
                                        if (actionElement != null && actionElement.isJsonObject()) {
                                            JsonObject actionObject =
                                                    actionElement.getAsJsonObject();
                                            IndentAction indentAction =
                                                    IndentAction.valueOf(
                                                            getAsString(
                                                                    actionObject.get(
                                                                            "indentAction"))); //$NON-NLS-1$
                                            Integer removeText =
                                                    getAsInt(
                                                            actionObject.get(
                                                                    "removeText")); //$NON-NLS-1$
                                            String appendText =
                                                    getAsString(
                                                            actionObject.get(
                                                                    "appendText")); //$NON-NLS-1$
                        ...
                        (JsonDeserializer<Comments>)
                                (json, typeOfT, context) -> {
                                    ...
                                    if (json.isJsonObject()) {
                                        ...
                                        lineComment =
                                                getAsString(
                                                        object.get("lineComment")); // $NON-NLS-1$
                                        JsonElement blockCommentElement =
                                                object.get("blockComment"); // $NON-NLS-1$
                                        if (blockCommentElement != null
                                                && blockCommentElement.isJsonArray()) {
                                            JsonArray blockCommentArray =
                                                    blockCommentElement.getAsJsonArray();
                                            if (blockCommentArray.size() == 2) {
                                                String blockCommentStart =
                                                        getAsString(blockCommentArray.get(0));
                                                String blockCommentEnd =
                                                        getAsString(blockCommentArray.get(1));
                ...
                .registerTypeAdapter(
                        CharacterPair.class,
                        (JsonDeserializer<CharacterPair>)
                                (json, typeOfT, context) -> {
                                    String open = null;
                                    String close = null;
                                    if (json.isJsonArray()) {
                ...
                .registerTypeAdapter(
                        AutoClosingPairConditional.class,
                        (JsonDeserializer<AutoClosingPairConditional>)
                                (json, typeOfT, context) -> {
                ...
                .registerTypeAdapter(
                        Folding.class,
                        (JsonDeserializer<Folding>)
                                (json, typeOfT, context) -> {
                                    // ex: {"offSide": true, "markers": {"start": "^\\s*/", "end":
                                    // "^\\s*"}}
                                    ...
                                    if (json.isJsonObject()) {
                                        ...
                                        offSide =
                                                getAsBoolean(
                                                        object.get("offSide"),
                                                        offSide); //$NON-NLS-1$
                                        JsonElement markersElement =
                                                object.get("markers"); // $NON-NLS-1$
                                        if (markersElement != null
                                                && markersElement.isJsonObject()) {
                                            JsonObject markersObject =
                                                    markersElement.getAsJsonObject();
                                            startMarker =
                                                    getAsString(
                                                            markersObject.get(
                                                                    "start")); //$NON-NLS-1$
                ...
                .registerTypeAdapter(
                        ...
                        (JsonDeserializer<IndentationRule>)
                                (json, typeOfT, context) -> {
                                    // ex: { "increaseIndentPattern": "...",
                                    ...
                                    if (json.isJsonObject()) {
                                        ...
                                        increaseIndentPattern =
                                                getAsString(
                                                        object.get(
                                                                "increaseIndentPattern")); //$NON-NLS-1$
                                        decreaseIndentPattern =
                                                getAsString(
                                                        object.get(
                                                                "decreaseIndentPattern")); //$NON-NLS-1$
                                    }
    ...
    /**
     * The language's brackets. This configuration implicitly affects pressing Enter around these
     * brackets.
     */
     ...
     * selected string is surrounded by the open and close characters. If not set, the autoclosing
     * pairs settings will be used.
     */
